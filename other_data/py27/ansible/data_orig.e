...
validation
validation
metadata tags from the object
handles elasticache creation and destruction
entity which we want to remove
handles cloudfront services
additional parameters passed when creating entity
securityrule
...
type of database object to grant/revoke
...
hash of all variables to find
list of servers to start or stop
list of snapshots to search
this is a systemd hostname manipulation strategy class - it uses the hostnamectl command
str error that occurred
the name of the group to validate/create
ansible math jinja2 filters
devices class
this class should only attempt to populate those facts that are mostly generic to all systems
...
this is a gentoo openrc hostname manipulation strategy class - it edits the /etc/conf
the new cpu value
condition to be fulfilled
a caching module backed by redis
the target alert policy
this is a debian family hostname manipulation strategy class - it edits the /etc/hostname file
aix-specific subclass of hardware defines memory and cpu facts
is an extra-simple tool/framework/api for doing 'remote things'
an optional dictionary representing the specification for additional module arguments if any
this is a hp-ux user manipulation class
reference on dictionary which contain information about
action which should be executed by service on entity
apache 2 4 mod_proxy lb balancer member
the list of server ids
controlpersist broken pipe
this is the openbsd service manipulation class - it uses rcctl 8 or /etc/rc
ansible core jinja2 filters
sub class for lists
this callback puts results into a host specific file in a directory in json format
the list of clc request objects
raw socket accelerated connection
a special view of vars_cache that adds values from the inventory when needed
optparser which sorts the options by opt before outputting --help
this is the main worker class for the executor pipeline which handles loading an action plugin to actually dispatch the task to
list of command arguments
list of dictionaries in the format of {'name': port}
reference on exception for which title should be
local chroot based connections
filter the result if its older than this
...
gcp service account email used to make requests
this is a base ansible callback class that does nothing new callbacks should
ansible dictionary containing configuration
...
reference on module which contain module launch
field name in list_managed_instances response. defaults
...
the clc-sdk.datacenter instance to use to lookup the group
this is the freebsd service manipulation class - it uses the /etc/rc conf
additional parameters passed to action
exception raised when a vlan cannot be found
an optional list representing the specification for additional module arguments that must be specified together
only for state "present": if true set
traffic groups class
create the cartesian product of lists
the module to validate
bad or incomplete options passed
ansible dictionary containing backends configuration only
displays information on modules installed in ansible libraries
resource management client for azure
simple encoder class to deal with json encoding of internal
dictionary which contains needed values for connection creation
the anti affinity policy id
failure to connect because of a proxy
time before the destroy operation is abandoned
kwargs to be extended
string - the server group to delete
create the connection object and client objects as required
class to interact with a pulp server
local lxc based connections
this is a freebsd-specific subclass of virtual it defines
this is a netbsd user manipulation class
this is a generic user manipulation class that is subclassed based on platform
the port to check and see if it exists
...
datacenter to get policies from
provision information class
this is a generic group manipulation class that is subclassed based on platform
needed to connect to an https url but no ssl library available to verify the certificate
class to handle authenticating user with galaxy api prior to performing cud operations
this is a openbsd user manipulation class
simulate time -- used for unit testing
this is a openbsd family hostname manipulation strategy class - it edits the /etc/myname file
...
pools class
name of the loadbalancer to delete
entity we want to update if exists
internal safeguards tripped something happened in the code that should never happen
service of the entity
selinux store
base name to assign the public ip address
the alert policy name
abstract selector class a selector supports registering file objects to be monitored
clc_sdk.parent - the parent group for {group}
either a list of privileges to grant/revoke
id of the firewall policy to get
takes a yaml-format inventory file and builds a list of groups and subgroups with their associated hosts and variable settings
name of a resource group
the number of days to keep the snapshot
image tag
clc_sdk.group - the parent group to start the walk
base exception for ejabberduser class object
failure to connect due to ssl validation failing
function which checks incorrect state of entity if it returns true exception is raised
...
the state for which the respective command name is required
dictionary in ansible-playbook format
the datacenter to search for the template
array of bind dicts
enabled / disabled
a dictionary of params to use to create the servers
connection to rhev-m
list of servers to add alert policy to
the name of the load balancer
name/path of the lower directory
an action runtime skip
this is the base class for all errors raised from ansible code and can be instantiated with two optional parameters beyond the
handles client authentication via cert/key
dict representing current http resource in avi controller
the params which should be passed to update method
enable or disable unsafe sell commands
the list of clc request objects
managed instance group object from libcloud
the ansiblemodule object
the datacenter to search for a network id
the alert policy id
...
object for lazy instantiation of disk image gce
this is a mac os x darwin group manipulation class
trunks class
the number of times to check the status for completion
ansible module reference
the overlay repository id to sync
str message to report
dictionary of request parameters for firewall policy
ip address and network manipulation filters
selective py callback plugin
name of the object's class
the port that the load balancer listens on
path to the temporary local working directory
firewall policy id to update
this is the openbsd network class
openbsd-specific subclass of hardware defines memory cpu and device facts
this is a sunos group manipulation class solaris doesn't have
an initialized gceautoscaler
func-based connections
the ansiblemodule instance to use
lazy loading of moved objects in six moves urllib_request
ignore invalid options
winrm connections over http/https
reference on keyset model from which reference on block
dictionary of parameters needed by the module
the list of server_ids to delete the snapshot
port or port range (example "8080", "8080-9090")
azure object
state which should entity has in order to finish task
...
a set subclass that keeps track of insertion time and persists the set in memcached
"present" to grant privileges "absent" to revoke
the blueprint package id
full path to location of file which should be read'ed
a list of dictionaries containing the nodes to set
the clc alert policy id
this object represents a user resource for an ejabberd server the
this is the aix service src manipulation class - it uses lssrc startsrc stopsrc and refresh for service control
client ssl profiles class
the source account alias for the firewall policy
the dataloader class is used to load and parse yaml or json content either from a given file name or from a string that was previously
error message text
something was detected early that is wrong about a playbook or data file
a group of ansible hosts
this class is used to consolidate the connection information for hosts in a play and child tasks where the task may override some
entity to be created or updated
list of ports and port ranges to add (e.g. ["8080", "8080-9090"])
a task is a language feature that represents a call to a module with given arguments and other parameters
list of database objects to grant/revoke
function which is executed before running the action
name/path of the upper directory
this is the netbsd service manipulation class - it uses the /etc/rc conf
some object types ("table", "sequence",
a derivative of roledefinition used by playbook code when a role is included for execution in a play
the underlying ansible module
...
if this condition is true raise exception
this is a freebsd group manipulation class
path on the file system that is mounted
the module to check for intended state
the list of server ids
whether to reload selinux policy after commit
lazy loading of moved objects in six moves urllib_parse
the absolute path of where to save the downloaded content to
additional parameters passed when removing entity
loads variables for groups and/or hosts
this class is the base class for all action plugins and defines code common to all actions
firewall policy to delete
number of times this command has been called by itself
user agent product. eg 'ansible-python-api'
name of lv snapshot
the id string of the load balancer pool
if true require credentials to be valid json. default is true
ceconfigmixin
netbsd-specific subclass of hardware defines memory and cpu facts
the loadbalancer id
any key=value pairs
the server to start or stop
the blue print package id
the ip protocol
the id string of the provided load balancer
the cli string
the layman's configuration to use optional
response from firewall creation api call
reference on pubnub account model from which reference
print statements during execution
version string to append to product. eg 'na' or '0.1'
clc-sdk.datacenter instance to search for the group
name of volume
name of the address
the clc-sdk instance
path to the repository. required
string - a text description of the group
the firewall policy id
this is a tcp connection info evaluation strategy class that utilizes information from linux's procfs
main class that handles daemontools can be subclassed and overridden in case
ssh based connections with paramiko
the clc server id
a spanner instance object
list of 'currentaction' strings to filter on. only
a callback failure
acme client class uses an acme account object and a csr to
dict data for avi api call
the clc-python-sdk instance to use
data about an individual host
string - the datacenter to walk (ex 'uc1')
name of the container
sequence lookup module used to generate some sequence of items
name of the alert policy
entity we want to run action on
...
set to 'default' if the rule is part of the default set of rules
ansible dictionary containing named_ports configuration
system information class
string - the group to check
a module failed somehow
instantiates a pushover object use it to send notifications
this is the aix network class
there are several ways a module and argument set can be expressed # legacy form for a shell command
string - the name of the parent group for group_name
structure that contains the fields
this is a timezone manipulation class for smartos instances
zone that the autoscaler is located in
for os_type 'windows' port used in rule allowing rdp access
exception raised when apache libcloud cannot be found
description for loadbalancer to be created
transforms a dict to with an argument spec this class will take a dict and apply an ansible argument spec to the
exception raised when a network or network domain cannot be found
dictionary of autoscaling parameters
this is a darwin mac os x user manipulation class
ansible logstash callback plugin ansible
a base class for implementing cli connections
this is the mac os x/darwin network class
certificates class
alias of account
path to the container container
the account alias
an initialized gce driver object
datacenter to create policy in
bool. push the image once it's tagged
lazy loading of moved objects in six moves urllib_error
this is a generic tcp connection info strategy class that relies on the psutil module which is not ideal for targets but necessary
lazy loading of moved objects in six moves urllib_robotparser
ansible dictionary containing autoscaling configuration
...
logs ansible-playbook and ansible runs to a syslog server in json format make sure you have in ansible
dict
makes ansible much more exciting on os x
the load balancing persistence type
pauses execution for a length or time or until input is received
id of the server to modify
virtualmachine object
list of servers to modify
a dictionary that maintains the order of keys as they are added this has become an abuse of the dict interface
used for the command and all options
the services to format. this is always a python set
local iocage based connections
freebsd-specific subclass of hardware defines memory and cpu facts
sub class for dictionaries
handles ecs services
path on disk to credentials file
...
optional string containing the registryid
this is a sunos user manipulation class - the main difference between this class and the generic user class is that solaris-type distros
this class is a helper to create easily vmware objects for pyvmomihelper
string - the parent of group_name
the ansible module to fetch command
...
the ansible module to fetch input parameters
an initialized gceinstancegroupmanager
this callback will report facts and reports to foreman https //theforeman org/
this is the timezone implementation for *bsd which works simply through updating the /etc/localtime symlink to point to a valid timezone name under
number of seconds we should wait until next condition check
the list of nodes to be updated to the pool
datacenter to delete a policy from
protocol ('tcp' or 'udp')
this class is used for manipulating pools subscriptions with rhsm
the ansiblemodule instance
a protocol to allow for the public ips
this is a aix user manipulation class
entity which was created
nodes class
dictionary of parameters to be used for search
this is a generic timezone manipulation class that is subclassed based on platform
pre-parsed optional variables used from a
a lookup failure
this is a timezone manipulation class for systemd-powered linux
this is a openbsd-specific subclass of virtual it defines
a custom context which intercepts resolve() calls and sets a flag internally if any variable lookup returns an ansibleunsafe value
helper class to template all variable content before jinja2 sees it this is
data about an individual task
the anti affinity policy name
function which is executed after running the action
dictionary of policy name
...
zone that the managed instance group is located in
the list of nodes to be removed
exception raised when credentials for dimension data cloudcontrol cannot be found
the new memory value
handles ecs tasks
a meta path importer to import six moves and its submodules
a role include is derived from a regular role to handle the special circumstances related to the - include_role
dictionary of string string pairs
ansible module containing the validated configuration for the deployment template
the id string of the pool
initialized ansible module object
reference on pubnub account model which will be
this is the netbsd network class
the id string of the load balancer
fail this step after this many seconds
the group to look for
lazy loading of moved objects
list of module names to build enum dependencies from
name of the managed instance group
panorama device group or if none then 'all'
...
the server list where the package needs to be installed
this is a netbsd group manipulation class
this ansible callback plugin mails errors to interested parties
this class handles the multiprocessing requirements of ansible by creating a pool of worker forks a result handler fork and a
gcp project id
...
this is the sunos service manipulation class - it uses the svcadm command for controlling services and svcs command for checking status
size of snapshot to create
a list of clc-sdk.request instances
value of entity parameter
name of the image. required
netconf
aws api client reference boto3
the datacenter the load balancer resides in
main class that handles daemontools can be subclassed and overridden in case
ansiblemodule
...
vlans class
name of lv to snapshot
name of the entity
logs playbook results per host in /var/log/ansible/hosts
create inventory groups based on variables
one of 'windows' or 'linux'. determins any default rules added to the security group
...
this is a freebsd hostname manipulation strategy class - it edits the /etc/rc
self ips class
extends complexdict to handle a list of dicts
clc_sdk.group - the child group to start the walk
the method used to generate the replacement lines
name of the policy
the group to count the servers
the overlay id to uninstall
this is an ansible callback plugin that sends status updates to a slack channel during playbook execution
this class is meant to be used as a api client for an ansible galaxy server
this class is responsible for interpreting the resulting data from an executed task and provides helper methods for determining
either a list of role names or "public"
handles cloudtrail configuration
fail with custom message
dictionary of tags from an azure object
rules class
this is the dense callback interface where screen estate is still valued
the ansible module to fetch username password and switch
handles elb creation and destruction
clc_sdk.group - the group to create
path to the directory to be archived
list of ports to allow for the public ips
a list of dictionaries containing the nodes to remove
list of tag keys or tag key value pairs
the dictionary of server parameters
this is a redhat hostname strategy class - it edits the /etc/sysconfig/network file
boolean - whether or not to provision a public ip for servers. skipped if false
the name of the parent group for group_name
...
this is the generic service manipulation class that is subclassed based on platform
...
crontab object to write time based crontab file user - the user of the crontab defaults to root
this is the sunos network class
command to run this can be a script and can use spacing
context in the network config to filter the diff
host inventory for ansible
name of the logical volume to mount
the list of server_ids to create the snapshot
the error message to raise if there is any error
the error message to throw in case of any error
a list of dictionaries containing the nodes to add
perform container management tasks
number to round up
the transport / connection_plugin had a fatal error
list of servers to add public ips to
string. name of a resource group
metaclass for the base object which is used to construct the class attributes based on the fieldattributes available
used in command modules to evaluate waitfor conditions
for os_type 'linux' port used in rule allowing ssh access
...
entity we want to remove
a short description of the server group used when creating
selinux mls/mcs range (defaults to 's0')
ssh based connections
reference on pubnub account model which will be used in
connection info needed
list of ports and port ranges to delete (e.g. ["8080", "8080-9090"])
function which is executed when checking if action should be executed
datacenter
headers to convert
apiresponse object returned from apisession
aws client connection
the list of nodes to check for
connection to the python sdk
the url of the remote repositories list to look for the overlay
bool. force tagging even it image already exists with the repository path
this is a generic hostname manipulation class that is subclassed based on platform
handles cloudformation services
interfaces class
darwin-specific subclass of hardware defines memory and cpu facts
the required argument dict for the package installation
netconf connections
helper class for galaxy which is used to parse both dependencies specified in meta/main
reference on pubnub application model from which
mapping of file objects to selector keys
this is a aix group manipulation class
the number of retry attempts to make prior to fail. default is 5
used for communicating with haproxy through its local unix socket interface
entity which was updated
code behind bin/ansible* programs
description of loadbalancer
the clc account alias
a simple stub class that allows us to add representers for our overridden object types
instance of seobject.portrecords
this class wraps the parsing and validation of the optional metadata within each role (meta/main
this is a generic bsd subclass of network using the ifconfig command
pluginloader loads plugins from the configured plugin directories
this class caches references to objects which are requested multiples times but not modified
firewall policy id to delete
a csv reader which will iterate over lines in the csv file "f", which is encoded in the given encoding
azure location name
this is a freebsd user manipulation class - it uses the pw command to manipulate the user database followed by the chpass command
instance of networkconfig to diff against
cli
int return code while executing an ansible command
...
cloudretry can be used by any cloud provider in order to implement a backoff algorithm/retry effect based on status code from exceptions
a task include is derived from a regular task to handle the special circumstances related to the - include
this is a linux-specific subclass of virtual it defines
common operations to manage users and roles
string - the name of the clc server group to delete
whether something changed in this module
...
the list of nodes to be added
a class to manage homebrew packages
a caching module backed by file based storage
name of the backend service
create inventory hosts and groups in the memory inventory
the inspection output for an existing network
dictionary of a policy name
this subclass of facts fills the distribution distribution_version and distribution_release variables to do so it checks the existence and content of typical files in /etc containing distribution information
name of loadbalancer
create a six moves urllib namespace that resembles the python 3 namespace
type of diff to perform. valid values are 'line',
the alert policy id
class to manage mac os user defaults
this is a sles hostname strategy class - it edits the /etc/hostname file
a valid azure location
the overlay id
ansible make module instance
entity we want to compare with ansible module values
the worker thread class which uses taskexecutor to run tasks read from a job queue and pushes results into a results queue
reference on block model from which reference on event
this is a alpine linux hostname manipulation strategy class - it edits the /etc/hostname file then run hostname -f /etc/hostname
processed ansible module
list of dict representing the fields
a repl that allows for running ad-hoc tasks against a chosen inventory (based on dominis' ansible-shell)
output to sanitize
function which is executed when waiting on result state
a single ansible host
list of servers to delete
the account alias to search
...
keys class
query filter
clc-sdk instance to use
the server object where the package needs to be installed
perform common tasks in nagios related to downtime and notifications
if true wait for condition if false don't wait
base name to use for the security group
this is a sunos-specific subclass of virtual it defines
list of clc-sdk.server instances to refresh
link of the entity
a class to manage homebrew casks
user inputted parameter
the clc-sdk instance to use
the alert policy
opts_string looks like 'discard foo=bar baz=greeble'
this is a solaris11 or later hostname manipulation strategy class - it execute hostname command
the clc server object
ansible had a problem while running a playbook
this is the linux service manipulation class - it is currently supporting a mixture of binaries and init scripts for controlling services started at
uuid of the server
the intended powerstate for the server
datacenter of the firewall policy
reference on authorized user for which one of accounts
handles ecs tasks
alias for account
the complete cli string to be executed on the target node s
