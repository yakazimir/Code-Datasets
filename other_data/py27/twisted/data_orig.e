...
stream client endpoint string parser for l{wrapclienttls} with l{hostnameendpoint}
a l{response} instance describes an http response received from an http server
...
helper for l{wsgiresource} which drives the wsgi application using a threadpool and hooks it up to the l{http
mixin that implements c{__eq__} and c{__ne__}
fake traceback object which can be passed to functions in the standard library l{traceback} module
a representation of a project that has a version
a completion "action" - provides completion possibilities for a particular command-line option
...
an interface for performing some basic kinds of i/o (particularly that i/o which might be useful for l{twisted
...
a l{via} is a sip via header representing a segment of the path taken by the request
implements lookup() to grab entries for responses from /proc/net/tcp
an email service
encodes c{decimal decimal} instances
an interface to the parts of the operating system which l{tuntapport} relies on
...
l{message} contains all the information represented by a single dns request or response
...
an in-memory implementation of basic i/o primitives useful in the context of unit testing as a drop-in replacement for parts of the c{os} module
a sip message
...
file is a resource that represents a plain non-interpreted file (although it can look for an extension like
forks a child process on connectionmade passing the socket as fd 0
a http/2 request was received that contained a http/2 pseudo-header field that is not recognised by twisted
...
trust an explicitly specified set of certificates represented by a list of l{openssl
...
a l{_warning} instance represents one warning emitted through the python warning system (l{warnings})
...
l{_messagestructure} is a helper base class for message structure classes representing the structure of particular kinds of messages as defined by
stream error exception
a listening port
process is already running
win32 event api methods @since 10
...
processreader i am a selectable representation of a process's output pipe such as
handle a xml-rpc request and store the state for a request in progress
internal this class manages state for a call to jelly()
an l{irenderable} is an object that may be rendered by the l{twisted
...
protocol supporting basic ssh functionality sending/receiving packets and message dispatch
basic implementation of connector
a utility class to retrieve the header and some lines of the body of a mail message
a l{commandlocator} is a collection of responders to amp l{command}s with the help of the l{command
...
add a failure
a l{_contextfactorywithcontext} is like a l{twisted
in some cases this acts as an account since it a source of text
l{missingtemplateloader} is raised when trying to render an element without a template loader i
a protocol that parses and verifies the checksum of an nmea sentence (in string form not l{nmeasentence}), and delegates to a receiver
...
log observer that writes to the python standard library's c{logging} module
...
l{failureelement} is an l{irenderable} which can render detailed information about a l{failure<twisted
...
a long string
a maildir-backed domain where membership is checked with a l{dirdbm <dirdbm
...
extend the standard library's c{testsuite} with a consistently overrideable c{run} method
handle isupport messages
i am a remote copy of a copyable object
an l{ibodyproducer} declared the number of bytes it was going to produce (via its c{length} attribute) and then produced a different number
...
a protocol for receiving l{ampbox}es - key/value pairs - via length-prefixed strings
...
an l{_ifixedgroupkexalgorithm} describes a key exchange algorithm with a fixed prime / generator group
l{systemftprealm} uses system user account information to decide what the home directory for a particular avatarid is
...
@ivar savestats if c{true}, save the stats information instead of the human readable format
...
l{isshprivatekey} credentials encapsulate an ssh public key to be checked against a user's private key
base class for esmtp client errors
l{_wrapperexception} is the base exception type for exceptions which include one or more other exceptions as the low-level causes
an l{ireactorpluggableresolver} is a reactor which can be customized with an l{iresolversimple}
...
@ivar _shouldshutdown set to c{true} if c{shutdown} should be called before calling c{close} on the underlying socket
...
l{defaultopensslcontextfactory} is a factory for server-side ssl context objects
...
a trivial newsstorage implementation using pickles contains numerous flaws and is generally unsuitable for any
an l{iargumenttype} can serialize a python object into an amp box and deserialize information from an amp box back into a python object
...
trivial wrapper around a list to give it an interface like a queue which the addition of also sending notifications by way of a deferred whenever
l{ftpclient} is a client implementation of the ftp protocol which exposes ftp commands as methods which return l{deferred}s
a class representing read/write/execute permissions for a single user category (i
...
this exception is raised on windows for compatibility with previous releases of filepath where unportable programs may have done "except
a factory for objects which provide l{twisted
an error indicating that the server sent a line which exceeded the maximum line length (l{lineonlyreceiver
...
a factory for producing ftp protocol instances @ivar timeout the protocol interpreter's idle timeout time in seconds
l{_rfc822messagestructure} represents the message structure of a i{message/rfc822} message
wraps a stream object so that all c{write} calls are wrapped in l{untilconcludes<twisted
a l{streamserverendpointservice} is an l{iservice} which runs a server on a listening port described by an l{istreamserverendpoint
a virtual hosting pop3 server
an ipv6 address
trivial protocol for communicating with a process and turning its output into the result of a l{deferred}
raised to indicate the current test has failed to pass
an attempt was made to use a socket with an address family (eg i{af_inet}, i{af_inet6}, etc) which is not supported by the reactor
...
@ivar protocol an instance of the protocol to which this transport is connected or none before the connection is
gives us information about the platform we're running on
the user was not authorized to log in
inherit from me to unjelly yourself directly with the c{setstatefor} convenience method
...
wrap a c{testresult} from the standard library's c{unittest} so that it supports the extended result types from trial and also supports
an object representing an nmea sentence
i am an object which pages out information
a credential factory defines a way to generate a particular kind of authentication challenge and a way to interpret the responses to these
interface that must be implemented by all plugins
file-like object used to read a deflated entry in a zipfile
l{_singlepartmessagestructure} represents the message structure of a non-i{multipart/*} message
an altitude
a l{contexttracker} provides a way to pass arbitrary key/value data up and down a call stack without passing them as parameters to the functions on
...
l{_textmessagestructure} represents the message structure of a i{text/*} message
a positioning beacon
...
a test loader finds tests from the functions modules and files that is asked to and loads them into a l{testsuite} or l{testcase}
internal class for tracking method calls
an l{ihostnameresolver} can resolve a host name and port number into a series of l{iaddress} objects
run a test
a loader for templates something usable as a value for l{twisted
...
a l{clientservice} maintains a single outgoing connection to a client endpoint reconnecting after a configurable timeout when a connection
...
the body l{ibodyproducer} for a request tried to write data after indicating it had finished writing data
...
an c{exception} that will be raised when the factory given to a l{istreamclientendpoint} returns l{none} from c{buildprotocol}
...
@ivar _tlsclientdefault always c{true}, indicating that this is a client connection and by default when tls is negotiated this class will act as
...
this wraps an event handler and translates notification in the helper l{win32reactor} thread into a notification in the primary reactor thread
...
a service
a provider of l{iprotocolnegotiationfactory} can provide information about the various protocols that the factory can create implementations of
imap4 client protocol implementation @ivar state a string representing the state the connection is currently
protocol interpreter for the file transfer protocol @ivar state the current server state
add an error
implements the digest-md5 sasl authentication mechanism
this credential is used when a credentialchecker has access to the hash of the username realm password as in an apache
...
l{_descriptorexchanger} is a mixin for l{binaryboxprotocol} which adds support for receiving file descriptors a feature offered by
...
protocols may implement l{ifiledescriptorreceiver} to receive file descriptors sent to them
a finite-state machine that parses formatted irc text
l{_commandtransport} is an ssh client i{transport} which includes a host key verification step before it will proceed to secure the connection
...
l{userdatabase} holds a traditional posix user data in memory and makes it available via the same api as l{pwd}
...
information about positioning beacons (a generalized term for the reference objects that help you determine your position such as satellites or cell
l{_sourcelineelement} is an l{irenderable} which can render a single line of source code
...
service to periodically call a function every c{step} seconds call the given function with the given arguments
a credentials checker which authenticates users out of a l{dirdbm <dirdbm
...
base class for business logic of initializing an xmlstream subclass this object to enable an xmlstream to initialize and authenticate
generate a call to the zsh _arguments completion function based on data in a usage
a request for a uri
l{ihostnameresolver} implementation that resolves hostnames by calling l{getaddrinfo} in a thread
l{ilogfilterpredicate} that filters out events with a log level lower than the log level for the event's namespace
designed to be mixed in to a protocol implementing class
base class for process and ptyprocess
write the stream of tokens and whitespace from l{tokenprinter}, formating tokens as html spans
i act as an indirect reference to an object accessed through a l{pb
...
a provider of l{iopensslcontextfactory} is capable of generating l{openssl
...
an l{iagentendpointfactory} provides a way of constructing an endpoint used for outgoing agent requests
...
a shortcut on win32
a stream server endpoint is a place that a l{factory} can listen for incoming connections
a fileauthority that is built up from python source code
a hint to a particular system
a l{protocol} implementation which wraps another one transparently decompressing received data
an attempt was made to use a socket of a type (eg i{sock_stream}, i{sock_dgram}, etc) which is not supported by the reactor
a single integer
a helper for implementing c{assertraises} this is a context manager and a
implements the plain sasl authentication mechanism
download to a file
uh
an application object which can receive l{ampbox} objects and dispatch them appropriately
use or subclass me to implement a command that starts tls
no route to host
an l{_igroupexchangekexalgorithm} describes a key exchange algorithm that uses group exchange between the client and server
...
an l{_ipaddress} represents the address of an ip socket endpoint providing common behavior for ipv4 and ipv6
...
l{filebodyproducer} produces bytes from an input file object incrementally and writes them to a consumer
...
a mail group member
checker that authenticates ssh public keys based on public keys listed in authorized_keys and authorized_keys2 files in user
command line options for c{twist}
a top-level container object which provides focus wrap-around and paint scheduling
@ivar focusedchild the contained widget which currently has focus or none
a unit test the atom of the unit testing universe
a factory for a socksv4 proxy
raised when attempted to override an existing resource
l{tlsmemorybioprotocol} is a protocol wrapper which uses openssl via a memory bio to encrypt bytes written to it before sending them on to the
...
smtp protocol that sends a single email based on information it gets from its factory a l{smtpsenderfactory}
...
i represent the very top of the python object-space the module list in c{sys
...
/etc/services parser @ivar services dict mapping service names to port protocol tuples
...
objects which provide l{ibodyproducer} write bytes to an object which provides l{iconsumer<twisted
...
connection was aborted locally using l{twisted
complete hostnames
l{_bypasstls} is used as the transport object for the tls protocol object used to implement c{starttls}
an abstract maildir-backed domain
a signature of a callable
file-like object instances of which are used as the value for the c{'wsgi
a thing for checking whether a checkout has a topfile
the requested action can't happen while offline
an l{iresolutionreceiver} receives the results of a hostname resolution in progress initiated by an l{ihostnameresolver}
...
i find tests inside function modules files -- whatever -- then return them wrapped inside a test (either a l{testsuite} or a l{testcase})
represents a stringprep'd jabber id
a fake c{urllib2 request} object for c{cookielib} to work with
an l{agent} wrapper which handles http redirects in the same fashion as web browsers
...
generate api documentation from source files using u{pydoctor<https //github
an l{_internalstate} is a helper object for a l{_moduleproxy}, so that it can easily access its own attributes bypassing its logic for delegating to
a client that connects to an nntp server and asks for articles new since a certain time
raised by public key checkers when they receive public key credentials that don't contain a signature at all but are valid in every other way
an in-memory dictionary factory for postfixtcpmapserver
...
a provider of l{iopensslclientconnectioncreator} can create l{openssl
l{resourcepublisher} exposes a remote api which can be used to respond to request
encode a unicode string on the wire as utf-8
raised on any attempt to get a method which has not been exposed
an l{_iprocesstransportwithconsumerandproducer} combines various interfaces to work around the issue that l{interfaces
constants describing log levels
@cvar compresslevel the compression level used by the compressor default to 9 highest
base for classes which can render part of a page
a l{hashedentry} is a representation of an entry in a known_hosts file where the hostname has been hashed and salted
...
object that provides ssh public keys based on public keys listed in authorized_keys and authorized_keys2 files in unix user
python module wrapper to hook module-level attribute access
mixin which adds write buffering
an http request
a supplementary interface for mailboxes which require cleanup on close
an l{istreamserverendpointstringparser} is like an l{istreamclientendpointstringparserwithreactor}, except for
...
a stub transport implementation used to support l{amp} over a l{processprotocol} transport
a l{url} represents a url and provides a convenient api for modifying its parts
...
a l{iaddress} implementation meant to provide stub addresses for l{itransport
@ivar _waiting a c{dict} mapping tuple keys of query name/type/class to deferreds which will be called back with the result of those queries
...
an error indicating that when trying to look up a mail exchange host a set of canonical name records was found which form a cycle and resolution was
an l{ipolicyforhttps} provides a policy for verifying the certificates of https connections in the form of a l{client connection creator
...
diffie-hellman key exchange with sha-1 as hash and oakley group 14 2048-bit modp group
...
l{_macparams} represents the parameters necessary to compute ssh mac message authenticate codes
...
like l{iworker}, but with the additional guarantee that the callables passed to c{do} will not be called exclusively with each other
...
adapter for pyunit-style c{testcase} subclasses that have undesirable id() methods
...
helper object which exposes l{iconsumer} on top of l{http11clientprotocol} for streaming request bodies to the server
a collection that can be locked from adding entities
an encapsulation of some crammd5 hashed credentials
the normal twisted waker will simply wake up the main loop which causes an iteration to run which in turn causes l{reactorbase
storage implementations can inherit from this class to get the easy-to-use c{notifymoderators} method which will take care of sending messages which
...
an l{ihostresolution} represents represents an in-progress recursive query for a dns name
an l{_iellipticcurveexchangekexalgorithm} describes a key exchange algorithm that uses an elliptic curve exchange between the client and server
...
a l{_commandchannel} executes a command in a session channel and connects its input and output to an l{iprotocol} provider
...
an interface implemented by a l{iprotocol} to indicate that it would like to be notified when tls handshakes complete when run over a tls-based
...
l{_frameelement} is an l{irenderable} which can render details about one frame from a l{failure<twisted
...
l{_multipartmessagestructure} represents the message structure of a i{multipart/*} message
...
a version of build_py that doesn't install the modules that aren't yet ported to python 3
the heading of a mobile object
indicates a lookup failed because there were no records matching the given c{name class type} triple
a domain delivery base class for use in an smtp server
a type of credentials were passed in with no knowledge of how to check them
...
l{_connectionready} is an l{sshconnection} an ssh service which only propagates the i{servicestarted} event to a l{deferred} to be handled
credential factory for http basic authentication @type authenticationrealm l{bytes}
a standard input/output endpoint @ivar _stdio a callable like l{stdio
...
@ivar tls c{false} to indicate the connection is in normal tcp mode c{true} to indicate that tls has been started and that operations must
provides options for selecting a reactor to install
l{unparsedentry} is an entry in a l{knownhostsfile} which can't actually be parsed therefore it matches no keys and no hosts
...
the location of the server s for a specific protocol and domain
authenticator to permit an xmlstream to authenticate against a jabber server as an external component (where the authenticator is initiating the
a l{boxdispatcher} dispatches '_ask', '_answer', and '_error' l{ampbox}es both incoming and outgoing to their appropriate destinations
...
sshprotocolchecker is a checker that requires multiple authentications to succeed
...
an l{iconsumer} proxy which enforces an exact length requirement on the total data written to it
l{_sigchldwaker} can wake up a reactor whenever c{sigchld} is received
l{forbiddenresource} is a specialization of l{errorpage} which returns the i{forbidden} http response code
...
a l{logbeginner} holds state related to logging before logging has begun and begins logging when told to do so
a receiver for int16-prefixed strings
l{errorpage} is a resource which responds with a particular parameterized status and a body consisting of html containing some
an implementation of the socksv4 protocol
protocol for decoding i{chunked} transfer-encoding as defined by rfc 2616 section 3
...
@ivar modulename the fully-qualified python name of the module of which the install callable is an attribute
a date -- year month day tuple
support for rfc2617 http digest authentication @cvar challenge_lifetime_secs the number of seconds for which an
placeholder for a l{testcase} inside a reporter as far as a l{testresult}
i represent a user's account with a chat service
l{resolver} implements recursive lookup starting from a specified list of root servers
a hierarchical bucket filter with a l{bucket} for each service
l{potentialdataloss} may be raised by a transfer encoding decoder's c{nomoredata} method to indicate that it cannot be determined if the
a flag representing whether a worker has been quit
a maildir-backed mailbox
l{threadedresolver} uses a reactor a threadpool and l{socket
...
l{ilogobserver} that stores events in a buffer of a fixed size : >>> from twisted
...
provide a deterministic easily-controlled implementation of l{ireactortime
...
l{responsefailed} indicates that all of the response to a request was not received for some reason
...
add a layer on top of another request that only uses the value of an x-forwarded-for header as the result of c{getclientip}
a factory for ssh servers
file used for unix socket is no good
l{_sourcefragmentelement} is an l{irenderable} which can render several lines of source code near the line number of a particular frame object
a lightweight wrapper for a db-api 'cursor' object
an i{edns} message
the guy that cleans up after you
subclass this to indicate that your protocol factory is only usable for servers
a weakly file-like object which can be used with l{knownhostsfile} to respond in the negative to all prompts for decisions
read-only mapping that turns a c{()}-suffix in key names into an invocation of the key rather than a lookup of the key
...
this mixin provides common behavior between python module and path entries since the mechanism for searching sys
...
object that provides ssh public keys based on a dictionary of usernames mapped to l{twisted
a mail destination
...
abstract implementation of an l{ifilepath}; must be completed by a subclass
an authoritative nameserver
a protocol factory for clients
udp port that supports multicasting
mixin for stream-oriented unix transports which uses sendmsg and recvmsg to offer additional functionality such as copying file descriptors into other
i am a reverse-reference to the peer's l{remotecache}
a tcp server endpoint interface
a l{cooperativetask} is a task object inside a l{cooperator}, which can be paused resumed and stopped
...
defines helper methods that can be added on to any l{usage
a simple description of an inetd service
l{cookieagent} extends the basic l{agent} to add rfc-compliant handling of http cookies
...
a collection of l{cachedplugin} instances from a particular module in a plugin package
...
xml-rpc client factory @ivar path the path portion of the url to which to post method calls
...
an l{applogger} attaches the configured log observer specified on the commandline to a l{serveroptions} object a custom l{logger
...
connections which have been switched to other protocols can no longer accept traffic at the amp level
adapter wrapping a l{twisted internet interfaces iopensslcontextfactory}
a l{plainentry} is a representation of a plain-text entry in a known_hosts file
...
an l{iterminaltransport} implementation which forwards calls to one or more other l{iterminaltransport}s
...
an l{sshcommandaddress} instance represents the address of an ssh server a username which was used to authenticate with that server and a command
backwards compatibility hack just like ipv4address in practice
a socket-like object which can be used to read udp datagrams from tunnel-like file descriptors managed by a l{memoryiosystem}
...
l{_existingconnectionhelper} implements l{_isshconnectioncreator} by handing out an existing ssh connection which is supplied to its
an ipv6 host address
a basis for reactors that use file descriptors
standard http endpoint destinations - tcp for http tcp+tls for https
udp socket methods
a receiver for int32-prefixed strings
l{resolverbase} is a base class for implementations of l{interfaces
...
resource that renders the results gotten from another server put this resource in the tree to cause everything below it to be relayed
...
an l{itransport}, l{iprocesstransport}, l{iconsumer}, and l{ipushproducer} provider for the l{iprotocol} instance passed to the process endpoint
...
use this command to switch from something amp-derived to a different protocol mid-connection
...
an anonymous implementation of iftpshell @type filesystemroot l{twisted
a descriptor which will proxy attribute access mutation and deletion to the l{_proxydescriptor
...
l{shadowdatabase} holds a shadow user database in memory and makes it available via the same api as c{spwd}
...
a l{filesystemlock} that allows for a l{deferred} to be fired when the lock is acquired
a dictionary was not formattable
a verbose reporter that prints the name of each test as it is running
exception passed to l{responsefailed} if we got a redirect without a c{location} header field
...
a request which was received by a l{resourcesubscription} and sent via pb to a distributed node
...
l{banana} implements the i{banana} s-expression protocol client and server
stream initializer that performs sasl authentication
factory for jabber xmlstream objects as a reconnecting client
@type to c{list} of c{str} @ivar to the addresses to which to deliver this message
this class is used to wrap a string-ified exception from the remote side to be able to reraise it
...
a provider of l{iopensslserverconnectioncreator} can create l{openssl
factory for smtp
generate html documentation using sphinx
a process has ended without apparent errors
serve files exactly as responses without generating a status-line or any headers
a l{_disconnectfactory} is a proxy for l{iprotocolfactory} that catches c{connectionlost} notifications and relays them
l{_newconnectionhelper} implements l{_isshconnectioncreator} by establishing a brand new ssh connection securing it and authenticating
a l{scrolledarea} contains another widget wrapped in a viewport and vertical and horizontal scrollbars for moving the viewport around
an c{exception} that will be raised when an l{istreamclientendpoint} is cancelled before it connects
...
this mixin implements l{ireactorwin32events} for another reactor by running a l{win32reactor} in a separate thread and dispatching work to it
a sip url
generic color attribute
ssl secured tcp client endpoint with an ipv4 configuration
see _ansicolorizer docstring
l{_shadowrecord} holds the shadow user data for a single user in l{shadowdatabase}
any errors logged in this suite will be reported to the l{testresult} object
mixin for character attributes that implements a c{__getattr__} method returning a new c{_normalattr} instance when attempting to access
...
an error indicating that when trying to look up a mail exchange host too many canonical name records which point to other canonical name records
an error occurred while authenticating
xmpp server's router
a gui window of a conversation with a specific person
wrapper which logs attribute lookups
a consumer for producers that finish
l{resourcescriptdirectory} is a resource which serves scripts from a filesystem directory
a null record
...
c{_chunkedtranferdecoder} raises l{_malformedchunkeddataerror} from its c{datareceived} method when it encounters malformed data
...
an l{iresource} implementation which delegates responsibility for all resources hierarchically inferior to it to a wsgi application
this is thrown if the authentication is valid but is not enough to successfully verify the user
...
an authority that loads u{bind zone files <https //en
encode floating-point values on the wire as their repr
local implementation of the manager commands
indicates a lookup failed for a name for which this server is authoritative because there were no records matching the given c{name class type}
...
a push producer also known as a streaming producer is expected to produce write to this consumer data on a continuous basis unless
l{cgiscript} is a resource which runs child processes according to the cgi specification
a l{iknownhostentry} is an entry in an openssh-formatted c{known_hosts} file
...
an object that implements c{__str__()} in order to defer the work of formatting until it's converted into a c{str}
...
l{protocolnegotiationsupport} defines flags which are used to indicate the level of npn/alpn support provided by the tls backend
i am a componentized object that will be made available to internal twisted file-based dynamic web content such as
l{tunnelflags} defines more flags which are used to configure the behavior of a tunnel device
...
wrap a l{iresource}, potentially applying an encoding to the response body generated
...
l{requestnotsent} indicates that an attempt was made to issue a request but for reasons unrelated to the details of the request itself the request
an l{ireactorpluggablenameresolver} is a reactor whose name resolver can be set to a user-supplied object
common implementation of c{abortconnection}
i report results from a run of a test suite
@cvar volatile list of attribute to remove from pickling
interface for account classes implementors of this interface should consider implementing
...
this class contains the logic necessary to decide which of the available system techniques should be used to detect the open file descriptors for
superclass for classes that implement the business of producing
wrapper for the default importer i e none
an endpoint for child processes @ivar _spawnprocess a hook used for testing the spawning of child process
...
a l{keyflattener} computes keys for the things within curly braces in pep-3101-style format strings as parsed by l{string
i am a l{filepath}-like object which can wrap a zip archive as if it were a directory
...
l{_userauth} implements the client part of ssh user authentication in the convenient way a user might expect if they are familiar with the
...
an l{istreamclientendpointstringparserwithreactor} is a parser which can convert a set of string c{*args} and c{**kwargs} into an
...
the operation could not be completed because the scheduler or one of its tasks was in an invalid state
this is a factory which produces protocols
raised when a command syntax is wrong
l{iconsumer} implementations can mix this in to get c{registerproducer} and c{unregisterproducer} methods which take care of keeping track of a
...
l{sshcommandclientendpoint} exposes the command-executing functionality of ssh servers
...
an option list parser class c{optflags} and c{optparameters} are lists of available parameters
