...
structured parse results to provide multiple means of access to the parsed data - as a list (c{len results })
wraps a requests request to mimic a urllib2 request
specify the exact platform you want valid
a value in a form 'foo 1.0 '
metavar format string - evaluated as mvarfmt % metavar
base warning used by this module
abstract base class for tries
passed to the superclass
the url of the file to be downloaded (assumed to be
a locator which scrapes html pages to locate downloads for a distribution
proxymanager does not support the supplied scheme
here are some common low-level expressions that may be useful in jump-starting parser development - numeric forms (l{integers<integer>}, l{reals<real>}, l{scientific notation<sci_real>})
...
the fields to send as a list of fieldname value
a rational version
...
the url of the index. if not specified the url for pypi is
metadata provider for egg directories usage :
the maximum number of connections to save in the pool
messagepack packer usage
...
the name of the requirement that needed this
base treebuilder implementation
install packages from - pypi and other indexes using requirement specifiers
general exception during uninstallation
optional dictionary bytes or file-like object to send
raised when an existing pool gets a request for a foreign host
a required metadata is missing
if specified names are looked up in this mapping
parse a requirement
...
if specified this must be a byte string containing
...
the instance of :class metadata describing this
mapping of file objects to selector keys
the root url to use for scraping
the version of python which the release relates
optional json to send in the body of the :class request
a base class for locators - things that locate distributions
the :class response <response> object which contains a server's response to an http request
if true parsing a constraints file
the content for this response was already consumed
the marker to interpret
the pathname of the file to be uploaded
the url to connect to
raised when lock creation fails within a user-defined period of time
a prettier/less verbose help formatter for optparse
instance of pip.wheel.wheelcache
a formatcontrol object or none. used to control
...
a variation on l{literal} which matches "close" matches that is strings with at most 'n' mismatching characters
general exception during installation
the logical name of the stream used for logging only
string-like object with an associated position and various extra methods
the legacy metadata of a release
this finds packages
registers a signal handler that calls finish on sigint
the keyword arguments to pass to the event's
...
just like l{parsefatalexception}, but thrown internally when an l{errorstop<and
exception when the codepoint is not valid in the context it is used
abstract selector class a selector supports registering file objects to be monitored
this locator uses special extended metadata not available on pypi and is the basis of performant dependency resolution in distlib
...
implements a 'write()' method which on windows will strip ansi character sequences from the text and if outputting to a tty will convert them into
matches if the current position is at the end of a word and is not followed by any character in a given set of c{wordchars}
an interface to see if request should cached or not
a :class distributionpath instance to search
optional how long to wait for the server to send data
a wheel file
exception thrown by l{parserelement validate} if the grammar could be improperly recursive
the identifier of the signer of the file
an http error occurred
the url provided was somehow invalid
base class for all connection pools such as :class
...
the ideal pip parent_dir for the source_dir
“push”-based encoder
caseless version of l{keyword}
raised when an attempt is made to unlock a file someone else locked
an invalid specifier was found users should refer to pep 440
the request to send
the category to search for entries
retry configuration
...
the pathname to the file containing the
a requirement of the form 'foo 1.0 ' or perhaps
this class locates distributions in a directory tree
whether we should actually verify the certificate
provides a unicode stream of characters to the htmltokenizer
the passphrase for the signer's
failed to decode response content
a requirement had a hash specified but was not pinned to a specific version
...
a workalike for hashes used when we're missing a hash for a requirement it computes the actual hash of the requirement and raises a hashmissing
a thin wrapper around an existing file object that provides a part of its data as an individual file
small proxy class that enables transparent compression detection for the stream interface (mode 'r|*')
if specified this must be a hasher value
a string or none. if none searches for packages
base exception for errors caused within a pool
the subscriber to be added (and called when the
the name of the timeout attribute to validate. this is
a generic ip network object
this locator uses pypi's json interface it's very limited in functionality
the pathname to the file containing the
if false is returned this will contain what
cookiejar object to add the cookies to
mini parser for detecting character encoding from meta elements
this class represents and manipulates 128-bit ipv6 networks
the url to use for xml-rpc
the requirement specifying the distribution to
optional whether the ssl cert will be verified
...
optionparser options that we may update
tuple in the format name value
the files to send as a list of (fieldname filename
exception for end of file headers
the link of the sdist for which this will cache wheels
...
the same as for :func urlretrieve in the
...
base exception
the base directory to explore under
urllib3 encountered an error when trying to rewind a body
lazy loading of moved objects in six moves urllib_parse
some other thread/process is locking the file
the type of the file being uploaded. this is the
a class to copy or create scripts from source scripts or callable specifications
a string or none. this is passed directly
a dictionary of exports mapping an export category to
...
the directive to process. this should be in a format
instance of pip.download.pipsession
where still-packed .whl files should be
the path of the .dist-info or .egg-info
converter to concatenate all matching tokens to a single string
an already-installed version conflicts with the requested version
a :class metadata instance defining at least a name
an error occurred extracting a resource
a generic ip object
reresents a character encoding such as utf-8 that can be used for decoding or encoding
low-level file object supports reading and writing
matches if current position is at the end of a line within the parse string
a hash was provided for a version-control-system-based requirement but we don't have a method for hashing those
a class representing an in-package resource such as a data file this is
dict of key/values to insert into cookiejar
if true the action is logged but no file is actually
if true parsing a constraint file rather than
...
an optional environment for marker evaluation
resource finder for file system resources
base class for other exceptions
error in parsed document
a requests session
dict with 2 special properties on initiation keys that are lists sets or tuples are converted to
...
a dictionary as described in the documentation for
matches if the current position is at the beginning of a word and is not preceded by any character in a given set of c{wordchars}
represent and manipulate single ipv4 addresses
raised when a pool runs out of connections and no more are allowed
optional the proxies dictionary to apply to the request
an attempt encapsulates a call to a target function that may end as a normal return value from the function or an exception depending on what
the distribution to add
optional any user-provided ssl certificate to be trusted
unknown metadata version number
api-compatibility wrapper for python openssl's connection-class
the data to be hashed
the stream to read from. this will typically a pipe
a httplib.httpmessage or analogous carrying the headers
an invalid operation was attempted on a value that doesn't support it
a failure to verify a package against known-good hashes :cvar order an int sorting hash exception classes by difficulty of
custom option parser which updates its defaults by checking the
the name of a hash implementation supported by hashlib
a list of distributions
path or url of requirements file
the path relative to .dist-info of the resource
general exception for extract errors
the lines to get the encoding of
a token that will never match
a string
:param headers an iterable of field-value pairs
because locators look for most recent versions they
raised when a socket timeout error occurs
this is an unsupported version
optional a requests-style dictionary of proxies used on this request
the path to a directory which contains the keys
base ipv4 object
a :class resource instance
raised when passing an invalid state to a timeout
provides access to package resources in the filesystem
a value error related to the address
the pip wheel cache for passing to
exception for unavailable compression methods
list installed packages including editables
lazy loading of moved objects in six moves urllib_error
lazy loading of moved objects in six moves urllib_robotparser
whether to trust url-based (#md5=...) hashes
a hash was provided for a version-control-system-based requirement but we don't have a method for hashing those
...
instance of pip.index.packagefinder
implements an ntlm authentication version of an urllib3 connection pool
abstract subclass of c{parserelement}, for combining and post-processing parsed tokens
there was an ambiguous exception that occurred while handling your request
wrap a list of lines in an object with readline() to make configparser happy
raised when the connection to a proxy fails
custom retries logic failed
our own requests.request object
provider based on a virtual filesystem
represents one page along with its url
if there is no expires header already fall back on last-modified using the heuristic from
a string or none. this is passed directly
raised when a registry operation with the archiving
a simple cache mapping names and dist-info paths to distributions
:class request instance to prepare with this
specify the exact implementation you want valid
raised when there is an error in command-line arguments
response length doesn't match expected content-length subclass of http_client
dictionary or cookiejar object to be added
the name of the package this belongs to. none to
a case-insensitive dict-like object
convenience mixin for classes who implement a :meth urlopen method such as :class ~urllib3
a string with a dict header
the header value to unquote
a dict of algorithm names pointing to hashes we
base class that all auth implementations derive from
manage resource extraction and packages
the options to check if not supplied defaults to
matches if current position is at the end of the parse string
a hash was needed for a requirement but is absent
a list of meta extras such as :test :build and
requires all given c{parseexpression}s to be found in the given order
verify installed packages have compatible dependencies
build wheel archives for your requirements and dependencies
tuple of short opt long opt e.g ('-f', '--format')
an instance of :class metadata which describes the
the base directory where the cache should be located
...
a list of sometimes-wanted dependencies
...
the provider we are trying to replace with
the header provided was somehow invalid
exception for missing and invalid extended headers
the list of locators to search
raised when the maximum number of retries is exceeded
optional a cookiejar to add the cookies to
the version to match against this instance
a distribution member of *dists* for which we are interested
this locator finds installed distributions in a path it can be useful for
a helper command to be used for command completion
response needs to be chunked in order to read it as chunks
matches if current position is at the beginning of a line within the parse string example :
token for matching words composed of characters i{not} in a given set will include whitespace in matched characters if not listed in the provided exclusion set - see example
a metadata value is invalid
represents a dependency graph between distributions
metadata handler for standalone pkg-info files usage :
a value error related to the netmask
“push”-based decoder
an iterable of extras used to evaluate the
...
the cache_dir being used by pip
...
where still packed archives should be written to
the pathname of the directory which contains the
...
a pip.index.formatcontrol object to limit
...
cli options
requires that at least one c{parseexpression} is found
the pathnames to add. these can be relative to the base
the name of the package
the number of worker threads you want to do i/o
wraps a httplib httpmessage to mimic a urllib addinfourl
a file was opened in text mode but requests determined its binary length
used as a container for an error reason supplied in a maxretryerror
the specification which is either a valid export
the path
attaches http basic authentication to the given request object
origin description of requirements
the urllib3 response object
the path to use as a list of directories. if not specified
optional if string path to ssl client cert file .pem
dictionary lookup object
a list of string versions of the form ["33", "32"],
raised when virtualenv or a command is not found
the pathname to add. this can be relative to the base
a list of strings or none. this is passed directly
the requirement
provide a xmlrpclib transport implementation via a pipsession
optional dictionary bytes or file-like object to send in the body of the :class request
provides a unicode stream of characters to the htmltokenizer
...
a dictionary of options controlling script generation
the name of an event
the timeout in seconds to be applied to requests
the distribution to compare with. to be equal to one
the provider we're trying to replace
this is normally the :class distributionpath
compatibility class is a cookielib cookiejar but exposes a dict
represent and manipulate single ipv6 addresses
demonstrate sql-based locking
the :class preparedrequest <preparedrequest> used to generate the response
specify the exact abi you want valid
...
the string to parse. e.g. foo-2.1
block when no free connections are available
abstract base level parser element class
if true, allow pre-release versions to be
this class takes care of tokenizing html
the pathname to the file to be signed
token to match a specified string ignoring case of letters
raised when there is something wrong with a given url input
the installrequirement that provoked this error with
...
the positional arguments to pass to the event's
the :class resource being cached
...
the path of the resource in the cache
provides information about a linux distribution
whether to append or prepend the subscriber to an
wraps a stream such as stdout acting as a transparent proxy for all attribute access apart from method 'write()', which is delegated to our
the link the string came from for logging on failure
extra keyword arguments used to configure the proxy manager
base class for errors arising from attempts to release the lock
unsupported python version according to requires-python package metadata
...
the content of the requirements file
...
the directive to validate
requests encountered an error when trying to rewind a body
timeout configuration
create a six moves urllib namespace that resembles the python 3 namespace
raised when something unexpected happens mid-request/response
same as :class httpconnectionpool, but https
the pathname where the downloaded file is to be
the list to append the read lines to
...
the optionparser options to update
base pip exception
a class implementing a cache for resources that need to live in the file system e
lock access to a file using atomic property of link 2
exception for empty headers
attempt to read or write metadata fields that are conflictual
the built-in http adapter for urllib3
the context used for name lookup
...
a list of optional components being requested
body should be httplib httpresponse like (have an fp attribute which
datastructure for representing an http url used as a return value for
i am a wrapper class for the pyopenssl context object i am responsible
if specified only entries with that name are returned
...
if true this instance will look for and return legacy
unsupported wheel
handles the loading of configuration files and providing an interface to accessing data within them
the header value provided was somehow invalid
distribution doesn't have an "extra feature" of the given name
map each requirement to the extras that demanded it
invalid wheel filename
the tarfile class provides an interface to tar archives
type of error handling. see :func codecs.register
