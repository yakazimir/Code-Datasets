...
pixelates the input according to its :attr ~pixelateeffect
:class recycledatamodelbehavior is the base class for the models that describes and provides the data for the
the main layout that is used to house the entire tabbedpanel strip including the blank areas in case the tabs don't cover the entire
...
a settings widget that displays settings panels with a sidebar to switch between them
...
keyboard interface that is returned by :meth windowbase
...
the builder is responsible for creating a :class parser for parsing a kv file merging the results into its internal rules templates etc
class for creating a slider widget
abstract camera widget class
recalibrate the inputs
metaclass to automatically register new widgets for the :class ~kivy
...
class used for tbe graphical representation of a :class kivy
spinner class see module documentation for more information
implementation of a :class filechoosercontroller which supports switching between multiple synced layout views
...
accordionitem class that must be used in conjunction with the :class accordion class
...
the :class selectableview mixin is used with list items and other classes that are to be instantiated in a list view or other classes
...
:class recognizer provides a gesture database with matching facilities
...
a settings widget that displays a single settings panel with *no* close button
actionitem class an abstract class for all actionbar widgets to create
base class for all spelling providers
context of a module you can access to the config with self
...
each :class knspace instance is a namespace that stores the named kivy objects associated with this namespace
actioncheck class see module documentation for more information
inputpostprocignorelist is a post-processor which removes touches in the ignore list
see module documentation
mixes the color channels of the input according to the order property
...
the :class layoutselectionbehavior can be combined with :class recyclelayoutmanagerbehavior to allow its derived classes
...
transition where the new screen rises from the screen centre becoming larger and changing from transparent to opaque until it
...
an :class effectbase with additional behavior to easily set and update uniform variables in your shader
motionevent factory is a class that registers all availables input factories
kinetic effect class see module documentation for more information
codeinput class used for displaying highlighted code
inputpostprocretaintouch is a post-processor to delay the 'up' event of a touch to reuse it under certains conditions
...
this class is used to contruct panel settings for use with a :class settings instance or subclass
...
a implementation of :class recycledatamodelbehavior that keeps the data in a indexable list
abstract class that represents an input event touch or non-touch
inputpostproctripletap is a post-processor to check if a touch is a triple tap or not
file chooser layout using an icon view
widget empty drawing 10000 widget + 1 root
class for implementing a file system view that can be used with the :class filechooser <filechooser>
...
transition class that uses a shader for animating the transition between 2 screens
...
represents a set of unistroke paths of user input ie data to be matched against a :class unistroketemplate object using the protractor algorithm
...
transitionbase is used to animate 2 screens within the :class screenmanager
...
a settings widget that displays settings panels as pages in a :class ~kivy
...
console interface this widget is created by create_console(), when the module is loaded
...
widget with the ability to apply a series of graphical effects to its children
...
a recyclelayoutmanagerbehavior is responsible for positioning views into the :attr recycleview
...
a base class for adapters interfacing with lists dictionaries or other collection type data adding selection view creation and management
...
inheriting from this class allows naming of the inherited objects which are then added to the associated namespace :attr knspace and accessible
base handler that catches exceptions in :func runtouchapp
markup text label
the content widget used by :class settingswithtabbedpanel it
a settings interface that displays a spinner at the top for switching between panels
...
implementation of camerabase using gstreamer :parameters
...
blurs the input vertically with the width given by :attr ~verticalblureffect
...
transition where the new screen 'falls' from the screen centre becoming smaller and more transparent until it disappears and
...
recycleviewbehavior provides a behavioral model upon which the :class recycleview is built
this is a convenience class that provides a header of type tabbedpanelheader and links it with the content automatically
...
put the result of a method which uses this non-data descriptor decorator in the instance dict after the first call effectively replacing the
...
blurs the input horizontally with the width given by :attr ~horizontalblureffect
