reemplaza los valores de array1 con los mismos valores de todos los siguientes arrays
lee la información de la siguiente tupla de relation la hace la tupla actual y la devuelve como una matriz asociativa en tuple
...
fija el modo para error_reporting
...
devuelve la representación textual de un país a partir de una constante de la clase gender
...
establece la imagen usada como mascara de la imagen
mover el iterador al siguiente object en el almacenamiento
pregunta al servidor para matar un subproceso de mysql
...
devuelve el nodo del atributo con nombre name para el elemento actual
decrementa numéricamente el valor según el value
define la escala horizontal para la página
...
comprueba si un punto de código tiene la propiedad de unicode white_space
...
realiza un relleno cuyo color de borde está definido por border
...
esta función interpreta una cadena xml en dos estructuras paralelas de array una index que contiene punteros a la ubicación de los valores adecuados en el array values
...
vuelca el búfer de salida lo devuelve como una cadena de caracteres y deshabilita el almacenamiento en el búfer de salida
devuelve el id efectivo de grupo efectivo numérico del proceso actual
devuelve información sobre enlaces de mysql abiertos y almacenados en caché
elimina el array de claves dado por keys del servidor
devuelve el estado online/offline del servidor
coincidir string regexiterator::current() o regexiterator::key() si la flag regexiterator::use_key está establecida) con la expresión regular
obtiene el número de la última excepción lanzada o la razón del fallo de la llamada a la función
añade ficheros de un directorio que corresponde con el patrón global pattern
crea un objeto de estado de gráficos sujeto a varias opciones
...
devuelve los atributos en una entrada
añade o elimina un perfil icc iptc o genérico de una imagen
un método conveniente que escala una imagen proporcionalmente a la mitad de su tamaño original
finalizar el texto actual del objeto
esta función elimina un hijo de la lista de hijos
reemplaza los colores en una imagen con una paleta de colores
especifica un prefijo de cadena con el que limitar los términos a los que hacer una faceta
establece una función de usuarioerror_handler para manejar los errores de un script
devuelve el formato de una imagen en particular de una secuencia
hace que el fichero nominado por path sea truncado al tamaño de exactamente length bytes
...
establece la posición de la siguiente salida de texto
inserta un nuevo registro en la base de datos
escribe el texto text dado en una imagen usando fuentes truetype
obtiene varios parámetros que se establecieron directamente con la función ps_set_parameter() o indirectamente por alguna de las otras funciones
devuelve el id de la fila de la sentencia insert más reciente realizada en la base de datos
inicia una nueva definición de plantilla
comprueba si la propiedad es protegida
aplica una codificación binaria a data por lo que se puede devolver con serguridad desde consultas ya que la api libsqlite subyacente no es sergura a nivel binario
envía la acción con parámetros al servicio a través de un proxy y obtiene los valores de forma sincronizada
computa el símbolo legendre de a y p
establece el canal mate de la imagen
establece el estilo de borde de todos los tipos de anotaciones
...
devuelve el número de protocolo asociado con el name de protocolo indicado por /etc/protocols
incrementa el valor según el value
este método devuelve un array con un número máximo de num_cursors cursores
...
modificar las entradas existentes en el directorio de ldap
establece varios parámetros
obtiene el valor del campo solicitado
devuelve la lista de clases que pertenecen a una extensión
obtiene la matriz 2d de valores empleados en este núcleo («kernel»)
...
establece el color de contorno actual a un valor de gris entre 0 y 1 inclusive
...
obtiene el contenido de algún parámetro pdflib de tipo string
reinicia los gráficos de vectores
trunca el archivo a size bytes
devuelve los datos del campo dado por field de la fila actual obtenida mediante oci_fetch()
obtiene el iterador interno que fué pasado a norewinditerator
...
verifica si la key especificada existe en la base de datos
asigna la base de datos activa para el identificador de enlace especificado
mueve el apuntador al índice de campo especificado
comprueba si esta clase está definida en el espacio de nombres especificado
...
esta función devuelve todos los controladores de pdo actualmente disponibles que se pueden usar en el parámetro dsn de pdo::__construct()
establece varios valores
crea una red neuronal de retropropagación estándar completamente conectada
consolida una versión de un documento
elimina un elemento del conjunto de sondeo
...
intenta crear una nueva base de datos en el servidor msql
establece un nuevo punto inicial del sistema de coordenadas
recupera el objeto controlador actual
devuelve los comentarios de documentación
devuelve un array con el cliente protocolo y la versión del servidor
...
revertir todos los cambios globales en el archivo
busca la posición en unidades de grafema de la primera ocurrencia de una cadena
comprueba si la cola contiene más nodos
devuelve un identificador de imagen que representa la imagen obtenida desde el nombre de fichero dado
realiza una operación comprobar y establecer cas por sus siglas en inglés), de forma que el ítem será guardado solamente si ningún otro cliente lo actualizó desde la última vez que este cliente lo obtuvo
retorna un nuevo objeto gearmantask
imprime una imagen gd en filename
copia los mensajes de correo especificados por msglist al buzón especificado
fuerza la escritura de todos los datos del búfer bzip2 para el puntero del fichero bz
coloca una imagen o plantilla en la página sujetas a varias opciones
comprueba si una variable con la key existe en la caché de usuario o no
devuelve el input repetido multiplier veces
crea un grupo de peticiones
borra una porción especificada de los datos internos de un lob comenzando en el índice dado por offset
procesa una petición soap llama a las funciones necesarias y envía una respuesta
...
recupera los hijos filtrados de el iterador interno
...
lee del puntero de fichero bzip2 pasado como parámetro
...
establece el valor de algún parámetro pdflib con el tipo numérico
...
la función cubrid_disconnect() cierra el gestor de conexión y se desconecta del servidor
deja de alojar el archivo o directorio en server_path
la función socket_recvfrom() recibe len bytes de información en buf desde name en el puerto port si el socket no es del tipo af_unix usando socket
clona los atributos de un objeto
deshace una transacción sin cerrarla
reemplazará la clave primaria actual de relation con la combinación de dominios especificados por domlist
compara array1 con array2 y devuelve la diferencia
...
obtiene el primer par clave-valor del map indicado en el domain dado
...
este método elimina una entrada de un hash y devuelve si la entrada se eliminó o no
...
serializa las variables pasadas y agrega el resultado al paquete determinado
invierte los colores en la imagen de referencia
este método se puede usar para leer el contenido de una archivo phar externo
...
devuelve el color descrito por el objeto gmagickpixel como un matriz
...
establece una instancia de una base de datos para path con mode usando handler
envía el estado de fallo para este trabajo indicando que el trabajo ha fallado de forma conocida al contrario que un fallo debido una excepción)
...
calcula el seno para cada valor de real y devuelve el array resultante
convierte value a una variante y lo asigna al objeto variant ; no se crean un nuevo objeto variante y el valor antiguo de variant es liberado
esta función devuelve el número de parámetros de la sentencia preparada especificada en query
...
devuelve el tamaño del blob recibido
escala una imagen a las dimensiones deseadas con un filtro
divide cada valor de real0 por el valor correspondiente de real1 y devuelve el array resultante
...
devuelve un arreglo indexado por el nombre y por la posición de la columna representando una fila de un bloque de resultados
constructs eventhttprequest object
hace un diff unificado que contiene diferencias entre la cadena old_data y la cadena new_data y devuelve esta
combina los elementos de uno o más arrays juntándolos de modo que los valores de uno se anexan al final del anterior
devuelve la fecha unix actual con microsegundos
...
imprime el texto text en la siguiente línea
obtiene el máximo de profundidad permitido
mueve el número de bytes especificados desde un buffer fuente al final del buffer actual
obtener la ruta del directorio para un backend dado
...
genera una desviación aleatoria desde la distribución f ratio de varianza con dfn grados de libertad en el numerador y dfd grados de libertad en el denominador
rebobina todas las instancias de iterador adjuntadas
coloca una página pdf y la escala
establece el número máximo de conexiones que podrán crear los nuevos agrupamientos
esta función devuelve el nombre de la tabla del campo especificado
...
registra una llamada rpc aunque no la envía inmediatamente
multiplicación de a por b y devuelve el resultado
obtener el número de campos en un archivo de base de datos
la clase cairofontface representa una fuente en particular en una inclinación en particular entre otras características pero no la transformación o el tamaño
devuelve el número de peticiones excepto las no manejadas
es una combinación de fbsql_fetch_row() y fbsql_fetch_assoc()
...
establece varios parámetros que son utilizados por muchas funciones
destruye una sesión
calcula el número de caracteres los cuales pueden ser incluidos dentro del ancho especificado
obtiene el valor de algún parámetro pdflib de tipo numérico
elimina uno de los campos usados para remarcación
si la palabra está correctamente escrita se devuelve true de otro modo se devuelve false
...
la función cubrid_column_names() se usa para obtener los nombres de las columnas del resutado de la consulta usando req_identifier
crea el especificado directory en el servidor ftp
divide n por d usando el rápido algoritmo de división exacta
obtiene el tamaño de la secuencia temporal
establece el estilo para ser usado por todas las funciones de dibujo de líneas como imageline() y imagepolygon() cuando se dibuja con el color especial img_color_styled o líneas de imágenes con el color img_color_styledbrushed
...
cierra el trazado lo rellena y lo contornea
busca la cadena haystack por una char_list
obtiene la o las versiones de una o más etiquetas id3 en un fichero mp3
devuelve el ancho del contorno usado para dibular perfiles del objeto
contecta el socket a un extremo remoto
la descripción de la función va aquí
establece las unidades de resolución de la imagen
establece un patrón de discontinuidad definido por una lista de opciones
...
obtiene la calidad de compresión de la imagen actual
establece el estilo de borde discontinuo para todos los tipos de anotaciones
devuelve el último mensaje de advertencia
...
resta b con a y devuelve el resultado
ensamblar un url
...
cambia el tamaño de una imagen a las dimensiones dadas y elimina cualquier perfil asociado
...
esta función crea una nueva instancia de la clase domelement
elimina una entrada del mapa
revertir todos los cambios hechos a una entrada en el índice dado
ejecuta una instancia de la clase yaf_application permite a esta instancia aceptar una petición y enviar dicha petición despacha el controlador/acción e interpreta la respuesta
devuelve el resto en punto flotante de dividir el dividendo x por el divisor y 
activa la verificación del certificado y servidor ssl activado por omisión)
...
fuerza a terminar al hilo referenciado
la función divide a n por d
comprueba si un atributo es editable
tiene por objeto usarse como una función de llamada de retorno para ob_start() para facilitar en envío de datos codificados con gz a los navegadores web que admiten la compresión de páginas web
abre un documento pdf basado en disco o virtual y lo prepara para un uso posterior
devuelve información acerca sobre determinado archivo de base de datos rrd
obtiene la cadena de la propiedad fc_nfkc_closure de un carácter
esta función convierte un número gmp an un integer nativo de php
...
anexa una línea recta del punto corriente al inicio del punto de la ruta
añade el servidor especificado a la provisión de servidores
la función obtiene información acerca de la entrada definida por su índice
establece la cadena dada como codificación
la descripción de la función va aquí
...
inicializa la interfaz de newt
...
es el único método que debería implementar un enrutamiento personalizado
obtiene un identificador único prefijado basado en la hora actual en microsegundos
aviso: se cargará el fichero en memoria
suaviza los contornos de una imagen mientras que se preserva todavía la información del borde
obtiene el valor del tiempo de espera para la retrollamada de inactividad
esta función prepara para una solicitud z39.50 scan request en la conexión yaz especificada
define la apariencia de la página para ajustarse al ancho de la delimitación de la caja y establece la posición superior de la página para el valor de top
este método activa la capacidad de usar funciones php como funciones xslt dentro de las hojas de estilo xsl
define el patrón de guía para la página
establece una conexión a un servidor msql
construye el árbol de of sdo_dataobjects desde la dirección dada al documento de instancia xml
añade un perfil nominado al objeto gmagick
dibuja un arco de circunferencia con centro en las coordenadas dadas
comprobar si la posición actual de directoryiterator es un fichero válido
intenta cambiar el grupo del archivo filename a group
devuelve código de error de la última solicitud snmp
obtiene la posición basada en cero de el iterator interno
genera el polinomio de comprobación de redundancia cíclica de 32 bits de longitud de str
devuelve el color del índice del mapa de colores especficado
crea un paquete wddx a partir de un simple valor
devuelve información sobre el descriptor de fichero proporcionado
simula una fotografía polaroid
obtiene el ancho total del texto número de caracteres numero de palabras y número de espacios
...
la función cubrid_load_from_glo() se usa para leer una información desde una instancia glo y guardarla en el fichero indicado
devuelve la siguiente clave de la base de datos y avanza el apuntador de clave interno
igual que datetime::sub() excepto que trabaja con datetimeimmutable
...
obtiene la densidad de tinta total de la imagen
añade una nueva imagen al objeto gmagick desde la posición actual del objeto de origen
establece el iterador de imagick a la última imagen
...
deja de registrar una variable global name de la sesión actual
igual que datetime::setdate() excepto que trabaja con datetimeimmutable
lee la información de un archivo bz comprimido y crear clases desde el código de bytes
la función geoip_country_code3_by_name() devolverá la abreviación de tres letras del país correspondientes al sitio web o dirección ip
...
convertir una cadena 8bit a una cadena quoted-printable de acuerdo a su definición en rfc2045 sección 6.7)
establece el máximo de épocas de salida
busca la primera ocurrencia de needle en haystack y devuelve la porción de haystack
funciona del mismo modo que eventbuffer::read() pero no vacía ningún dato del buffer
añade una nueva página al documento y especifica varias opciones
esta función url codifica el string dado de acuerdo con rfc 3986
retorna el identificador único de una tarea
crea un nuevo objeto domdocument
la función stream_select() acepta matrices de flujos y espera a que cambien su estado
intenta borrar una base de datos del servidor msql
...
cierra un elemento de estructura u otro elemento de contenido
comprueba si existe la constante dada y está definida
intenta crear una nueva base de datos en el servidor asociado con el identificador de enlace especificado
este método se emplea para establecer una clase propia que será utilizada cuando se invoque a splfileinfo::openfile()
obtiene la información de estado sobre el buzón dado por mailbox
construye un objeto zmqsocket
...
recupera las estadísticas sobre el proceso con el id de proceso pid
acepta matrices de sockets y las espera para cambiar el estado
obtiene el propósito de renderización de la imagen
...
ordena las entradas por valores usando un algoritmo de ordenado natural sensible a mayúsculas
...
desinicializa la interfaz de newt
escala la entradas y salidas de los datos de entrenamiento al rango especificado
es la versión insensible a mayúsculas-minúsculas de natsort()
crea y devuelve un recurso socket también referido como un extremo de comunicación
...
inicia el alojamiento de local_path a server_path
retorna el estado para la tarea en ejecución
hace lo mismo que la función readlink de c
...
esta es una interfaz de getrusage 2 
establece el color del borde de todos los tipos de anotaciones
...
la biblioteca sqlite se puede compilar en los modos compatibles iso-8859-1 o utf-8
este método no es muy útil para el uso normal de mongodb
coloca una imagen cargada anteriormente en la página
...
este método actualiza una entrada con un nuevo valor y devuelve si la entrada fue actualizada
...
el límite de candidatas es un límite de cuánto podrían ser entrenadas las neuronas candidatas
retorna el identificador único para este trabajo
...
establece el ancho actual de línea
abre una conexión explícita ssl-ftp al host especificado
...
obtiene o asigna el nombre de usuario usado en la conexión
define la apariencia de la página para ajustarse a la delimitación de la caja ubicada dentro de la página en la ventana
cambia el valor de píxeles individuales basados en la instensidad de cada píxel comparado con el umbral
...
retorna el valor del código de error errno en caso de que se obtenga un gearman_errno
codifica la cadena dada de acuerdo al rfc 3986
comprime la cadena dada y la devuelve como datos codificados con bzip2
devuelve un string que describe el último error asociado con req
asigna un valor a el offset especificado
se llama cuando hay un siguiente elemento disponible
establece el color de relleno y contorno actual a un valor de gris entre 0 y 1 inclusive
...
dibuja una línea desde el punto actual a otro punto
...
cierra el enlace con una base de datos interbase que esté asociado con un id de conexión devuelto por ibase_connect()
obtiene la cuenta total
convierte el string dado por input con campos en formato csv y devuelve un array que contiene los campos leídos
comprueba si el fichero que el objecto splfileinfo hace referencia existe es un fichero regular
añade una tarea en segundo plano para ser ejecutada en paralelo junto con otras tareas
obtiene un array de métodos de una clase
define propietario y contraseñas de usuario para el documento
esta función es un alias de: soapfault::soapfault()
convierte el string codificado en internal_encoding a output_encoding
la función cubrid_num_cols() se usa para obtener el número de columnas del resultado de la consulta
crea un objeto blob
envía una única consulta no hay soporte para múltiples consultas) a la base de datos actualmente activa en el servidor asociado con el identificador de enlace especificado por link_identifier
convierte un gestor de conexión de mysql a uno de mysqlnd
dibuja una elipse con centro en las coordenadas especificadas
abre un objeto slob
exporta píxeles de imagen en bruto a una matriz
activa la solititud de información detallada útil para depurar la información de depuración es almacenada en el miembro debuginfo
...
establece la opacidad a usar cuando de dibuja untilizando el color de relleno o la textura de relleno
une los elementos de uno o más arrays de modo tal que los valores de uno sean añadidos al final del anterior
convierte las referencias a string numéricas del string str en un bloque de caracteres especificado
...
devuelve la cadena del uri del elemento raíz
añade un iterador
añade una nueva imagen al objeto imagick desde la posición actual del objeto de origen
devuelve el nombre de protocolo asociado con el numero de procotolo como lo indica /etc/protocols
devuelve una cadena con el primer caracter str en máyusculas si el caracter es alfabético
devuelve las banderas de campo del campo especificado
...
carga una fuente postscript type 1 desde el fichero filename
devuelve la geometría de la página asociada con la imagen en un array con las claves width height x e y
esta función permite juguetear con las opciones de odbc para una conexión o resultado de consulta en particular
obtiene el campo de bits de status de la conexión
exporta la llave fingerprint
...
la función cubrid_connect_with_url() se usa para establecer el entorno de conexión a su servidor usando la información de conexión pasada con un argumento de cadena de url
...
devuelve el nombre de una entrada usando el nombre de la entrada
...
la fracción de cambio de salida en cascada es un número entre 0 y 1 que determina lo grande que debería ser el cambio del valor de una fracción de fann_get_mse() en fann_get_cascade_output_stagnation_epochs() durante el entrenamiento de las conexiones de salida para que el entrenamiento no se estanque
determina si el elemento actual directoryiterator se puede leer
transforma el nodo origen a un string aplicando la hoja de estilos dada por el método xsltprocessor::importstylesheet()
decrementa un valor numérico de un ítem por el offset especificado
devuelve un identificador de imagen que representa la imagen obtenida desde el nombre de fichero dado
busca una fuente y la prepara para un uso posterior
los ficheros se almacenan en dos colecciones
devuelve el código de error de la última llamada a una función fdf
...
reinica la sentencia preparada al estado previo a su ejecución
calcula los valores singulares de la matriz a
...
esta es la forma más limpia de obtener una base de datos
esta función es un alias de: soapparam::soapparam()
devuelve la escala de un campo
cambia el valor del color de cualquier píxel que coincida con el objetivo y esté en la zona inmediata
crea un nuevo objeto domattr
libera una referencia previmente adquirida potencialmente volviendo a convertir una referencia fuerte en una débil
este método no está hecho para ser llamado directamente
...
añade un encabezado soap a ser devuelto en la respuesta de la petición actual
abrirá el archivo de relación file localmente para un acceso rápido sin ningún overhead de cliente/servidor
retorna el identificador única para esta tarea
crea un nuevo botón
...
crea un archivo que contiene los pares clave-valor especificados
devuelve el género de una etiqueta id3
entrena una época con los datos de entrenamiento almacenados en data
recupera el objeto petición actual
devuelve la ruta completa del archivo phar en ejecución
es similar a memcached::set() pero la operación fallará si la clave dada por key no existe en el servidor
obtiene el nombre de fichero que contiene una función definida por el usuario
averigua si la imagen image es una imagen de color verdadero
...
recupera las estadísticas sobre todos los procesos en ejecución
obtener la clave actual
envía la infomración data especificada a través del socket
exportaciones a la reflexión
se llama cuando la iteración termina o cuando recursivetreeiterator::valid() retorne false
devuelve el arco tangente de arg en radianes
este método implementa un algoritmo que ordena string alfanuméricas en la forma en que un humano seguiría manteniendo la asociación clave/valor
verifica si hay más conjuntos de resultados disponibles así como permitir el acceso al siguiente conjunto de resultados mediante odbc_fetch_array() odbc_fetch_row() odbc_result() etc
establece un parámetro de autenticación en la clave key con el valor value
asocia un artefacto con una imagen
desencripta y verifica un texto dado y retorna información sobre la firma
añade una etiqueta a una imagen
determina si el carácter esepecficado es permisible como el incio de un identificador de java
...
devuelve true si la imagen tiene un canal mate en caso contrario false
...
establece una conexión a un servidor sybase
convierte de un juego de caracteres cirílico a otro juego de caracteres cirílico
la función cubrid_error_msg() se usa para obtener el mensaje de error que ocurrió durante la utilización de la api cubrid
cerrar la conexión con el servidor imap cyrus
establece el valor de una propiedad
habilita las codificaciones chinas simplificadas
...
establece el color descrito por el objeto gmagickpixel con una cadena p.ej
comprueba si el índice index solicitado existe
establece el valor de retorno para este trabajo indica cómo se ha completado el trabajo
renderiza el objeto imagickdraw a la imagen actual
establece el timestamp de oauth para las subsecuentes peticiones
devuelve una cadena que contiene una representación octal del argumento numero dado
...
no se debería llamar directamente
establece una función a ser llamada en intervalos regulares
obtiene los flags del campo especificado en un resultado
aplica la función definida por el usuario callback a cada elemento del array
...
activa un objeto de estado de gráficos
...
obtiene tlos pulsos por segundo de la imagen
...
habilita el manejo de información de columna binaria
...
convierte a texto
se usa para cerrar todos los blob/clob devueltos desde cubrid_lob_get()
activa o desactiva el modo pasivo
define el color de relleno para la página
cierra el archivo zip archivo dado
devuelve una copia exacta de un resource de datos de entrenamiento de fann
...
obtendrá el valor la etiqueta del encabezado del fichero rpm determinado y la devolverá
...
escribe una secuencia de imágenes a un gestor de archivo abierto
...
obtiene el grupo del fichero
obtiene los colores de un índice especificado
trae filas de un identificador de resultado que tuvo la bandera dbx_result_unbuffered establecida
...
devuelve la cadena álbum de una etiqueta id3
establece el número máximo de términos a devolver
revierte todas las sentencias pendientes de la conexión
rellena el campo de información de un documento key con value
establece un nuevo valor de expiración para la clave dada
inicia el bucle principal en el servidor raíz
quita un value en la parte superior de la cola
es similar a mssql_fetch_array() con la única diferencia de que se devuelve un objeto en vez de un array
...
obtiene o asigna el password usado en una conexión
advertencia esta función no está documentada actualmente solamente se encuentra disponible la lista de parámetros
lee hasta length bytes desde el descriptor de fichero fd empezando en offset
...
crea un objeto grande y devuelve el oid del mismo
devuelve el valor apuntado por un cierto objeto
crea una instancia detenida del testigo evprepare
crea un string corrompido
construye un nuevo recursivecachingiterator que consiste en pasar un iterator
finaliza el trazado actual sin rellenarlo o contornearlo
recupera el valor boost de un campo en particular
divide a con b y devuelve el entero resultante
libera una consulta preparada
obteber las estadísticas de una tabla y sus índices
busca un nombre de color de impresión incorporado o crea un color de impresión con nombre desde el color de relleno actual
obtener el número de capas de la red neuronal
agrega dos números
retorna si un iterador puede ser creado para la entrada actual
elimina una entrada en particular en un directorio de ldap
obtiene todas las cabeceras de respuesta nsapi
la descripción de la función va aquí
prepara una sentencia sql para su ejecución y devuelve un objeto de la clase sqlite3stmt
devuelve la frecuencia del cronómetro en pulsos por segundo
...
la función passthru() es parecida a la función exec() que ejecuta un command
la descripción de la función va aquí
...
devuelve el código de error de la llamada más reciente a una función
esta es una función avanzada y no debería usarse a menos que se sepa lo que se está haciendo
es similar a pclose() excepto que este solo funciona con procesos abiertos mediante proc_open()
disminuye el valor asociado a la key por 1 o como se especifica por dec_by
devuelve todos los campos de ordenación
en orden de preferencia esta función devuelve la zona horaria predeternimada mediante: leer la zona horaria usando la función date_default_timezone_set() si hay alguna sólo antes de php 5.4.0: leer la variable de entorno tz si no está vacía) leer el valor de la opción ini date.timezone si está establecida) sólo antes de php 5.4.0: preguntar al sistema operativo siempre que el so lo soporte y lo permita 
asigna el locking y isolation de la transacción
define el color de trazo para la página
la función geoip_isp_by_name() devolverá el nombre del proveedor de servicios de internet isp que tenga asignado una dirección ip
computa la diferencia de los arrays usando una función tipo llamada de retorno para la comparación de datos
se llama cuando la iteración finaliza cuando recursiveiteratoriterator::valid() retorne false
define un objecto específico como controlador para las peticiones soap en lugar de solo una clase como en soapserver::setclass()
cambia el directorio actual al especificado
establece la combinación de entrelazado de la imagen
calcula la intersección de arrays con una comprobación de índices adicional compara la información y los índices mediante funciones de retrollamada por separado
esta función fuerza una escritura de todas las salidas del buffer a un recurso apuntado por el archivo handle
carga los datos desde un puntero de archivo a un archivo remoto en el servidor ftp
lee desde una entrada de directorio abierta
devuelve el número de error estándar devuelto por el último comando ldap
esta función cierra el recurso abierto por finfo_open()
la función cubrid_lob2_tell64() se usa para indicar la posición del cursor de un objeto lob
divide un string en una matriz mediante una expresión regular
añade un nombre de glifo y/o un valor unicode a una codificación hecha a medida
habilita las codificaciones japonesas
...
establece la propiedad tamaño del punto de objeto
esta función consulta el repositorio url y devuelve una lista de ficheros y directorios opcionalmente de una revisión específica
envia la query a la base de datos activa en el servidor
esta función libera una clave privada creada por openssl_pkey_new()
devuelve el número de linea final
...
la funcionalidad de memcached::addbykey() es equivalente a la de memcached::add() con la excepción de que server_key se puede utilizar para que la clave dada por key corresponda a un servidor específico
esta función calcula la solución del sistema de ecuaciones lineales con una matriz cuadrada a y múltipes lados derechos b
...
borra todos los mensajes marcados para eliminación por imap_delete() imap_mail_move() o imap_setflag_full()
ordena a phar interceptar fopen() readfile() file_get_contents() opendir() y todas las funciones relacionadas con estadísticas
comprueba si el parámetro es pasado por referencia
devuelve todas las estadísticas de los campos
obtiene el último acceso del elemento actual directoryiterator
útil para controlar los errores acaecidos en los métodos estáticos cuando no hay un objeto para obtener el código de error de ellos
obtiene el número total de éxitos de la caché
quita las barras de un string con comillas escapadas
la base de datos envía respuestas en lotes de documentos de hasta 4 mb de documentos por repuesta
devuelve una fila de datos del resultado asociado al identificador de resultado result
debe ser usado para reemplazar el valor de un valor existente en una key determinada
esta función le permite sobrescribir el dominio actual en una búsqueda en un mensaje único
aplica un filtro digital que mejora la calidad de una imagen con ruido
define la apariencia de la página para ajustarse al alto de la ventana
dibuja una línea entre dos puntos dados
comprueba si la función reflejada es una closure
usado para obtener el valor de una propiedad
verifica si hay eventos fam pendientes
establece los parámetros de la cookie definidos en el archivo php.ini
...
cierra el archivo pdf generado y aplica varias opciones
devuelve el número de error de la última función cubrid
carga fuente type1 de el fichero dado y la registra en el documento pdf
detiene la ejecución del compilador
es una versión extendida de sybase_fetch_row()
desplaza el mapa de colores de una imagen por el número de posiciones dados
...
examina un certificado para ver si se puede usar para el propósito purpose especificado
devuelve una array de errores de la llamada a la función de mysqli más reciente que puede tener éxito o no
...
yaf_route_simple obtendrá la información de la ruta desde una cadena de consulta
...
ejecuta una tarea de prioridad alta y retorna un string con el resultado
...
establece el indicador de posición para el apuntador al archivo dado en el desplazamiento de bytes fijado en el flujo del archivo
...
define el espaciado de carácter de la página
consigna la transacción abierta actualmente haciendo que todos los cambios hechos a la base de datos sean permanentes
la llamada al método disconnect method en un objeto samconnection desconecta el script php de un servidor de mensajería
devuelve el i-nodo de el cambio de tiempo para el fichero
detiene una base de datos en un servidor local o remoto
obtener un manejador de fichero para la entrada definido por su nombre
se usa para escapar ciertos caracteres de un string antes de eviarlo al sevidor de la base de datos
indica si un atributo en el espacio de nombres namespaceuri llamado localname existe como miembro del elemento
renombra un archivo o un directorio en el servidor ftp
...
inicia un nuevo paquete wddx para adición incremental de variables
habilita la incorporación de fuentes japonesas
recupera la variable server
chequea si está activa o no la conexión con el servidor
la monitorización se detiene si toma más de nseconds segundos
crea un núcleo («kernel») desde una matriz 2d de valore
desactiva un evento específico que se encuentra en el buffer
devuelve el ancho de la imagen
obtiene la posición actual para la impresión del texto
...
devuelve el nombre de la tabla que contiene el campo especificado
guarda los datos del documento en una secuencia temporal
establece una conexión con un servidor informix
cierra el descriptor que se utiliza para escribir en el registro del sistema
la función geoip_id_by_name() devolverá el tipo de conexión a internet correspondiente a un sitio web o a una dirección ip
devuelve el número de error de cualquier error bzip2 devuelto por el puntero del fichero dado
devuelve un identificador de imagen que representa la imagen obtenida desde el nombre de fichero dado
ejecuta el comando ftp list y devuelve el resultado como un array
mueve todos los datos del buffer indicado en el parámetro buf al final del actual eventbuffer
renderiza todos los comandos de dibujo precedentes
...
devuelve el valor de variant redondeado a decimals lugares decimales
establece la base de datos activa actual en el servidor asociado con el identificador de enlace especificado
...
obtiene el tamaño de un objeto grande
...
la descripción de la función va aquí
una función de tipo callable usada por la opción de contexto log_cmd_insert al insertar un documento
abre una base de datos sqlite 3
incrementa artificialmente la carga tomanto delay segundos para ejecutarse
...
copia los datos desde la tabla al fichero especificado por filename empleanto delimiter como delimitador de campos y la lista de campos fields
añade un servidor de trabajo a este trabajador
llama a una función o método definido por el usuario dado por el parámetro function
...
esta función crea una nueva instancia de la clase domtext
cierra el manejador de db_handle dado
devuelve el tipo del fichero dado
valida el documento basado en su dtd
inicia una capa para la subsiguiente impresión en la página
la descripción de la función va aquí
devuelve información interna de donde se lanzó la excepción
obtiene el valor del canal alfa de la imagen
...
esta función crea una nueva instancia de la clase domattr
anexar una curva bezier a la ruta actual
