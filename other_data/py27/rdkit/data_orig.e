...
this is used to represent decision trees _dectreenode_s are simultaneously the roots and branches of decision trees
a composite model using bayesian statistics in the decision proxy **notes**
base class for nodes which filter their input assumptions
we must at minimum support forward iteration
a composite model **notes**
a data set for holding general data floats ints and strings **note**
important attributes - mol the molecules whose signature is being worked with
jjk 02/01/00
...
base class for nodes which dump output assumptions
...
implement back propagation algorithm on pp 153-154 of patterson i don't *think* that i've made any assumptions about the connectivity of
creates a python based property function that can be added to the global property list
this class is used to represent color components red green blue
new fps come back with all additional fields from the
used for calculating descriptors this is the central point for descriptor calculation
...
convenience class for dealing with files with comments blank all whitespace lines and lines beginning with comment
maintains a sorted list of a particular number of data elements
a class for picking the top n overall best matches across a library connect to a database and build molecules
a class for storing clusters/data **general remarks**
pdf logically separates text and graphics drawing you can change the coordinate systems for text and graphics independently
a screener that only returns the top n hits found
this class is intended to abstract away many of the details of interacting with databases
...
base class for nodes which supply things assumptions
at times we need to put something in the place of standard output
...
this is a base class used by knnclassificationmodel and knnregressionmodel to represent a k-nearest neighbor predictor
filter out molecules matching one or more smarts patterns there is a count associated with each pattern
the standard hyperbolic tangent function
base class for nodes which filter their input assumptions
class for storing the details of a fingerprinting run
dbfp supplier supporting only forward iteration >>> from rdkit import rdconfig
lazy loading of moved objects
this is used to represent a k-nearest neighbor classifier
sigfactory's are used by creating one setting the relevant parameters then calling the getsignature() method each time a
note the default face is 'times' and is set in fontmapping py
this is a little utility class for saving and restoring the default drawing parameters of a canvas
pdf documents can have basic information embedded viewable from file | document info in acrobat reader
canonical-smiles based duplicate filter assumptions
abstract base class for descriptor calculators
this canvas is meant for generating encapsulated postscript files eps used for inclusion in other documents thus really only
this class represents font typeface size and style
supplies pickled objects from a db result set sample usage
create a six moves urllib namespace that resembles the python 3 namespace
test the python implementation of the various descriptors
used for calculating descriptors for molecules
supplies molecules from a db result set sample usage
...
class stringsegment contains the intermediate representation of string segments as they are being parsed by the xmlparser
...
dumps smiles output assumptions
this status bar displays clear and quit buttons as well as the location of the mouse and if the left button is down
a node in a neural network
used to return all compounds that have a similarity to the probe beyond a threshold value
...
a class for picking the best matches across a library connect to a database
filtermatcher - this class allows creation of python based filters
a data set for holding quantized data **note**
supports random access
>>> vc = vectcollection() >>> bv1 = datastructs
this is your bog standard tree class
dbfp supplier supporting random access >>> import os
smiles supplier sample usage
base class important attributes
this works by accumulating a list of strings containing pdf page marking operators as you call its methods
used to draw ugly rotated text
class to cluster a set of bits based on their correllation
transforms molecules by removing atoms matching smarts patterns assumptions
lazy loading of moved objects in six moves urllib_response
a container class to package a composite model with a descriptor calculator so that objects needing predictions (compounds molecules etc
