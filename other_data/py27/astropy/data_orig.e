...
a fast binary search tree implementation for indexing using the bintrees library
...
create a patch representing a spherical circle - that is a circle that is formed of all the points that are within a certain angle of the central
convolution kernel base class
base class for frames which are collections of :class ~astropy
in a deluxetable_ the data is enclosed in \startdata and \enddata
...
reader for table header without a header set the start of header line number to none, which tells the basic
...
invalid unit string as defined in the standards for astronomical catalogues version 2
create filter kernel from list or array
container for meta information like name description format this is
rawdatadiff is just a special case of imagedatadiff where the images are one-dimensional and the data is treated as a 1-dimensional array of
creates a dict like object that wraps an existing dict or other mutablemapping, along with a dict of *key aliases* that translate
...
this exception should be used to indicate that an uncertainty instance has not been associated with a parent ~astropy
...
support the centre de données astronomiques de strasbourg <http //cds
...
the given attribute was not supported on the given element until the specified votable version however the version declared in the file is
...
this is used to return a list-like sequence over all the values in the header for a given commentary keyword such as history
...
compute the lomb-scargle periodogram this implementations here are based on code presented in [1]_ and [2]_;
diff two table data arrays it doesn't matter whether the data originally
the attribute is not defined in the specification
result of signature bind call holds the mapping of arguments
undefined value
the supported datatypes are : double float bit boolean unsignedbyte short int long
...
representation of points in 3d spherical coordinates (using the physics convention of using phi and theta for azimuth and inclination
...
header class for which the column definition line starts with the comment character
...
a mapping for an asinh stretch preserving colours independent of brightness x = asinh(q i - minimum /stretch)/q
indicates that a given index cannot handle the supplied query
one dimensional gaussian model
base class for asynchronous vo service requests using :py class concurrent
a log stretch
...
helper class to control one of the coordinates in the :class ~astropy
base class for all hdus which are not corrupted
indicates that an input table is inconsistent in some way
a pseudo-list of table rows allowing for retrieval of rows by indexed column values
this is a generic class for wrapping a header in such a way that you can use the header's slice/filtering capabilities to return a subset of cards
...
the width attribute is meant to indicate to the application the number of characters to be used for input or output of the
gps time seconds from 1980-01-06 00 00 00 utc for example 630720013
...
a base class for simple elements such as field param and info that don't require any special parsing or outputting machinery
raised when the vo service database can not be updated possibly due to a network outage
...
if no version number is explicitly given in the votable file the parser assumes it is written to the votable 1
diff two header objects
the precision attribute is meant to express the number of significant digits either as a number of decimal places (e
fits record class
...
a warning that is issued when the configuration defaults which should be generated at build-time are missing
...
the column fields as defined using field elements do not match those in the headers of the embedded fits file
...
an exception that is raised when the configuration defaults which should be generated at build-time are missing
...
the attribute requires the value to be a valid xml token as defined by xml 1
...
this class is intended to handle the same format addressed by the ordinary :class basic writer but it acts as a wrapper for underlying c
earlier versions of the votable specification used a gref attribute on the link element to specify a glu reference
inputter where lines ending in continuation_char are joined with the subsequent line
a joint formatter/locator
this specifies the default database that sesame will query when using the name resolve mechanism in the coordinates
...
a vo service query failed due to a network error or malformed arguments
one dimensional power law model with a break
irreducible units are the units that all other units are defined in terms of
the abstract base class of all unit formats
one group of the random group data
logarithmic physical units expressed in db parameters
...
a function which returns the current ascii key that is down if no ascii key is down the null string is returned
base class for one-dimensional fittable models
the given element was not supported inside of the given element until the specified votable version however the version declared
parses the various angle formats including * 01 02 30
...
xml ids must match the following regular expression : ^[a-za-z_][a-za-z0-9_\
...
1d trapezoid kernel
inverse transformation for ~astropy
base table data reader
class which contains the definition of one column e g ttype,
represents a fits binary table column format
the size specifier for a char or unicode field must be only a number followed optionally by an asterisk
two dimensional box model
polyconic projection - sky to pixel
for x format in binary tables
invalid access url
represents an infinite value for purposes of tuple comparison
...
descriptor for attributes of column that are associated with keywords in the fits header and describe properties of the column as specified in
flrw cosmology with a constant dark energy equation of state and no spatial curvature
one dimensional moffat model
the vo catalog database retrieved from the www is designed for a newer version of vo
...
extract column names from a deluxetable_
chandra x-ray center seconds from 1998-01-01 00 00 00 tt
iers table class targeted to iers b provided by iers itself
an asinh stretch
a context manager (for use with the with statement) that will seed the numpy random number generator rng to a specific value and then restore
...
base class for the stretch classes which when called with an array of values in the range [0 1], return an transformed array of values
a physical or astronomical constant
general cone search exception
gnomonic projection - pixel to sky
the votable spec states if a cell contains an array or complex number it should be
bayesian blocks fitness for regular events this is for data which has a fundamental "tick" length so that all
...
list of tuples that sorts in a specific order that makes sense for astropy table column attributes
the index class makes it possible to maintain indices on columns of a table so that column values can be queried
...
bayesian blocks fitness for point measures parameters
...
a context manager that allows for special indexing modes which are intended to improve performance
a square root stretch
the given char or unicode string is too long for the specified field length
samp client exceptions
a faster version of the ordinary :class tab reader that uses the optimized c parsing engine
...
a faster version of the ordinary :class csv writer that uses the optimized c parsing engine
the votable element must contain at least one resource element
flrw cosmology with a cosmological constant and curvature
header object for compressed image hdus designed to keep the compression header and the underlying image header properly synchronized
...
in order to name the columns of the numpy record array each field element must have either an id or name attribute
data reader for rdb data starts reading at line 2
this class is used to set up the astropy logging
split line based on fixed start and end positions for each col in self
reader for table data without a header data starts at first uncommented line since there is no header line
...
some votable files write missing floating-point values in non-standard ways such as "null" and "-"
a warning that is issued when the configuration value specified in the astropy configuration file does not match the type expected for that
2d trapezoid kernel
base class for the interval classes which when called with an array of values return an interval computed following different
...
class which implements the function minimized by knuth_bin_width parameters
bayesian blocks fitness for binned or unbinned events parameters
the number of rows explicitly specified in the nrows attribute does not match the actual number of rows (tr elements) present
...
a specialized streamhandler that logs info and debug messages to stdout and all other messages to stderr
...
placeholder for when a more general cosmology class is implemented
samp hub exception
...
warning class for when a string column is assigned a value that gets truncated because the base numpy string length
configuration parameters for astropy utils iers
lazy loading of moved objects
the votable specification uses the attribute name id with uppercase letters to specify unique identifiers
...
the attribute must be a valid uri as defined in rfc 2396 <http //www
stereographic projection - pixel to sky
the 3 datatypes defined in the votable specification and supported by vo
a descriptor for classes that have a meta property
represent date as python standard library ~datetime datetime object
in a deluxetable <http //fits
...
logarithmic physical units expressed in magnitudes parameters
this error indicates additional sections in a section with a __many__ repeated section
the content-type attribute must use mime content-type syntax as defined in rfc 2046 <http //tools
...
this is a terrible hack to support simple image access protocol results from archive
...
this class provides a wrapper around an actual index object to make index slicing function correctly
logarithmic unit containing a physical one usually logarithmic units are instantiated via specific subclasses
...
warning class for cases when a table column is replaced via the table
...
the parser has encountered an element that does not exist in the specification or appears in an invalid context
...
base class for models with custom bounding box templates (methods that return an actual bounding box tuple given some adjustable parameters--see
...
this metaclass makes methods of a class automatically have their docstrings filled in from the methods they override in the base
a subclass class for wcsaxes
superclass for quantities of specific physical type
metaclass that adds timeformat and timedeltaformat to the time_formats and time_delta_formats registries respectively
...
represents the "next largest" version of a given value so that for all valid comparisons we have
configuration parameters for astropy io votable
wcs objects perform standard wcs transformations and correct for sip_ and distortion paper_ table-lookup transformations based
...
base class for bayesian blocks fitness functions derived classes should overload the following method
normalization class to be used with matplotlib
a faster version of the :class commentedheader reader which looks for column names in a commented line
...
since nan's can not be represented in integer fields directly a null value must be specified in the field descriptor to support reading
...
base class for times that represent the interval from a particular epoch as a floating point multiple of a unit time interval (e
...
a class that displays either a progressbar or spinner depending on whether the total size of the operation is
table column
...
class to format the header s of a fits file for display by the fitsheader tool essentially a wrapper around a hdulist object
...
an exception that is used only as a placeholder to indicate to the ipython exception-catching mechanism that the astropy
...
indicates that one of the specified options for fast reading is invalid
...
internal class to replace the csv writer writerow and writerows functions so that in the case of delimiter=' ' and
...
an element in a binary search tree containing a key data and references to children nodes and
...
a variant of tableloc allowing for row retrieval by indexed order rather than data values
...
classes should use this metaclass if they wish to use ordereddescriptor attributes which are class attributes that "remember" the order in which
...
base class to support string besselian and julian epoch dates such as 'b1950
slsqp optimization algorithm and least squares statistic
context manager to set a temporary path for the astropy download cache primarily for use with testing (though there may be other applications
base metaclass that defines the interface for n-dimensional datasets with associated meta information used in astropy
specialized dict subclass to represent attributes of a column and return items() in a preferred order
...
read or write a restructuredtext simple format table <http //docutils
...
flat iterator object to iterate over quantities a quantityiterator iterator is returned by q
...
this metaclass exists because the unit constructor should sometimes return instances that already exist
...
a special list of table indices allowing for retrieval by column name s
get the lines from the table input and return a list of lines
conic orthomorphic projection - sky to pixel
a representation of a scaled logarithm of a number with a unit parameters
