...
this module holds the base basespatialbackend object which is instantiated by each spatial database backend with the features
...
parses a variable token and its optional filters all as a single string and return a list of tuples of the filter name and arguments
base exception
a rfc2388 multipart/form-data parser
an sql searched case expression
document type definition is forbidden
a proxy for the _order database field that is used when meta
...
very insecure algorithm that you should *never* use stores sha1 hashes with an empty salt
...
if set to true true, the legacy format
takes a pair of projectstates and compares them to see what the first would need doing to make it match the second (the second
argument is a geometry return type is double that is passed in by reference as the last argument
a onetoonefield is essentially the same as a foreignkey with the exception that it always carries a "unique" constraint with it and the reverse
sets new model options that don't directly affect the database schema (like verbose_name permissions ordering)
provides the ability to combine one or two objects with some connector
...
takes a migration instance and is able to produce the contents of the migration file from it
...
an adapter class for cursor variables that prevents the wrapped object from being converted into a string when used to instantiate an oracleparam
this class and its subclasses are responsible for emitting schema-changing statements to the databases - model creation/removal/alteration field
deserialize xml
...
this class encapsulates all backend-specific differences that pertain to creation and destruction of the test database
the base geometry field -- maps to the opengis specification geometry type
raised when a irreversible migration is about to be reversed
a deserialized model
the exception used for syntax errors during parsing or rendering
upload handler that streams data into a temporary file
you cannot access raw_post_data from a request that has multipart/* post data if it has been accessed via post
the geographic queryset
raised when there's a bad migration (unreadable/bad format/etc
the 'geometry_columns' view from postgis see the postgis
secure password hashing using the bcrypt algorithm recommended this is considered by many to be the most secure algorithm but you
...
used internally as a consistent exception type to catch from save (see the docstring for sessionbase
...
this backend is to be used in conjunction with the remoteusermiddleware found in the middleware module of this package and is used when the server
...
wraps accesses to a dictionary so that certain values those starting with the specified prefix are passed through a function before being returned
...
a list containing remainder of the token split by spaces
...
middleware that requires a present and correct csrfmiddlewaretoken for post requests that have a csrf cookie and sets an outgoing
...
stub serializer to hold exception raised during registration this allows the serializer registration to cache serializers and if there
...
object to wrap callable appearing in settings * not to call in the debug page (#21345)
compactly serializes instances of the message class as json
the permissions system provides a way to assign permissions to specific users and groups of users
middleware for web-server provided authentication on logon pages
lazy loading of moved objects in six moves urllib_request
formerly a middleware for invalidating a user's sessions that don't correspond to the user's current session authentication hash
...
this class encapsulates all backend-specific differences such as the way a backend performs ordering or calculates the id of a recently-inserted
...
class that serves as a wrapper for geos c functions and will use thread-safe function variants when available
...
customized argumentparser class to improve some error messages and prevent systemexit in several occasions as systemexit is unacceptable when a
...
a mixin class that adds the fields and methods necessary to support django's group and permission model using the modelbackend
...
command that allows to copy or symlink static files from different locations to the settings
...
powers the optimization process where you provide a list of operations and you are returned a list of equal or shorter length - operations
...
abstract base class for password hashers when creating your own hasher you need to override algorithm
...
a type that indicates the contents are an sql fragment and the associate parameters
...
the envelope object is a c structure that contains the minimum and maximum x y coordinates for a rectangle bounding box
this class that wraps an ogr feature needs to be instantiated from a layer object
any error having to do with uploading files
a thin wrapper around mysqldb's normal cursor class so that we can catch particular exception instances and reraise them with the right types
...
delete queries are done through this class since they are more constrained than general queries
...
a mixin to provide a default regex property which can vary by active language
...
custom argparse action for ogrinspect keywords that require a string list
...
used by join construction code to indicate the point at which a multi-valued join was attempted (if the caller wants to treat that
...
provides an iterator which converts the results of raw sql queries into annotated model instances
...
a logging filter that checks the return value of a given callable which takes the record-to-be-logged as its only parameter to decide whether to
...
the size of the request excluding any file uploads exceeded settings
a form that creates a user with no privileges from the given username and password
this middleware compresses content if the browser allows gzip compression
iterable returned by queryset values() that yields a dict
a form used to change the password of a user in the admin interface
this is just a base class for the proxy class created in the closure of the lazy function
...
the number of fields in a get or post request exceeded settings
...
managementform is used to keep track of how many form instances are displayed on the page
a version of emailmessage that makes it easy to send multipart/alternative messages
intentionally simple parent class for all views only implements
a url resolver that always matches the active language code as url prefix
secure password hashing using the bcrypt algorithm this is considered by many to be the most secure algorithm but you
wraps an ogr data source object
this class encapsulates all backend-specific introspection utilities
create a model's table
wraps a gdal raster band needs to be obtained from a gdalraster object
if true, requires that the username does not match an
...
this exception is raised when an invalid index is encountered and has the 'silent_variable_feature' attribute set to true
...
"common" middleware for taking care of some basic operations - forbids access to user-agents in settings
wraps a gdal/ogr data source driver
detail view of a single object on a single date this differs from the standard detailview by accepting a year/month/day in the url
represents an actual message that can be stored in any of the supported storage classes typically session- or cookie-based and rendered in a view
a field whose clean() method calls multiple field clean() methods
validate whether the password is of a minimum length
a database query predicate is impossible
a base file finder to be used for custom staticfiles finder classes
mixin for combining with a lookup expecting a text lhs from a jsonfield key lookup
...
signal subclass that allows the sender to be lazily specified as a string of the app_label
...
incredibly insecure algorithm that you should *never* use stores unsalted md5 hashes without the algorithm prefix also accepts md5 hashes with an
...
the purpose of this class is to store the actual translation function upon receiving the first call to that function
a abstract uploaded file (temporaryuploadedfile and inmemoryuploadedfile are the built-in concrete subclasses)
holder for user configured settings
a class to organize and coordinate reporting on exceptions
a class that maps ogr layers to geodjango models
return the selected language catalog as a json object
mixin class for views manipulating date-based data
tries to store all messages in the first backend storing any unstored messages in each subsequent backend backend
a javascript lexer >>> lexer = jslexer()
...
a field plus data
renames a model
this is a very simple middleware that parses a request and decides what translation object to install in the current
simple appconfig which does not do automatic discovery
list of objects published on a given day
takes two lists of operations - ones that will be used for the database and ones that will be used for the state change
a single raw sql query
iterable that yields a model instance for each row
a validator designed for hstore to require/restrict keys
iterable returned by queryset values_list(flat=false)
invalid to_field was passed to admin view via url query string
special subclass of string which actually references a current settings value
...
a unicode python 2 / str python 3 subclass that has been specifically marked as "safe" for html output purposes
...
a management command which takes one or more arbitrary arguments labels on the command line and does something with each of
