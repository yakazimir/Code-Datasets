this function kills the process process by sending the signal sigkill
this function clears and redisplays all visible frames
this variable is a list of suffixes strings to try adding to the specified program file name
this function makes window the selected window and the window selected within its frame see basic windows and selects that frame
this function returns a string consisting of the printed base-ten representation of number
this function returns filename’s final “extension”, if any after applying file-name-sans-versions to remove any version/backup part
this function returns the value of the parameter parameter a symbol of frame
this function selects frame raises it should it happen to be obscured by other frames and tries to give it the x server’s focus
remove the last n items of list and return a copy
this function looks up key in table and returns its associated value—or default if key has no association in table
this function is the non-destructive counterpart of delete
this function returns the longest common substring of all possible completions of string in collection
this function executes the shell command command synchronously
this is like get-file-buffer except that it can return any buffer visiting the file possibly under a different name
this function returns a string that represents in shell syntax an argument whose actual contents are argument
this function sets one or more attributes of face for frame
if non-nil edebug binds print-level to this value while printing results
this is like indent-rigidly except that it doesn’t alter lines that start within strings or comments
this function returns t if object is a syntax table
this function returns the real gid of the emacs process
this function makes window the window selected within the frame frame
this function selects a live window one count places from the selected window in the cyclic ordering of windows
this function defines a local menu bar item named name to run imenu
this function returns as a string the text matched in the last search or match operation
this command unloads the library that provided feature feature
this variable lets you specify alternative font registries to try if a given registry is specified and doesn’t exist
this function implements the heart of adaptive fill mode it chooses a fill prefix based on the text between from and to typically the start and end of a paragraph
this function asks the user a series of questions reading a single-character answer in the echo area for each one
this macro defines obsolete-face as an alias for current-face and also marks it as obsolete indicating that it may be removed in future
this function divides dividend by divisor and returns the quotient
this function outputs character to stream
this function returns the first association with value value in alist
this function auto-saves all buffers that need to be auto-saved
this function kills the buffer buffer-or-name freeing all its memory for other uses or to be returned to the operating system
repeatedly calls fn presumably for side-effects passing in integers from 0 through num-1
this function defines a package
this function returns t if filename is the name of an existing directory nil otherwise
if this variable is non-nil it means that the user plans to use global abbrevs only
if this is non-nil lines exactly as wide as the window not counting the final newline character are not continued
the value of this option should be a display action
this function saves the contents of the current buffer in its visited file if the buffer has been modified since it was last visited or saved
this function returns t if object is a marker nil otherwise
this convenience macro marks the function obsolete-name obsolete and also defines it as an alias for the function current-name
this function returns t if object is a window configuration
this function attempts to make window as small as possible in both dimensions without deleting it or resizing its frame
this function is the opposite of listp it returns t if object is not a list
this function returns a string or vector containing the key sequence that invoked the present command plus any previous commands that generated the prefix argument for this command
thread the expr through the forms insert x as the last item
this function returns the meaning of function as a function
this function returns non-nil if buffer-or-name is a minibuffer
if value1 is a relative value for the face attribute attribute returns it merged with the underlying value value2 otherwise if value1 is an absolute value for the face attribute attribute returns value1 unchanged
this function returns the previous sibling of the window window
this variable determines how much time should elapse before command characters echo
if process is a datagram connection or server this function sets its remote peer address to address
return the position at which button ends
if val evaluates to non-nil bind it to var and do then
this is similar to ewoc-refresh except that only nodes in ewoc are updated instead of the entire set
this function returns the total number of characters in the current buffer
this function stores object as the new car of cons replacing its previous car
this function reads abbrev definitions from a file named filename previously written with write-abbrev-file
this function returns non-nil if tty is the controlling terminal of the emacs session tty can be a terminal object a frame meaning the terminal for that frame or nil meaning the terminal for the selected frame
return the next button after for next-button or before for previous-button position pos in the current buffer
this function returns the selected window which is always a live window
this variable specifies special text property handling conditions for yanked text
this function returns t if object is a byte-code function
this function announces that feature is now loaded or being loaded into the current emacs session
this function tests whether its arguments are numerically equal and returns t if they are not and nil if they are
this returns the standard case table
return non-nil if the current token’s parent is among parents
this function populates the current buffer with entries
this function displays the message message which may be either a string or a buffer
if this variable is non-nil read-file-name ignores case when performing completion
setup smie navigation and indentation
this function defines a new fontset according to the specification string fontset-spec
this function deletes symbol from the obarray obarray
split the list each time item is found
this predicate tests whether its argument is a special form and returns t if so nil otherwise
this function returns the “logical and” of the arguments the nth bit is set in the result if and only if the nth bit is set in all the arguments
set the representation type of the current buffer
this moves point to the start of node in ewoc
return the window area recorded in position
this function returns the current depth of activations of the minibuffer a nonnegative integer
this function returns a list of image file extensions supported by the current imagemagick installation
this function displays the buffer specified by buffer-or-name in some window other than the selected window
this command specifies coding-system as the coding system to use for encoding terminal output from terminal
this function returns the number of color cells the screen supports
this function returns a new marker that points to the beginning of the accessible portion of the buffer
this function returns the raw syntax descriptor for the character in the buffer after position pos taking account of syntax properties as well as the syntax table
return the property prop of abbrev or nil if the abbrev has no such property
this function returns t if object is a coding system name or nil
this function sets the variable symbol by inserting element into the old value which must be a list at the position specified by order
this function returns t if the screen is a color screen
this function returns the value of the font property property for font
this function creates a buffer-local binding in the current buffer for variable a symbol
this function returns a string made up of count repetitions of character
if pos-or-prop is a marker or number this function returns a non-nil value if the text at that position is invisible
this command inserts the last character typed it does so count times before point and returns nil
zip lists with fill-value padded onto the shorter lists the
this function returns a list of the color names that are defined and supported on frame frame default the selected frame
this function displays a pop-up dialog box and returns an indication of what selection the user makes
this function moves overlay to buffer and places its bounds at start and end
this variable specifies the site-customization file to load before the user’s init file
this function adds an item to the tool bar by modifying tool-bar-map
this function kills the text in the region defined by start and end
this function is used for making non-global tool bar items
if this variable is non-nil indentation functions can insert tabs as well as spaces
if this variable is non-nil the default emacs makes backups by copying in cases where renaming would change either the owner or the group of the file
this variable controls processing of ‘eval ’ in ‘-*-’ lines or local variables lists in files being visited
this function returns the weak value that was specified for hash table table
this function returns information about the network interface named ifname
this variable if non-nil inhibits the startup screen
this function moves point forward count lines to the beginning of the line
wrap string s with prefix and optionally suffix
this command replaces the minibuffer contents with the value of the nth next newer history element that matches pattern a regular expression
like this-command-keys except that it always returns the events in a vector so you don’t need to deal with the complexities of storing input events in a string see strings of events
this function returns the effective gid of the emacs process
this function rounds float to the next lower integral value and returns that value as a floating-point number
this function returns a new vector containing all the elements of sequences
take a list of n functions and return a function that takes a
this function returns buffer’s modification-count
the function x-get-resource retrieves a resource value from the x window defaults database
the value of this variable is a list of imagemagick image types which emacs may attempt to render using imagemagick
this function signals the quit condition with (signal 'quit nil)
mapconcat applies function to each element of sequence the results which must be strings are concatenated
this function returns an alist listing all the parameters of terminal and their values
this deletes each node in nodes from ewoc
this function returns the height of the screen in pixels
this returns t if time value t1 is less than time value t2
this command adjusts the height or width of window to fit the text in it
this is like minibuffer-contents except that it does not copy text properties just the characters themselves
this function returns the binding for key according to the current active keymaps
perform the action specified by a button at location pos
this function calls the function predicate for each of the windows in the cyclic order of windows in turn passing it the window as an argument
this function returns the “exclusive or” of its arguments the nth bit is set in the result if and only if the nth bit is set in an odd number of the arguments
this function returns the first child window of the internal window window—the topmost child window for a vertical combination or the leftmost child window for a horizontal combination
this function returns the minibuffer window used for frame frame
this function returns the buffer position of the previous beginning or end of an overlay before pos
if function was previously registered as a fontification function using jit-lock-register this function unregisters it
this function returns an md5 hash
this function tries to display buffer by splitting the largest or least recently-used window typically one on the selected frame
this function uses read-key to read and return a single character
this function converts the lisp representation of a network address to a string
this function reports the scroll bar type for window window
this function is the most general way to get input from the minibuffer
apply fn to each element of tree while preserving the tree structure
this inserts a button with the label label at point using text properties
this function returns the value of the attribute attribute for face on frame
if this variable is non-nil sort-subr and the other buffer sorting functions ignore case when comparing strings
define the new command command a symbol
this function inserts the abbrev expansion of abbrev replacing the text between start and end
if this variable is non-nil emacs makes backups by copying for files with multiple names hard links
this is the command bound to tab in most editing modes
this stores value as the value of the property property in the property list plist
this function associates sentinel with process
this buffer-local variable controls how the buffer boundaries and window scrolling are indicated in the window fringes
if s is shorter than len pad it with padding on the right
this function returns non-nil if the buffer is narrowed and nil otherwise
are there both lower case and upper case letters in s
this function renders the parsed html in dom into the current buffer
this command is similar to package-upload-file but instead of prompting for a package file it uploads the contents of the current buffer
this function returns the standard category table
returns the capabilities of the notification server a list of symbols
this function should be called when the operation is finished
this function returns the width of the left and right margins of window as a cons cell of the form (left
this function returns the number of consecutive button presses that led up to event
this macro declares theme a symbol as the name of a custom theme
this function decodes the text in string according to coding-system
return the indices of all elements in list satisfying the
this function returns the left pixel edge of window window
this function adds its arguments together
this function returns the standard syntax table which is the syntax table used in fundamental mode
this function returns a list of positions markers or integers that record all the information on the text that the last search matched
threads s through the forms inserts s as the last item
this function returns the proper fill column value to use for filling the text around point
this function returns the value of dividend modulo divisor in other words the remainder after division of dividend by divisor but with the same sign as divisor
this function destructively removes all elements eq to object from list and returns the resulting list
this sets the face for the fringe bitmap bitmap to face
this function builds a prec2 table from a table of precedences precs
this variable specifies the number of seconds to keep indicating the matching parenthesis
this returns a list of the objects in ring in order newest first
this returns t if frame can really display the color color or at least something close to it
this function ignores any arguments and returns nil
apply fn to the first item in list that is the header
this variable specifies how many of the newest backup versions to keep in the dired command
this function returns the processor run time used by emacs as a list of four integers high low microsec picosec using the same format as current-time see time of day
this function makes table the category table for the current buffer
this function returns non-nil if the display supports the saveunder feature
this function scrolls the selected window count columns to the left or to the right if count is negative
if the value of this variable is non-nil that means display-buffer may display buffers by making new frames
the function frame-live-p returns non-nil if the frame frame has not been deleted
this option is how you inform emacs whether the window manager transfers focus when the user moves the mouse
create a deep copy of list
this function returns the integer remainder after division of dividend by divisor
this command considers a region of text as a single paragraph and fills it
this special form returns function-object without evaluating it
this makes a button from beg to end in the current buffer using text properties
if all vals evaluate to true bind them to their corresponding
this command is like down-list but it also pays attention to nesting of tokens other than parentheses such as begin
this function resizes window by delta increments
this function returns the address of the last byte emacs has allocated divided by 1024
this predicate returns a non-nil value if object is a frame and nil otherwise
this is the major mode command for fundamental mode
this variable determines whether files may be written out that do not end with a newline
this returns a cons cell (header
this function returns the fractional vertical position of a scroll bar event within the scroll bar
this function returns non-nil if position is within the range of text currently visible on the screen in window
return an infinite copy of list that will cycle through the
flatten num levels of a nested list
return the index of the first element in the given list which
return the last item of list or nil on an empty list
this function aborts the command that requested the innermost recursive edit including minibuffer input by signaling quit after exiting the recursive edit
use fn to reduce elements of list tree
this removes the element element from buffer-invisibility-spec
if this variable is non-nil emacs auto-saves buffers in the files they are visiting
this function returns a newly-allocated uninterned symbol whose name is name which must be a string
this function returns a file name based on filename which fits the conventions of the current operating system
this function returns t if object is an integer or a marker nil otherwise
this function removes all the associations from hash table table so that it becomes empty
return a new list with the items in list grouped into n-sized sublists
this function returns a copy of sequence
this function returns the value of the environment variable var as a string
this command displays the contents of the syntax table of buffer by default the current buffer in a help buffer
this function moves backward across arg default 1 balanced expressions
this function creates a suitably named buffer for visiting filename and returns it
this function returns a vector containing the last 300 input events from the keyboard or mouse
return a list with element at nth position in list replaced with x
this function inserts the strings and/or characters args into the current buffer at point moving point forward
return the result of applying fn to initial-value and the
this function deletes blank lines surrounding point
this function returns the number of lines between the positions start and end in the current buffer
this makes a button from beg to end in the current buffer and returns it
this function is the guts of all the file-visiting functions
this function concatenates all its argument bytes and makes the result a unibyte string
this command replaces the just-yanked entry from the kill ring with a different entry from the kill ring
this function returns the height in lines of the body of window window
this function returns a live window the one following window in the cyclic ordering of windows
this is like re-search-forward except that it performs the full backtracking specified by the posix standard for regular expression matching
return a new font spec using the specifications in arguments which should come in property-value pairs
this function tries to display buffer in a window previously showing it
this sets the parent keymap of keymap to parent and returns parent
the value of this variable lists the imagemagick image types which should never be rendered using imagemagick regardless of the value of imagemagick-enabled-types
return a list of matched positions for regexp in string
the purpose of throw is to return from a return point previously established with catch
this returns a list of numbers that count the number of objects created in this emacs session
this returns number converted to an integer by rounding towards zero
this function returns a list that indicates which x displays emacs has a connection to
this function simply pauses for seconds seconds without updating the display
this variable always local in all buffers controls whether and where to put scroll bars in windows displaying the buffer
this function returns the element of sequence indexed by index
is s anything but nil or the empty string
this function returns the name of the machine you are running on as a string
this command reindents the current line inserts a newline at point and then indents the new line the one following the newline just inserted
this function returns a translation table based on the argument translations
this is the same as (cdr cdr cons-cell or nthcdr 2 cons-cell
this function deletes a process killing it with a sigkill signal
this function returns the total height in lines of the window window
file-name-completion usually ignores file names that end in any string in this list
grade elements of list using comparator relation yielding a
this function deletes from alist all the elements whose car is eq to key much as if you used delq to delete each such element one by one
this function returns a list of coding systems that could be used to encode the text of string
unzip lists
this function converts the count words after point to all lower case moving point over as it does
join all the string arguments into one string
this function sets the value of the propname property of process to value
this function creates a list with objects as its elements
bind variables according to varlist then eval body
return non-nil if the next token is among tokens
this function sets the file mode or permissions of filename to mode
this variable controls how recenter-top-bottom behaves when called with no argument
this sets the header and footer of ewoc to the strings header and footer respectively
this function sets the selinux security context for filename to context
this returns number converted to floating point
this function returns the current global keymap
this command selects a buffer visiting the file filename like find-file but it marks the buffer as read-only
this variable if non-nil specifies the same behavior as backup-by-copying-when-mismatch but only for certain user-id values namely those less than or equal to a certain number
this function returns t if point is at the end of a line
this function returns the list of buffers recently re-shown in window via switch-to-prev-buffer
if the value of this variable is non-nil the default split-window-below behaves as described above
this variable is an alist that specifies the coding systems to use for reading and writing particular files
this function removes all elements equal to object from sequence and returns the resulting sequence
this function returns a string that is the name to use for a non-numbered backup file for file filename
select columns from table
this function defines a set of variable settings for the named class which is a symbol
this function returns the window with the largest area height times width
this function returns t if object is a buffer nil otherwise
this sets the :stipple attribute of face to pattern
this function reads a string in the minibuffer assisting the user by providing completion
if the file filename is a symbolic link the file-symlink-p function returns its non-recursive link target as a string
this returns number converted to an integer by rounding towards the nearest integer
this function returns a list of coding systems that could be used to encode a text between from and to
this command is identical to word-search-forward except that the beginning or the end of string need not match a word boundary unless string begins or ends in whitespace
this function sends the text in the region defined by start and end as standard input to process
appends fill-value to the end of each list in lists such that they
this function sets the fringe widths of window window
this function returns the syntax class of character represented by its designator character see syntax class table
this function provides a convenient way to find an image satisfying one of a list of image specifications specs
this function extracts from parser state state the last position scanned in the parse which was at top level in grammatical structure
return the timestamp in position
this list specifies which warning types should not be logged in the warnings buffer
this is like next-single-char-property-change but scans back from pos instead of forward and returns the minimum valid position in object if no change is found
this function searches backward in the current buffer for a string of text that is matched by the regular expression regexp leaving point at the beginning of the first text found
this function reads a lisp expression using the minibuffer evaluates it then returns the result
this variable determines whether the higher-level replacement functions should preserve case
the - function serves two purposes negation and subtraction
this function multiplies its arguments together and returns the product
this function returns the value of the propname property of process
compute flat outer product of lists using function fn
this function returns the next sibling of the window window
make a new list from the elements of args
this inserts object into ring making it the newest element and returns object
if this is non-nil lisp programs and the emacs user can use the mark even when it is inactive
set button’s prop property to val
this command loads the file filename
this function asks the user a question expecting input in the minibuffer
this command sets the value of the environment variable named variable to value
this function returns an alist of the emacs-recognized extended attributes of file filename
this predicate returns a non-nil value if object is a terminal that is live
if this option is nil a frame’s size is usually rounded to a multiple of the current values of that frame’s frame-char-height and frame-char-width
this variable specifies the base left margin column
this command runs a garbage collection and returns information on the amount of space in use
this function returns the position at which overlay ends as an integer
the value of this variable is an alist of package archives recognized by the emacs package manager
adaptive fill mode matches this regular expression against the text starting after the left margin whitespace if any on a line the characters it matches are that line’s candidate for the fill prefix
this function returns the buffer that overlay belongs to
this function reads a single key
this function sets the query flag of process to flag
if all vals evaluate to true bind them to their corresponding
function returns t if object is a symbol whose name starts with ‘:’, interned in the standard obarray and returns nil otherwise
this function returns the list of version numbers of the gui window system running on display such as the x server on gnu and unix systems
the value of this variable is a list of directories to search for programs to run in subprocesses
apply calls function with arguments just like funcall but with one difference the last of arguments is a list of objects which are passed to function as separate arguments rather than a single list
this is the regular expression for recognizing the beginning of a line that starts or separates paragraphs
this function returns a new marker that points to the present position of point in the current buffer
this returns the value of the property property stored in the property list plist
return a list of matches for regex in string
this returns the time difference t1 - t2 between two time values in the same format as a time value
destructive set list to the cdr of list
build a list from seed using fun
this command attempts to reduce window’s height as much as possible while still showing its full buffer but no less than window-min-height lines
this macro removes function from the function stored in place
this function completes the minibuffer contents and exits if confirmation is not required i
return the product of list
this function iconifies frame frame
this function returns t if object is a cons cell nil otherwise
this function returns a list of the overlays that overlap the region beg through end
this function returns the sentinel of process
this function returns the current buffer’s mark position as an integer or nil if no mark has ever been set in this buffer
this is like next-property-change but scans back from pos instead of forward
this function makes keymap the new current global keymap
this is like next-single-property-change except that it considers overlay properties as well as text properties and if no change is found before the end of the object it returns the maximum valid position in object rather than nil
this function sets window’s value of parameter to value and returns value
return the proper offset to align the current token with the parent
this function takes a prec2 grammar table and returns an alist suitable for use in smie-setup
this function sets the base remapping of face in the current buffer to specs
this function is the most basic function for building new list structure
return a function that computes the least fixpoint of fn
this function discards the undo list of buffer-or-name and disables further recording of undo information
this is the major mode used in inactive minibuffers
this function makes keymap the new local keymap of the current buffer
this function returns nil if the file filename is not locked
string-equal is another name for string=
this function returns non-nil if window is the currently active minibuffer window
indent current token as a separator
this function parses a sexp in the current buffer starting at start not scanning past limit
this variable specifies the directory name for creating temporary files
return a list with element at nth position in list replaced with (func nth n list
this is a synonym for ding
this function returns the total height of window window in pixels
this function returns non-nil if process is alive
if string-or-char is a string this function capitalizes the initials of the words in string-or-char without altering any letters other than the initials
this function returns the contents of the field specified by pos as a string
this function returns the y coordinate of the topmost row of window equivalent to the top entry in the list returned by window-edges
if non-nil edebug tries to remove any of its own instrumentation when showing the results of expressions
define a ‘button type’ called name a symbol
this function returns t if a file named filename exists and you can execute it
if this is non-nil the byte compiler generates compiled files that are set up for dynamic loading of documentation strings
this function returns a string representing the emacs uptime—the elapsed wall-clock time this instance of emacs is running
return a list of -filter pred list -remove pred list ), in one pass through the list
this function returns the absolute value of number
apply fn to each item in list splitting it each time fn returns a new value
this is an alist specifying default values of frame parameters for all emacs frames—the first frame and subsequent frames
this function returns an alist of attributes of face
this function returns the character following point in the current buffer
this function parses and binds or evaluates as appropriate any local variables specified by the contents of the current buffer
this command inserts spaces between the words of the current line so that the line ends exactly at fill-column
this function returns the first window displaying buffer-or-name in the cyclic ordering of windows starting from the selected window see cyclic window ordering
return the position that beginning-of-line count would move to
this variable specifies the directory name for creating certain temporary files which are likely to be small
replace conses with fn and evaluate the resulting expression
this is like next-property-change except that it considers overlay properties as well as text properties and if no change is found before the end of the buffer it returns the maximum buffer position rather than nil in this sense it resembles the corresponding overlay function next-overlay-change rather than next-property-change
this function searches forward from point for a “word” match for string
this function replaces all the horizontal whitespace surrounding point with either one space or no space according to the context
this function calls describe-bindings to display a list of all the subcommands of the prefix key of the most recent key sequence
this function reports a warning using message as the message and type as the warning type
this returns t if object is a ring nil otherwise
this function finds and opens a file of lisp code evaluates all the forms in it and closes the file
this function moves forward across arg default 1 balanced groups of parentheses
split s up to n times into substrings bounded by matches for regexp separator
this function returns a unibyte string containing the same sequence of characters as string
this function sets the variable symbol by consing element onto the old value if element is not already a member of that value
this function returns t if object is a char-table and nil otherwise
this function converts the string string into base 64 code
this function returns t if the file filename exists and is a regular file (not a directory named pipe terminal or other i/o device)
convert s to dashed-words
this function returns the string that is symbol’s name
this function is identical to get except that if symbol is the name of a function alias it looks in the property list of the symbol naming the actual function
this command selects a buffer visiting the file filename using an existing buffer if there is one and otherwise creating a new buffer and reading the file into it
this function tries to return a relative name that is equivalent to filename assuming the result will be interpreted relative to directory an absolute directory name or directory file name
if this variable is non-nil a period followed by just one space does not count as the end of a sentence and the filling functions avoid breaking the line at such a place
rotate list n places to the right with n negative rotate to the left
require special confirmation to execute command from now on and alter the user’s init file so that this will apply to future sessions
call function for every advice that was added to the named function symbol
this function returns the character preceding point in the current buffer
this macro provides a way to initialize the variable var as a collection for completion in a lazy way not computing its actual contents until they are first needed
this function returns non-nil if object is an autoload object
this function reads a password prompting with prompt
this function sets the access and modification times of filename to time
this construct displays a message in the echo area temporarily during the execution of body
this function selects a field’s data from the nested alist struct
this function determines whether any command input is currently available to be read
this function marks the current buffer as modified if flag is non-nil or as unmodified if the flag is nil
string-lessp is another name for string<
this function makes char case-invariant with syntax syntax
this function returns t if object is an overlay
this function returns the value of the property named property in symbol’s property list
this function returns the interned symbol whose name is name
this function returns the process named name a string or nil if there is none
return pred x for the first list item where pred x is non-nil else nil
this function moves point to the left margin of the current line
this function is like window-pixel-edges except that it returns the edge pixel coordinates relative to the top left corner of the display screen
this function returns the current buffer
this function creates and returns an overlay that belongs to buffer and ranges from start to end
this option specifies the minimum total height in lines of any window
this function adds the element element to buffer-invisibility-spec
return t if button has button-type type or one of type’s subtypes
this option specifies the minimum total width in columns of any window
if this option is t whenever a scrolling command moves point off-window emacs tries to adjust point to keep the cursor at its old vertical position in the window rather than the window edge
this function removes keywords from font-lock-keywords for the current buffer or for major mode mode
this function returns non-nil if window is dedicated to its buffer and nil otherwise
if this variable is non-nil buffers that are visiting files have auto-saving enabled by default
this function returns the timer responsible for animating image if there is one
this command alphabetically sorts lines in the region between start and end
this function activates the change group that handle designates
this sets up a timer that calls the function function with arguments args at time time
this function returns t if object is a live window and nil otherwise
this variable if non-nil specifies a function for reading buffer names
this command tries to work out appropriate settings to produce your preferred style of indentation
this function cancels and undoes all the changes in the change group specified by handle
this function returns a list of all live terminal objects
this function sets window’s previous buffers to the value of prev-buffers
this function returns t if object is a string nil otherwise
this function constructs and returns a byte-code function object with elements as its elements
this command encodes the text from start to end according to coding system coding-system
this function is an alternative interface to playing a sound file specifying an optional volume and device
this function returns the visibility status of frame frame
this function tests whether each argument is greater than or equal to the following argument
this function stores value as the value of the property propname for the character char
this function returns the byte at character position pos in the current buffer
the value of this variable is the number of seconds of idle time that should cause auto-saving
this function returns locale data item for the current posix locale if available
return non-nil if infix is infix of list
this command switches to the last buffer in the local buffer list of the selected frame
return non-nil if the current token’s parent is actually a sibling
this function returns a list of all font entities that match the font spec font-spec
these move point to the previous or next respectively argth node in ewoc
this function returns the file name to use for auto-saving the current buffer
this function returns a new string that is made by copying string and then replacing any format specification in the copy with encodings of the corresponding objects
this function sends process the contents of string as standard input
this function returns the entire property list of the character at position in the string or buffer object
this is the basic function for evaluating an expression
convert each word's first character to upper case and the rest to lower case in s
this function returns t if object is nil and returns nil otherwise
this function makes window display buffer-or-name
this command displays the disassembled code for object
this variable controls whether emacs redraws the entire screen after it has been suspended and resumed
this command displays the documentation of the current buffer’s major mode and minor modes
this returns the query flag of process
this function updates the cross reference data in the *help* buffer which is used to regenerate the help information when the user clicks on the ‘back’ or ‘forward’ buttons
this function returns the value of point in the current buffer as an integer
this constructs and returns a new ewoc with no nodes and thus no data elements
call fn index item for each item in list
this predicate function works like looking-at but without updating the match data
this function removes left margin indentation from the text between from and to
this function sets the syntax entry for char according to syntax-descriptor
this function returns a nondeleted process associated with the buffer specified by buffer-or-name
this function returns the property list of symbol
this function returns the width in pixels of window’s vertical scrollbar
this function returns the beginning of the field specified by pos
this function creates and returns a new frame displaying the current buffer
this variable controls whether to process file-local variables
this command indents the current line like the previous nonblank line by calling indent-relative with t as the unindented-ok argument
this function swaps the text of the current buffer and that of its argument buffer
after emacs reads your init file it initializes auto-save-list-file-name if you have not already set it non-nil based on this prefix adding the host name and process id
return a function of two arguments that first applies
this function moves point backward skipping characters that match character-set until limit
this function completely replaces the text property list for the text between start and end in the string or buffer object
this swaps the foreground and background colors of face face
this command decodes the text from start to end according to coding system coding-system
this command copies the directory named dirname to newname
this command inserts contents of register reg into the current buffer
if this option is non-nil then rather than stopping at the first suffix that exists load tests them all and uses whichever file is the newest
the print function is a convenient way of printing
this function defines an abbrev named name in abbrev-table to expand to expansion and call hook with properties props see abbrev properties
this buffer-local variable specifies the maximum width of filled lines
this command sorts the lines in the region between beg and end comparing them alphabetically by a certain range of columns
this function returns non-nil if string1 is a prefix of string2 i
return the indices of all elements in list equal to the query
this function returns the process plist of process
this function returns the first buffer in the buffer list other than buffer
this function loads the custom theme named theme from its source file looking for the source file in the directories specified by the variable custom-theme-load-path
this variable’s value is a function to use for making backup file names
this function runs an abnormal hook by calling all the hook functions in hook passing each one the arguments args
this variable determines whether or not to make backup files
this macro defines variable as a variable with initial value value and docstring and marks it as automatically buffer-local
this function returns a translation table made from vec that is an array of 256 elements to map bytes (values 0 through #xff) to characters
this function removes a relative remapping previously added by face-remap-add-relative
this function returns the buffer specified by buffer-or-name
this command displays a description of the current display table in a help buffer
this special form returns object without evaluating it
this function replaces environment variable references in filename with the environment variable values
return the width thickness in pixels of window’s bottom divider
this function completes the minibuffer contents by at most a single word
this function prints a trace of lisp function calls currently active
this function returns t if object is a number either integer or floating point or a marker nil otherwise
does s end with suffix
this function takes several prec2 tables and merges them into a new prec2 table
this variable specifies the minimum distance between the baseline and the underline in pixels when displaying underlined text
this function signals a buffer-read-only error if the current buffer is read-only
return t if string is a multibyte string nil otherwise
this command inserts spaces or tabs before point up to the next tab stop column defined by tab-stop-list
this function returns t if feature has been provided in the current emacs session
this command renames the file filename as newname
this function returns window’s value for parameter
this function returns a list of all processes that have not been deleted
the function frame-parameters returns an alist listing all the parameters of frame and their values
this function warps the mouse like set-mouse-position except that x and y are in units of pixels rather than units of characters
this function configures a serial port connection
this function is used mainly for operating on keyboard macros but it can also be used as a rough inverse for key-description
this function returns delta if the size of window can be changed vertically by delta lines
this function lets you specify the grammar using a bnf notation
this predicate function does what string-match does but it avoids modifying the match data
this function lets you compare two substrings of the same buffer or two different buffers
functions to call before edebug is used
this predicate tests whether its argument is a macro and returns t if so nil otherwise
the value of this variable is the number of newest versions to keep when a new numbered backup is made
define a binding in map for key with value binding just like define-key but position the binding in map after the binding for the event after
this function is the same as libxml-parse-html-region except that it parses the text as xml rather than html so it is stricter about syntax
this function requests function-name to invoke the debugger each time it is called
the function backtrace-frame is intended for use in lisp debuggers
this function moves point with respect to the text currently displayed in the selected window
this function stores value in the extra slot slot of display-table
this function returns non-nil if symbol is a special variable
this function destroy the fringe bitmap identified by bitmap
this function returns the mode bits of filename—an integer summarizing its read write and execution permissions
concatenate s and suffix
this returns the list of active keymaps that would be used by the command loop in the current circumstances to look up a key sequence
the function x-parse-geometry converts a standard x window geometry string to an alist that you can use as part of the argument to make-frame
this function returns non-nil if built-in zlib decompression is available
this command fills each of the paragraphs in the region from start to end
this command displays the previous buffer in window
this function sets the binding of key in the current local keymap to binding
this function destructively removes all equal duplicates from list stores the result in list and returns it
this function returns the list x with the last element or the last n elements removed
this function concatenates list-of-strings into a single string quoting each string as necessary
this function returns the nearest live window in direction direction as seen from the position of window-point in window window
this function sets the current buffer’s mark to position and pushes a copy of the previous mark onto mark-ring
this function returns the current buffer’s local keymap or nil if it has none
return non-nil if list contains element
this function returns the marker that represents the current buffer’s mark
this function reports whether a color name is meaningful
this command selects a buffer visiting the file filename but does so in a window other than the selected window
this function converts a symbolic file mode specification in modes into the equivalent integer
this function tries to display buffer in a window at the bottom of the selected frame
this function is similar to make-translation-table but returns a complex translation table rather than a simple one-to-one mapping
if string is already a unibyte string this function returns string itself
this predicate function returns non-nil if the mouse pointer displayed on frame is visible otherwise it returns nil
this function tests whether each argument is strictly greater than the following argument
if a given family is specified but does not exist this variable specifies alternative font families to try
this function returns the largest value that a valid character codepoint can have
this function returns the leftmost child window of window if window is an internal window whose children form a horizontal combination
this function moves point to the end of the buffer or the limits of the accessible portion when narrowing is in effect setting the mark at the previous position except in transient mark mode when the mark is already active
this function indents all lines starting between start inclusive and end exclusive sideways by count columns
this variable if non-nil should be a string which is inserted into the *scratch* buffer when emacs starts up
this function returns filename with any file version numbers backup version numbers or trailing tildes discarded
convert the first word's first character to upper case and the rest to lower case in s
this function is like read-file-name but allows only directory names as completion alternatives
this function returns the line number in the current buffer corresponding to the buffer position pos
return t if button-type type is a subtype of supertype
this function resizes frame to a height of height lines
this returns non-nil if table is a hash table object
this variable controls line truncation in partial-width windows
this function converts its argument seconds into a string of years days hours etc
this function returns the indentation of the current line which is the horizontal position of the first nonblank character
this command which is the default binding for c-l acts like recenter except if called with no argument
this function returns a list of the character sets of highest priority that contain characters in the current buffer between positions beg and end
return a list with args
this function returns the backing store capability of the display
return the thing around or next to point as a string
this function returns the coding system to use by default for performing operation with arguments
this function searches for the executable file of the named program and returns the absolute file name of the executable including its file-name extensions if any
this function moves point to the beginning of the buffer or the limits of the accessible portion when narrowing is in effect setting the mark at the previous position (except in transient mark mode if the mark is already active it does not set the mark
this function returns the body height or body width of window
this function returns a new function which when called will call func with the list of arguments composed from args and additional arguments specified at the time of the call
this function returns a list of the modifiers that event has
this function makes frame frame invisible
this function runs byte-compile-file on files specified on the command line
this function returns the value of the variable sentence-end if non-nil
this returns t if object is a bool-vector and nil otherwise
these are the basic trigonometric functions with argument arg measured in radians
this function returns a description of the position of the mouse
this function returns the frame for which the window configuration config was made
this function returns the position that marker points to or nil if it points nowhere
this command creates a directory named dirname
this function forces emacs to update the current buffer’s mode line and header line based on the latest values of all relevant variables during its next redisplay cycle
this function returns the subtype symbol of char-table
allow command a symbol to be executed without special confirmation from now on and alter the user’s init file see init file so that this will apply to future sessions
this function returns the list of character sets see character sets supported by coding-system
this function inserts a portion of buffer from-buffer-or-name into the current buffer before point
this function returns the position of the beginning of the region as an integer
this function delays the initialization of symbol to the next emacs start
this function returns the character of glyph code glyph
this is equivalent to make-hash-table but with a different style argument list
this function resembles insert-buffer-substring except that it processes the text according to yank-handled-properties and yank-excluded-properties
concatenate prefix and s
this function tells font lock mode to run the lisp function function any time it has to fontify or refontify part of the current buffer
this function attempts to give all windows on the selected frame approximately the same share of the screen area
this command saves the region defined by start and end on the kill ring including text properties but does not delete the text from the buffer
this function specifies the custom theme theme’s face settings
evaluate form and return its value but inform coverage testing that form’s value should always be the same
this function does the main work of reporting progress of your operation
sort list stably comparing elements using comparator
this function returns a list of all coding system names symbols
this function takes a search path string such as the value of the path environment variable and splits it at the separators returning a list of directory names
this function stops the process process by sending the signal sigtstp
this function returns the position of the start of the text matched by the last regular expression searched for or a subexpression of it
this function returns information about the fringes of a window window
these add a new node encapsulating data adding it to ewoc before or after node respectively
this function makes process see an end-of-file in its input
this function returns the prompt string of the currently active minibuffer
this function returns a symbol naming the primitive type of object
this function restores the configuration of windows and buffers as specified by configuration for the frame that configuration was created for
this command adjusts the size of frame to display the contents of its buffer exactly
this variable is an older variant of scroll-conservatively
the value of this variable is the maximum length to which the kill ring can grow before elements are thrown away at the end
this function completes the text in the current buffer between the positions start and end using collection
this function returns t if object is a command
if s is longer than len cut it down to len - 3 and add at the end
this function returns the duration of the emacs initialization see startup summary in seconds as a string
this variable if non-nil enables transient mark mode
the value of this variable specifies the amount of consing before a garbage collection occurs as a fraction of the current heap size
this variable specifies a function for creating a new frame in order to make a new window for displaying a buffer
this command replaces any spaces and tabs around point with a single space or n spaces if n is specified
this function stores object as the new cdr of cons replacing its previous cdr
this function returns the symbol in obarray whose name is name or nil if obarray has no symbol with that name
this function returns a list of all the active minor mode bindings of key
set the font property property in the font-spec font-spec to value
if this variable is non-nil you can invoke commands such as find-file that use minibuffers even while the minibuffer window is active
like set-buffer-modified-p but does not force redisplay of mode lines
this function returns t if a file named filename appears to exist
save all abbrev definitions except system abbrevs for all abbrev tables listed in abbrev-table-name-list in the file filename in the form of a lisp program that when loaded will define the same abbrevs
are all the letters in s in upper case
this function converts the string or vector key to a list of individual events which you can put in unread-command-events
this function returns the index of the start of the first match for the regular expression regexp in string or nil if there is no match
this function deletes specified text properties from the text between start and end in the string or buffer object
this function capitalizes all words in the region defined by start and end
this function signals an error named by error-symbol
this function inserts the strings and/or characters args into the current buffer at point moving point forward
this function moves marker to position in buffer
this function tries to split window and return the newly created window
this function returns t if object represents an emacs subprocess nil otherwise
split s into list of words
this returns a copy of the property list of overlay
this function returns a regular expression whose only exact match is string
this function returns the documentation string of category category in category table table
this function returns a list of all possible completions of string
this function scrolls the selected window count columns to the right or to the left if count is negative
this function searches for a file whose name is filename in a list of directories given by path trying the suffixes in suffixes
this function tests to see whether object is a member of list
this function returns the boundaries of the field on which collection will operate assuming that string holds the text before point and suffix holds the text after point
if this variable is non-nil emacs always makes backup files by copying
like plist-put except that it compares properties using equal instead of eq
if this variable is non-nil the completion commands automatically display a list of possible completions whenever nothing can be completed because the next character is not uniquely determined
this function returns a list containing all the elements of lists
this sets the left-margin property on the text from from to to to the value margin
this function returns the current position of the end of the minibuffer prompt if a minibuffer is current
this predicate tests whether its argument is floating point and returns t if so nil otherwise
this function returns the total width in columns of the window window
this returns the start position of node
this function returns the contents of the entire accessible portion of the current buffer as a string
return bitwise and of bool vectors a and b
the value of this variable is a list of event types that serve as alternative “help characters”
this function returns non-nil if object is a mouse movement event
this function returns the current time represented as a list of four integers sec-high sec-low microsec picosec
this function empties out the table of events for this-command-keys to return
this function returns the text that would be inserted into the buffer by replace-match but without modifying the buffer
this function returns its argument string possibly modified such that the result can be safely concatenated with another string or juxtaposed with another string in a buffer without disrupting the relative layout of this string and the next one on display
this command replaces the minibuffer contents with the value of the nth more recent history element
this is the regular expression for recognizing the beginning of a line that separates paragraphs
returns up to the len first chars of s
this function works like insert except that it processes the text in string according to the yank-handler text property as well as the variables yank-handled-properties and yank-excluded-properties see below before inserting the result into the current buffer
this function returns the number of bytes in string
this macro declares that the theme named theme has been fully specified
this function returns t if display has a mouse available nil if not
return t if every t value in a is also t in b nil otherwise
this function returns the first association with value value in alist
this function closes a notification with identifier id
this function returns the size of the text of window’s buffer in pixels
this variable allows the user to specify the dimensions of graphical displays returned by display-mm-height and display-mm-width in case the system provides incorrect values
this function returns the current mode for reading keyboard input
the function member tests to see whether object is a member of list comparing members with object using equal
this function converts string-or-char which should be either a character or a string to upper case
if non-nil this buffer-local variable holds a regular expression that specifies what text can appear before the open-parenthesis that starts a defun
this function is the guts of query-replace and related commands
this option lets you customize indentation on a per-mode basis
takes a function fn and fewer than the normal arguments to fn
this function defines tabname a symbol as an abbrev table name i
this is another name for set-marker
this function stores value in extra slot n of char-table
this function converts the region from beg to end from base 64 code into the corresponding decoded text
return the font object that is being used to display the character at position position in the window window
this variable controls whether splitting a window shall make a new parent window
this function stores definition in the function cell of symbol
when this variable is non-nil no end-of-line conversion is done no matter which coding system is specified
return bitwise or of bool vectors a and b
return non-nil if object is one of the two canonical boolean values t or nil
this function rounds float to the next higher integral value and returns that value as a floating-point number
this is the same as (car car cons-cell
return button’s button-type
you can specify more complex ways of choosing a fill prefix automatically by setting this variable to a function
this function returns t if regexp matches the text immediately before point
this function returns t if object1 and object2 have equal components and nil otherwise
this special form declares that a function is a command and that it may therefore be called interactively via m-x or by entering a key sequence bound to it
call fn with every item in list while pred item is non-nil
this function returns a two-level deep copy of alist it creates a new copy of each association so that you can alter the associations of the new alist without changing the old one
this function returns a list of face attributes corresponding to font
this function returns the “vendor” that provided the window system software as a string
this function stops emacs and returns control to the superior process
this command calls the function in indent-line-function to indent the current line in a way appropriate for the current major mode
this predicate tests whether its argument is a number either integer or floating point and returns t if so nil otherwise
this function appends the text string to the first entry in the kill ring and makes the yanking pointer point to the combined entry
this function reads a lisp object using the minibuffer and returns it without evaluating it
return true if list and list2 has the same items
return a lambda which destructures its input as match-form and executes body
this function creates and returns a vector whose elements are the arguments objects
if this is non-nil normal evaluation of defining forms such as defun and defmacro instruments them for edebug
this function moves forward across arg default 1 balanced expressions
this function reads and returns a character of command input
this function renames the current buffer to newname
this function lets you take the cdr of a cons cell while avoiding errors for other data types
this function returns the index of the current frame number for image counting from 0
this function reads the
this function processes files synchronously in a separate process
this function sets symbol’s property list to plist
this function unpacks data from the unibyte string or byte array bindat-raw according to spec
this returns the sum of two time values one of which ought to represent a time difference rather than a point in time
this function sets the prop property to value for the text between start and end in the string or buffer object
this function returns the binding for command key in the current global keymap or nil if it is undefined there
this function works like assoc except that key must be a string or symbol and comparison is done using compare-strings
this function returns the message currently being displayed in the echo area or nil if there is none
this function removes function from the hook variable hook
this normal hook runs before a buffer is saved in its visited file regardless of whether that is done normally or by one of the hooks described above
this function reads and returns a register name prompting with prompt and possibly showing a preview of the existing registers and their contents
this creates and returns an empty category table
this function moves point to the start of the screen line count screen lines down from the screen line containing point
this function returns non-nil if all the face attributes in attributes are supported see face attributes
take list of unary predicates preds and return a unary
this function deletes all frames on terminal and frees the resources used by it
if this variable is non-nil calling recenter with a nil argument redraws the frame
if this variable has a non-nil value the default running the command eval-expression causes debug-on-error to be temporarily bound to to t
this function returns the width of the screen in millimeters or nil if emacs cannot get that information
add the advice function to the named function symbol
this function creates a listing of all current key bindings and displays it in a buffer named *help*
return the pixel width and height of the object in position as a cons cell (width
for the customization option symbol add value to the list of reasonable values
this function removes images in buffer between positions start and end
this function tests for the falsehood of condition
this function returns the buffer position of the next beginning or end of an overlay after pos
return a new list with the items in list grouped into n-sized sublists
this function returns t if the current buffer has been auto-saved since the last time it was read in or saved
this function returns t if object is a live buffer a buffer which has not been killed nil otherwise
this function searches backward from point for string
zip lists together group the head of each list followed by the
this function returns a live window which is heuristically the “least recently used” window
this function works like insert-file-contents except that it does not run find-file-hook and does not do format decoding character code conversion automatic uncompression and so on
this function deletes the frame frame
if alist has a non-nil allow-no-window entry then this function does not display buffer
this function modifies keyboard-translate-table to translate character code from into character code to
splits s into a list of strings on newline characters
this command finds the precise file name for library library
return the buffer position in character units corresponding to given byte-position in the current buffer
this function inserts text in the trace buffer
this function creates a new list whose elements are the elements of list but in reverse order
this function creates a new syntax table
if non-nil edebug binds print-circle to this value while printing results
this function sets the buffer major mode and parses local variables see auto major mode
this function runs an abnormal hook by calling each hook function stopping if one of them “succeeds” by returning a non-nil value
this function returns t if the symbol has an object in its function cell nil otherwise
this function defines a new category with name char and documentation docstring for the category table table
select column from table
return a sequence of the nodes in tree in depth-first search order
this function returns t if object is an atom nil otherwise
this function locks the file filename if the current buffer is modified
this function returns the height in pixels of window’s header line
this holds the nominal email address of the user who is using emacs
this function associates the color name name with color number number on the terminal
this function returns the current vertical scroll position of window
this function deletes the text between positions start and end in the current buffer and returns a string containing the text just deleted
this function deletes all spaces and tabs around point
this function returns a cons cell (col
this function puts the window state state into window
this variable determines whether the debugger is called when an error is signaled and not handled
replace conses with fn nil with initial-value and evaluate
this function makes characters l and r a matching pair of case-invariant delimiters
return a list of -take-while pred list -drop-while pred list ), in no more than one pass through the list
this function reads a string from the minibuffer and returns it
this function sets the emacs-recognized extended file attributes for filename
this function returns a non-nil value if the :slant attribute of face is italic or oblique and nil otherwise
this function returns a list of the edge coordinates of window
this function specifies the argument list signature as the correct way to call function
this function returns t if object is a string or nil
this variable controls whether to make a single non-numbered backup file or multiple numbered backups
this function marks the current buffer as auto-saved
this function cancels any narrowing in the current buffer so that the entire contents are accessible
replace the last occurence of old with new in list
this function returns t if object is a category table otherwise nil
this function sets the process plist of process to plist
this function returns number-or-marker minus 1
return the buffer position in position
this function returns a coding system which uses the end-of-line conversion of eol-coding and the text conversion of text-coding
this function returns the smallest of its arguments
this function capitalizes strings or characters
this function returns t if file is a file in directory dir or in a subdirectory of dir
this function returns the documentation string of face face or nil if none was specified for it
this function converts the text keyseq-text a string constant into a key sequence a string or vector constant
this variable controls which scalable fonts to use
this function returns the symbol network for a network connection or server serial for a serial port connection or real for a real subprocess
return a new list with all duplicates removed
this list specifies which warning types should not be displayed immediately for the user
delete trailing whitespace in the region defined by start and end
this function returns the current buffer’s recorded last file modification time as a list of the form high low microsec picosec
this command fills each paragraph in the region according to its individual fill prefix
this function creates and returns a copy of table
return a new list containing only the elements that are members of both list and list2
this function unlocks the file being visited in the current buffer if the buffer is modified
this function returns non-nil if object is a mouse position list in either of the formats documented in click events); and nil otherwise
this command inserts the entire accessible contents of from-buffer-or-name which must exist into the current buffer after point
return t if pred x is non-nil for all x in list else nil
this function returns the x coordinate of the leftmost column of window equivalent to the left entry in the list returned by window-edges
this returns the ending position of event
this function returns a name that would be unique for a new buffer—but does not create the buffer
this function takes one or more normal hook variable names as arguments and runs each hook in turn
the value of this variable is the number of lines of continuity to retain when scrolling by full screens
return the pixel-based x and y coordinates relative to the upper left corner of the object in position as a cons cell (dx
if this variable is nil switch-to-buffer displays the buffer specified by buffer-or-name at the position of that buffer’s point
this function moves point forward across characters having syntax classes mentioned in syntaxes a string of syntax class characters
if this variable is non-nil then the various find-file commands check for wildcard characters and visit all the files that match them when invoked interactively or when their wildcards argument is non-nil
if this is non-nil activation of the minibuffer raises the frame that the minibuffer window is in
this function deletes the entire text of the current buffer not just the accessible portion leaving it empty
this function finds the closest color among the known colors supported for display to that described by the rgb value rgb a list of color values
this is like insert-buffer-substring except that it does not copy any text properties
this function tests whether all its arguments are numerically equal and returns t if so nil otherwise
the function specified by this option is called to automatically hide frames
this function inserts the contents of file filename into the current buffer after point
return the first x in list where pred x is non-nil else nil
this function returns a list of available font family names
this function erases the editable contents of the minibuffer that is everything except the prompt if a minibuffer is current
call this command after using smie-config-guess to save your settings for future sessions
this command inserts before point the text at the front of the kill ring
the function default-boundp tells you whether symbol’s default value is nonvoid
this command replaces the buffer text with the text of the visited file on disk
this function sets the width of frame measured in characters
return s if it's s-present otherwise return nil
this function returns filename minus its extension if any
this function returns a symbol whose name tells what window system is used for displaying frame which defaults to the currently selected frame
return a list whose elements are elements from list selected
this function sets the access control list for filename to acl
the value of this variable should be either nil or a fraction f between 0 and 1
start a terminal-emulator for a serial port in a new buffer
return button’s text label
this command displays what is contained in register reg
take n-ary function and n arguments and specialize some of them
take a unary predicate pred and return a unary predicate
this function alters the parameters of frame frame based on the elements of alist
this function specifies a pair of corresponding letters one upper case and one lower case
this returns the current nominal size of table
this function creates and returns a new full keymap
this function returns symbol’s default value
apply fn to each element of tree and make a list of the results
this function modifies the existing fontset name to use the font matching with font-spec for the character character
remove the advice function from the named function symbol
this function reads the name of a customizable variable and returns it as a symbol
report the width and type of scroll bars specified for window
the buffer-local value of this variable holds the symbol for the current major mode
removes an existing file watch specified by its descriptor
this function converts the category set category-set into a string containing the characters that designate the categories that are members of the set
this function returns a copy of abbrev-table—a new abbrev table containing the same abbrev definitions
this function deletes from alist all the elements whose cdr is eq to value
this function sets the parameter parm of terminal to the specified value and returns the previous value of that parameter
return a new list with only the members of list that are not in list2
splice lists generated by fun in place of elements matching pred in list
this variable holds a list of functions that try to determine a coding system for a file based on its undecoded contents
this function compares two strings a character at a time
this function clears an image cache removing all the images stored in it
this function returns a non-nil value if object is an abbrev table
return the tail of list starting from the first item for which pred item returns nil
this function returns the width in columns of the character char if it were displayed in the current buffer
this function returns the value stored in symbol’s value cell
this function returns the first character in string
this function returns t if object is a keymap nil otherwise
return non-nil if the current token is the first on the line
this function pushes the text string onto the kill ring and makes the yanking pointer point to it
call function for characters in charset
this function returns t if point is at the end of the buffer
this function returns a non-nil value if object is a named face a lisp symbol or string which serves as a face name
this function returns the user name corresponding to emacs’s real uid
this function returns the default file permissions as an integer
convert s to uppercamelcase
this function moves window’s bottom edge by delta lines
this function returns a list specifying the previous contents of window
this command displays a description of the contents of the current buffer’s case table
this is the default file name for reading and saving abbrevs
this function returns non-nil if the current major mode is derived from any of the major modes given by the symbols modes
this command quits window and buries its buffer
if val evaluates to non-nil bind it to var and execute body
if function is a byte-code function object this immediately finishes loading the byte code of function from its byte-compiled file if it is not fully loaded already
this function marks variable a symbol automatically buffer-local so that any subsequent attempt to set it will make it local to the current buffer at the time
this function returns the interactive form of function
this function creates and returns a new frame on display taking the other frame parameters from the alist parameters
this command recompiles every ‘
this command closes the most recently opened and not yet closed block
if this variable is nil emacs does not lock files
this is the same as (car cdr cons-cell or nth 1 cons-cell
this function is the handy way to add function function to hook variable hook
this returns t if variable either has a buffer-local value in buffer buffer or is automatically buffer-local
this macro defines a pop-up menu and/or menu bar submenu whose contents are given by menu
replacements is a list of cons-cells each car is replaced with cdr in s
edebug binds debug-on-error to this value if debug-on-error was previously nil
convert all adjacent whitespace characters to a single space
this variable is used by the function delete-auto-save-file-if-necessary
return the window that position is in
this function performs redisplay provided there is no pending input from the user then waits seconds seconds or until input is available
this predicate tests whether its argument is zero and returns t if so nil otherwise
this function lets you cycle conveniently through all the frames on the current display from an arbitrary starting point
this function returns all parameters of window and their values
this is the soft limit for the acceptable size of an undo list
this function returns the contents of extra slot n of char-table
this function returns a copy of list with all elements removed which are eq to object
this function returns the contents of the register reg or nil if it has no contents
this function returns the name of a directory that is not magic
this extracts the data encapsulated by node and returns it
return the last n items of list in order
this function invokes the editor command loop
this function parses the command line that emacs was called with processes it and amongst other things loads the user’s init file and displays the startup messages
this function resumes the previously suspended terminal device tty where tty has the same possible values as it does for suspend-tty
this function returns the current depth of recursive edits
this command marks the current location of point as the beginning of an abbrev
this function makes buffer-or-name the current buffer and displays it in some window preferably not the window previously selected
this command makes a symbolic link to filename named newname
this function marks obsolete-name as obsolete
this function returns the current 1-minute 5-minute and 15-minute system load averages in a list
this function returns arg and has no side effects
if this variable is non-nil the prefixes specified by a group’s :prefix keyword are omitted from tag names whenever the user customizes the group
this function removes the binding of key from the current local map
these add a new node encapsulating data putting it respectively at the beginning or end of ewoc’s chain of nodes
this option specifies how many seconds to wait between execution steps in trace mode or continue mode
if this is non-nil fit-window-to-buffer can resize windows horizontally
this macro defines a generic mode command named mode a symbol not quoted
this function moves forward into arg default 1 levels of parentheses
this function returns the remapping for command a symbol given the current active keymaps
this function resumes execution of the process process by sending it the signal sigcont
this function returns the minimum accessible value of point in the current buffer
this function copies the sign of x2 to the value of x1 and returns the result
this function raises frame frame default the selected frame
this function scrolls forward by count lines in the selected window
if this variable is non-nil then save-buffer protects against i/o errors while saving by writing the new file to a temporary name instead of the name it is supposed to have and then renaming it to the intended name after it is clear there are no errors
this function returns the current gap size of the current buffer
this variable specifies errors which should not enter the debugger regardless of the value of debug-on-error
this function sorts list stably though destructively and returns the sorted list
this function returns the process id of the emacs process as an integer
this function decodes a character that is assigned a code-point in charset to the corresponding emacs character and returns it
this function returns the number of screens associated with the display
this macro creates a new list whose car is element and whose cdr is the list specified by listname and saves that list in listname
call fun on each node of tree that satisfies pred
is s1 less than s2
this function returns a cons cell (s
this is like get-char-property but gives extra information about the overlay that the property value comes from
if the value of this variable is non-nil display-buffer is allowed to split an existing window to make a new window for displaying in
this sets the :weight attribute of face to normal if bold-p is nil and to bold otherwise
this function calls its argument function for each element of char-table that has a non-nil value
this function returns a string describing character in the standard emacs notation for characters that appear in text—like single-key-description except that control characters are represented with a leading caret which is how control characters in emacs buffers are usually displayed
this function returns the minimum size of window
replace first item in list satisfying pred with result of rep called on this item
the value of this variable is the help character—the character that emacs recognizes as meaning help
this function returns a list of all the overlays that cover the character at position pos in the current buffer
this function returns the parser state that the parser would reach at position pos starting from the beginning of the buffer
this function returns t if object is an array
if emacs is idle this function returns the length of time emacs has been idle as a list of four integers sec-high sec-low microsec picosec using the same format as current-time see time of day
this function returns window’s display table or nil if there is none
this function chooses a plausible coding system for decoding the text from start to end
this returns number converted to an integer by rounding downward towards negative infinity
this function returns a value that describes what color should ideally look like on frame
this function returns the selinux context of the file filename as a list of the form user role type range
this function is similar to window-edges but the returned edge values are for the text area of the window
returns the longest prefix s1 and s2 have in common
this function returns a list of the keymaps of currently enabled minor modes
destructive set cdr to the cons of car and cdr
this function sets the propname property of charset to the given value
this function displays string temporarily at the end of the minibuffer text for a few seconds or until the next input event arrives whichever comes first
remove whitespace at the end of s
this function returns the current gap position in the current buffer
this function gives process the filter function filter
this command replaces the minibuffer contents with the value of the nth previous older history element
return a new list with the items in list grouped into n-sized sublists at offsets step apart
this function clears the table of defined colors for a text terminal
if s is longer than len wrap the words with newlines
this function searches alist for a match for key
this function returns t when the calling function was called using call-interactively
if non-nil the value should be a regular expression describing the end of a sentence including the whitespace following the sentence
this function sets the insertion type of marker marker to type
this command adds to the prefix argument for the following command
this is just like forward-char except that it moves in the opposite direction
return information on the notification server a list of strings
this is similar to eval-region but the arguments provide different optional features
this function returns a category name a character which is not currently defined in table
this function sets the display table of window to table
this function is a convenience for defining tool bar items which are consistent with existing menu bar bindings
this function installs the variable customizations specified by args
this macro enables you to easily define setf methods for simple cases
this function reads a shell command from the minibuffer prompting with prompt and providing intelligent completion
this function returns t if the selected window is the only live window and nil otherwise
this is like next-char-property-change but scans back from pos instead of forward and returns the minimum buffer position if no change is found
this function inserts in the current buffer a directory listing for directory file formatted with ls according to switches
this function disables the custom theme named theme
this function returns position information corresponding to pixel coordinates x and y in a specified frame or window frame-or-window which defaults to the selected window
this insert a button with the label label at point and returns it
this works like substring but discards all text properties from the value
this function returns a regular expression matching a character in the list of characters chars
this returns the object in ring found at index index
this function returns x raised to power y
this function is used when building emacs just before the runnable emacs is dumped
this command enables recording undo information for buffer buffer-or-name so that subsequent changes can be undone
this function is like setq-default except that symbol is an ordinary evaluated argument
return a new list of the first n items in list or all items if there are fewer than n
this function returns the overall prompt string of keymap or nil if it has none
this macro defines a help command named fname that acts like a prefix key that shows a list of the subcommands it offers
remove suffix if it is at end of s
these are the values for print-length and print-level used by eval-expression and thus indirectly by many interactive evaluation commands see evaluating emacs-lisp expressions in the gnu emacs manual
this function returns the display-start position of window window
this function returns the definition of key in keymap
return the sum of list
convert the internet address vector ip to a string in the usual dotted notation
this function returns the property list of the character set charset
create a shallow copy of list
this function returns t if image spec has a mask bitmap
this function sets the accessible portion of the current buffer to start at start and end at end
these return respectively the previous node and the next node of node in ewoc
this function sends a signal to process process
lsh which is an abbreviation for logical shift shifts the bits in integer1 to the left count places or to the right if count is negative bringing zeros into the vacated bits
this function returns t if object is a list vector string bool-vector or char-table nil otherwise
this function is like match-beginning except that it returns the position of the end of the match rather than the position of the beginning
this function displays the buffer specified by buffer-or-name in a new frame
this command shows a list of shadowed emacs lisp files
this normal hook runs after a buffer has been saved in its visited file
these functions return the height and width of the main display area of frame measured in pixels
this function converts the region from beg to end into base 64 code
this function sets the value of property prop recorded in overlay to value
this function lets you take the car of a cons cell while avoiding errors for other data types
this function returns the name of buffer as a string
this function returns the charset of highest priority containing the character at position pos in the current buffer
this function reads the name of a command and returns it as a lisp symbol
while a sentinel or filter function is running this function returns non-nil if emacs was waiting for keyboard input from the user at the time the sentinel or filter function was called or nil if it was not
this function returns the value of the extra slot slot of display-table
return a new list with the first item matching pred removed
this function converts all of the letters in the region defined by start and end to upper case
this function returns the buffer that marker points into or nil if it points nowhere
take a comparison function comparator and a list and return
if you set debug-on-event to a special event see special events emacs will try to enter the debugger as soon as it receives this event bypassing special-event-map
this is the basic function for undoing elements of an undo list
if this buffer-local variable is non-nil then emacs displays ‘…’ at the end of a line that is followed by hidden text
this function return a non-nil value if theme a symbol is the name of a custom theme
the value of this buffer-local variable is the spacing between tab stops used for displaying tab characters in emacs buffers
this function establishes the proper major mode and buffer-local variable bindings for the current buffer
this function returns a string containing a copy of the text of the region defined by positions start and end in the current buffer
this function returns the live window at the frame-relative coordinates x and y on frame frame
this function joins the line point is on to the previous line deleting any whitespace at the join and in some cases replacing it with one space
this function returns the process marker for process which is the marker that says where to insert output from the process
remove an object from ring and return that object
this function returns the actual number of entries in table
this function moves point backward across characters whose syntax classes are mentioned in syntaxes
this function returns the last link of list
this function reports the current insertion type of marker
this function makes buffer-or-name the current buffer
this function returns t if the characters of the two strings match exactly
this function regenerates the text of ewoc
this function returns a new object representing frame’s current window configuration
this function returns t if you have permission to open existing files in the directory whose name as a file is dirname otherwise or if there is no such directory it returns nil
this function is like read-char except that if the first character read is an octal digit (0–7), it reads any number of octal digits but stopping if a non-octal digit is found and returns the character represented by that numeric character code
this function sets up a change group for buffer buffer which defaults to the current buffer
this function returns the parent window of window
this function returns t if object is a window whether or not it displays a buffer
if this variable is non-nil read-buffer ignores case when performing completion
this function creates a new hash table according to the specified arguments
this command is identical to word-search-backward except that the beginning or the end of string need not match a word boundary unless string begins or ends in whitespace
this function returns the pid of process
this function returns a floating-point number corresponding to the significand sig and exponent exp
this function returns a live window the one preceding window in the cyclic ordering of windows
this function defines the function or macro named function so as to load automatically from filename
this function runs an abnormal hook by calling each hook function in turn stopping if one of them “fails” by returning nil
this function is like start-process-shell-command but uses start-file-process internally
this command deletes the text between positions start and end in the current buffer and returns nil
this function makes charsets the highest priority character sets
this function returns t if year is a leap year
this function computes the file name for a new backup file for filename
changes an n-arity function fn to a 1-arity function that
this returns the rehash threshold of table
call button’s action property
this function computes and sets header-line-format for the tabulated list buffer see header lines and assigns a keymap to the header line to allow sort entries by clicking on column headers
adaptive fill mode is enabled when this variable is non-nil
this returns a list of numbers starting with from and incrementing by separation and ending at or just before to
this function replaces all occurrences of the character old-char with the character new-char in the region of the current buffer defined by start and end
convert s to upper case
this function makes a backup of the file visited by the current buffer if appropriate
this returns the node in ewoc found at zero-based index n
shut down transaction queue queue waiting for all pending transactions to complete and then terminate the connection or child process
this command reads input and specifies a prefix argument for the following command
this function displays a pop-up menu and returns an indication of what selection the user makes
return the concatenation of the result of mapping fn over list
this function performs a replacement operation on a buffer or string
this function returns a frame configuration list that describes the current arrangement of frames and their contents
return t if pred x is nil for all x in list else nil
this variable specifies the default radix for sort-numeric-fields to parse numbers
this function displays a description of the display table display-table in a help buffer
this option can be used to specify margins around frames to be fit by fit-frame-to-buffer
this is like read-key-sequence except that it always returns the key sequence as a vector never as a string
this function puts value onto symbol’s property list under the property name property replacing any previous property value
this function checks the validity of coding-system
this function rounds float towards zero to an integral value and returns that value as a floating-point number
this function is the general text-sorting routine that subdivides a buffer into records and then sorts them
does s start with prefix
this function moves point to the beginning of the current line
this function scans forward count balanced parenthetical groupings from position from
this function returns the height of text line line in window
this function returns the terminal name that process is using for its communication with emacs—or nil if it is using pipes instead of a terminal see process-connection-type in asynchronous processes
this function returns non-nil if face face specifies a non-nil :inverse-video attribute
the value of this variable is the list of properties to remove from inserted text
this function returns the selected frame
return copy of list starting from index from to index to
this function returns the exit status of process or the signal number that killed it
this function moves forward to the argth end of a defun
this command copies the file oldname to newname
this function returns t if object is a cons cell or nil
this command replaces the minibuffer contents with the value of the nth previous older history element that completes the current contents of the minibuffer before the point
this function returns an alist of attributes for the process specified by its process id pid
this function returns the width of window window in pixels
if this is non-nil the commands eval-defun eval-region eval-buffer and eval-current-buffer instrument all forms even those that don’t define anything
this function performs the loading specified by autoload which should be an autoload object
return the number of elements that are t in bool vector a
this function reads an input method name with the minibuffer prompting with prompt
this function returns the value of charsets property propname
this function is used to open a termscript file that will record all the characters sent by emacs to the terminal
this function generates a string that can be used as a unique file name
this function returns the face number of face face
this function returns non-nil if window has no other window above or below it in its frame i
this function returns t if displaying a buffer named buffer-name with display-buffer would put it in the selected window
this variable specifies the mouse pointer style for void text areas
return t if pred x is non-nil for any x in list else nil
like next-frame but cycles through all frames in the opposite direction
this function returns the handler function for file name file or nil if there is none
zip the two lists list1 and list2 using a function fn this
this variable determines whether the debugger is called when quit is signaled and not handled
take a comparison function comparator and a list and return
this function exits from the innermost recursive edit including minibuffer input
if transient mark mode is enabled or force is non-nil this function deactivates the mark and runs the normal hook deactivate-mark-hook
this function expands the wildcard pattern pattern returning a list of file names that match it
this function converts filename to an absolute file name
this is a version of butlast that works by destructively modifying the cdr of the appropriate element rather than making a copy of the list
returns t if object is a symbol that names a character set nil otherwise
this function returns the window that was selected when the minibuffer was entered
this function defines the symbol new-alias as a variable alias for symbol base-variable
this function is the inverse of decode-time
return the image or string object in position either nil an image (image
this function calls the interactively callable function command providing arguments according to its interactive calling specifications
this function returns the name of the help buffer which is normally *help*; if such a buffer does not exist it is first created
this variable specifies a list of directories for searching for bitmap files for the :stipple attribute
this function sets the value in char-table for a range of characters range
this function creates and returns a transaction queue communicating with process
this function returns a new string containing one character character
given a syntax descriptor desc a string this function returns the corresponding raw syntax descriptor
this function outputs the printed representation of object to stream
this function returns the end of the field specified by pos
this function sets the base remapping of face to its default value which inherits from face’s global definition
this function is like member except that object should be a string and that it ignores differences in letter-case and text representation upper-case and lower-case letters are treated as equal and unibyte strings are converted to multibyte prior to comparison
this function tries to display buffer in a window below the selected window
this function places a boundary element in the undo list
this command replaces buffer-or-name with some other buffer in all windows displaying it
return the position at which button starts
this function sets the default permissions for new files created by emacs and its subprocesses
return a new list of the items in list for which pred returns a non-nil value
this function reports a warning using the value of (format message args
this function completes the file name filename in directory directory
if the value is non-nil then comments are treated as whitespace by the functions in this section and by forward-sexp scan-lists and scan-sexps
this function returns a hash code for lisp object obj
this function positions point in window at position position in window’s buffer
this function reads the name of a buffer and returns it as a string
this function inserts a newline then indents the new line the one following the newline just inserted according to the major mode
this function returns a font entity that best matches the font spec font-spec on frame frame
set the property prop of abbrev to value val
this function replaces olddef with newdef for any keys in keymap that were bound to olddef
this function sets the binding for key in keymap
this function sends a notification to the desktop via d-bus consisting of the parameters specified by the params arguments
this function beeps or flashes the screen see visible-bell below
this function warps the mouse to position x y in frame frame
this function returns a list representing the window tree for frame frame
this function tries immediately to redisplay
like y-or-n-p except that if the user fails to answer within seconds seconds this function stops waiting and returns default
this function returns the first association for key in alist comparing key against the alist elements using equal see equality predicates
return a list of all items in list except for the last
this function converts a time value into calendrical information
this function returns t if the file filename1 is newer than file filename2
this variable defines the maximum depth allowed in calls to eval apply and funcall before an error is signaled (with error message "lisp nesting exceeds max-lisp-eval-depth")
this function signals an error with an error message constructed by applying format see formatting strings to format-string and args
this function returns a buffer named buffer-or-name
a list of regular expressions that specify buffers that should be displayed in the selected window
this function returns the xlfd x logical font descriptor a string matching font
return the image object in position either nil or an image (image
the value of this variable is the maximum size of mark-ring
this variable’s value is an alist of parameter values used when creating the initial frame
this command switches to the next buffer in window thus undoing the effect of the last switch-to-prev-buffer command in window
this function returns non-nil if window is a minibuffer window
used in keymaps to undefine keys
this option specifies the size of the scroll margin—a minimum number of lines between point and the top or bottom of a window
this function reads a coding system using the minibuffer prompting with string prompt and returns the coding system name as a symbol
this function returns a list of coding systems that could be used to encode all the character sets in the list charsets
this function converts seconds the number of seconds since the epoch to a time value and returns that
return a list of iterated applications of fun to init
this function returns the binding for key in the current local keymap or nil if it is undefined there
this function returns number-or-marker plus 1
this function returns the documentation string of face as a face
the value of this variable is a mode line construct that controls the contents of the mode-line
this function moves backward across arg default 1 balanced groups of parentheses
this function sets the binding of key in the current global map to binding
this function returns the largest of its arguments
slices s up at every index matching regexp
this function returns the currently active minibuffer window or nil if there is none
this function sets the buffer associated with process to buffer
return a new list with the concatenation of the elements in the supplied lists
this command provides help for the current mode
this command activates input method input-method for the current buffer
this function returns a multibyte string containing the same sequence of characters as string
this function returns point-max if flag is greater than 0 point-min otherwise
this function returns t if display supports selections
this function is the default value of blink-paren-function
this function reads a string that is a color specification either the color’s name or an rgb hex value such as #rrrgggbbb
this function marks window as dedicated to its buffer if flag is non-nil and non-dedicated otherwise
in s is the first letter upper case and all other letters lower case
return a function fn composed n times with itself
this command sorts the region between start and end alphabetically as specified by record-regexp and key-regexp
this function returns a string containing the emacs standard notation for the input events in sequence
this function moves point to the end of the current line
is s a number
return the last x in list where pred x is non-nil else nil
if non-nil an expression to test for at every stop point
this function returns the current syntax table which is the table for the current buffer
if this variable is non-nil it specifies the initial execution mode for edebug when it is first activated
if non-nil edebug binds print-length to this value while printing results
the value of asin arg is a number between -pi/2 and pi/2 inclusive whose sine is arg
this creates and returns an indirect buffer named name whose base buffer is base-buffer
this function specifies the custom theme theme’s variable settings
this function returns the proper left margin value to use for filling the text around point
this function is like call-process-shell-command but uses process-file internally
this function moves back to the argth beginning of a defun
takes a list of functions and returns a fn that is the
this is the upper limit for the acceptable size of an undo list
this function returns a list of character sets of highest priority that contain characters in string
this function sets the display-start position of window to position in window’s buffer
this function enters the debugger
this function dumps the current state of emacs into an executable file to-file
this function returns the number of words between the positions start and end in the current buffer
this function returns t if display is a graphic display capable of displaying several frames and several different fonts at once
this function returns the name under which the user is logged in
this function returns the coding system used for decoding keyboard input from terminal
this function returns t if a file named filename exists and you can read it
this function returns a list of attributes of file filename
if this variable is nil then blink-matching-open does nothing
if non-nil continue defining or executing any keyboard macro that is executing outside of edebug
this function returns an efficient regular expression that will match any of the strings in the list strings
if s is shorter than len pad it with padding on the left
this command inserts newlines into the current buffer before point
are all the letters in s in lower case
this function returns the editable contents of the minibuffer that is everything except the prompt as a string if a minibuffer is current
this function is like get-char-property except that it pays attention to properties’ stickiness and overlays’ advancement settings instead of the property of the character at
this returns non-nil if plist contains the given property
this function makes table the syntax table for the current buffer
return the tail of list without the first n items
a list of buffer names for buffers that should be displayed in the selected window
this function returns the file name of the device used by terminal
this function returns t if point is at the beginning of the buffer
this function sends string to terminal without alteration
this function reads and returns a character of command input
this function returns t if object is a built-in function
this command fills the paragraph at or after point
this function moves point in the current buffer forward skipping over a given set of characters
the function scans text for a change in the prop property then returns the position of the change
this variable affects the behavior of forward-word and everything that uses it
this variable holds an alist of frame parameters see frame parameters which is used by the default function in pop-up-frame-function to make a new frame
this function returns the truename of the file filename
this function is just like forward-word except that it moves backward until encountering the front of a word rather than forward
this function discards the contents of the terminal input buffer and cancels any keyboard macro that might be in the process of definition
this function puts value in the value cell of symbol
this function returns a non-nil value if filename is a possible name for a backup file
convert s to its initials
this command visits filename like find-file does but it does not perform any format conversions see format conversion character code conversions see coding systems or end-of-line conversions see end of line conversion
this function reads a file name prompting with prompt and providing completion
this sets the :underline attribute of face to underline
if this variable is non-nil switch-to-prev-buffer and switch-to-next-buffer may switch to a buffer that is already visible on the same frame provided the buffer was shown in the relevant window before
this function returns the proper justification style to use for filling the text around point
this function filters the buffer text between start and end using a function specified by the variable filter-buffer-substring-function and returns the result
this function acts on the text between start and end adding the face face to the face text property
this function calls function once for each of the associations in table
return a new list of the non-nil results of applying fn to the items in list
this command sorts lines in the region between start and end comparing them numerically by the fieldth field of each line
separate list into an alist whose keys are fn applied to the
this function tries to display buffer by choosing an existing window and displaying the buffer in that window
this predicate tests whether its argument is an integer and returns t if so nil otherwise
these functions return the height and width of frame measured in lines and columns
this predicate returns non-nil if object is a valid case table
this function returns a copy of overlay
this function removes the image with specification spec from the image cache of frame frame
this function deletes overlay
this variable controls the display of the startup echo area message
move to the nth next button or nth previous button if n is negative
this command deletes count characters backward changing tabs into spaces
this variable’s value specifies the style of justification to use for text that doesn’t specify a style with a text property
this function returns t if variable a symbol is not void and nil if it is void
this function returns the combination limit for window
this function deletes the text of the field specified by pos
return a new list containing the elements of list and elements of list2 that are not in list
this function attempts to make window as large as possible in both dimensions without resizing its frame or deleting other windows
this function returns the number of screen lines in the text from beg to end
this function adds keymap as a transient keymap which takes precedence over other keymaps for one or more subsequent keys
this function displays a message in the echo area
this predicate returns t if its floating-point argument is a nan nil otherwise
this function is used to ask a user how to proceed after an attempt to modify an buffer visiting file filename when the file is newer than the buffer text
this is a wrapper for completion-table-dynamic that saves the last argument-result pair
this function acts like eq except when both arguments are numbers
this function returns the length of list with no risk of either an error or an infinite loop
count occurrences of regexp in s'
this returns number converted to an integer by rounding upward towards positive infinity
this variable if non-nil in a particular buffer tells save-buffers-kill-emacs and save-some-buffers if the second optional argument to that function is t to offer to save that buffer just as they offer to save file-visiting buffers
this macro creates a buffer-local binding in the current buffer for variable and gives it the buffer-local value value
replaces old with new in s
this sets the :inverse-video attribute of face to inverse-video-p
this function returns the acl for the file filename
this function redirects focus from frame to focus-frame
remove whitespace at the beginning and end of s
this function returns a copy of the tree tree
when expr in non-nil thread it through the first form (via [-->](#---x-form-rest-more)),
this function sets the debug-on-exit flag of the stack frame level levels down the stack giving it the value flag
this variable controls how scrolling is done automatically when point moves off the screen or into the scroll margin
this sets the right-margin property on the text from from to to to the value margin
if this option is non-nil fit-window-to-buffer can fit a frame to its buffer
this function returns non-nil if position pos in the current buffer is on a link
take a nested list l and return its contents as a single flat list
to generate repeat events the number of milliseconds between successive button presses must be less than the value of double-click-time
return a new list of all elements in list separated by sep
this function puts image image in front of pos in the current buffer
this function returns t if point is at the beginning of a line
insert the strings strings just like the function insert-before-markers but inherit any sticky properties from the adjoining text
this function returns the name of the font of face face
this command displays a list of characters in the character set charset
return the reverse of s
compute the least fixpoint of fn with initial input list
return the actual row and column in position as a cons cell (col
if s is shorter than len pad it with spaces so it is centered
this command specifies coding-system as the coding system to use for decoding keyboard input from terminal
this function “constrains” new-pos to the field that old-pos belongs to—in other words it returns the position closest to new-pos that is in the same field as old-pos
this function returns the state of window as a lisp object
this is the regular expression describing line-beginnings that separate pages
return a new list with the last item matching pred removed
this function returns t if object is a live window or an internal window in a window tree
this function behaves exactly like error except that it uses the error symbol user-error rather than error
the value of this variable is the number of bytes of storage that must be allocated for lisp objects after one garbage collection in order to trigger another garbage collection
this variable gives major modes a way to specify not to break a line at certain places
convert s to titleized words
this function returns buffer’s character-change modification-count
this function is like assoc in that it returns the first association for key in alist but it makes the comparison using eq instead of equal
this function compares two window configurations as regards the structure of windows but ignores the values of point and mark and the saved scrolling positions—it can return t even if those aspects differ
this function scrolls the text in the selected window so that point is displayed at a specified vertical position within the window
this function scans string for the above special sequences and replaces them by what they stand for returning the result as a string
this command inserts the contents of file file converting it according to format format
this function returns the width of the screen in pixels
this function makes the byte compiler warn that the variable obsolete-name is obsolete
this returns the day number within the year corresponding to time
this function will tell you whether a process has given control of its terminal to its own child process
this function returns a new vector consisting of length elements each initialized to object
this function tries to restore the state of window that existed before its buffer was displayed in it
this variable defines the limit on the total number of local variable bindings and unwind-protect cleanups see cleaning up from nonlocal exits that are allowed before emacs signals an error (with data "variable binding depth exceeds max-specpdl-size")
this function returns the number of elements in sequence
the attributes of remote files can be cached for better performance
this function reports a warning using the value of (format message args
this behaves like scroll-up except that if the selected window cannot be scrolled any further and the value of the variable scroll-error-top-bottom is t it tries to move to the end of the buffer instead
the value of this variable is a list of completion style symbols to use for performing completion
this function returns non-nil if images of type type can be loaded and displayed
edebug binds debug-on-quit to this value if debug-on-quit was previously nil
this command expands the abbrev before point if any
like start-process this function starts a new asynchronous subprocess running program in it and returns its process object
this function returns a list containing all the elements of sequences
counts the number of items in list where pred item is non-nil
this function returns the nth element of list
this command deletes the file filename
this function gives the file named oldname the additional name newname
this function searches forward in the current buffer for a string of text that is matched by the regular expression regexp
if non-nil this variable is a string that specifies a file or directory for emacs to display after starting up instead of the startup screen
this function reads the first textual lisp expression from the text in string
this function returns the variable at the end of the chain of aliases of variable
this function checks whether a window window occupies the frame-relative coordinates coordinates and if so which part of the window that is
this function writes the current buffer into file filename makes the buffer visit that file and marks it not modified
this is like looking-at except that it performs the full backtracking specified by the posix standard for regular expression matching
if non-nil edebug tests coverage of all expressions debugged
this function behaves like equal in all cases but also requires that for two strings to be equal they have the same text properties
this function returns the binary exponent of x
this function returns the parent of char-table
this function returns the value of the parameter parameter a symbol of terminal
this variable holds the default input method for commands that choose an input method
this function clears out the record of the last modification time of the file being visited by the current buffer
special mode is a basic major mode for buffers containing text that is produced specially by emacs rather than directly from a file
this function reports the scroll bar type settings for frame frame
this function returns a list of pixel coordinates for the edges of window
this function returns the height in pixels of window’s header line
this function returns the absolute file name of the file that buffer is visiting
this macro allows for more complex setf expansions than the previous form
if this option is non-nil kill-region does not signal an error if the buffer or text is read-only
this function appends the contents of the region delimited by start and end in the current buffer to the end of file filename
this function creates a new frame and displays the buffer in that frame’s window
this function returns t if object is a vector
this function returns the current time as a floating-point number of seconds since the epoch
this function returns t if file filename is an absolute file name nil otherwise
used only in one-line paragraphs this regular expression acts as an additional check of the validity of the one available candidate fill prefix the candidate must match this regular expression or match comment-start-skip
this function converts string-or-char which should be either a character or a string to lower case
take a predicate pred and a list and return the index of the
this returns non-nil if the face face displays differently from the default face
this command loads the library named library
this function returns the real uid of the user
this function returns the nondirectory part of filename
this function returns the status of process-name as a symbol
this function sets window’s vertical scroll position to lines
this function recenters the overlays of the current buffer around position pos
when the given expression matches the string this function returns a list
the function map-keymap calls function once for each binding in keymap
this function returns a list of strings listing the names of user groups on the system
this function returns the frame that the window window belongs to
this function inserts before point a description of the abbrev table named name
this function searches forward from point for an exact match for string
this function sets point in the current buffer to the value position
this function reads a regular expression as a string from the minibuffer and returns it
this function deletes the current buffer’s auto-save file if delete-auto-save-files is non-nil
the value of this variable is a list of functions to be called after a file is visited
this function returns the fringe bitmaps of the display line containing position pos in window window
this function copies string and searches it for matches for regexp and replaces them with rep
this function returns the current display-width of the minibuffer prompt if a minibuffer is current
this function returns the directory part of filename as a directory name see directory names or nil if filename does not include a directory part
this function returns a copy of string with the text properties properties added
visiting
visiting
visiting
read
read
read
scroll
scroll
scroll
string
string
string
object
object
object
activate
activate
activate
append
append
append
4.3
4.3
4.3
place
place
place
end
end
end
chars
chars
chars
plist
plist
plist
processp
processp
processp
by
by
by
field
field
field
list
list
list
split
split
split
left
left
left
capitalize
capitalize
capitalize
write
write
write
at
at
at
oldname
oldname
oldname
fringe
fringe
fringe
keys
keys
keys
items?
items?
items?
gid
gid
gid
18.2
18.2
18.2
display
display
display
buffer
buffer
buffer
column
column
column
wildcards
wildcards
wildcards
replace
replace
replace



undo
undo
undo
exclusive
exclusive
exclusive
feature
feature
feature
table
table
table
first
first
first
list
list
list
2.7
2.7
2.7
slot
slot
slot
vendor
vendor
vendor
print
print
print
byte-position
byte-position
byte-position
21.3
21.3
21.3
process
process
process
find
find
find
auto-mode-list
auto-mode-list
auto-mode-list



visiting
visiting
visiting
face
face
face
name
name
name
prev
prev
prev
window
window
window
window
window
window
file
file
file
read
read
read
by
by
by
char
char
char
font
font
font
file
file
file
14
14
14
auto
auto
auto
create
create
create
number-or-markers
number-or-markers
number-or-markers
text
text
text
ewoc
ewoc
ewoc
capabilities
capabilities
capabilities
focus-frame
focus-frame
focus-frame
tabulated
tabulated
tabulated
count
count
count
ewoc
ewoc
ewoc
min
min
min
none?
none?
none?
insert
insert
insert
glyph
glyph
glyph
alias
alias
alias
file
file
file
frame
frame
frame
unpack
unpack
unpack
list
list
list
table
table
table
plist
plist
plist
parse
parse
parse
styles
styles
styles
recompile
recompile
recompile
recenter
recenter
recenter
next
next
next
refresh
refresh
refresh
completion
completion
completion
accessible
accessible
accessible
fceiling
fceiling
fceiling
insert
insert
insert
buffer
buffer
buffer
make
make
make
buffer
buffer
buffer
contextual
contextual
contextual
terminal
terminal
terminal
input
input
input
file
file
file
registry
registry
registry
obsolete
obsolete
obsolete
activate
activate
activate
below
below
below



year
year
year
substitute
substitute
substitute
18.5
18.5
18.5
no-confirm
no-confirm
no-confirm
total
total
total
table
table
table
prev
prev
prev
display
display
display
delimiter
delimiter
delimiter
let*
let*
let*
prompt
prompt
prompt
margin
margin
margin
pop
pop
pop
maps
maps
maps
table
table
table
fboundp
fboundp
fboundp
inhibit
inhibit
inhibit
serial
serial
serial
fringes
fringes
fringes
append
append
append
self
self
self
lax
lax
lax
click
click
click
buffer
buffer
buffer
resize
resize
resize
noforce
noforce
noforce
insertion
insertion
insertion
charset
charset
charset
encode
encode
encode
vertical
vertical
vertical
yes
yes
yes
visited
visited
visited
from
from
from
input
input
input
user
user
user
xref
xref
xref
infix
infix
infix
8.4
8.4
8.4
defaults
defaults
defaults
save
save
save
change
change
change
left
left
left



buffer
buffer
buffer
let
let
let
address
address
address
indent
indent
indent
lisp
lisp
lisp
delimiter
delimiter
delimiter
message
message
message
fringes
fringes
fringes
p
p
p
recent
recent
recent
standard
standard
standard
margin
margin
margin
lisp_object
lisp_object
lisp_object
following
following
following
with
with
with
up
up
up
video
video
video
boundary
boundary
boundary
hooks
hooks
hooks
tree
tree
tree
bitmap
bitmap
bitmap
minibuffer
minibuffer
minibuffer
command
command
command
prev
prev
prev



debugger-args
debugger-args
debugger-args
eval
eval
eval
face
face
face
real
real
real
window
window
window
systems
systems
systems
threshold
threshold
threshold
only
only
only
mouse
mouse
mouse
line
line
line
p
p
p
ellipses
ellipses
ellipses
complete
complete
complete
category
category
category
switch
switch
switch
string
string
string
file
file
file
documentation
documentation
documentation
supports
supports
supports
recursive
recursive
recursive
into
into
into
flat
flat
flat
set
set
set
3
3
3
reporter
reporter
reporter



timer
timer
timer
movement
movement
movement
window
window
window
boundp
boundp
boundp
s2
s2
s2
bool
bool
bool
parents
parents
parents
regexps
regexps
regexps
match
match
match
default
default
default
any?
any?
any?
cancel
cancel
cancel
digit
digit
digit
success
success
success
smie
smie
smie
time
time
time
7.13
7.13
7.13
smie
smie
smie
describe
describe
describe
count-current
count-current
count-current
lessp
lessp
lessp
save
save
save
other
other
other
subr
subr
subr
mark
mark
mark
default
default
default
p
p
p
screen
screen
screen
file
file
file
hf
hf
hf
kill
kill
kill
6
6
6
config
config
config
left
left
left
buffer
buffer
buffer
orfn
orfn
orfn
bobp
bobp
bobp
intern
intern
intern
byte
byte
byte
keyboard
keyboard
keyboard
excluded
excluded
excluded
change
change
change
make
make
make
delq
delq
delq
search
search
search
selected
selected
selected
depth
depth
depth
helper
helper
helper
window
window
window
params
params
params
8
8
8
notifications
notifications
notifications
prompt
prompt
prompt
window
window
window
window
window
window
posn
posn
posn
add
add
add
fill
fill
fill
line
line
line
all
all
all
dynamic
dynamic
dynamic
orfn
orfn
orfn
selected
selected
selected
button
button
button
eop
eop
eop
results
results
results
upcase
upcase
upcase
table
table
table
chars
chars
chars
regexp
regexp
regexp
max
max
max
number-or-marker1
number-or-marker1
number-or-marker1
whitespace
whitespace
whitespace
find
find
find
point
point
point
substring
substring
substring
position
position
position
adaptive
adaptive
adaptive
abbrev
abbrev
abbrev
window
window
window
month
month
month
down
down
down
width
width
width
sum
sum
sum
frame
frame
frame
menubar
menubar
menubar
do
do
do
alist
alist
alist
unhandled
unhandled
unhandled
rigidly
rigidly
rigidly
listify
listify
listify
change
change
change
set
set
set
matching
matching
matching
8.6
8.6
8.6
find
find
find
focus
focus
focus
featurep
featurep
featurep
load
load
load
window
window
window
of
of
of
theme
theme
theme
end
end
end
set
set
set
image
image
image
pixel
pixel
pixel
success
success
success
bufferp
bufferp
bufferp
frame
frame
frame
fill-value
fill-value
fill-value
keymap
keymap
keymap
locked
locked
locked
regexps
regexps
regexps
char
char
char
fill
fill
fill
valid
valid
valid
unpack
unpack
unpack
real
real
real
resume
resume
resume
link
link
link
nosqueeze
nosqueeze
nosqueeze
group
group
group
partially
partially
partially
after
after
after
boundary
boundary
boundary
make
make
make
widen
widen
widen
constrain
constrain
constrain
activate
activate
activate
sum
sum
sum
vector
vector
vector
process
process
process
child
child
child
charsets
charsets
charsets
bars
bars
bars
sentinel
sentinel
sentinel
height
height
height
not-this-window
not-this-window
not-this-window
day
day
day
overlays
overlays
overlays
elements
elements
elements
smie
smie
smie
global
global
global
set
set
set
19.1
19.1
19.1
define
define
define
some
some
some
Helper
Helper
Helper
combination
combination
combination
zlib
zlib
zlib
split
split
split
listname
listname
listname
gc
gc
gc
zip
zip
zip
dir
dir
dir
parent
parent
parent
window
window
window
move
move
move
sort
sort
sort
buffer
buffer
buffer
bitmap
bitmap
bitmap
fringe
fringe
fringe
package
package
package
process
process
process
extra
extra
extra
s
s
s
symbol
symbol
symbol
some
some
some
small
small
small
min
min
min
frame
frame
frame
ppss
ppss
ppss
sorted
sorted
sorted
barf
barf
barf
marker
marker
marker
local
local
local



swap
swap
swap
window
window
window
version
version
version
mode
mode
mode
starts
starts
starts
threshold
threshold
threshold
autoload
autoload
autoload
properties
properties
properties
visible
visible
visible
make
make
make
char
char
char
else
else
else
set
set
set
file
file
file
pos
pos
pos
height
height
height
set-minimum
set-minimum
set-minimum
position
position
position
make
make
make
coordinates
coordinates
coordinates
p
p
p
char
char
char
command
command
command
if
if
if
p
p
p
window
window
window
untabify
untabify
untabify
universal
universal
universal
group
group
group
column
column
column
pos
pos
pos
2
2
2
string
string
string
set
set
set
aggressively
aggressively
aggressively
double
double
double
char
char
char
time
time
time
format
format
format
p
p
p
kbd
kbd
kbd
ding
ding
ding
year
year
year
previous
previous
previous
text
text
text
frame
frame
frame
x
x
x
count
count
count
abbrev
abbrev
abbrev
level
level
level
time
time
time
infile
infile
infile
void
void
void
call
call
call
when
when
when
previous
previous
previous
display
display
display
name
name
name
next
next
next
redirect
redirect
redirect
family
family
family
ring
ring
ring



height
height
height
frequent
frequent
frequent
do-not-terminate
do-not-terminate
do-not-terminate
types
types
types
backward
backward
backward
define
define
define
add
add
add
type
type
type
pop
pop
pop
count
count
count
fields
fields
fields
function
function
function
find
find
find
resource
resource
resource
delims
delims
delims
locate
locate
locate
button
button
button
and
and
and
only
only
only
description
description
description
x
x
x
ceiling
ceiling
ceiling
emacs
emacs
emacs
read
read
read
bold
bold
bold
type
type
type
overlays
overlays
overlays
bool
bool
bool
smie
smie
smie
history
history
history
gap
gap
gap
keywords
keywords
keywords
marker
marker
marker
on
on
on
/
/
/
subst
subst
subst
beginning
beginning
beginning
19
19
19
delete
delete
delete
indices
indices
indices
prefix
prefix
prefix
words
words
words
each
each
each
partial
partial
partial



17.11
17.11
17.11
lockfiles
lockfiles
lockfiles
initial
initial
initial
cons-cell
cons-cell
cons-cell
times
times
times
y-limit
y-limit
y-limit
below
below
below
posix
posix
posix
has
has
has
parameter
parameter
parameter
window
window
window
document
document
document
fontset
fontset
fontset
internal
internal
internal
bars
bars
bars
redirect
redirect
redirect
define
define
define
ignore-auto
ignore-auto
ignore-auto
file
file
file
compare-fn
compare-fn
compare-fn
emacs
emacs
emacs
buffer
buffer
buffer
obarray
obarray
obarray
sep
sep
sep
modes
modes
modes
mark
mark
mark
final
final
final
motion
motion
motion
archives
archives
archives
car
car
car
window
window
window
expand
expand
expand
numbers-or-markers
numbers-or-markers
numbers-or-markers
void
void
void
right
right
right
directory
directory
directory
store
store
store
passwd
passwd
passwd
abbrev
abbrev
abbrev
current
current
current



to
to
to
beginning
beginning
beginning
to
to
to
inverse-video-p
inverse-video-p
inverse-video-p
signal
signal
signal
terminal
terminal
terminal
rigidly
rigidly
rigidly
put
put
put
window
window
window
size
size
size
tty
tty
tty
2.3
2.3
2.3
font-lock-list
font-lock-list
font-lock-list
bool
bool
bool
mm
mm
mm
block
block
block
buffer
buffer
buffer
up
up
up
normal
normal
normal
more-numbers-or-markers
more-numbers-or-markers
more-numbers-or-markers
small
small
small
find
find
find
precious
precious
precious
syntaxes
syntaxes
syntaxes
make
make
make
p
p
p
15
15
15
set
set
set
name
name
name
match
match
match
insert
insert
insert
previous
previous
previous
total
total
total
obsolete-face
obsolete-face
obsolete-face
alist
alist
alist
text
text
text
prefix
prefix
prefix
write
write
write
comments
comments
comments
size
size
size
remapping
remapping
remapping
file
file
file
11
11
11
tabname
tabname
tabname
height
height
height
args
args
args
default
default
default
17.6
17.6
17.6
window
window
window
min-width
min-width
min-width
hack
hack
hack
display
display
display
config
config
config
print
print
print
char
char
char
just
just
just
4.1
4.1
4.1
ppss
ppss
ppss
ewoc
ewoc
ewoc



variable
variable
variable
mail
mail
mail
while
while
while
sensibly
sensibly
sensibly
interactive
interactive
interactive
window
window
window
in
in
in
pad
pad
pad
bold-p
bold-p
bold-p
filename
filename
filename
upcase
upcase
upcase
file
file
file
sort
sort
sort
unused
unused
unused
set
set
set
buffer
buffer
buffer
with
with
with
pos
pos
pos
select
select
select
eval
eval
eval
than
than
than
uppercase?
uppercase?
uppercase?
unwrap
unwrap
unwrap
signal
signal
signal
char
char
char
minibuffer
minibuffer
minibuffer
read
read
read
redisplay
redisplay
redisplay
equal
equal
equal
file
file
file
cadr
cadr
cadr
pixels-p
pixels-p
pixels-p
ok
ok
ok
map
map
map
buffers
buffers
buffers
prepare
prepare
prepare
recursive
recursive
recursive
s
s
s
binding
binding
binding
fixup
fixup
fixup
to
to
to
arguments
arguments
arguments
number-or-marker2
number-or-marker2
number-or-marker2
tab
tab
tab
save
save
save
combination
combination
combination
into
into
into
group
group
group
system
system
system
transformer
transformer
transformer
final
final
final
input
input
input
copy
copy
copy
auto
auto
auto
character-set
character-set
character-set
7.1.2
7.1.2
7.1.2
sibling
sibling
sibling
colon
colon
colon
copy
copy
copy
charsetp
charsetp
charsetp
symbolic
symbolic
symbolic
inactive
inactive
inactive
set
set
set
fringe
fringe
fringe
numeric?
numeric?
numeric?
commandp
commandp
commandp
buffer
buffer
buffer
threat
threat
threat
start
start
start
check
check
check
process
process
process
translation
translation
translation
completing
completing
completing
p
p
p
word
word
word
hash
hash
hash
colors
colors
colors
number
number
number
suffix
suffix
suffix
context
context
context
window
window
window
directory
directory
directory
find
find
find
select
select
select
pixel
pixel
pixel
save
save
save
delete
delete
delete
recenter
recenter
recenter
tabs
tabs
tabs
theme
theme
theme
maybe
maybe
maybe
gv
gv
gv
pixel
pixel
pixel
set
set
set
name
name
name
set
set
set
window
window
window
load
load
load
display
display
display
xref
xref
xref
precious
precious
precious
list
list
list
command
command
command
overlay
overlay
overlay
lock
lock
lock
list
list
list
file
file
file
cons
cons
cons
window
window
window
file
file
file
user
user
user
temp
temp
temp
end
end
end
buffer
buffer
buffer
rear-advance
rear-advance
rear-advance
eval
eval
eval
start
start
start
set
set
set



class
class
class
ok-if-already-exists
ok-if-already-exists
ok-if-already-exists
search
search
search
open
open
open
directory
directory
directory
abbrev
abbrev
abbrev
parameter
parameter
parameter



conversion
conversion
conversion
selective
selective
selective
string-or-char
string-or-char
string-or-char
linked
linked
linked
insert
insert
insert
prec2s
prec2s
prec2s
custom
custom
custom
sel
sel
sel
from
from
from
file
file
file
geometry
geometry
geometry
buffer
buffer
buffer
set
set
set
name
name
name
read
read
read
nowarn
nowarn
nowarn
time
time
time
sort
sort
sort
buffer
buffer
buffer
repeat-count
repeat-count
repeat-count
help-map
help-map
help-map
errors
errors
errors
auto
auto
auto
bool
bool
bool
looking
looking
looking
ignore
ignore
ignore
discard
discard
discard
definition
definition
definition
run
run
run
popup
popup
popup



split
split
split
is
is
is
set
set
set



inherit
inherit
inherit
name
name
name
larger
larger
larger
tabulated
tabulated
tabulated
cons
cons
cons
rename
rename
rename
bold
bold
bold
prompt
prompt
prompt
conservatively
conservatively
conservatively
message
message
message
sin
sin
sin
sexp
sexp
sexp
file
file
file
smie
smie
smie
cons*
cons*
cons*
def
def
def
define
define
define
one
one
one
log
log
log



field
field
field
merge
merge
merge
population
population
population
nomodes
nomodes
nomodes
paragraph
paragraph
paragraph
backward
backward
backward
abort
abort
abort
to
to
to
let
let
let
file
file
file
set
set
set
group
group
group
iteratefn
iteratefn
iteratefn
button
button
button
dom
dom
dom
coding
coding
coding
7.1.1
7.1.1
7.1.1
cycle
cycle
cycle
inherit
inherit
inherit
for
for
for
rigidly
rigidly
rigidly
image
image
image
window
window
window
vscroll
vscroll
vscroll
set
set
set
throw
throw
throw
largest
largest
largest
region
region
region
current
current
current
switch
switch
switch
cons*
cons*
cons*
underline
underline
underline
window
window
window
bottom
bottom
bottom
on
on
on
theme
theme
theme



slot
slot
slot
in
in
in
sound
sound
sound
with
with
with
of
of
of
from-string
from-string
from-string
types
types
types
6.6
6.6
6.6
limit
limit
limit
user
user
user



left
left
left
tag
tag
tag
set
set
set
rule
rule
rule
whitespace
whitespace
whitespace
fold
fold
fold
buffer
buffer
buffer
accept-defaults
accept-defaults
accept-defaults
p
p
p
tab
tab
tab
last
last
last
syntax
syntax
syntax
ask
ask
ask
reset
reset
reset
match
match
match
emacs
emacs
emacs
on
on
on
advertised
advertised
advertised
reduce
reduce
reduce
justification
justification
justification
eol-coding
eol-coding
eol-coding
emacs
emacs
emacs
kill
kill
kill
auto
auto
auto
recursion
recursion
recursion
windows
windows
windows
handle
handle
handle
justify
justify
justify
reduce
reduce
reduce
sequence
sequence
sequence
p
p
p
field
field
field
parse
parse
parse
consp
consp
consp
list
list
list
menu
menu
menu
window
window
window
beginning
beginning
beginning
file
file
file
context
context
context
dups
dups
dups
font
font
font
edebug
edebug
edebug
exit
exit
exit
string
string
string
1
1
1
s
s
s
file
file
file
coding
coding
coding
window
window
window
caar
caar
caar
check
check
check
collapse
collapse
collapse
s
s
s
left
left
left
display
display
display
make
make
make
slice
slice
slice
preview
preview
preview
pixel
pixel
pixel
assoc
assoc
assoc
preceding
preceding
preceding
notifications
notifications
notifications
up
up
up
edge
edge
edge
list
list
list
term
term
term
ewoc
ewoc
ewoc
remove
remove
remove
requirements
requirements
requirements
variable
variable
variable
size
size
size
edebug
edebug
edebug
variable
variable
variable
total
total
total
properties
properties
properties
color
color
color
bar
bar
bar
overlay
overlay
overlay
completion
completion
completion
buffer
buffer
buffer
p
p
p
toplevel
toplevel
toplevel
map
map
map



parse
parse
parse
charset
charset
charset
face
face
face
keymap
keymap
keymap
warning
warning
warning
on
on
on
docstring
docstring
docstring
paren
paren
paren
size
size
size
eol
eol
eol
relative
relative
relative
lists
lists
lists
privileged
privileged
privileged
keymap
keymap
keymap
frame
frame
frame
p
p
p



ends
ends
ends
reduce
reduce
reduce
macro
macro
macro
entry
entry
entry
frame
frame
frame
macro-only
macro-only
macro-only
marker
marker
marker
spec
spec
spec
expression
expression
expression
get
get
get
21.2
21.2
21.2
infix?
infix?
infix?
remove
remove
remove
length
length
length
backward
backward
backward
file
file
file
&rest
&rest
&rest
replace
replace
replace
n
n
n
id
id
id
search
search
search
pixel
pixel
pixel
arg-descriptor
arg-descriptor
arg-descriptor



button
button
button
p
p
p
config1
config1
config1
minibuffers
minibuffers
minibuffers
modifiers
modifiers
modifiers
screen
screen
screen
no
no
no
remove
remove
remove
command
command
command
table
table
table
paragraph
paragraph
paragraph
with
with
with
height
height
height
window
window
window
posn
posn
posn
require
require
require
tool
tool
tool
overlays
overlays
overlays
backward-only
backward-only
backward-only
left
left
left
ewoc
ewoc
ewoc
make
make
make
file
file
file
shadows
shadows
shadows
yank
yank
yank
indicate
indicate
indicate
set
set
set
for
for
for
keep-backup-version
keep-backup-version
keep-backup-version
escapes
escapes
escapes
doc
doc
doc
25
25
25
directory
directory
directory
replace
replace
replace
substitute
substitute
substitute
buffer
buffer
buffer
window
window
window
symbol
symbol
symbol
only-in-line
only-in-line
only-in-line
object1
object1
object1
restore
restore
restore
previous
previous
previous
left
left
left
eql
eql
eql
nth
nth
nth
describe
describe
describe
file
file
file
multibyte
multibyte
multibyte
buffer
buffer
buffer
other
other
other
edit
edit
edit
p
p
p
char
char
char
10.5
10.5
10.5
trim
trim
trim
key
key
key
put
put
put
string
string
string
vector
vector
vector
ring
ring
ring
rule
rule
rule
frame-or-window
frame-or-window
frame-or-window
window
window
window
7.1.7
7.1.7
7.1.7
erase
erase
erase
file
file
file



product
product
product
save
save
save
map
map
map
marker
marker
marker
bindat
bindat
bindat
edebug
edebug
edebug
34
34
34
file
file
file
uptime
uptime
uptime
string
string
string
zerop
zerop
zerop
view
view
view



p
p
p
interactive
interactive
interactive
string
string
string
push
push
push
preserve-extended-attributes
preserve-extended-attributes
preserve-extended-attributes
directory
directory
directory
window
window
window
system
system
system
delims
delims
delims
uid
uid
uid
all
all
all
iterate
iterate
iterate
display
display
display
run
run
run
top
top
top
max
max
max
register
register
register
s
s
s
prompt
prompt
prompt
set
set
set
function
function
function
s
s
s
file
file
file
mode
mode
mode
font
font
font
search
search
search
minibuffer
minibuffer
minibuffer
keywordp
keywordp
keywordp
from
from
from
unload
unload
unload
min
min
min
pointer
pointer
pointer
maps
maps
maps
ip
ip
ip
6.7
6.7
6.7
set
set
set
2.6
2.6
2.6
15.2
15.2
15.2
sans
sans
sans
region
region
region
help
help
help
fill
fill
fill
movement
movement
movement
deftheme
deftheme
deftheme
windowp
windowp
windowp



locate
locate
locate
of
of
of
scroll
scroll
scroll
other
other
other
error
error
error
new
new
new
error
error
error
for
for
for
login
login
login
string2
string2
string2
variables
variables
variables
numeric
numeric
numeric
translation
translation
translation
modes
modes
modes
current
current
current
let*
let*
let*
file
file
file
delete
delete
delete
lockfiles
lockfiles
lockfiles
left
left
left
window
window
window
file
file
file
window
window
window
mode
mode
mode
seq
seq
seq
deactivate
deactivate
deactivate
button
button
button
md5
md5
md5
run
run
run
p
p
p
first
first
first
charset
charset
charset
remap
remap
remap
preserve-uid-gid
preserve-uid-gid
preserve-uid-gid
getenv
getenv
getenv
string
string
string
of
of
of
object
object
object



end1
end1
end1
switch
switch
switch
file
file
file
char
char
char
end
end
end
to
to
to
when
when
when
find
find
find
positions
positions
positions
set
set
set
window
window
window
to
to
to
uptime
uptime
uptime
keys
keys
keys
backup
backup
backup
string
string
string
set
set
set
value1
value1
value1
21
21
21
count
count
count
all
all
all
fit
fit
fit
forward
forward
forward
atom
atom
atom
row
row
row
sit
sit
sit
coding-system
coding-system
coding-system
select
select
select
overlay
overlay
overlay
p
p
p
face
face
face
datagram
datagram
datagram
delete
delete
delete
ref
ref
ref
s
s
s
char-table
char-table
char-table
ifname
ifname
ifname
parameters
parameters
parameters
minibuffer
minibuffer
minibuffer
display-table
display-table
display-table
translations
translations
translations
window
window
window
errors
errors
errors



insert
insert
insert
12.3
12.3
12.3
buffer
buffer
buffer
hook
hook
hook
frequent
frequent
frequent
dotimes
dotimes
dotimes
file
file
file
default
default
default
command
command
command
7.7
7.7
7.7
from
from
from



include
include
include
listify
listify
listify
find
find
find
buffer
buffer
buffer
geom
geom
geom
chars
chars
chars
coding
coding
coding
map
map
map
reseat
reseat
reseat
code-point
code-point
code-point
tq
tq
tq



pending
pending
pending
char
char
char
popup
popup
popup
get
get
get
fold-wildcards
fold-wildcards
fold-wildcards
readable
readable
readable
appendp
appendp
appendp
circle
circle
circle
read
read
read
forward
forward
forward
if
if
if
height
height
height
marker
marker
marker
bindings
bindings
bindings
paragraph
paragraph
paragraph
file
file
file
state
state
state
description
description
description
word
word
word
file
file
file
visible-ok
visible-ok
visible-ok
base-name
base-name
base-name
get
get
get
input
input
input
specpdl
specpdl
specpdl
match
match
match
input
input
input
datagram
datagram
datagram
read
read
read
more
more
more
bar
bar
bar
attributes
attributes
attributes
highest
highest
highest
pop
pop
pop
with
with
with
s
s
s
byte
byte
byte
string1
string1
string1



recenter
recenter
recenter
bar
bar
bar
system
system
system
under
under
under
file
file
file
9.3
9.3
9.3
s1
s1
s1
word
word
word
start
start
start
identity
identity
identity
backup
backup
backup
skip
skip
skip
button
button
button
font
font
font
separate
separate
separate
olp
olp
olp
upload
upload
upload
base64
base64
base64
set
set
set
family
family
family
on
on
on
insert
insert
insert
assoc
assoc
assoc
p
p
p
obsolete
obsolete
obsolete
invisibility
invisibility
invisibility
char
char
char
table
table
table
boundaries
boundaries
boundaries
count
count
count
text
text
text
edge
edge
edge
>>
>>
>>
zip
zip
zip
alias
alias
alias
c
c
c
dashed
dashed
dashed
>prec2
>prec2
>prec2
char
char
char
argument
argument
argument
prefer
prefer
prefer
on
on
on
of
of
of
coding
coding
coding
p
p
p
from-file
from-file
from-file
bytes
bytes
bytes
make
make
make
make
make
make
vector
vector
vector
wrap
wrap
wrap
clone
clone
clone
depth
depth
depth
recompile
recompile
recompile
file
file
file
noselect
noselect
noselect
mnemonics
mnemonics
mnemonics
window
window
window
union
union
union
item
item
item
only
only
only
init
init
init
button
button
button
bool
bool
bool
map
map
map
p
p
p
bindat-raw
bindat-raw
bindat-raw
selinux
selinux
selinux
sibling
sibling
sibling
value
value
value
define
define
define
case
case
case
divider
divider
divider
match
match
match
case
case
case
horizontally
horizontally
horizontally
directory
directory
directory
make
make
make
p
p
p
beginning
beginning
beginning
truncate
truncate
truncate
element
element
element
setter
setter
setter
describe
describe
describe
child
child
child
attributes
attributes
attributes
propname
propname
propname
available
available
available
erase
erase
erase
windows
windows
windows
reindent
reindent
reindent
5.1
5.1
5.1
overlayp
overlayp
overlayp
current
current
current
p
p
p
screen
screen
screen
current
current
current
with
with
with
modes
modes
modes
adaptive
adaptive
adaptive
display-message
display-message
display-message
property
property
property
read
read
read
4
4
4
pid
pid
pid
char
char
char
p
p
p
name
name
name
including
including
including
coding
coding
coding
desc
desc
desc
from
from
from
substring
substring
substring
completion
completion
completion
coding
coding
coding



passwd
passwd
passwd
invert
invert
invert
system
system
system
nomessage
nomessage
nomessage
change
change
change
18.3
18.3
18.3
parameter
parameter
parameter
current
current
current
file
file
file
nondirectory
nondirectory
nondirectory
function
function
function
grade
grade
grade
19.3
19.3
19.3
in
in
in
type
type
type
frame
frame
frame
indentation
indentation
indentation
parent
parent
parent
live
live
live
plist
plist
plist
preserve
preserve
preserve
key
key
key
invalidate
invalidate
invalidate
string
string
string
buffer
buffer
buffer
selected
selected
selected
float
float
float
format-string
format-string
format-string
substitute
substitute
substitute
imagemagick
imagemagick
imagemagick
scroll
scroll
scroll
byte
byte
byte
set
set
set
ring
ring
ring
custom
custom
custom
unset
unset
unset
undefined
undefined
undefined
symbolic
symbolic
symbolic
equal
equal
equal
dashed
dashed
dashed
>>
>>
>>
discard
discard
discard
setter
setter
setter
hook
hook
hook
position
position
position
fundamental
fundamental
fundamental
cut
cut
cut
upload
upload
upload
jit
jit
jit
actual
actual
actual
literally
literally
literally
enter
enter
enter
from
from
from
limit
limit
limit
function
function
function
as
as
as
s
s
s
13
13
13
face
face
face
shell
shell
shell
speed
speed
speed
10
10
10
abbrev
abbrev
abbrev
indirect
indirect
indirect
with
with
with
mapreduce
mapreduce
mapreduce
line
line
line
syntax
syntax
syntax
information
information
information
destroy
destroy
destroy
bus
bus
bus
jit
jit
jit
13.5
13.5
13.5
smie
smie
smie
abbrev
abbrev
abbrev



subr
subr
subr
window
window
window
list
list
list



parent
parent
parent
universal
universal
universal
all
all
all
display
display
display
buffers
buffers
buffers
set
set
set



name
name
name
p
p
p
bindat
bindat
bindat
starts
starts
starts
login
login
login
display
display
display
operator
operator
operator
last
last
last
max-height
max-height
max-height
percentage
percentage
percentage
uc
uc
uc
horizontally
horizontally
horizontally
window
window
window
backtrace
backtrace
backtrace
selective
selective
selective
window
window
window
font
font
font
text
text
text
open
open
open
line
line
line
absolute
absolute
absolute
fix-return
fix-return
fix-return
extended
extended
extended
footer
footer
footer
print
print
print
completion
completion
completion
primitive
primitive
primitive
window
window
window
local
local
local
name
name
name
line
line
line
selected
selected
selected
face
face
face



bar
bar
bar
command
command
command
timeout
timeout
timeout
item
item
item
global
global
global
parameters
parameters
parameters
elem
elem
elem
play
play
play
file
file
file
auto
auto
auto
mm
mm
mm
17.2
17.2
17.2
recent
recent
recent
perform
perform
perform
tables
tables
tables
margin
margin
margin
map
map
map
alternatives
alternatives
alternatives
edit
edit
edit
base64
base64
base64
graphic
graphic
graphic
split
split
split
number
number
number
guess
guess
guess
default
default
default
live
live
live
cells
cells
cells
display
display
display
mail
mail
mail
encode
encode
encode
left
left
left
get
get
get
dimensions
dimensions
dimensions
find
find
find
ratio
ratio
ratio
1value
1value
1value
handler
handler
handler
start
start
start
eval
eval
eval
smie
smie
smie
button
button
button
overlay
overlay
overlay
vectorp
vectorp
vectorp
buffer
buffer
buffer
transient
transient
transient
property
property
property
interactive-call
interactive-call
interactive-call
elt
elt
elt
initials
initials
initials
use
use
use
s
s
s
name
name
name
minor
minor
minor
window
window
window
argument
argument
argument
string
string
string
append
append
append
up
up
up
char
char
char
t1
t1
t1
indent
indent
indent
current
current
current
set
set
set
13.4
13.4
13.4
inherit-input-method
inherit-input-method
inherit-input-method
add
add
add
overlay
overlay
overlay
coding
coding
coding
point
point
point
nocopy
nocopy
nocopy
current
current
current
buffer
buffer
buffer
capitalize
capitalize
capitalize
subrp
subrp
subrp
s
s
s
indices
indices
indices
frame
frame
frame
approximate
approximate
approximate
window
window
window
function
function
function
lc
lc
lc
titleized
titleized
titleized
case
case
case
backing
backing
backing
temporary
temporary
temporary
autoloadp
autoloadp
autoloadp
next
next
next
set
set
set
display
display
display
case
case
case
region
region
region
window
window
window
indent
indent
indent
base
base
base
union
union
union
string
string
string
refresh
refresh
refresh
8.3
8.3
8.3
on
on
on
flag
flag
flag
beforep
beforep
beforep
minibuffer
minibuffer
minibuffer
remove
remove
remove
ewoc
ewoc
ewoc
char
char
char
nconc
nconc
nconc
regexp
regexp
regexp
lexical
lexical
lexical
undo
undo
undo
to-eop
to-eop
to-eop
startup
startup
startup
link
link
link
extra
extra
extra
pattern
pattern
pattern
maximize
maximize
maximize
y
y
y
char
char
char



window
window
window
8.7
8.7
8.7
p
p
p
clear
clear
clear
fit
fit
fit
mode
mode
mode
class
class
class
spec
spec
spec
message
message
message
>prec2
>prec2
>prec2



numeric?
numeric?
numeric?
from-map
from-map
from-map
facep
facep
facep
direction
direction
direction
minimum
minimum
minimum
edebug
edebug
edebug
minibufferp
minibufferp
minibufferp
counts
counts
counts
to
to
to
edebug
edebug
edebug
matching
matching
matching
7.14
7.14
7.14
keyseq-text
keyseq-text
keyseq-text
read
read
read
after
after
after
read
read
read
terminal
terminal
terminal
slot
slot
slot
string
string
string
grammar
grammar
grammar
backup
backup
backup
predicate
predicate
predicate
space
space
space
coding
coding
coding
margin
margin
margin
p
p
p
partition
partition
partition
idle
idle
idle
supertype
supertype
supertype
file
file
file
suspend
suspend
suspend
notifications
notifications
notifications
group
group
group
flag
flag
flag
plist
plist
plist
human
human
human
auto
auto
auto
line
line
line
matching
matching
matching
blank
blank
blank
continue-echo
continue-echo
continue-echo
progress
progress
progress
selections
selections
selections
color
color
color
last
last
last



table
table
table
coordinates
coordinates
coordinates
continue
continue
continue
then
then
then
contains?
contains?
contains?
according
according
according
tab
tab
tab
36
36
36
right
right
right
matches
matches
matches
file
file
file
global
global
global
substring
substring
substring
at
at
at
buffer
buffer
buffer
lock
lock
lock
position
position
position
present?
present?
present?
adjust
adjust
adjust
cells
cells
cells
follows
follows
follows
keys
keys
keys
setenv
setenv
setenv
process
process
process
modes
modes
modes
local
local
local
min
min
min
define
define
define
syntax
syntax
syntax
mode
mode
mode
1.1
1.1
1.1
filter
filter
filter
p
p
p
event
event
event
filter
filter
filter
directory
directory
directory
minibuffer
minibuffer
minibuffer
minibuffer
minibuffer
minibuffer
x
x
x
posn
posn
posn
set
set
set
delay
delay
delay
font
font
font
at
at
at
titleized
titleized
titleized
t2
t2
t2



delete
delete
delete
property
property
property
configurations
configurations
configurations
absolute
absolute
absolute
command
command
command
hook
hook
hook
edges
edges
edges
delete
delete
delete
table
table
table
22
22
22
make
make
make
width
width
width
/=
/=
/=
file
file
file
snarf
snarf
snarf
goto
goto
goto



keyboard
keyboard
keyboard
put
put
put
region
region
region
fix
fix
fix
indirect
indirect
indirect
forward
forward
forward
acl
acl
acl
rigid
rigid
rigid
close
close
close
region
region
region
subsetp
subsetp
subsetp
region
region
region
set
set
set
controlling
controlling
controlling
divisor
divisor
divisor
point
point
point
keymapp
keymapp
keymapp
configuration
configuration
configuration
current
current
current
event
event
event
sxhash
sxhash
sxhash
p
p
p
start
start
start
screens
screens
screens
capabilities
capabilities
capabilities
reenter
reenter
reenter
convention
convention
convention
command
command
command
selected
selected
selected
absolute
absolute
absolute
lines
lines
lines
infix?
infix?
infix?
completion
completion
completion
frame
frame
frame
keymap
keymap
keymap
p
p
p
markers
markers
markers
id-format
id-format
id-format
s
s
s
window
window
window
gv
gv
gv
get
get
get
ok-if-exists
ok-if-exists
ok-if-exists
call
call
call
from
from
from
insert
insert
insert
command
command
command
table
table
table
save
save
save
bitmap
bitmap
bitmap
window
window
window
char
char
char
category
category
category
read
read
read
choice
choice
choice
help
help
help
pretty-printer
pretty-printer
pretty-printer
window
window
window
p
p
p
narrow
narrow
narrow
replacements
replacements
replacements
area
area
area
init-value
init-value
init-value
file
file
file
recenter
recenter
recenter
quietly
quietly
quietly
header
header
header
echo
echo
echo
char
char
char
parameters
parameters
parameters
face
face
face
process
process
process
newline
newline
newline
body
body
body
buffer
buffer
buffer
39
39
39
flat
flat
flat
file
file
file



has
has
has
current
current
current
to
to
to
set
set
set
untabify
untabify
untabify
string
string
string
floor
floor
floor
current-name
current-name
current-name
directory
directory
directory
window
window
window
variables
variables
variables
time-value
time-value
time-value
blink
blink
blink
wildcard
wildcard
wildcard
minibuffer
minibuffer
minibuffer
number
number
number
single
single
single
notification
notification
notification
mark
mark
mark
face
face
face
titleize
titleize
titleize
conversion
conversion
conversion
name
name
name
defined
defined
defined
matching
matching
matching
buffer
buffer
buffer
switch
switch
switch
margins
margins
margins
file
file
file
get
get
get
transient
transient
transient
p
p
p
9.4
9.4
9.4
s
s
s
reset
reset
reset
switch
switch
switch
base64
base64
base64
redraw
redraw
redraw
differs
differs
differs
9
9
9
get
get
get
sort
sort
sort
send
send
send
minibuffer
minibuffer
minibuffer
difference
difference
difference
ffloor
ffloor
ffloor
copy-contents
copy-contents
copy-contents
alist
alist
alist
major
major
major
process
process
process
translate
translate
translate
2.4
2.4
2.4
kill
kill
kill
buffer
buffer
buffer
frame
frame
frame
add
add
add
start
start
start
member
member
member
auto
auto
auto
mode
mode
mode
2.11
2.11
2.11
10.3
10.3
10.3
seconds
seconds
seconds
copy
copy
copy
integer
integer
integer
smie
smie
smie
strings
strings
strings
key
key
key
hash
hash
hash
map
map
map
warning
warning
warning
4.7
4.7
4.7
bindings
bindings
bindings
accessible
accessible
accessible
maps
maps
maps
window
window
window
charset
charset
charset
insert
insert
insert
skip
skip
skip
remap
remap
remap
supported
supported
supported
yank
yank
yank
file
file
file
nbutlast
nbutlast
nbutlast
element
element
element
forms
forms
forms
fill
fill
fill
hf
hf
hf
pixel
pixel
pixel
as
as
as
mask
mask
mask
event
event
event
pixelwise
pixelwise
pixelwise
by
by
by
mode
mode
mode
scroll
scroll
scroll
custom
custom
custom
trailing
trailing
trailing
word
word
word
conservatively
conservatively
conservatively
timestamp
timestamp
timestamp
process
process
process
ints-or-markers
ints-or-markers
ints-or-markers
version
version
version
opt
opt
opt
case
case
case
config2
config2
config2
margin
margin
margin
use
use
use
change
change
change
file
file
file
color
color
color
tool
tool
tool
s
s
s
match-form
match-form
match-form



tool
tool
tool
count
count
count
17.1
17.1
17.1
applify
applify
applify
allow-empty
allow-empty
allow-empty
p
p
p
28
28
28
parent
parent
parent
from
from
from
load
load
load
sequences
sequences
sequences
p
p
p
style-variant-p
style-variant-p
style-variant-p
forms
forms
forms
mark
mark
mark
trim
trim
trim
lru
lru
lru
translation
translation
translation
setup
setup
setup
batch
batch
batch
p
p
p
defvar
defvar
defvar
port
port
port
by
by
by
terminal
terminal
terminal
full
full
full
prev
prev
prev
12.5
12.5
12.5
x
x
x
counts
counts
counts
user
user
user
property
property
property
mark
mark
mark
keywords
keywords
keywords
minimize
minimize
minimize
ignored
ignored
ignored
suffix
suffix
suffix
length
length
length
indent
indent
indent
get
get
get
smie
smie
smie
text
text
text
argument
argument
argument
file
file
file
header
header
header
save
save
save
tabulated
tabulated
tabulated
at
at
at
node
node
node
p
p
p
specpdl
specpdl
specpdl
iconify
iconify
iconify
sign
sign
sign
or
or
or
and
and
and
terminal
terminal
terminal
path
path
path
nochange-regexp
nochange-regexp
nochange-regexp
package
package
package
6.5
6.5
6.5
eval
eval
eval
display
display
display
mark
mark
mark
millisec
millisec
millisec
fixfn
fixfn
fixfn
read
read
read
present?
present?
present?
frame
frame
frame
buffer
buffer
buffer
than
than
than
bars
bars
bars
s
s
s
17.7
17.7
17.7
dimensions
dimensions
dimensions
17.5
17.5
17.5
get
get
get
invisible
invisible
invisible
and
and
and
when
when
when
display
display
display
point
point
point
asin
asin
asin
or
or
or
previous
previous
previous
memory
memory
memory
product
product
product
clear
clear
clear
documentation
documentation
documentation
fixfn
fixfn
fixfn
member
member
member
name
name
name
count
count
count
integerp
integerp
integerp
hooks
hooks
hooks
generic
generic
generic
code
code
code
ewoc
ewoc
ewoc
intersection
intersection
intersection
oldmap
oldmap
oldmap
bindat-idx
bindat-idx
bindat-idx
region
region
region
symbolic
symbolic
symbolic
regexp
regexp
regexp
from
from
from
executable
executable
executable
re
re
re



for
for
for
norecord
norecord
norecord
popup
popup
popup
locale
locale
locale
leap
leap
leap
backup
backup
backup
advice
advice
advice
partition
partition
partition
columns
columns
columns
text
text
text
time
time
time
exclusive
exclusive
exclusive
end
end
end
category
category
category
attributes
attributes
attributes
sequence
sequence
sequence
insert
insert
insert
while
while
while
quoted
quoted
quoted
variables
variables
variables
disable
disable
disable
case
case
case
charset
charset
charset
is
is
is
end
end
end
on
on
on
lines
lines
lines
1.2
1.2
1.2
local
local
local
invalidate
invalidate
invalidate
modify
modify
modify
from-buffer-or-name
from-buffer-or-name
from-buffer-or-name
dividend
dividend
dividend
skip
skip
skip
event
event
event
buffer
buffer
buffer
lax
lax
lax
pad
pad
pad
to
to
to
local
local
local
p
p
p
maphash
maphash
maphash
rgb
rgb
rgb
insertion
insertion
insertion
get
get
get
index
index
index
system
system
system
scroll
scroll
scroll
library
library
library
format
format
format
inverse
inverse
inverse
trim
trim
trim
fname
fname
fname
first
first
first
forward
forward
forward
self
self
self
move
move
move
faces
faces
faces



item
item
item
standard
standard
standard
list
list
list
varlist
varlist
varlist
decode
decode
decode
rule
rule
rule
window
window
window
gap
gap
gap
button
button
button
limit
limit
limit
filename
filename
filename
tree
tree
tree
car
car
car
nextrecfun
nextrecfun
nextrecfun
autoload
autoload
autoload
display
display
display
window
window
window
and
and
and
p
p
p
insert
insert
insert
set
set
set
mark
mark
mark
buffer
buffer
buffer
debug
debug
debug
face
face
face
echo
echo
echo
butlast
butlast
butlast
put
put
put
normal
normal
normal
overlay
overlay
overlay
suppress
suppress
suppress
memq
memq
memq
insert
insert
insert
make
make
make
18.4
18.4
18.4
systems
systems
systems
position
position
position
page
page
page
charset
charset
charset
key
key
key
unibyte
unibyte
unibyte
differs
differs
differs
byte
byte
byte
subexp
subexp
subexp
regexp
regexp
regexp
quietly
quietly
quietly
function
function
function
enable
enable
enable
read
read
read
compare
compare
compare
serial
serial
serial
mode
mode
mode
substitute
substitute
substitute
button
button
button
p
p
p
shell
shell
shell
function-list
function-list
function-list
status
status
status
syntax
syntax
syntax
26
26
26
and
and
and
table
table
table
current
current
current
x-limit
x-limit
x-limit
window
window
window
count
count
count
call
call
call
display
display
display
remember-pos
remember-pos
remember-pos
global
global
global
previous
previous
previous
file
file
file
to
to
to
startkeyfun
startkeyfun
startkeyfun
rotate
rotate
rotate
extension
extension
extension
include
include
include
help
help
help
stop
stop
stop
fields
fields
fields
to
to
to
newname
newname
newname
dedicated
dedicated
dedicated
buffer
buffer
buffer
pixel
pixel
pixel
backup
backup
backup
hash
hash
hash
notifications
notifications
notifications
shr
shr
shr
this
this
this
acl
acl
acl
subfeature
subfeature
subfeature
name
name
name
width
width
width
file
file
file
p
p
p
function-object
function-object
function-object
subtract
subtract
subtract
query
query
query
defun
defun
defun
header
header
header
to-code
to-code
to-code
display
display
display
text
text
text
current
current
current
p
p
p
attribute
attribute
attribute



kill
kill
kill
display
display
display
to
to
to
last
last
last
single
single
single
find-file
find-file
find-file
adjust
adjust
adjust
site
site
site
average
average
average
locals
locals
locals
posn
posn
posn
dump
dump
dump
than
than
than
rehash
rehash
rehash
window
window
window
versions
versions
versions
set
set
set
read
read
read
clone
clone
clone
name
name
name
remove
remove
remove
string=
string=
string=
lock
lock
lock
flush
flush
flush
run
run
run
indent
indent
indent
p
p
p
round
round
round
setcar
setcar
setcar
5
5
5
all
all
all
forward
forward
forward
create
create
create
2.1
2.1
2.1
property
property
property
indent
indent
indent
looking
looking
looking
help
help
help
setter
setter
setter
cancel
cancel
cancel
require
require
require
window
window
window
file
file
file
case
case
case
name
name
name
mode
mode
mode
s
s
s
properties
properties
properties
hash
hash
hash
space
space
space
replace
replace
replace
matched
matched
matched
format-args
format-args
format-args
compile
compile
compile
forward
forward
forward
unlispify
unlispify
unlispify
stringp
stringp
stringp
replacement
replacement
replacement
buffer2
buffer2
buffer2
s
s
s
set
set
set
juxt
juxt
juxt
remove
remove
remove
posn
posn
posn
remove
remove
remove
table
table
table
string
string
string
mode-only
mode-only
mode-only
symlink
symlink
symlink
complete
complete
complete
fix
fix
fix
echo
echo
echo
char
char
char
table
table
table
start1
start1
start1
use-mouse-action
use-mouse-action
use-mouse-action
buffer
buffer
buffer
dir
dir
dir
squeeze-after
squeeze-after
squeeze-after
s
s
s
definitions
definitions
definitions
special
special
special
positions
positions
positions
width
width
width
encode
encode
encode
libxml
libxml
libxml
push
push
push
display
display
display
file
file
file
move
move
move
find
find
find
tty
tty
tty
add
add
add
help-line
help-line
help-line
r
r
r
context
context
context
delete
delete
delete
dedicated
dedicated
dedicated
fringe
fringe
fringe
to
to
to
backup
backup
backup
equal
equal
equal
face
face
face
string
string
string
delete
delete
delete
backing
backing
backing
goto
goto
goto
unregister
unregister
unregister
delimited-flag
delimited-flag
delimited-flag
local
local
local
allowed
allowed
allowed
numberp
numberp
numberp
standard
standard
standard
visited
visited
visited
wildcards
wildcards
wildcards
set
set
set



modified
modified
modified
posn
posn
posn
status
status
status
remove
remove
remove
update
update
update
play
play
play
coding
coding
coding
only
only
only
font
font
font
iterate
iterate
iterate
new
new
new
syntax
syntax
syntax
frame
frame
frame
in
in
in
live
live
live
12
12
12
ppss
ppss
ppss
local
local
local
process
process
process
expand
expand
expand
missing-ok
missing-ok
missing-ok
header
header
header
completion
completion
completion
33
33
33
buffer
buffer
buffer
divider
divider
divider
of
of
of
next
next
next
special
special
special
case-fold
case-fold
case-fold
charset
charset
charset
newdef
newdef
newdef
partition
partition
partition
live
live
live
boundaries
boundaries
boundaries
omit-nulls
omit-nulls
omit-nulls
excluded
excluded
excluded
global
global
global
buffer
buffer
buffer
groups
groups
groups
set
set
set
command
command
command
gethash
gethash
gethash
get
get
get
read
read
read
10.4
10.4
10.4
p
p
p
even
even
even
set
set
set
method
method
method
undo
undo
undo
variables
variables
variables
to
to
to



send
send
send
markers
markers
markers
first
first
first
char
char
char
form
form
form
file
file
file
active
active
active
string
string
string
subtract
subtract
subtract
make
make
make
window
window
window
buffer
buffer
buffer



system
system
system
undo
undo
undo
history
history
history
window
window
window
compare
compare
compare
terminal
terminal
terminal
history
history
history
edebug
edebug
edebug
for
for
for
table
table
table
resize
resize
resize
file
file
file
xml
xml
xml
zone
zone
zone
yank
yank
yank
substrings
substrings
substrings
remote
remote
remote
shrink
shrink
shrink
23
23
23
name
name
name
vconcat
vconcat
vconcat
font
font
font
branch
branch
branch
variable
variable
variable
down
down
down
overlay
overlay
overlay
in
in
in
sensibly
sensibly
sensibly
name
name
name
buffer
buffer
buffer
reverse
reverse
reverse
minor
minor
minor
right
right
right
object2
object2
object2
modes
modes
modes
backup
backup
backup
key
key
key
set
set
set
saved
saved
saved
offer
offer
offer
grade
grade
grade
to
to
to
exit
exit
exit
feature
feature
feature
end
end
end
max
max
max
same
same
same
day
day
day
start
start
start
bitmaps
bitmaps
bitmaps
syntax
syntax
syntax
delete
delete
delete
syntax
syntax
syntax
switches
switches
switches
documentation
documentation
documentation
window
window
window



shadows
shadows
shadows
buffer-name
buffer-name
buffer-name
width
width
width
edebug
edebug
edebug
frame
frame
frame
first
first
first
match
match
match
buffer
buffer
buffer
modtime
modtime
modtime
p
p
p
reduce
reduce
reduce
key
key
key
dedicated
dedicated
dedicated
actual
actual
actual
mouse
mouse
mouse
detect
detect
detect
and
and
and
send
send
send
access-type
access-type
access-type
partition
partition
partition
path
path
path
y
y
y
push
push
push
region
region
region
match
match
match
position
position
position
minibuffer
minibuffer
minibuffer
adaptive
adaptive
adaptive
gap
gap
gap
subsetp
subsetp
subsetp
before
before
before
action
action
action
file
file
file
mapc
mapc
mapc
subtype
subtype
subtype
kept
kept
kept
no-enable
no-enable
no-enable
generate
generate
generate
uid
uid
uid
posn
posn
posn
any?
any?
any?
state
state
state
menubar
menubar
menubar
separate
separate
separate
pretend
pretend
pretend
get
get
get
17.9
17.9
17.9
redraw
redraw
redraw
chars
chars
chars
struct
struct
struct
scroll
scroll
scroll
timeout
timeout
timeout
systems
systems
systems
to
to
to
margins
margins
margins
vars-vals
vars-vals
vars-vals



no-remap
no-remap
no-remap
advertised
advertised
advertised
decode
decode
decode
category
category
category
color
color
color
local
local
local
italic
italic
italic
vscroll
vscroll
vscroll
comments
comments
comments
minor
minor
minor
field
field
field
new
new
new
use
use
use
parameter
parameter
parameter
menu
menu
menu
sequencep
sequencep
sequencep
process
process
process
at
at
at
unlock
unlock
unlock
1.8
1.8
1.8
syntax-descriptor
syntax-descriptor
syntax-descriptor
file
file
file
fundamental
fundamental
fundamental
for
for
for
24
24
24
edebug
edebug
edebug
user
user
user
index
index
index
elem
elem
elem
global
global
global
zip
zip
zip
number
number
number
make
make
make
p
p
p
balance
balance
balance
save
save
save
standard
standard
standard
index
index
index
command
command
command
terminal
terminal
terminal
hash
hash
hash
abbrev
abbrev
abbrev
process
process
process
file
file
file
unibyte
unibyte
unibyte
p
p
p
char
char
char
sexp
sexp
sexp
case
case
case
p
p
p
query
query
query
node
node
node
define
define
define
skip
skip
skip
boundaries
boundaries
boundaries
>
>
>
operation
operation
operation
modtime
modtime
modtime
word
word
word
rm
rm
rm
plist
plist
plist
pos
pos
pos
get
get
get
list
list
list
copying
copying
copying
s
s
s
ewoc
ewoc
ewoc
lines
lines
lines
modes
modes
modes
right
right
right
edit
edit
edit
address
address
address
chars
chars
chars
error
error
error
mouse
mouse
mouse
p
p
p
posix
posix
posix
image
image
image
file
file
file
less?
less?
less?
package
package
package
display
display
display
trace
trace
trace
syntax
syntax
syntax
noerror
noerror
noerror
window
window
window
to
to
to
termscript
termscript
termscript
outside-margins
outside-margins
outside-margins
width
width
width
abbrev
abbrev
abbrev
cache
cache
cache
right
right
right
garbage
garbage
garbage
up
up
up



flatten
flatten
flatten
some
some
some
select
select
select
args
args
args
read
read
read
message
message
message
compile
compile
compile
x
x
x
with?
with?
with?
status
status
status
derived
derived
derived
at
at
at
frame
frame
frame
set
set
set
pop
pop
pop
truncate
truncate
truncate
fill
fill
fill
all?
all?
all?
resource
resource
resource
regular
regular
regular
to
to
to
locate
locate
locate
17
17
17
max-width
max-width
max-width
height
height
height
down
down
down
encode
encode
encode
range
range
range
initial
initial
initial
word
word
word
fill
fill
fill
frame
frame
frame
byte
byte
byte
vector
vector
vector
integer
integer
integer
edges
edges
edges
timeout
timeout
timeout
data
data
data
shr
shr
shr
s
s
s
in
in
in
region
region
region
file
file
file
truename
truename
truename
login
login
login
fill
fill
fill
presence
presence
presence
field
field
field
display
display
display
stop-before
stop-before
stop-before
tick
tick
tick
raise
raise
raise
uppercase?
uppercase?
uppercase?
name
name
name
31
31
31
21.1
21.1
21.1
edebug
edebug
edebug
args
args
args
replace
replace
replace
imenu
imenu
imenu
front-advance
front-advance
front-advance



ignore-case
ignore-case
ignore-case
next
next
next
read
read
read
less
less
less
region
region
region
same
same
same
delete
delete
delete
encode
encode
encode
height
height
height
extract
extract
extract
delta
delta
delta
upcase
upcase
upcase
remq
remq
remq
insert
insert
insert
kill
kill
kill
extensions
extensions
extensions
lookup
lookup
lookup
capitalized?
capitalized?
capitalized?
divisors
divisors
divisors
marker
marker
marker
group
group
group
make
make
make
buffer
buffer
buffer
string
string
string
format
format
format
extensions
extensions
extensions
provide
provide
provide
saved
saved
saved
location
location
location
overlay
overlay
overlay
window
window
window
buffer
buffer
buffer
insertion
insertion
insertion
undo
undo
undo
indirect
indirect
indirect
rotate
rotate
rotate
tty
tty
tty
register
register
register
button
button
button
scroll
scroll
scroll
user
user
user
handled
handled
handled
sort
sort
sort
predicate
predicate
predicate
limit
limit
limit
19.6
19.6
19.6
table
table
table
rules-function
rules-function
rules-function
marker
marker
marker
on
on
on
theme
theme
theme
properties
properties
properties
text
text
text
file
file
file
kill
kill
kill
text
text
text
floatp
floatp
floatp
val
val
val
binding
binding
binding
block
block
block
text
text
text
regexp
regexp
regexp
take
take
take
char
char
char
special
special
special
property
property
property
matches
matches
matches
lisp
lisp
lisp
add
add
add
element
element
element
set
set
set
dash
dash
dash
mode
mode
mode
keep
keep
keep
setq
setq
setq
backward
backward
backward
arglist
arglist
arglist
invert
invert
invert
20
20
20
context
context
context
local
local
local
obsolete
obsolete
obsolete
file
file
file
test
test
test
find
find
find
flag
flag
flag
back
back
back
less
less
less
interpose
interpose
interpose
stipple
stipple
stipple
versions
versions
versions
up
up
up
create
create
create
parse
parse
parse
system
system
system
window
window
window
safe
safe
safe
arrayp
arrayp
arrayp
left
left
left
inactive
inactive
inactive
mapc
mapc
mapc
last
last
last
fill
fill
fill
x
x
x
p
p
p
escape-from-edge
escape-from-edge
escape-from-edge
new-alias
new-alias
new-alias
configuration
configuration
configuration
forward
forward
forward
swap
swap
swap
set
set
set
ignore
ignore
ignore
description
description
description
pos
pos
pos
number-of-newlines
number-of-newlines
number-of-newlines
process
process
process
buffer
buffer
buffer
find
find
find
frame
frame
frame
where
where
where
partially
partially
partially
frame
frame
frame
5.3.1
5.3.1
5.3.1
window
window
window
dir
dir
dir
backup
backup
backup
parent
parent
parent
configuration
configuration
configuration
get
get
get
no-line-break
no-line-break
no-line-break
minibuffer
minibuffer
minibuffer
parameters
parameters
parameters
n
n
n
try
try
try
cache
cache
cache
keys
keys
keys
frame
frame
frame
function-name
function-name
function-name



flag
flag
flag
rename
rename
rename
s
s
s
strong
strong
strong
font-spec
font-spec
font-spec
form
form
form
macro
macro
macro
sequence
sequence
sequence
>prec2
>prec2
>prec2
precs
precs
precs
string
string
string
juxt
juxt
juxt
display
display
display
rehash
rehash
rehash
p
p
p
find
find
find
filter
filter
filter
window
window
window
spec
spec
spec
display
display
display



regular
regular
regular
lock
lock
lock
file
file
file
even
even
even
paragraph
paragraph
paragraph
kill
kill
kill



defined
defined
defined
unzip
unzip
unzip
sentence
sentence
sentence
print
print
print
live
live
live
screen
screen
screen
frame
frame
frame
face
face
face
on
on
on
charset
charset
charset
19.9
19.9
19.9
visible
visible
visible
display
display
display
unintern
unintern
unintern
rassq
rassq
rassq
form
form
form
recent
recent
recent
theme
theme
theme
invisibility
invisibility
invisibility
to
to
to
buffer
buffer
buffer
print
print
print
put
put
put
images
images
images
type
type
type
next
next
next
separator
separator
separator
key
key
key
current
current
current
auto
auto
auto
set
set
set
p
p
p
list
list
list
local
local
local
trash
trash
trash
if
if
if
largest
largest
largest
mismatch
mismatch
mismatch
files
files
files
7.12
7.12
7.12
copy
copy
copy
until
until
until
process
process
process
table
table
table
y
y
y
expand
expand
expand
set
set
set
to
to
to
tty
tty
tty
string
string
string
frame
frame
frame
unbury
unbury
unbury
lookup
lookup
lookup
syntax
syntax
syntax
newer
newer
newer
beep
beep
beep
auto
auto
auto
face
face
face
entry
entry
entry
cut
cut
cut
expansion
expansion
expansion
escapes
escapes
escapes
posnp
posnp
posnp
error
error
error
abbrev
abbrev
abbrev
forward
forward
forward
ring
ring
ring
put
put
put
window
window
window
null
null
null
ok
ok
ok
key
key
key
mode
mode
mode
buffers
buffers
buffers
on
on
on
+
+
+
remote
remote
remote
%
%
%
memory
memory
memory
change
change
change
restore
restore
restore
buffer
buffer
buffer
start2
start2
start2
list
list
list
condition
condition
condition
to
to
to
until
until
until
done
done
done
search
search
search
name
name
name
button
button
button
hide
hide
hide
read
read
read
require-match
require-match
require-match
read
read
read
process
process
process
19.2
19.2
19.2
to
to
to
emacs
emacs
emacs
attributes
attributes
attributes
kbd
kbd
kbd
arg
arg
arg
lines
lines
lines
file
file
file
type
type
type
new-pos
new-pos
new-pos
overlay
overlay
overlay
file
file
file
exec
exec
exec



fonts
fonts
fonts
cache
cache
cache
buffer
buffer
buffer
buf
buf
buf
p
p
p
generic
generic
generic
modified
modified
modified
col
col
col
left
left
left
coding
coding
coding
horizontal
horizontal
horizontal
overflow
overflow
overflow
fill
fill
fill
pid
pid
pid
buffer
buffer
buffer
alist
alist
alist
code
code
code
17.3
17.3
17.3
temporary
temporary
temporary
eof
eof
eof
exists
exists
exists
unbury
unbury
unbury
s
s
s
replace
replace
replace
window
window
window
table
table
table
blink
blink
blink
value
value
value
child
child
child
paragraph
paragraph
paragraph
boundp
boundp
boundp
term
term
term
width
width
width
scroll
scroll
scroll
scroll
scroll
scroll
notification
notification
notification
acl
acl
acl
ask
ask
ask
type
type
type
s
s
s
substitute
substitute
substitute
s
s
s
file
file
file
bytes
bytes
bytes
looking
looking
looking
compare
compare
compare
backward
backward
backward
modify
modify
modify
children
children
children
inhibit-quit
inhibit-quit
inhibit-quit
names
names
names
s
s
s
s
s
s
graphic
graphic
graphic
lists
lists
lists
6.1
6.1
6.1
color
color
color
prev
prev
prev
of
of
of
sort
sort
sort
docstrings
docstrings
docstrings
convert
convert
convert
command
command
command
all?
all?
all?
split
split
split
complete
complete
complete
switch
switch
switch
function
function
function
ip
ip
ip
completing
completing
completing
record-flag
record-flag
record-flag
running
running
running
x
x
x
to
to
to
frame
frame
frame
change
change
change
drop
drop
drop
system
system
system
control
control
control
17.10
17.10
17.10
debug
debug
debug
upper
upper
upper
register
register
register
defun
defun
defun
eof
eof
eof
in
in
in
unlock
unlock
unlock
attribute
attribute
attribute
set
set
set
initialize
initialize
initialize
on
on
on
event
event
event
set
set
set
line
line
line
shell
shell
shell
p
p
p
initial
initial
initial
type
type
type
window
window
window



words
words
words
kind
kind
kind
face
face
face
suffixes
suffixes
suffixes
offer
offer
offer
read
read
read
save
save
save
bindat
bindat
bindat
button
button
button
case
case
case
add
add
add
system
system
system
property
property
property
min
min
min
area
area
area
running
running
running
window
window
window
date
date
date
input
input
input
regexp
regexp
regexp
char
char
char
jit
jit
jit
concat
concat
concat
change
change
change
and
and
and
video
video
video
27
27
27
file
file
file
multibyte
multibyte
multibyte
pop
pop
pop
table
table
table
item
item
item
quit
quit
quit
copy
copy
copy
fill
fill
fill
keys
keys
keys
face
face
face
map
map
map
current
current
current
reverse
reverse
reverse
mouse
mouse
mouse
in
in
in
simple
simple
simple
defvaralias
defvaralias
defvaralias
help-text
help-text
help-text
at
at
at
13.1
13.1
13.1
rule
rule
rule
mark
mark
mark
field
field
field
unload
unload
unload
data
data
data
r
r
r
page
page
page
char
char
char
steps
steps
steps
initials
initials
initials
try
try
try
visited
visited
visited
family
family
family
fun
fun
fun
contents
contents
contents
abbrev
abbrev
abbrev
with
with
with
fit
fit
fit
string
string
string
check-timers
check-timers
check-timers
window
window
window
frames
frames
frames
priority
priority
priority
results
results
results
window
window
window
12.11
12.11
12.11
truename
truename
truename
enable
enable
enable
fill
fill
fill



tty
tty
tty
completions
completions
completions
previous
previous
previous
inhibit
inhibit
inhibit
while
while
while
beginning
beginning
beginning
end
end
end
before
before
before
rule
rule
rule
spec
spec
spec
after
after
after
6.8
6.8
6.8
menu
menu
menu
compile
compile
compile
set
set
set
cdr
cdr
cdr
custom
custom
custom
2.5
2.5
2.5



ignore
ignore
ignore
2.2
2.2
2.2
save
save
save
default
default
default
abbrevs
abbrevs
abbrevs
supersession
supersession
supersession
forward
forward
forward
local
local
local
max
max
max
destroy
destroy
destroy
slot
slot
slot
ewoc
ewoc
ewoc
applify
applify
applify
completion
completion
completion
2.8
2.8
2.8
case-table
case-table
case-table
process
process
process
mapreduce
mapreduce
mapreduce
variable
variable
variable
at
at
at
18
18
18
properties
properties
properties
change
change
change
local
local
local
recursive
recursive
recursive
default
default
default
action-alist
action-alist
action-alist
>
>
>
define
define
define
message
message
message
unfold
unfold
unfold
region
region
region
delete
delete
delete
filename1
filename1
filename1
run
run
run
describe
describe
describe
current
current
current
about
about
about
face
face
face
limit
limit
limit
p
p
p
supports
supports
supports
replace
replace
replace
switch
switch
switch
executable
executable
executable
tree
tree
tree
display
display
display
volume
volume
volume
window
window
window
mapcat
mapcat
mapcat
address
address
address
color
color
color
tty
tty
tty
redraw
redraw
redraw
goto
goto
goto
exists
exists
exists
delete
delete
delete
type
type
type
tree
tree
tree
display
display
display
table
table
table
lax
lax
lax
p
p
p
mode
mode
mode
element
element
element
user
user
user
indexed
indexed
indexed
old-pos
old-pos
old-pos
list
list
list
clear
clear
clear
3.3
3.3
3.3
code
code
code
find
find
find
delay
delay
delay
mask
mask
mask
edebug
edebug
edebug
attribute
attribute
attribute
up
up
up
add
add
add
disable
disable
disable
set
set
set
when
when
when
combine
combine
combine
path
path
path
scroll
scroll
scroll
auto
auto
auto
slice
slice
slice
max
max
max
auto
auto
auto
write
write
write
close
close
close
systems
systems
systems
local
local
local
b
b
b
folder
folder
folder
bars
bars
bars
inhibit
inhibit
inhibit
wrap
wrap
wrap
vecp
vecp
vecp
3.4
3.4
3.4
step
step
step
char
char
char
table
table
table
copying
copying
copying
then
then
then
completion
completion
completion
read
read
read
ewoc
ewoc
ewoc
bottom
bottom
bottom
colors
colors
colors
get
get
get
1
1
1
list
list
list
interpose
interpose
interpose
height
height
height
buffer
buffer
buffer
scratch
scratch
scratch
run
run
run
category
category
category
other
other
other
looking
looking
looking
alist
alist
alist
address
address
address
local
local
local
styles
styles
styles
marker
marker
marker
imagemagick
imagemagick
imagemagick
x
x
x
edebug
edebug
edebug
kbd
kbd
kbd
padding
padding
padding
hack
hack
hack
timestamp
timestamp
timestamp
s
s
s
newer
newer
newer
window
window
window
plist
plist
plist
define
define
define
directory
directory
directory
space
space
space
revert
revert
revert
message
message
message
cdr
cdr
cdr
overlay
overlay
overlay
horizontal
horizontal
horizontal
device
device
device
process
process
process
notifications
notifications
notifications
choice
choice
choice
range
range
range
string
string
string
smie
smie
smie
file
file
file
tq
tq
tq
properties
properties
properties
gv
gv
gv
buffer
buffer
buffer
nodisp
nodisp
nodisp
match
match
match
rule
rule
rule
charset
charset
charset
window
window
window
indices
indices
indices
down
down
down
undo
undo
undo
edebug
edebug
edebug
17.4
17.4
17.4
current
current
current
dired
dired
dired
end2
end2
end2
table
table
table
double
double
double
take
take
take
name
name
name
backup
backup
backup
next
next
next
scroll
scroll
scroll
height
height
height
selinux
selinux
selinux
n
n
n
delete
delete
delete
x2
x2
x2
newline
newline
newline
cycle
cycle
cycle
truncate
truncate
truncate
table
table
table
x1
x1
x1
path
path
path
vertical
vertical
vertical
clear
clear
clear
focus
focus
focus
resizable
resizable
resizable
objects
objects
objects
parent
parent
parent
force
force
force
file
file
file
goto
goto
goto
must-suffix
must-suffix
must-suffix
region
region
region
6.2
6.2
6.2
actor
actor
actor
ring
ring
ring
set
set
set



process
process
process
decode
decode
decode
stop-comment
stop-comment
stop-comment
modified
modified
modified
disable
disable
disable
p
p
p



attribute-alist
attribute-alist
attribute-alist
larger
larger
larger
process
process
process
row
row
row
notify
notify
notify
signature
signature
signature
to
to
to
posn
posn
posn
balance
balance
balance
last
last
last
args
args
args
init
init
init
circle
circle
circle
to
to
to
add
add
add
buffer
buffer
buffer
null
null
null
syntax
syntax
syntax
geometry
geometry
geometry
year
year
year
37
37
37
p
p
p
blink
blink
blink
file
file
file
area
area
area
buffer
buffer
buffer
information
information
information
forward
forward
forward
server
server
server



create
create
create
run
run
run
to
to
to
opt
opt
opt
word
word
word
body
body
body
forward
forward
forward
dont-downcase-last
dont-downcase-last
dont-downcase-last
invisible
invisible
invisible
top
top
top
startup
startup
startup
string
string
string
advice
advice
advice
contents
contents
contents
or
or
or
collect
collect
collect
capitalized?
capitalized?
capitalized?
noauto
noauto
noauto
makehash
makehash
makehash
posn
posn
posn
no
no
no
regexp
regexp
regexp
properties
properties
properties
up
up
up
princ
princ
princ
attributes
attributes
attributes
minibuffer
minibuffer
minibuffer
extract
extract
extract
file
file
file
30
30
30
p
p
p
necessary
necessary
necessary
linked
linked
linked
hook
hook
hook



x
x
x
history
history
history
bolp
bolp
bolp
1.4
1.4
1.4
alias
alias
alias
copying
copying
copying
buffer
buffer
buffer
sans
sans
sans
set
set
set
copy
copy
copy
load
load
load
color
color
color
ignored
ignored
ignored
window
window
window
table
table
table
modified
modified
modified
command
command
command
switch-frame-ok
switch-frame-ok
switch-frame-ok
mnemonics
mnemonics
mnemonics
prefer
prefer
prefer
word
word
word
region
region
region
period
period
period
location
location
location
window
window
window
property
property
property
symlink
symlink
symlink
fetch
fetch
fetch
butlast
butlast
butlast
window
window
window
subtype
subtype
subtype
after-find-file-from-revert-buffer
after-find-file-from-revert-buffer
after-find-file-from-revert-buffer
button
button
button
default
default
default
key
key
key
advice
advice
advice
auto
auto
auto
downcase
downcase
downcase
x
x
x
separate
separate
separate
use
use
use
stop
stop
stop
8.5
8.5
8.5
logxor
logxor
logxor
buffer
buffer
buffer
insert
insert
insert
union
union
union
set
set
set
make
make
make
input
input
input
files
files
files
column
column
column
file
file
file
on
on
on
pointer
pointer
pointer
other
other
other
8.2
8.2
8.2
ellipses
ellipses
ellipses
nobreak
nobreak
nobreak
font
font
font
auto
auto
auto
tree
tree
tree
configuration
configuration
configuration
variable
variable
variable
bidi
bidi
bidi
up
up
up
x
x
x
selections
selections
selections
setq
setq
setq
text-coding
text-coding
text-coding
system
system
system
backward
backward
backward
describe
describe
describe
process
process
process
point
point
point
next
next
next
insert
insert
insert
alist
alist
alist
buffer
buffer
buffer
subclass
subclass
subclass
21.4
21.4
21.4
resize
resize
resize
list
list
list
format
format
format
fill
fill
fill
put
put
put
l
l
l
r
r
r
shell
shell
shell
make
make
make
in-map
in-map
in-map
no-message
no-message
no-message
all
all
all
prodfn
prodfn
prodfn
file
file
file
relative
relative
relative
all
all
all
vector
vector
vector
parameter
parameter
parameter
buffer
buffer
buffer
expression
expression
expression
image
image
image
about
about
about
*
*
*
window
window
window
invisible
invisible
invisible
custom
custom
custom
killp
killp
killp
buffer
buffer
buffer
window
window
window
space
space
space
full
full
full
booleanp
booleanp
booleanp
visited
visited
visited
in
in
in
replace
replace
replace
put
put
put
p
p
p
remove
remove
remove
lazy
lazy
lazy
window
window
window
minimize
minimize
minimize
s
s
s
file
file
file
maximize
maximize
maximize
scroll
scroll
scroll
size
size
size
eval
eval
eval
priority
priority
priority
setplist
setplist
setplist
field
field
field
operation
operation
operation
describe
describe
describe
set
set
set
suffixes
suffixes
suffixes
read
read
read
scroll
scroll
scroll
terminal
terminal
terminal
frame
frame
frame
window
window
window
end
end
end
prompt
prompt
prompt
delete
delete
delete
set
set
set
n
n
n
sit
sit
sit
pixelwise
pixelwise
pixelwise
symbol
symbol
symbol
exit
exit
exit
find
find
find
newline
newline
newline
buffers
buffers
buffers
control
control
control
define
define
define
kept
kept
kept
advice
advice
advice
end
end
end
exec
exec
exec
syntax
syntax
syntax
window
window
window
configure
configure
configure
list
list
list
help
help
help
posn
posn
posn
20.2
20.2
20.2
recenter
recenter
recenter
greedy
greedy
greedy
file
file
file
register
register
register
size
size
size
substring
substring
substring
readable
readable
readable
event
event
event
to
to
to
keystrokes
keystrokes
keystrokes
find
find
find
calling
calling
calling
ordered
ordered
ordered
property
property
property
rawfile
rawfile
rawfile
unwrap
unwrap
unwrap
frame
frame
frame
to
to
to
position
position
position
nosuffix
nosuffix
nosuffix
force
force
force
collapse
collapse
collapse
mm
mm
mm
time
time
time
fset
fset
fset
hook
hook
hook
keymap
keymap
keymap
notify
notify
notify
position
position
position
face
face
face
lines
lines
lines
image
image
image
according
according
according
extended
extended
extended
fonts
fonts
fonts
get
get
get
network
network
network
width
width
width
whole
whole
whole
before
before
before
close
close
close
subexp-depth
subexp-depth
subexp-depth
keymaps
keymaps
keymaps
new-char
new-char
new-char
bytecode
bytecode
bytecode
string
string
string
window
window
window
link
link
link
read
read
read
opt
opt
opt
min-height
min-height
min-height
coverage
coverage
coverage
thing
thing
thing
undo
undo
undo
filter
filter
filter
barf
barf
barf
>grammar
>grammar
>grammar
time
time
time
prefix
prefix
prefix
s
s
s



versions
versions
versions
mode
mode
mode
ignore
ignore
ignore
nondirectory
nondirectory
nondirectory
posn
posn
posn
s
s
s
debug
debug
debug
seq
seq
seq
if
if
if
p
p
p
order
order
order
after
after
after
chop
chop
chop
initial
initial
initial
safe
safe
safe
modifiers
modifiers
modifiers
clear
clear
clear
list1
list1
list1
file
file
file
modified
modified
modified
macro
macro
macro
just
just
just
prec2s
prec2s
prec2s
reg
reg
reg
word
word
word
frames
frames
frames
cddr
cddr
cddr
make
make
make
underline
underline
underline
p
p
p
p
p
p
expt
expt
expt
how
how
how
line
line
line
next
next
next
waiting
waiting
waiting
partial
partial
partial
29
29
29
maybe
maybe
maybe
with
with
with
name
name
name
dynamic
dynamic
dynamic
docstrings
docstrings
docstrings
process
process
process
name
name
name
concat
concat
concat
functions
functions
functions
delete
delete
delete
failure
failure
failure
buffer
buffer
buffer
active
active
active
mode
mode
mode
ewoc
ewoc
ewoc
type
type
type
beg
beg
beg
pos
pos
pos
process
process
process
auto
auto
auto
print
print
print
help
help
help
wrap
wrap
wrap
charset
charset
charset
trace
trace
trace
write
write
write
break
break
break
defun
defun
defun
disable
disable
disable
window
window
window
define
define
define
dump
dump
dump



buffer
buffer
buffer
animate
animate
animate
region
region
region
words
words
words
set
set
set
ftruncate
ftruncate
ftruncate
last
last
last
combine
combine
combine
table
table
table
window
window
window
prev
prev
prev
prefix
prefix
prefix
frame
frame
frame
queue
queue
queue
enter
enter
enter
select
select
select
hash
hash
hash
lowercase?
lowercase?
lowercase?
end
end
end
keyword-list
keyword-list
keyword-list
variable
variable
variable
alist
alist
alist
integer1
integer1
integer1
change
change
change
as
as
as
p
p
p



table
table
table
buffer1
buffer1
buffer1
at
at
at
font
font
font
indentation
indentation
indentation
newer
newer
newer
vector
vector
vector
justify
justify
justify
forward
forward
forward
distinct
distinct
distinct
quit
quit
quit
fixup
fixup
fixup
defvar
defvar
defvar
area
area
area
strings
strings
strings
edges
edges
edges
global
global
global
from
from
from
at
at
at
bytecode
bytecode
bytecode
col
col
col
update
update
update
locals
locals
locals
edges
edges
edges
preserve-modes
preserve-modes
preserve-modes
old-char
old-char
old-char
sequence
sequence
sequence
buffer
buffer
buffer
lock
lock
lock
process
process
process
available
available
available
!cdr
!cdr
!cdr
keep
keep
keep
watch
watch
watch
name
name
name
height
height
height
match
match
match



called
called
called
package
package
package
hook
hook
hook
regexp-flag
regexp-flag
regexp-flag
ignore
ignore
ignore
kill
kill
kill
system
system
system
add
add
add
define
define
define
make
make
make
network
network
network
substring
substring
substring
buffer
buffer
buffer
spec
spec
spec
data
data
data
marker
marker
marker
method
method
method
register
register
register
frame
frame
frame
on
on
on
full-directory-p
full-directory-p
full-directory-p
ring
ring
ring
partition
partition
partition
descriptor
descriptor
descriptor
obsolete
obsolete
obsolete
custom
custom
custom
function
function
function
replace
replace
replace
config
config
config
overlay
overlay
overlay
if
if
if
scalable
scalable
scalable
slice
slice
slice
minibuffer
minibuffer
minibuffer
region
region
region
calling
calling
calling
image
image
image
case
case
case
versions
versions
versions
no
no
no
window
window
window
keyboard
keyboard
keyboard
method
method
method
temp
temp
temp
p
p
p
at
at
at
version
version
version
coding
coding
coding
image
image
image



suppress
suppress
suppress
or
or
or
window
window
window
threat
threat
threat
translation
translation
translation
fns
fns
fns
window
window
window
vector
vector
vector
background-p
background-p
background-p
faces
faces
faces
variable
variable
variable
max
max
max



indent
indent
indent
next
next
next
backup
backup
backup
set
set
set
6.4
6.4
6.4
lambda
lambda
lambda
hide
hide
hide
imenu
imenu
imenu
locked
locked
locked
prepare
prepare
prepare
smie
smie
smie
config
config
config
revert
revert
revert
charset
charset
charset
label
label
label
register
register
register
ring
ring
ring
sexp
sexp
sexp
default
default
default
load
load
load
char
char
char
keystrokes
keystrokes
keystrokes
table
table
table
mapcat
mapcat
mapcat
if
if
if
12.6
12.6
12.6
contents
contents
contents
current
current
current
literally
literally
literally
function
function
function
p
p
p
files
files
files
previous
previous
previous
window
window
window
kill
kill
kill
define
define
define
make
make
make
one
one
one
region
region
region
for
for
for
help
help
help
vec
vec
vec
multibyte
multibyte
multibyte
match
match
match
variables
variables
variables
columns
columns
columns
do
do
do
set
set
set
to
to
to
info
info
info
defs
defs
defs
s
s
s
line
line
line
extension
extension
extension
buffer
buffer
buffer
deactivate
deactivate
deactivate
table
table
table
remove
remove
remove
progress
progress
progress
obj
obj
obj
idle
idle
idle
time
time
time
this
this
this
collection
collection
collection
properties
properties
properties
backward
backward
backward
char
char
char
alternatives
alternatives
alternatives
attributes
attributes
attributes
keep-margins
keep-margins
keep-margins
replace
replace
replace
s
s
s
load
load
load
copy
copy
copy
number
number
number
custom
custom
custom
menu
menu
menu
prec2
prec2
prec2
max
max
max
define
define
define
string
string
string
attributes
attributes
attributes
buffer
buffer
buffer
adaptive
adaptive
adaptive
table
table
table
all-frames
all-frames
all-frames
make
make
make
shell
shell
shell
set
set
set
derived
derived
derived
10.6
10.6
10.6
nobreak
nobreak
nobreak
positions
positions
positions
screen
screen
screen
hour
hour
hour
case
case
case
abs
abs
abs
less?
less?
less?
blink
blink
blink
2.12
2.12
2.12
buffer
buffer
buffer
bool
bool
bool
height
height
height
predicate
predicate
predicate
save
save
save
message
message
message
buffer
buffer
buffer
tree
tree
tree
mark
mark
mark
insert
insert
insert
aggressively
aggressively
aggressively
point
point
point
s
s
s
base
base
base
set
set
set
citation-regexp
citation-regexp
citation-regexp
keep
keep
keep
file
file
file
first
first
first
current
current
current
time
time
time
name
name
name
remapping
remapping
remapping
indices
indices
indices
all
all
all
selected
selected
selected
length
length
length
to
to
to
leap
leap
leap
find
find
find
p
p
p
plist
plist
plist
define
define
define
file
file
file
in
in
in
line
line
line
indent
indent
indent
frame
frame
frame
pop
pop
pop
make
make
make
move
move
move
contents
contents
contents



time
time
time
words
words
words
no
no
no
level
level
level
count-final-newline
count-final-newline
count-final-newline
take
take
take
partition
partition
partition
quit
quit
quit
query
query
query
each
each
each
p
p
p
p
p
p
posn
posn
posn
of
of
of
starting-name
starting-name
starting-name
set
set
set
get
get
get
string
string
string
enable
enable
enable
list
list
list
file
file
file
sexp
sexp
sexp
batch
batch
batch
keyword-args
keyword-args
keyword-args
display
display
display
region
region
region
let*
let*
let*
put
put
put
rule
rule
rule
bar
bar
bar
face
face
face
obsolete
obsolete
obsolete



coding
coding
coding
all
all
all
window
window
window
items?
items?
items?
left
left
left
4.4
4.4
4.4
substring
substring
substring
buffer
buffer
buffer
dups
dups
dups
from-code
from-code
from-code
depth
depth
depth
fn
fn
fn
frame
frame
frame
standard
standard
standard
6.3
6.3
6.3
let
let
let
add
add
add
buffer
buffer
buffer
table
table
table
image
image
image
buffer
buffer
buffer
font
font
font
buffer
buffer
buffer
file
file
file
minibuffer
minibuffer
minibuffer
internal
internal
internal
display
display
display
s
s
s
key-regexp
key-regexp
key-regexp
click
click
click
handler
handler
handler
nodes
nodes
nodes
words
words
words
table
table
table
p
p
p
window
window
window
base
base
base
tree
tree
tree
case
case
case
fontset
fontset
fontset
code
code
code
process
process
process
current-face
current-face
current-face
fill
fill
fill
primitive
primitive
primitive
vector
vector
vector
partial
partial
partial
frame
frame
frame
region
region
region
line
line
line
fit
fit
fit
mode
mode
mode
trailing
trailing
trailing
unset
unset
unset
get
get
get
coding
coding
coding
minutes
minutes
minutes
reindent
reindent
reindent
count
count
count
alist
alist
alist
bar
bar
bar
button
button
button
sort
sort
sort
s
s
s
hook
hook
hook
preserve
preserve
preserve
path
path
path
char
char
char
some
some
some
stream
stream
stream
10.2
10.2
10.2
7
7
7
directory
directory
directory
italic
italic
italic
process
process
process
point
point
point
pixel
pixel
pixel
match
match
match
face
face
face
scroll
scroll
scroll
marker
marker
marker
nlistp
nlistp
nlistp
append
append
append
current
current
current
process-name
process-name
process-name
recursive
recursive
recursive
definition
definition
definition
key
key
key
name
name
name
animate
animate
animate
overlay
overlay
overlay
seed
seed
seed
decode
decode
decode
focus
focus
focus
move
move
move
get
get
get
to-file
to-file
to-file
translate
translate
translate
set
set
set
values
values
values
difference
difference
difference
read
read
read
omit-port
omit-port
omit-port
split
split
split
save
save
save



restore
restore
restore
lambda
lambda
lambda
parse
parse
parse
display
display
display
mouse
mouse
mouse
matching
matching
matching
if
if
if
pad
pad
pad
put
put
put
category
category
category
numeric
numeric
numeric
buffer
buffer
buffer
category
category
category
seconds
seconds
seconds
bnf
bnf
bnf
garbage
garbage
garbage
table
table
table
keep-time
keep-time
keep-time
view
view
view
in
in
in
info
info
info
merge
merge
merge
display
display
display
stipple
stipple
stipple
map
map
map
table
table
table
same
same
same
shrink
shrink
shrink
point
point
point
name
name
name
value
value
value
file
file
file
width
width
width
sibling
sibling
sibling
serial
serial
serial
key
key
key
overlay
overlay
overlay
ewoc
ewoc
ewoc
mismatch
mismatch
mismatch
property
property
property
p
p
p
num
num
num
handled
handled
handled
set
set
set
s
s
s
create
create
create
label
label
label
functions
functions
functions
recursion
recursion
recursion
inhibit
inhibit
inhibit
let*
let*
let*
char
char
char
tabs
tabs
tabs
category-set
category-set
category-set
p
p
p
this
this
this



hook
hook
hook
len
len
len
under
under
under
error-symbol
error-symbol
error-symbol
debug
debug
debug
custom
custom
custom
find
find
find
search
search
search
adaptive
adaptive
adaptive
base64
base64
base64
text
text
text
video
video
video



p
p
p
p
p
p
var
var
var
window
window
window
coding
coding
coding
unlispify
unlispify
unlispify
binding
binding
binding
drop
drop
drop
hookvars
hookvars
hookvars
char
char
char
with
with
with
syntax
syntax
syntax
need-vector
need-vector
need-vector
make
make
make
in
in
in
types
types
types
category
category
category
bitmap
bitmap
bitmap
timer
timer
timer
weakness
weakness
weakness
reporter
reporter
reporter
privileged
privileged
privileged
set
set
set
attributes
attributes
attributes
buffer
buffer
buffer
put
put
put
by
by
by
bury-or-kill
bury-or-kill
bury-or-kill
minibuffer
minibuffer
minibuffer
buffer
buffer
buffer
smie
smie
smie
setup
setup
setup
buffer
buffer
buffer
delete
delete
delete
interactively
interactively
interactively
width
width
width
to
to
to
load
load
load
function
function
function
window
window
window
or
or
or
face
face
face
minibuffer
minibuffer
minibuffer
only
only
only
mixedcase?
mixedcase?
mixedcase?
disassemble
disassemble
disassemble
ends
ends
ends
number
number
number
lax
lax
lax
zip
zip
zip
save
save
save
base-buffer
base-buffer
base-buffer
image
image
image
all
all
all
interactive-p
interactive-p
interactive-p
pair
pair
pair
record-regexp
record-regexp
record-regexp
inhibit
inhibit
inhibit
func
func
func
display
display
display
make
make
make
props
props
props
initials
initials
initials
extended
extended
extended
table
table
table
abbrevs
abbrevs
abbrevs
mixedcase?
mixedcase?
mixedcase?
or
or
or
to
to
to
intern
intern
intern
insert
insert
insert
break
break
break
find
find
find
symbol
symbol
symbol



update
update
update
bidi
bidi
bidi
lowercase?
lowercase?
lowercase?
beginning
beginning
beginning
2.16
2.16
2.16
make
make
make
read
read
read
concat
concat
concat
progress
progress
progress
mode-and-header-line
mode-and-header-line
mode-and-header-line
after
after
after
sort
sort
sort
pixel
pixel
pixel
newline
newline
newline
prefix
prefix
prefix
abbrev
abbrev
abbrev
run
run
run
system
system
system
map
map
map
with?
with?
with?
delete
delete
delete
fold
fold
fold
steps
steps
steps
mismatch
mismatch
mismatch
resizable
resizable
resizable
easy
easy
easy
xml
xml
xml
depth
depth
depth
defun
defun
defun
justification
justification
justification
delete
delete
delete
find
find
find
no-cursor-in-echo-area
no-cursor-in-echo-area
no-cursor-in-echo-area
indicate
indicate
indicate
col
col
col
markers
markers
markers
center
center
center
put
put
put
s
s
s
process
process
process
file
file
file
sibling
sibling
sibling
bottom
bottom
bottom
gc
gc
gc
generate
generate
generate
log
log
log
set
set
set
table
table
table
frame
frame
frame
info
info
info
contents
contents
contents
smie
smie
smie
component
component
component
with
with
with
elements
elements
elements
extension
extension
extension
argument
argument
argument
id
id
id
allowed
allowed
allowed
to
to
to
back
back
back
32
32
32
process
process
process
object
object
object
list2
list2
list2
simple
simple
simple
current-group
current-group
current-group
current
current
current
prompt
prompt
prompt
active
active
active
network
network
network
with
with
with
population
population
population
debug
debug
debug
tick
tick
tick
name
name
name
easy
easy
easy
hf
hf
hf
not-selected
not-selected
not-selected
at
at
at
buffer
buffer
buffer
terminal
terminal
terminal
debug
debug
debug
preserve
preserve
preserve
narrow
narrow
narrow
display
display
display
provide
provide
provide
map
map
map
ordered
ordered
ordered
error
error
error
9.2
9.2
9.2
parameter
parameter
parameter
name
name
name
iconify
iconify
iconify
use-float
use-float
use-float
lessp
lessp
lessp
icon
icon
icon
words
words
words
elem
elem
elem
window
window
window
end
end
end
buffer
buffer
buffer
abbrev
abbrev
abbrev
define
define
define
paragraphs
paragraphs
paragraphs
point
point
point
height
height
height
multibyte
multibyte
multibyte
id
id
id
color
color
color
rm
rm
rm
to
to
to
35
35
35
ldexp
ldexp
ldexp
abbrev
abbrev
abbrev
up
up
up
or
or
or



separation
separation
separation
mapconcat
mapconcat
mapconcat
charsets
charsets
charsets
auto
auto
auto
overflow
overflow
overflow
continue
continue
continue
binding
binding
binding
charset
charset
charset
text
text
text
make
make
make
face
face
face



process
process
process
regexp
regexp
regexp
exit
exit
exit
separator
separator
separator
initialize
initialize
initialize
function
function
function
tq
tq
tq
enabled
enabled
enabled
pos-or-prop
pos-or-prop
pos-or-prop
abort
abort
abort
insert
insert
insert
get
get
get
for-call-interactively
for-call-interactively
for-call-interactively
fetch
fetch
fetch
row
row
row
framep
framep
framep
byte
byte
byte
offset
offset
offset
colon
colon
colon
key
key
key
memory
memory
memory
minibuf
minibuf
minibuf
after
after
after
syntax
syntax
syntax
visible
visible
visible
test
test
test
send
send
send
clrhash
clrhash
clrhash
y
y
y
previous
previous
previous
file
file
file
ratio
ratio
ratio
key
key
key
at
at
at
fringes
fringes
fringes
detect
detect
detect



tool
tool
tool
coding
coding
coding
eol
eol
eol
backup-option
backup-option
backup-option
gid
gid
gid
change
change
change
1+
1+
1+
read
read
read
bnf
bnf
bnf
bar
bar
bar
exit
exit
exit
flush
flush
flush
upper
upper
upper
enable
enable
enable
read
read
read
collect
collect
collect
3.1
3.1
3.1
syntax
syntax
syntax
min
min
min
insert
insert
insert
backup
backup
backup
resume
resume
resume
from
from
from
threshold
threshold
threshold
margins
margins
margins
binding
binding
binding
sentence
sentence
sentence
context
context
context
pop
pop
pop
abbrev
abbrev
abbrev
raise
raise
raise
contains?
contains?
contains?
expand
expand
expand
display
display
display
minimum
minimum
minimum
average
average
average
save
save
save
wildcards
wildcards
wildcards
special
special
special
setcdr
setcdr
setcdr
process
process
process
mini
mini
mini
guess
guess
guess
copying
copying
copying
dotimes
dotimes
dotimes
string
string
string
registry
registry
registry



select
select
select
intersection
intersection
intersection
p
p
p
window
window
window
description
description
description
repeat
repeat
repeat
directory
directory
directory
child
child
child
38
38
38
variables
variables
variables
by
by
by
group
group
group
prop
prop
prop
frame
frame
frame
theme
theme
theme
this
this
this
s
s
s
types
types
types
scroll
scroll
scroll
string
string
string
smie
smie
smie
marker
marker
marker
zlib
zlib
zlib
capitalize
capitalize
capitalize
set
set
set
local
local
local
abbrev-table
abbrev-table
abbrev-table
17.8
17.8
17.8
mode
mode
mode
eolp
eolp
eolp
values
values
values
12.12
12.12
12.12
value
value
value
obsolete-name
obsolete-name
obsolete-name
from
from
from
p
p
p
narrowed
narrowed
narrowed
hack
hack
hack
8.1
8.1
8.1
pad
pad
pad
change
change
change
regex
regex
regex
relative
relative
relative
file
file
file
in
in
in
p
p
p
continue
continue
continue
command
command
command
imagemagick
imagemagick
imagemagick
exec
exec
exec
prev
prev
prev
executable
executable
executable
underline
underline
underline
set
set
set
face
face
face
in-string
in-string
in-string
and
and
and
describe
describe
describe
integers
integers
integers
fonts
fonts
fonts
xlfd
xlfd
xlfd
assq
assq
assq
mode
mode
mode
s
s
s
size
size
size
base-modes
base-modes
base-modes
indexed
indexed
indexed
time
time
time
get
get
get
color
color
color
s
s
s
remap
remap
remap
cookie
cookie
cookie
line
line
line
window
window
window
table
table
table
columns
columns
columns
set
set
set
1.3
1.3
1.3
string<
string<
string<
images
images
images
buffer
buffer
buffer
keyboard
keyboard
keyboard
pred
pred
pred
enabled
enabled
enabled
controlling
controlling
controlling
bolp
bolp
bolp
parse
parse
parse
13.3
13.3
13.3
modify
modify
modify
single
single
single
eobp
eobp
eobp
edit
edit
edit
tty
tty
tty
visible
visible
visible
propertize
propertize
propertize
window
window
window
keys
keys
keys
number-or-marker
number-or-marker
number-or-marker
p
p
p
window
window
window
prompter
prompter
prompter
interactively
interactively
interactively
thing
thing
thing
list
list
list
frame
frame
frame
region
region
region
get
get
get
keep-record
keep-record
keep-record
supported
supported
supported
destination
destination
destination
relative
relative
relative
coverage
coverage
coverage
width
width
width
backtrace
backtrace
backtrace
character
character
character
gid
gid
gid
dired
dired
dired
menu
menu
menu
string
string
string
prefixes
prefixes
prefixes
click
click
click
visible
visible
visible
debug
debug
debug
interactively
interactively
interactively
by
by
by
p
p
p
glyph
glyph
glyph
list
list
list
or
or
or
beginning
beginning
beginning
scroll
scroll
scroll
map
map
map
12.9
12.9
12.9
make
make
make
subfeatures
subfeatures
subfeatures
sleep
sleep
sleep
base-only
base-only
base-only
inside
inside
inside
column
column
column
property
property
property
copy
copy
copy
pop
pop
pop
buffer
buffer
buffer
writable
writable
writable
double
double
double
comment-list
comment-list
comment-list
load
load
load
file
file
file
waiting
waiting
waiting
backward
backward
backward
list-of-strings
list-of-strings
list-of-strings
time
time
time
describe
describe
describe
p
p
p
by
by
by
remap
remap
remap
hash
hash
hash
major
major
major
enter
enter
enter
substring
substring
substring
inverse
inverse
inverse
button
button
button
12.1
12.1
12.1
by
by
by
server
server
server
face
face
face
live
live
live
time
time
time
p
p
p
blank
blank
blank
intersection
intersection
intersection
1.7
1.7
1.7
safe
safe
safe
coding
coding
coding
as
as
as
case
case
case
history
history
history
isnan
isnan
isnan
window
window
window
10.8
10.8
10.8
indent
indent
indent
float
float
float
regexp
regexp
regexp
properties
properties
properties
create
create
create
table
table
table
10.1
10.1
10.1
bindings
bindings
bindings
camel
camel
camel
process
process
process
list
list
list
event
event
event
mod
mod
mod
to
to
to
string
string
string
termscript
termscript
termscript
remove
remove
remove
base-url
base-url
base-url
soft
soft
soft
pixel
pixel
pixel
base
base
base
before
before
before
file
file
file
decode
decode
decode
coding
coding
coding
property
property
property
plist
plist
plist
hook
hook
hook
margin
margin
margin
edebug
edebug
edebug
individual
individual
individual
key
key
key
failure
failure
failure
all
all
all
sound
sound
sound
scan
scan
scan
width
width
width
value2
value2
value2
re
re
re
remove
remove
remove
ignored
ignored
ignored
buffer
buffer
buffer
read
read
read



to
to
to
table
table
table
image
image
image
to
to
to
types
types
types
selinux
selinux
selinux
p
p
p
frame
frame
frame
p
p
p
element
element
element
window
window
window
face
face
face
warning
warning
warning
narrowed
narrowed
narrowed
splice
splice
splice
table
table
table
prepend
prepend
prepend
as
as
as
noselect
noselect
noselect
byte
byte
byte
trailing
trailing
trailing
rassq
rassq
rassq
file
file
file
set
set
set
matched
matched
matched
default
default
default
initial
initial
initial
copy
copy
copy
reenter
reenter
reenter
defs
defs
defs
read
read
read
frame
frame
frame






archives
archives
archives
windows
windows
windows
dirname
dirname
dirname
item
item
item
shared
shared
shared
symbol
symbol
symbol
system
system
system
copy
copy
copy
choice
choice
choice
properties
properties
properties
find
find
find
resolvers
resolvers
resolvers
process
process
process
convert
convert
convert
window
window
window
init
init
init
real
real
real
indirect
indirect
indirect
rassoc
rassoc
rassoc
table
table
table
site
site
site
macrop
macrop
macrop
13.2
13.2
13.2
notify
notify
notify
update
update
update
name
name
name
comparator
comparator
comparator
groups
groups
groups
buffer
buffer
buffer
raise
raise
raise
one
one
one
scan
scan
scan
top
top
top
method
method
method
pending
pending
pending
endkeyfun
endkeyfun
endkeyfun
p
p
p
p
p
p
same
same
same
quit
quit
quit
warning
warning
warning
focus
focus
focus
p
p
p
font
font
font
open
open
open
alternatives
alternatives
alternatives
backward
backward
backward
exit
exit
exit
quote
quote
quote
char
char
char
buffer
buffer
buffer
margin
margin
margin
search
search
search
default
default
default
set
set
set
edebug
edebug
edebug
pop
pop
pop
assoc
assoc
assoc
left
left
left



unfold
unfold
unfold
vector
vector
vector
edebug
edebug
edebug
name
name
name
buffer
buffer
buffer
decode
decode
decode



invisibility
invisibility
invisibility
interface
interface
interface
ewoc
ewoc
ewoc
command
command
command
last
last
last
buffer-or-name
buffer-or-name
buffer-or-name
apply
apply
apply
strings
strings
strings
face
face
face



marker
marker
marker
line
line
line
nodes
nodes
nodes
reporter
reporter
reporter
from
from
from
toplevel
toplevel
toplevel
sig
sig
sig
button
button
button
process
process
process



paren
paren
paren
until
until
until
fontset-spec
fontset-spec
fontset-spec
upcase
upcase
upcase
syntax
syntax
syntax
!cons
!cons
!cons
individual
individual
individual
configure
configure
configure
frame
frame
frame
remove
remove
remove
base
base
base
fixedcase
fixedcase
fixedcase
smie
smie
smie
fringe
fringe
fringe
save
save
save
perform
perform
perform
process
process
process
regexp
regexp
regexp
var-val
var-val
var-val
table
table
table
line
line
line
date
date
date
times
times
times



minibuffer
minibuffer
minibuffer
color
color
color
unibyte
unibyte
unibyte
eval
eval
eval
splice
splice
splice
smie
smie
smie
customizations
customizations
customizations
configuration
configuration
configuration
configuration
configuration
configuration
temporary
temporary
temporary
nomsg
nomsg
nomsg
sentinel
sentinel
sentinel
noundo
noundo
noundo
quote
quote
quote
unzip
unzip
unzip
mouse
mouse
mouse
each
each
each
previous
previous
previous
not
not
not
sentinel
sentinel
sentinel
file
file
file
listp
listp
listp
literally
literally
literally
called
called
called
document
document
document
quit
quit
quit
delete
delete
delete
current
current
current
on
on
on
vendor
vendor
vendor
make
make
make
window
window
window
direction
direction
direction
sit
sit
sit
skip
skip
skip
next
next
next
olddef
olddef
olddef
valid
valid
valid
subst
subst
subst
line
line
line
when
when
when
display
display
display
string
string
string
watch
watch
watch
find
find
find
subtype
subtype
subtype
temp
temp
temp
theme
theme
theme
seconds
seconds
seconds
property
property
property
fontset
fontset
fontset
input
input
input
hash
hash
hash
yes
yes
yes
not
not
not



window
window
window
prefixes
prefixes
prefixes
set
set
set
iteratefn
iteratefn
iteratefn
margin
margin
margin
alternatives
alternatives
alternatives
mouse
mouse
mouse
strong
strong
strong
coding
coding
coding
gc
gc
gc
cache
cache
cache
startup
startup
startup
next
next
next
approximate
approximate
approximate
region
region
region
visit
visit
visit
convention
convention
convention
help
help
help
inhibit-null
inhibit-null
inhibit-null



send
send
send
delete
delete
delete
motion
motion
motion
prodfn
prodfn
prodfn
frame
frame
frame
current-only
current-only
current-only
next
next
next
16
16
16
variables
variables
variables
process
process
process
property
property
property
program
program
program
properties
properties
properties
sentence
sentence
sentence
in
in
in
chop
chop
chop
s
s
s
lru
lru
lru
add
add
add
position
position
position
dialog
dialog
dialog
transient
transient
transient
server
server
server
action
action
action
modtime
modtime
modtime
frexp
frexp
frexp
at
at
at
name
name
name
dialog
dialog
dialog
state
state
state
file
file
file



re
re
re
no
no
no
end
end
end
minibuffer
minibuffer
minibuffer
name
name
name
fit
fit
fit
10.7
10.7
10.7
shared
shared
shared
save
save
save
set
set
set
drop
drop
drop
interface
interface
interface
digit
digit
digit
bottom
bottom
bottom
filename2
filename2
filename2



new
new
new
presence
presence
presence
minibuffers
minibuffers
minibuffers
next
next
next
file
file
file
configurations
configurations
configurations
safe
safe
safe
9.1
9.1
9.1
put
put
put
entry
entry
entry
no
no
no
terminal
terminal
terminal
process
process
process
base-variable
base-variable
base-variable
create
create
create



locale
locale
locale
4.2
4.2
4.2
region
region
region
p
p
p
object
object
object
input-method
input-method
input-method
unregister
unregister
unregister
offset
offset
offset
width
width
width
12.2
12.2
12.2
defined
defined
defined
windows
windows
windows
search
search
search
coding
coding
coding
logb
logb
logb
window
window
window
lazy
lazy
lazy
whitespace
whitespace
whitespace
table
table
table
width
width
width
prev-buffers
prev-buffers
prev-buffers
table
table
table
window
window
window
inhibit-capture-property
inhibit-capture-property
inhibit-capture-property
real
real
real
fill
fill
fill
single
single
single
make
make
make
done
done
done
>grammar
>grammar
>grammar
ewoc
ewoc
ewoc
file
file
file
process
process
process
directory
directory
directory
tab
tab
tab
screen
screen
screen
category
category
category
merge
merge
merge
reduce
reduce
reduce
do
do
do
vector
vector
vector
inactive
inactive
inactive
set
set
set



backward
backward
backward
docstring
docstring
docstring
buffer
buffer
buffer
at
at
at
mode
mode
mode
windows
windows
windows
map
map
map
literal
literal
literal
region
region
region
pointer
pointer
pointer
get
get
get
time
time
time
area
area
area
syntax
syntax
syntax
char
char
char
warn
warn
warn
delete
delete
delete
rep
rep
rep
buffer
buffer
buffer
process
process
process
delete
delete
delete
Snarf
Snarf
Snarf
no-mini
no-mini
no-mini
when
when
when
add
add
add
posix
posix
posix
noconfirm
noconfirm
noconfirm



library
library
library
target-depth
target-depth
target-depth
to
to
to
sleep
sleep
sleep
pair
pair
pair
partial
partial
partial
next
next
next
for
for
for
none?
none?
none?
lwarn
lwarn
lwarn
whitespace
whitespace
whitespace
5.3
5.3
5.3






local
local
local
redisplay
redisplay
redisplay
process
process
process
p
p
p
rename
rename
rename
column
column
column
e
e
e
attribute
attribute
attribute
before
before
before
tree
tree
tree
soft
soft
soft
ring
ring
ring
time
time
time
markerp
markerp
markerp
drop
drop
drop
vscroll
vscroll
vscroll
mode
mode
mode
s
s
s



string
string
string
scratch
scratch
scratch
a
a
a
default
default
default



name
name
name
available
available
available
get
get
get
face
face
face
frame-number
frame-number
frame-number
backtrace
backtrace
backtrace
equal-test
equal-test
equal-test
edebug
edebug
edebug
copying
copying
copying
smie
smie
smie
assq
assq
assq



setup
setup
setup
by
by
by
flatten
flatten
flatten
step
step
step
tokens
tokens
tokens
beginning
beginning
beginning
preds
preds
preds
complete
complete
complete
code
code
code
shell
shell
shell
font
font
font
sequence
sequence
sequence
downcase
downcase
downcase
edges
edges
edges
setup
setup
setup
buffer
buffer
buffer



format
format
format
active
active
active
quit
quit
quit
all
all
all
type
type
type
name
name
name
screens
screens
screens
stop
stop
stop
get
get
get
prefix
prefix
prefix
window
window
window
sexp
sexp
sexp
plist
plist
plist
kept
kept
kept
syntax
syntax
syntax
halt-test
halt-test
halt-test
command
command
command
follows
follows
follows
contents
contents
contents
y
y
y
yank
yank
yank
name
name
name
ref
ref
ref
minibuffer
minibuffer
minibuffer
window
window
window
byte
byte
byte
read
read
read
close
close
close
reuse
reuse
reuse
3.2
3.2
3.2
start
start
start
expression
expression
expression
minibuffer
minibuffer
minibuffer
length
length
length
before-p
before-p
before-p
constrain
constrain
constrain
coding
coding
coding
window
window
window
completion
completion
completion
minibuffer
minibuffer
minibuffer
insert
insert
insert
unibyte
unibyte
unibyte



suppress
suppress
suppress
completion
completion
completion
supersession
supersession
supersession
old
old
old
endrecfun
endrecfun
endrecfun
frame
frame
frame
buffer
buffer
buffer
backward
backward
backward
=
=
=
emacs
emacs
emacs
mode
mode
mode



s
s
s
make
make
make
19.8
19.8
19.8
flatten
flatten
flatten
min
min
min
nth
nth
nth
sort
sort
sort
input
input
input
slot
slot
slot
enable
enable
enable
indentation
indentation
indentation
&optional
&optional
&optional
list
list
list
buffer
buffer
buffer
completion
completion
completion
up
up
up
apply
apply
apply
face
face
face
names
names
names
weakness
weakness
weakness
suspend
suspend
suspend
char
char
char
other
other
other
s
s
s
imagemagick
imagemagick
imagemagick
with?
with?
with?
replacement
replacement
replacement
delay
delay
delay
enable
enable
enable
ewoc
ewoc
ewoc
hook
hook
hook
xlfd
xlfd
xlfd
system
system
system
parameters
parameters
parameters
sans
sans
sans
buffer
buffer
buffer
numeric
numeric
numeric
library
library
library
keyboard
keyboard
keyboard
bitmaps
bitmaps
bitmaps
read
read
read
bindat
bindat
bindat
percentage
percentage
percentage
nosep
nosep
nosep
query-flag
query-flag
query-flag
key
key
key
store
store
store
center
center
center
char
char
char
read
read
read
at
at
at
justification
justification
justification
user
user
user
search
search
search
precs
precs
precs
yank
yank
yank
tick
tick
tick
to
to
to
mark
mark
mark
command-loop
command-loop
command-loop
preceding
preceding
preceding
member
member
member
prec2
prec2
prec2
necessary
necessary
necessary
quote
quote
quote
display
display
display
exp
exp
exp
buffer
buffer
buffer
window
window
window
following
following
following
unused
unused
unused
initial-value
initial-value
initial-value
types
types
types
window
window
window
process
process
process
>=
>=
>=
syntax
syntax
syntax
set
set
set
copysign
copysign
copysign
set
set
set
yank
yank
yank
lsh
lsh
lsh
extra
extra
extra
completions
completions
completions
seconds
seconds
seconds



specs
specs
specs
lines
lines
lines
newline
newline
newline
auto
auto
auto
cons
cons
cons
directory
directory
directory
body
body
body
find
find
find
camel
camel
camel
insert
insert
insert
condition
condition
condition
join-following-p
join-following-p
join-following-p
recursive
recursive
recursive
font
font
font
7.1.9
7.1.9
7.1.9
name
name
name
no
no
no
completion
completion
completion
paragraphs
paragraphs
paragraphs
3.5
3.5
3.5



inhibit
inhibit
inhibit
by
by
by
scalable
scalable
scalable
inverse
inverse
inverse
same
same
same
size
size
size
confirm
confirm
confirm
right
right
right
change
change
change
chars
chars
chars
titleize
titleize
titleize
substrings
substrings
substrings
including
including
including
buffer
buffer
buffer
mouse
mouse
mouse
preview
preview
preview
get
get
get
window
window
window



key
key
key
lines
lines
lines
command
command
command
logand
logand
logand
quoted
quoted
quoted
file
file
file
name
name
name
distinct
distinct
distinct
string
string
string
16.5
16.5
16.5
debug
debug
debug
conversion
conversion
conversion
p
p
p
unique
unique
unique
frame
frame
frame
inside
inside
inside
table
table
table
use
use
use
p
p
p
quote
quote
quote
s
s
s
get
get
get
server
server
server
prepend
prepend
prepend
split
split
split
unhandled
unhandled
unhandled
set
set
set
pixel
pixel
pixel
frame
frame
frame
if
if
if
copy
copy
copy
window
window
window
fields
fields
fields
libxml
libxml
libxml
the value of this variable is a full keymap used by electric command future mode
the value of this variable is a buffer position an integer or a marker for expand-abbrev to use as the start of the next abbrev to be expanded
the value of this variable is the buffer for which abbrev-start-location has been set
this is the abbrev-symbol of the most recent abbrev expanded
this is the location of the most recent abbrev expanded
this is the exact expansion text of the most recent abbrev expanded after case conversion if any
the value of this variable is a function that expand-abbrev will call with no arguments to do the expansion
this variable is set non-nil by defining or altering any abbrevs except system abbrevs
this is a list of symbols whose values are abbrev tables
this variable holds the name of the directory which should contain the file "doc" that contains documentation strings for built-in and preloaded functions and variables
this variable is automatically buffer-local in all buffers its value specifies the buffer’s display table
the value of this variable is the standard display table which is used when emacs is displaying a buffer in a window with neither a window display table nor a buffer display table defined
if this variable is non-nil when a command returns to the command loop then the command loop does not check for those text properties and does not move point out of sequences that have them
if you set this variable to a non-nil value the feature of moving point out of these sequences is completely turned off
this variable controls the type of device used to communicate with asynchronous subprocesses
this variable holds a list of functions that are called by emacs redisplay as needed just before doing redisplay
the value of this buffer-local variable should be a function of no arguments to be called after self-inserting a character from the table auto-fill-chars
this variable specifies the function to use for auto-fill-function if and when auto fill is turned on
a char table of characters which invoke auto-fill-function when self-inserted—space and newline in most language environments
this variable specifies major modes to use for scripts that specify a command interpreter in a ‘#!’ line
this variable’s value is an alist with elements of the form (regexp
this works like magic-mode-alist except that it is handled only if auto-mode-alist does not specify a mode for this file
this variable contains an association list of file name patterns regular expressions and corresponding major mode commands
this buffer-local variable is the name of the file used for auto-saving the current buffer
this normal hook is run whenever an auto-save is about to happen
the value of this buffer-local variable is the length of the current buffer when it was last read in saved or auto-saved
this variable if non-nil specifies a file for recording the names of all the auto-save files
the value of this variable should be a string whose syntax is a lisp comment
the value of this variable names an emacs lisp file where the autoload calls should go
if the value of this variable is non-nil case is not considered significant in completion
this is a list of regular expressions
this variable is non-nil when emacs is running in batch mode
if this is non-nil it specifies how emacs should “ring the bell”
if the value of this buffer-local variable is non-nil the default emacs performs bidirectional reordering for display
if the value of this buffer-local variable is the symbol right-to-left or left-to-right all paragraphs in the buffer are assumed to have that specified direction
the value of this variable should be a function of no arguments to be called whenever a character with close parenthesis syntax is inserted
the value of this variable is a function that filter-buffer-substring will call to do the actual work
this obsolete variable is a wrapper hook whose members should be functions that accept four arguments fun start end and delete
the value of this obsolete variable should be a list of functions that accept a single string argument and return another string
this buffer-local variable contains the name of the file being visited in the current buffer or nil if it is not visiting a file
this buffer-local variable holds the abbreviated truename of the file visited in the current buffer or nil if no file is visited
this buffer-local variable holds the file number and directory device number of the file visited in the current buffer or nil if no file or a nonexistent file is visited
this buffer-local variable specifies a string to display in a buffer listing where the visited file name would go for buffers that don’t have a visited file name
this is a normal hook run whenever the buffer list changes
this buffer-local variable records the number of times a buffer has been displayed in a window
this buffer-local variable records the time at which a buffer was last displayed in a window
the case table for the ascii character set
this variable holds a list of functions to call before any buffer modification
this variable holds a list of functions to call after any buffer modification
this variable is a normal hook that is run whenever a buffer is changed that was previously in the unmodified state
if this variable is non-nil all of the change hooks are disabled none of them run
the value of this variable is a char-table see char-tables that specifies for each character its unicode general_category property as a symbol
the value of this variable is a char-table that specifies for each character a symbol whose name is the script to which the character belongs according to the unicode standard classification of the unicode code space into script-specific blocks
the value of this variable is a char-table that specifies the width of each character in columns that it will occupy on the screen
the value of this variable is a char-table that specifies for each character whether it is printable or not
the value is a list of all defined character set names
the value of this variable should be a display action which is treated with the highest priority by display-buffer
this variable’s value is a list of recent complex commands each represented as a form to evaluate
this variable records the name of the previous command executed by the command loop the one before the current command
this variable is set up by emacs just like last-command but never altered by lisp programs
this variable stores the most recently executed command that was not part of an input event
this variable records the name of the command now being executed by the editor command loop
this has the same value as this-command except when command remapping occurs see remapping commands
this variable holds the last input event read as part of a key sequence not counting events resulting from mouse menus
this variable is set to the last input event that was read by the command loop as part of a command
this variable records which frame the last input event was directed to
this normal hook is run by the editor command loop before it executes each command
this normal hook is run by the editor command loop after it executes each command including commands terminated prematurely by quitting or by errors
the value of this variable is t once the command line has been processed
this variable is an alist of user-defined command-line options and associated handler functions
the value of this variable is the list of command-line arguments passed to emacs
the value of this variable is the list of command-line arguments that have not yet been processed
this variable’s value is a list of functions for handling an unrecognized command-line argument
this variable controls whether overwrite mode is in effect
this alist provides a mapping for the versions of emacs that are associated with versions of a package listed in the :package-version keyword
the value of this variable is the completion table used for completion in the minibuffer
this variable’s value is the predicate that completing-read passes to try-completion
this variable determines whether emacs asks for confirmation before exiting the minibuffer completing-read binds this variable and the function minibuffer-complete-and-exit checks the value before exiting
this variable holds a list of commands that cause emacs to ask for confirmation before exiting the minibuffer if the require-match argument to completing-read is confirm-after-completion
completing-read uses this value as the local keymap when an exact match of one of the completions is not required
completing-read uses this value as the local keymap when an exact match of one of the completions is required
this is a sparse keymap that simply unbinds spc because filenames can contain spaces
the value of this abnormal hook should be a list of functions which are used to compute a completion table for completing the text at point
this variable stores a list of available completion styles
this variable is used to specify extra properties of the current completion command
this variable’s value is the program to run to generate a directory listing for the function insert-directory
if this variable is non-nil scan-sexps treats all non-ascii characters as symbol constituents regardless of what the syntax table says about them
this variable contains the default global keymap that maps emacs keyboard input to commands
this variable is an alist describing keymaps that may or may not be active according to the values of certain variables
this variable allows major modes to override the key bindings for particular minor modes
if non-nil this variable holds a keymap to use instead of the buffer’s local keymap any text property or overlay keymaps and any minor mode keymaps
if non-nil this variable holds a keymap to use instead of overriding-local-map the buffer’s local keymap text property or overlay keymaps and all the minor mode keymaps
if this variable is non-nil the value of overriding-local-map or overriding-terminal-local-map can affect the display of the menu bar
this variable holds a keymap for special events
this variable holds a list of keymap alists to use for emulation modes
the function kill-all-local-variables runs this normal hook before it does anything else
a normal hook run by make-frame before it creates the frame
an abnormal hook run by make-frame after it creates the frame
this variable specifies the list of frame parameters that a newly created frame inherits from the currently selected frame
the default value of separators for split-string
this variable is the standard obarray for use by intern and read
this buffer-local variable controls how the cursor looks in a selected window showing the buffer
the value of this variable is a list of themes loaded into emacs
this variable is an alist specifying which coding systems to use for a subprocess depending on which program is running in the subprocess
this variable is an alist that specifies the coding system to use for network streams
this variable specifies the coding systems to use for subprocess and network stream input and output when nothing else specifies what to do
this variable’s value is a list of locations in which to search for image files
the value of this variable is a list of warnings to be displayed after the current command has finished
this is a normal hook which is run by the emacs command loop after post-command-hook in order to to process and display delayed warnings
if this buffer-local variable is non-nil the buffer will have its state saved in the desktop file at desktop save
alist with elements (major-mode
this alist holds the class symbols and the associated variable settings
this alist holds directory names their assigned class names and modification times of the associated directory local variables file if there is one
if nil directory-local variables are ignored
the value of this variable should be a function
this variable specifies the width of the left margin in character cell (a
this variable specifies the width of the right margin in character cell units
this is an alist of dynamic libraries and external library files implementing them
if this is non-nil the byte compiler generates compiled files that are set up for dynamic function loading
this variable controls where the cursor appears when a message is displayed in the echo area
this normal hook is run whenever the echo area is cleared—either by message nil or for any other reason
normally displaying a long message resizes the echo area to display the entire message
this buffer-local variable records the coding system used for saving the buffer and for writing part of the buffer with write-region
this variable specifies the coding system for saving the buffer by overriding buffer-file-coding-system
i/o operations for files and subprocesses set this variable to the coding system name that was used
the variable file-name-coding-system specifies the coding system to use for encoding file names
normally errors caught by condition-case never invoke the debugger
if you set debug-on-message to a regular expression emacs will enter the debugger if it displays a matching message in the echo area
the value of this variable is a list of the values returned by all the expressions that were read evaluated and printed from buffers including the minibuffer by the standard emacs commands which do this
this variable holds a list of events waiting to be read as command input
this variable records the last terminal input event read whether as part of a command or explicitly by a lisp program
this variable lets lisp programs “press” the modifier keys on the keyboard
this terminal-local variable is the translate table for keyboard characters
this variable holds an alist which maps property names to a list of alternative property names
this variable holds a property list giving default values for text properties
the value of this variable is an alist whose elements have the form (face
this is a list of regular expressions
this buffer-local variable holds the alist of file-local variable settings
emacs calls this hook immediately before applying file-local variables stored in file-local-variables-alist
emacs calls this hook immediately after it finishes applying file-local variables stored in file-local-variables-alist
this variable holds a list of variables that should not be given local values by files
the value of this buffer-local variable is the default directory for the current buffer
this variable provides a way to override the filling of paragraphs
this variable provides a way to override how the filling functions such as fill-region and fill-paragraph move forward to the next paragraph
if this variable is non-nil the filling functions do not delete newlines that have the hard text property
this variable is set by major modes to specify how to fontify text in that mode
if the font-lock-multiline variable is set to t font lock will try to add the font-lock-multiline property automatically on multiline constructs
this variable specifies scaling for certain faces
a function run just before redisplay
a list of functions for write-region to call
the value of this variable if non-nil should be a function
each function in this list is called by insert-file-contents with one argument the number of characters inserted and with point at the beginning of the inserted text
this list contains one format definition for each defined file format
this variable states the format of the visited file
this variable specifies the format to use for auto-saving
this variable specifies how to compute a name for a frame when you have not explicitly specified one
this variable specifies how to compute the name for an iconified frame when you have not explicitly specified the frame title
this variable is set automatically by emacs
this variable specifies the mapping from logical cursor type to the actual fringe bitmaps displayed in the right fringe
this buffer-local variable specifies the mapping from logical fringe indicators to the actual bitmaps displayed in the window fringes
the fringes normally appear between the display margins and the window text
this variable if non-nil specifies the width of the left fringe in pixels
this variable if non-nil specifies the width of the right fringe in pixels
this is a normal hook that is run at the end of garbage collection
this variable is t if emacs is nearly out of memory for lisp objects and nil otherwise
this variable contains the total number of garbage collections done so far in this emacs session
this variable contains the total number of seconds of elapsed time during garbage collection so far in this emacs session as a floating-point number
the value of this variable is a char-table which defines glyphless characters and how they are displayed
the value of this variable if non-nil is the current glyph table
this variable local in every buffer specifies how to display the header line for windows displaying the buffer
the value of this variable is a local keymap for characters following the help key c-h
if this variable is non-nil its value is a form to evaluate whenever the character help-char is read
this variable holds a function to print help for a prefix key
this variable holds the name of the directory in which emacs finds certain documentation and text files that come with emacs
this abnormal hook is run after loading a file
this variable is non-nil if emacs is in the process of loading a file and it is nil otherwise
when emacs is in the process of loading a file this variable’s value is the name of that file as emacs found it during the search described earlier in this section
this variable specifies an alternate expression-reading function for load and eval-region to use instead of read
this variable specifies the number of seconds an image can remain in the cache without being displayed
this variable contains a list of type symbols for image formats which are potentially supported in the current configuration
this variable is an alist mapping image types to file name extensions
this variable if non-nil is a list that specifies regular expressions for finding definitions for imenu
this variable controls whether matching against the regular expressions in the value of imenu-generic-expression is case-sensitive t the default means matching should ignore case
this variable is an alist of syntax table modifiers to use while processing imenu-generic-expression to override the syntax table of the current buffer
if this variable is non-nil its value should be a function that finds the next “definition” to put in the buffer index scanning backward in the buffer from point
if this variable is non-nil its value should be a function to return the name for a definition assuming point is in that definition as the imenu-prev-index-position-function function would leave it
this variable specifies the function to use for creating a buffer index
this normal hook is run once just before loading all the init files (site-start
this normal hook is run once just after loading all the init files (site-start
this normal hook is run once just after handling the command line arguments
this normal hook is very similar to emacs-startup-hook
this variable holds the absolute file name of the user’s init file
this variable holds the name of the
this is a normal hook run when an emacs frame gains input focus
this is a normal hook run when an emacs frame loses input focus
this variable holds the default input stream—the stream that read uses when the stream argument is nil
if non-nil this variable enables the reading of circular and shared structures
this variable holds the name of the input method now active in the current buffer
this variable defines all the supported input methods
the value of this variable is the largest integer that emacs lisp can handle
the value of this variable is the smallest integer that emacs lisp can handle
the value of this variable is the function to call to invoke the debugger
if this variable is non-nil it says to call the debugger before the next eval apply or funcall
this variable records the debugging status of the current interactive command
this variable holds the list of killed text sequences most recently killed first
this variable’s value indicates which element of the kill ring is at the “front” of the ring for yanking
this variable specifies which kinds of invisible properties actually make a character invisible
if this is non-nil its value specifies the current input method function
this variable’s value is the number of key sequences processed so far in this emacs session
this variable contains the string or vector that defines the keyboard macro that is currently executing
this variable is non-nil if and only if a keyboard macro is being defined
this variable is the definition of the most recently defined keyboard macro
this normal hook is run when a keyboard macro terminates regardless of what caused it to terminate reaching the macro end or an error which ended the macro prematurely
this variable specifies the character to use to ask to see the next line of a menu
before confirming unsaved changes kill-buffer calls the functions in the list kill-buffer-query-functions in order of appearance with no arguments
this is a normal hook run by kill-buffer after asking all the questions it is going to ask just before actually killing the buffer
this variable if non-nil in a particular buffer tells save-buffers-kill-emacs and save-some-buffers to save this buffer (if it’s modified) without asking the user
this normal hook is run by kill-emacs before it kills emacs
when save-buffers-kill-terminal is killing emacs it calls the functions in this hook after asking the standard questions and before calling kill-emacs
this variable holds a list of functions for computing text properties
if this variable’s value is non-nil it is a symbol which is used as a text property name
the value of this variable is a list of directories to search when loading files with load
if this variable has a non-nil value null bytes are ignored when detecting the encoding of a region or a string
if this variable has a non-nil value iso-2022 escape sequences are ignored when detecting the encoding of a region or a string
if non-nil this variable holds a function for finding the beginning of a defun
if non-nil this variable holds a function for finding the end of a defun
this is a list of suffixes indicating compiled or source emacs lisp files
this is a list of suffixes that indicate representations of the same file
this variable specifies the coding system to use for decoding system error messages and—on x window system only—keyboard input for encoding the format argument to format-time-string and for decoding the return value of format-time-string
this variable specifies the locale to use for generating system error messages
this variable specifies the locale to use for formatting time values
this variable provides a way of transferring killed text from other programs when you are using a window system
this variable provides a way of communicating killed text to other programs when you are using a window system
this variable holds a list of handlers whose use is presently inhibited for a certain operation
the operation for which certain handlers are presently inhibited
this buffer-local variable says whether this buffer’s file has been backed up on account of this buffer
this variable’s value is a function to be called on certain occasions to decide whether a file should have backup files
if this variable is non-nil backups are inhibited
the mathematical constant e (2
the mathematical constant pi (3
the total number of cons cells that have been allocated so far in this emacs session
the total number of floats that have been allocated so far in this emacs session
the total number of vector cells that have been allocated so far in this emacs session
the total number of symbols that have been allocated so far in this emacs session
the total number of string characters that have been allocated so far in this session
the total number of miscellaneous objects that have been allocated so far in this session
the total number of intervals that have been allocated so far in this emacs session
the total number of strings that have been allocated so far in this emacs session
normally the menu bar shows global items followed by items defined by the local maps
this normal hook is run by redisplay to update the menu bar contents before redisplaying the menu bar
the value of this variable must be a function which is called by completing-read to actually do its work
if the value of this variable is nil standard functions that read from the minibuffer don’t add new elements to the history list
the default history list for minibuffer history input
a history list for arguments to query-replace and similar arguments to other commands
a history list for file-name arguments
a history list for buffer-name arguments
a history list for regular expression arguments
a history list for arguments that are names of extended commands
a history list for arguments that are shell commands
a history list for arguments that are lisp expressions to evaluate
a history list for arguments that are faces
this is a normal hook that is run whenever the minibuffer is entered
this is a normal hook that is run whenever the minibuffer is exited
the current value of this variable is used to rebind help-form locally inside the minibuffer see help functions
if the value of this variable is non-nil it should be a window object
this variable specifies the frame to use for the minibuffer window by default
the value of this variable is a list of all minor mode commands
this is a normal hook run by run-mode-hooks
this is a normal hook run by run-mode-hooks
this variable holds the value of the mode line construct that displays information about the language environment buffer coding system and current input method
this variable holds the value of the mode line construct that displays whether the current buffer is modified
this variable identifies the current frame
this variable identifies the buffer being displayed in the window
the variable vc-mode buffer-local in each buffer records whether the buffer’s visited file is maintained with version control and if so which kind
this variable is used to show whether default-directory for the current buffer is remote
this variable is used to identify emacsclient frames
this buffer-local variable holds the “pretty” name of the current buffer’s major mode
this buffer-local variable contains the mode line information on process status in modes used for communicating with subprocesses
this variable holds an association list whose elements specify how the mode line should indicate that a minor mode is active
this variable holds a mode line construct that by default appears in the mode line just after the which-func-mode minor mode if set else after mode-line-modes
this variable’s value is the function to be used by indent-for-tab-command and various other indentation commands to indent the current line
if non-nil the value of this variable is a function for mouse-position to call
an abnormal hook run by delete-terminal
the value of this variable is a list of symbols that are the features loaded in the current emacs session
if this variable is non-nil it should be a function that is called with no arguments to choose an enclosing range of text for refontification for the command m-o m-o font-lock-fontify-block
this variable specifies additional properties other than font-lock-face that are being managed by font lock mode
function to use for fontifying the buffer
function to use for unfontifying the buffer
function to use for fontifying a region
function to use for unfontifying a region
on some systems when emacs reads the output from a subprocess the output data is read in very small blocks potentially resulting in very poor performance
the value of this variable is the default output stream—the stream that print functions use when the stream argument is nil
if this is non-nil that means to print quoted forms using abbreviated reader syntax e
if this variable is non-nil then newline characters in strings are printed as ‘\n’ and formfeeds are printed as ‘\f’
if this variable is non-nil then unibyte non-ascii characters in strings are unconditionally printed as backslash sequences by the print functions prin1 and print that print with quoting
if this variable is non-nil then multibyte non-ascii characters in strings are unconditionally printed as backslash sequences by the print functions prin1 and print that print with quoting
the value of this variable is the maximum number of elements to print in any list vector or bool-vector
the value of this variable is the maximum depth of nesting of parentheses and brackets when printed
if non-nil this variable enables detection of circular and shared structure in printing
if non-nil this variable enables detection of uninterned symbols see creating symbols in printing
if non-nil that means number continuously across print calls
this variable holds a vector used internally by printing to implement the print-circle feature
this variable specifies how to print floating-point numbers
this variable holds the string to display to call attention to a particular line or nil if the arrow feature is not in use
this variable holds a marker that indicates where to display the overlay arrow
this variable’s value is a list of variables each of which specifies the position of an overlay arrow
this variable specifies the pointer shape to use ordinarily in the emacs frame for the text pointer style
this variable specifies the pointer shape to use when the mouse is over mouse-sensitive text
if this is non-nil it should be a function that moves to an earlier buffer position where the parser state is equivalent to nil—in other words a position outside of any comment string or parenthesis
this variable holds the raw prefix argument for the current command
the value of this variable is the raw prefix argument for the next editing command
the raw prefix argument value used by the previous command
this variable if non-nil specifies a function to use to handle errors that return control to the emacs command loop
the value of this variable is the number of bytes of pure storage allocated so far
this variable determines whether defun should make a copy of the function definition in pure storage
if this variable is non-nil then emacs quits immediately unless inhibit-quit is non-nil
this variable determines whether emacs should quit when quit-flag is set to a value other than nil
this buffer-local variable specifies whether the buffer is read-only
if this variable is non-nil then read-only buffers and depending on the actual value some or all read-only characters may be modified
if non-nil this should be a function that accepts the same arguments as read-file-name
this keymap is used by read-shell-command for completing command and file names that are part of a shell command
this variable holds the total number of input events received so far from the terminal—not counting those generated by keyboard macros
if this variable is non-nil it should be a regular expression that says how to search for whitespace
the value of this variable is a function that can be used by indent-region as a short cut
this buffer-local variable is either nil or a function for font lock mode to call to determine the region to scan and fontify
this variable is an alist of elements of the form (name
this variable specifies the application name that x-get-resource should look up
this variable specifies the instance name that x-get-resource should look up
if this variable is non-nil emacs does not look up x resources and x resources do not have any effect when creating new frames
revert-buffer binds this variable to a non-nil value while it is working
the value of this variable is the function to use to revert this buffer
the value of this variable specifies the function to use to insert the updated contents when reverting this buffer
this normal hook is run by the default revert-buffer-function before inserting the modified contents
this normal hook is run by the default revert-buffer-function after inserting the modified contents
the value of this variable specifies a function to call to check whether a buffer needs reverting
the value of this variable is a list of functions to be called before writing out a buffer to its visited file
this works just like write-file-functions but it is intended for hooks that pertain to the buffer’s contents not to the particular visited file or its location
this variable always local in all buffers specifies the width of the buffer’s scroll bars measured in pixels
this variable holds a special keymap that defines the valid user responses for perform-replace and the commands that use it as well as y-or-n-p and map-y-or-n-p
this variable holds a keymap that extends query-replace-map by providing additional keybindings that are useful in multi-buffer replacements
this variable specifies a function that perform-replace calls to search for the next string to replace
this variable specifies a function that perform-replace calls to search for the next regexp to replace
the value of this variable is a list of the keywords to highlight
non-nil means that regular expression matching for the sake of font-lock-keywords should be case-insensitive
this buffer-local variable enables selective display
emacs supports saving state via a hook called emacs-save-session-functions
this variable is used to define the maximum size of image that emacs will load
a list of functions to be called before playing a sound
when this variable is non-nil point-left and point-entered hooks are not run and the intangible property has no effect
if this variable is non-nil it specifies a function called to display help strings
if this variable is non-nil it specifies the coding system to use for reading a file or for input from a synchronous subprocess
this works much like coding-system-for-read except that it applies to output rather than input
this is the abbrev table for mode-independent abbrevs
the value of this buffer-local variable is the mode-specific abbreviation table of the current buffer
the value of this variable is a list of elements of the form (mode
this is the local abbrev table used in fundamental mode in other words it is the local abbrev table in all buffers in fundamental mode
this is the local abbrev table used in text mode
this is the local abbrev table used in lisp mode
this variable holds an alist which defines the default rear-stickiness of various text properties
the value of this variable is a string the name of a directory that contains programs that come with gnu emacs and are intended for emacs to invoke
this variable is a normal hook that emacs runs before suspending
this variable is a normal hook that emacs runs on resuming after a suspension
this variable indicates whether a call of process-file changes remote files
if the value of this variable is non-nil font lock does not do syntactic fontification only search-based fontification based on font-lock-keywords
this variable holds the syntax table to use for fontification of comments and strings
if this variable is non-nil it should be a function to move point back to a position that is syntactically at “top level” and outside of strings or comments
if this variable is non-nil it should be a function to determine which face to use for a given syntactic element a string or a comment
if this is non-nil the syntax scanning functions like forward-sexp pay attention to syntax text properties
this variable if non-nil should store a function for applying syntax-table properties to a specified stretch of text
this abnormal hook is run by the syntax parsing code prior to calling syntax-propertize-function
this variable holds the standard gnu configuration name for the hardware/software configuration of your system as a string
the value of this variable is a symbol indicating the type of operating system emacs is running on
this variable is a list of strings each describing one environment variable
this variable holds the list of environment variables emacs inherited from its parent process when emacs started
this variable holds a string that says which character separates directories in a search path as found in an environment variable
this variable holds the program name under which emacs was invoked
this variable holds the directory from which the emacs executable was invoked or nil if that directory cannot be determined
if non-nil this is a directory within which to look for the lib-src and etc subdirectories
this variable holds the erase character that was selected in the system’s terminal driver before emacs was started
this buffer-local variable specifies the format of the tabulated list data
this buffer-local variable specifies the entries displayed in the tabulated list buffer
this normal hook is run prior to reverting a tabulated list buffer
the value of this variable is the function called to insert an entry at point including its terminating newline
the value of this variable specifies the current sort key for the tabulated list buffer
this normal hook is run by with-output-to-temp-buffer before evaluating body
this normal hook is run by with-output-to-temp-buffer after displaying the temporary buffer
if the value of this variable is non-nil emacs loads a terminal-specific initialization file as follows (load (concat term-file-prefix (getenv "term"))) you may set the term-file-prefix variable to nil in your init file if you do not wish to load the terminal-initialization file
this variable is a normal hook that emacs runs after initializing a new text terminal
the function read-regexp may use the value of this variable to determine its list of default regular expressions
if this variable is nil then read-from-minibuffer and read-string strip all text properties from the minibuffer input before returning it
this is the default local keymap for reading from the minibuffer
this built-in variable is the keymap used as the minibuffer local keymap in the function read-no-blanks-input
this variable specifies the current buffer’s text representation
if this variable is non-nil it tells scroll-other-window which buffer’s window to scroll
if an editor command sets this variable non-nil then the editor command loop deactivates the mark after the command returns if transient mark mode is enabled
the mark is active when this variable is non-nil
these normal hooks are run respectively when the mark becomes active and when it becomes inactive
the value of this buffer-local variable is the list of saved former marks of the current buffer most recent first
by default the global map binds [tool-bar] as follows (global-set-key [tool-bar] `(menu-item ,(purecopy "tool bar") ignore :filter tool-bar-make-keymap)) the function tool-bar-make-keymap in turn derives the actual tool bar map dynamically from the value of the variable tool-bar-map
if this variable is non-nil the tool bar automatically resizes to show all defined tool bar items—but not larger than a quarter of the frame’s height
if this variable is non-nil tool bar items display in raised form when the mouse moves over them
this variable specifies an extra margin to add around tool bar items
this variable specifies the shadow width for tool bar items
this variable specifies the height of the border drawn below the tool bar area
this variable holds a keymap that describes the character sequences sent by function keys on an ordinary character terminal
this variable holds a keymap similar to input-decode-map except that it describes key sequences which should be translated to alternative interpretations that are usually preferred
this variable is another keymap used just like input-decode-map to translate input events into other events
this is the default translation table for decoding
this is the default translation table for encoding
self-inserting characters are translated through this translation table before they are inserted
if this buffer-local variable is non-nil it defines a wrap prefix which emacs displays at the start of every continuation line
if this buffer-local variable is non-nil it defines a line prefix which emacs displays at the start of every non-continuation line
this buffer-local variable’s value is the undo list of the current buffer
this variable is normally nil but the undo commands bind it to t
this variable holds a list of hooks to be scanned before unloading a library to remove functions defined in the library
this variable says which user’s init files should be used by emacs—or nil if none
if this buffer-local variable is non-nil emacs lisp files and buffers are evaluated using lexical binding instead of dynamic binding
this variable holds a list of all variables of type defvar_bool
the value of this variable indicates the time at which emacs was built
the value of this variable is the version of emacs being run
the major version number of emacs as an integer
the minor version number of emacs as an integer
if this variable is non-nil the line-move scroll-up and scroll-down functions will automatically modify the vertical scroll position to scroll through display rows that are taller than the height of the window for example in the presence of large images
the value of this variable is a list of functions to be called when find-file or find-file-noselect is passed a nonexistent file name
this buffer-local variable if set to a non-nil value makes save-buffer behave as if the buffer were visiting its file literally i
this list defines the meaning and severity order of the warning severity levels
if non-nil the value is a function to generate prefix text for warnings
programs can bind this variable to t to say that the next warning should begin a series
when this variable is non-nil it specifies a fill prefix to use for filling each warning’s text
this variable specifies the format for displaying the warning type in the warning message
the value of this variable is an alist that associates the names of loaded library files with the names of the functions and variables they defined as well as the features they provided or required
this variable holds a list of functions that emacs should call before redisplaying a window with scrolling
this variable holds a list of functions to be called if the size of any window changes for any reason
a normal hook that is run every time you change the window configuration of an existing frame
this variable is an alist specifying which parameters get saved by current-window-configuration and window-state-get and subsequently restored by set-window-configuration and window-state-put
if this variable is non-nil some standard functions do not process window parameters
this variable specifies the marker insertion type see marker insertion types of window-point
if this buffer-local variable is non-nil the size of any window displaying the buffer cannot normally be changed
this terminal-local variable tells lisp programs what window system emacs is using for displaying the frame
this variable holds the value of window-system used for the first frame created by emacs during startup
if this variable is non-nil certain motion functions including forward-word forward-sentence and forward-paragraph ignore field boundaries
this variable’s value should be an alist with one element for each system-specific keysym
the name of the keysym that should stand for the alt modifier respectively for meta hyper and super
if this variable is non-nil the function yank-pop uses its value instead of delete-region to delete the text inserted by the previous yank or yank-pop command
%-constructs in the mode line
a sample function description
a sample variable description
looking up and expanding abbreviations
saving abbrevs in files
abbrev properties
abbrev table properties
abbrev tables
abbrevs and abbrev expansion
abstract display example
abstract display functions
abstract display
accepting output from processes
access to documentation strings
accessing mouse events
accessing scroll bar events
accessing variable values
acknowledgments
active display table
active keymaps
adaptive fill mode
defining new setf forms
adjusting point after commands
ways to compose advices
advising emacs lisp functions
advising named functions
alias menu items
anonymous functions
old lisp features in emacs 23
applying customizations
evaluating macro arguments repeatedly
other features of argument lists
arithmetic operations
functions that operate on arrays
array type
arrays
association list type
association lists
creating an asynchronous process
atomic change groups
face attribute functions
automatic face assignment
auto filling
how emacs chooses a major mode
automatic indentation of code
auto-saving
autoload type
autoload
autoloading
backquote
backtracking in specifications
backup files
naming backup files
backups and auto-saving
base 64 encoding
basic char syntax
basic completion functions
basic faces
basic major modes
basic parameters
basic concepts of emacs windows
batch mode
beeping
bidirectional display
examples of byte unpacking and packing
functions to unpack and pack bytes
describing data layout
bitwise operations on integers
blinking parentheses
bool-vector type
bool-vectors
drawing lists as box diagrams
edebug breakpoints
breaks
buffer basics
examining buffer contents
motion to an end of the buffer
buffer file name
the buffer gap
buffer internals
the buffer list
buffer modification
buffer names
buffer parameters
buffer text notation
buffer type
buffer-local variables
buffers and windows
buffers
building emacs
building cons cells and lists
button buffer commands
button properties
button types
button-down events
buttons
byte compilation
packing and unpacking byte arrays
byte-code function objects
byte-code function type
c dialect
c integer types
calling functions
case changes
case conversion in lisp
the case table
explicit nonlocal exits catch and throw
categories
caveats
change hooks
changing file names and attributes
changing key bindings
changing text properties
character classes
char-table type
char-tables
character codes
character display
motion by characters
character properties
character sets
character type
checking whether to stop
checksum/hash
additional options for displaying buffers
choosing a window for display
read syntax for circular objects
classifying events
classification of list forms
cleaning up from nonlocal exits
click events
defining clickable text
closures
emacs lisp coding conventions
basic concepts of coding systems
coding systems
color names
counting columns
constructs for combining conditions
command history
information from the command loop
command loop
command loop overview
command-line arguments
user-level insertion commands
tips on writing comments
comments
common item keywords
comparing text
comparison of numbers
byte-compilation functions
tips for making compiled code fast
compiler errors
macros and byte compilation
minibuffer commands that do completion
completion in ordinary buffers
completion variables
completion
composite types
conditionals
cons cell and list types
lists and cons cells
variables that never change
contents of directories
parameters to control parsing
control structures
controlling the active keymaps
conventions
converting text representations
coordinates and windows
primitives to manipulate advices
coverage testing
creating copying and deleting directories
creating and deleting buffer-local bindings
creating buffers
creating frames
creating hash tables
creating keymaps
functions that create markers
creating strings
creating and interning symbols
control-character syntax
the current buffer
cursor parameters
custom themes
customization types
customization settings
customizing fringe bitmaps
customizing search-based fontification
cyclic ordering of windows
datagrams
debugger commands
the lisp debugger
debugging lisp programs
the declare form
telling the compiler that a function is defined
decoding process output
dealing with compressed data
dedicated windows
default coding systems
the default value of a buffer-local variable
defining abbrevs
defining commands
defining faces
defining functions
defining hash comparisons
defining images
defining macros
defining menus
defining minor modes
defining new types
defining global variables
defining symbols
delayed warnings
deleting frames
deleting processes
deleting windows
deleting text
defining derived modes
describing characters for help messages
desktop notifications
desktop save mode
dialog boxes
directory local variables
directory names
disabling commands
disabling multibyte characters
disassembled byte-code
action functions for display-buffer
display feature testing
displaying in the margins
the display property
display tables
emacs display
displaying faces
displaying messages in the echo area
distinguish interactive calls
documentation strings and compilation
documentation basics
tips for documentation strings
documentation
dotted pair notation
drag and drop
drag events
proper use of dynamic binding
dynamic binding
dynamically loaded libraries
dynamic loading of individual functions
easy menu
echo area customization
edebug and macros
edebug display update
evaluation
edebug execution modes
miscellaneous edebug commands
edebug options
edebug recursive edit
edebug views
edebug
editing types
emulating mode line formatting
encoding and i/o
accessing the entire match data
equality predicates
entering the debugger on an error
error messages
error symbols and condition names
errors
evaluation during compilation
evaluating macro arguments in expansion
evaluation list buffer
eval
evaluation notation
evaluation
event examples
miscellaneous event input features
modifying and translating input events
examining text properties
major mode examples
examples of catch and throw
excess close parentheses
excess open parentheses
excursions
expansion of a macro call
explicit entry to the debugger
explicit encoding and decoding
extended file attributes
extended menu items
face attributes
functions for working with faces
face remapping
faces for font lock
faces
defining and using fields
file attributes
file local variables
file locks
file name completion
file name components
functions that expand filenames
file names
notifications on file changes
files
filling
process filter functions
finding all frames
searching for overlays
floating-point basics
floating-point type
focus events
font and color parameters
font lock basics
font lock mode
font lock multiline
looking up fonts
font selection
font type
fontsets
forcing redisplay
overview
piecemeal specification
round-trip specification
file format conversion
format of descriptions
format of keymaps
formatted text properties
formatting strings
kinds of forms
frame configuration type
frame configurations
frame parameters
frame titles
frame type
frames
fringe bitmaps
fringe cursors
fringe indicators
fringe size and position
fringes
accessing function cell contents
entering the debugger on a function call
documentation strings of functions
evaluation of function forms
symbol function indirection
function keys
naming a function
determining whether a function is safe to call
function type
functions for key lookup
functions
garbage collection
general escape syntax
generalized variables
select among command alternatives
generic modes
geometry
getting out of emacs
global break condition
global variables
glyphless character display
glyphs
generic
generic
generic
defining customization groups
writing code to handle errors
hash table access
hash table type
hash tables
window header lines
help functions
high-level completion functions
hooks for loading
hooks
horizontal scrolling
how programs do loading
idle timers
image cache
image descriptors
image formats
imagemagick images
images
imenu
adjustable “tab stops”
indentation
indenting macros
generic
indirect buffers
debugging infinite loops
information about files
information from markers
inheritance and keymaps
the init file
initial input
initial frame parameters
inline functions
input events
input focus
input functions
input methods
input modes
input streams
sending input to processes
inserting text
instrumenting macro calls
instrumenting for edebug
integer basics
integer type
interactive call
code characters for interactive
examples of using interactive
internals of the debugger
internals of the kill ring
introduction to evaluation
introduction to buffer-local variables
introduction to minibuffers
introduction
invisible text
invoking the debugger
invoking the input method
iteration
jumping
commands for binding keys
key binding conventions
key lookup
key sequence input
key sequences
keyboard events
keyboard macros
menus and the keyboard
keymap basics
keymap type
keymaps and minor modes
keymaps
substituting key bindings in documentation
functions for killing
kill ring concepts
killing buffers
killing emacs
distinguishing kinds of files
components of a lambda expression
lambda expressions
layout parameters
lazy computation of text properties
levels of font lock
lexical binding
conventional headers for emacs libraries
library search
line height
coding systems in lisp
lisp data types
lisp history
accessing elements of lists
moving over balanced expressions
modifying list variables
predicates on lists
lists
load suffixes
loading non-ascii characters
loading
local variables
locales
locating files in standard places
logging messages in *messages*
low-level font representation
low-level kill ring
low-level network access
low-level parsing
lisp macro evaluation
macro type
macros
making certain file names “magic”
maintaining undo lists
major mode conventions
major modes
making backup files
making buttons
window management parameters
managing overlays
manipulating buttons
mapping functions
margins for filling
marker insertion types
marker type
markers
the match data
standard mathematical functions
memory usage
the menu bar
menu example
menu keymaps
menu separators
meta-character syntax
minibuffer commands
completion and the minibuffer
minibuffer contents
minibuffer history
minibuffer miscellany
minibuffer windows
minibuffers and frames
minibuffers
conventions for writing minor modes
minor modes
miscellaneous system events
misc network facilities
getting help about a major mode
mode hooks
mode line basics
the data structure of the mode line
mode line format
the top level of mode line control
variables used in the mode line
indentation controlled by major mode
major and minor modes
buffer modification time
modifying existing list structure
modifying menus
modifying strings
motion and syntax
indentation-based motion commands
motion events
motion commands based on parsing
motion
mouse events
menus and the mouse
mouse position
mouse tracking
moving marker positions
multi-file packages
multi-frame images
multiline font lock constructs
asking multiple y-or-n questions
multiple terminals
features
narrowing
examining text near point
testing availability of network features
network options
make-network-process
network servers
network connections
nil and t
non-ascii characters
non-ascii characters in strings
nonlocal exits
nonprinting characters in strings
why text properties are not intervals
making and deleting numbered backup files
numbers
numeric conversions
reading lisp objects with the minibuffer
object internals
declaring functions obsolete
operator precedence grammars
other character modifier bits
other display specifications
other font lock variables
other hash table functions
other image types
receiving output from processes
output functions
output streams
variables affecting output
the overlay arrow
overlay properties
overlay type
overlays
overview of markers
creating and maintaining package archives
packaging basics
preparing lisp code for distribution
access to frame parameters
parser state
parsing expressions
parsing html and xml
pattern matching case statement
pixel specification for spaces
property lists outside symbols
property lists and association lists
point
pointer shape
pop-up menus
adapting code using the old defadvice
position parameters
finding the parse state for a position
positions
posix regular expression searching
postscript images
precalculated fontification
predicates for strings
predicates on markers
type predicates for numbers
prefix command arguments
prefix keys
primitive function type
indentation primitives
printed representation and read syntax
printing in edebug
printing notation
common problems using macros
process buffers
process information
process internals
process type
processes
how emacs processes errors
processor run time
profiling
programmed completion
emacs programming tips
programming types
reporting operation progress
properties in the mode line
property lists
text property search functions
pure storage
querying before exit
quitting windows
quitting
quoted character input
quoting
raising and lowering frames
random numbers
reading and printing lisp objects
read-only buffers
reading a password
reading file names
reading from files
reading input
reading one event
functions that rearrange lists
recombining windows
recording input
recursive editing
recursive minibuffers
refreshing the screen
backslash constructs in regular expressions
complex regexp example
regular expression functions
regular expression searching
special characters in regular expressions
indenting an entire region
region to fontify after a buffer change
registers
regular expressions
other topics related to functions
absolute and relative file names
indentation relative to previous lines
remapping commands
backup by renaming or by copying?
repeat events
how many times is the macro expanded?
repeated loading
replacing the text that matched
display specs that replace the text
resizing windows
x resources
reverting
managing a fixed-size ring of objects
rounding operations
running hooks
saving buffers
saving and restoring the match data
scanning for character sets
scanning keymaps
motion by screen lines
scroll bars
search and replace
search-based fontification
searching and case
searching and matching
searching the active keymaps
selecting a representation
selecting windows
selective display
self-evaluating forms
sentinels detecting process status changes
sequences
sequence types
sequences arrays and vectors
sequencing
communicating with serial ports
session management
altering list elements with setcar
altering the cdr of a list
using lists as sets
the setf macro
setting hooks
setting variable values
shell arguments
showing images
how to signal an error
sending signals to processes
a simple lambda expression example
a simple example of a macro
simple match data access
simple menu items
simple packages
simple types
frame size and position
size of displayed text
size parameters
skipping characters
customizing indentation
defining the grammar of a language
sample indentation rules
helper functions for indentation rules
specifying indentation rules
defining tokens
smie setup and features
living with a weak parser
simple minded indentation engine
some terms
sorting text
sound output
source breakpoints
special events
special forms
properties with special meanings
specification examples
specification list
specified spaces
specifying a coding system for one operation
performance of byte-compiled code
splicing into lists
splitting windows
standard abbrev tables
generic
standard file names
generic
generic
standard symbol properties
standard regular expressions used in editing
starting up emacs
summary sequence of actions at startup
stickiness of text properties
stream type
introduction to reading and printing
string and character basics
conversion of characters and strings
searching for strings
string type
strings and characters
putting keyboard events in strings
functions that create subprocesses
subroutines of visiting
substituting for a character code
local variables in macro expansions
suspending emacs
swapping text between two buffers
switching to a buffer in a window
symbol components
symbol forms
accessing symbol properties
symbol properties
symbol type
symbols
creating a synchronous process
syntactic font lock
syntax table concepts
table of syntax classes
syntax descriptors
debugging invalid lisp syntax
syntax flags
syntax for strings
syntax of regular expressions
syntax properties
syntax table functions
syntax table internals
syntax tables
operating system environment
operating system interface
accessing other processes
tabulated list mode
temporary displays
terminal i/o encoding
terminal input
terminal output
terminal parameters
terminal type
terminal-specific initialization
test coverage
testing accessibility
comparison of characters and strings
reading text strings with the minibuffer
motion by text lines
text properties
text properties in strings
text representations
text terminal colors
text
textual scrolling
the echo area
the kill ring
the mark
the outside context
the region
time calculations
time conversion
time of day
parsing and formatting times
timers for delayed execution
tips for defining variables robustly
generic
tool bars
trace buffer
transaction queues
keymaps for translating sequences of events
translation of characters
transposition of text
trapping errors
truenames
truncation
type keywords
type predicates
undo
generating unique file names
unloading
user identification
user-chosen coding systems
user-level deletion commands
using the debugger
using edebug
using interactive
using lexical binding
usual display conventions
variable aliases
defining customization variables
scoping rules for variable bindings
variables with restricted values
variables
functions for vectors
vector type
vectors
version information
vertical fractional scrolling
visibility of frames
visiting files
functions for visiting files
when a variable is “void”
waiting for elapsed time or input
warning basics
warning options
tips for avoiding compiler warnings
warning variables
reporting warnings
what is a function?
which file defined a certain symbol
window configuration type
window configurations
window dividers
window frame parameters
window history
hooks for window scrolling and changes
window internals
window parameters
windows and point
window sizes
the window start and end positions
window system selections
window systems
window type
windows and frames
windows
motion by words
writing emacs primitives
writing to files
wrong time
operating on x11 keysyms
xbm images
xpm images
functions for yanking
yanking
yes-or-no queries
