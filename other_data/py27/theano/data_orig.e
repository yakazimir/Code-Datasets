...
tensorsolve
tensorsolve
basic l{linker} subclass that calls the perform method on each l{op} in the l{functiongraph} in the order given by l{linker
...
2d tensor containing kernels which are applied at every pixel
...
return a sparse matrix having random values from a multinomial density having number of experiment n and probability of succes
...
an :term apply instance is a node in an expression graph which represents the application of an op to some input variable nodes producing some
...
implement advancedincsubtensor1 on the gpu but use function only avail on compute capability 2
...
a functiongraph represents a subgraph bound by a set of input variables and a set of output variables ie a subgraph that specifies a theano function
this op builds a pooling descriptor for use in the other pooling operations
a type that allows no values
the function to call
a compiled theano function a variable an apply or
gradient wrt filters for abstractconv3d
raised by a feature's on_attach callback method if the functiongraph attempting to attach the feature already has a functionally identical
sinh x = (exp x - exp -x / 2
an operation to asynchronously receive an array to a remote host using mpi
if true this will print for apply node that
...
number of kernels/filters to apply
the upper bound of the gradient value
multiplicative inverse also called reciprocal
useful base class for l{linker}s which keep all nodes in the graph and run a thunk associated with each node
...
retrieves the list local optimizers based on the optimizer flag's value from equilibriumoptimizer by calling the method query
...
the destroyhandler class detects when a graph is impossible to evaluate because of aliasing and destructive operations
...
whether to treat the first axis of a and b as a batch
...
modified bessel function of the first kind of order 1
base class for corrmm, corrmm_gradweights and corrmm_gradinputs
this is an optimization that can push operations performed at the end of the inner graph of scan to outside of scan
...
the filename being considered used in diff generation only
if true then whenever this config option changes the
...
a list of expressions not to backpropagate
use this as a decorator or context manager to change the value of theano config variables
a dict where we store the ids of printed node
"transpose" of conv3d (conv3d implements multiplication by an implicitly defined matrix w
removes all applications of an op by transferring each of its outputs to the corresponding input
...
get a output_types_preference object by passing a dictionary unary_out_lookup({int8 int32 float32 complex128})
...
base class for meta-optimizers that try a set of localoptimizers to replace a node and choose the one that executes the fastest
lazy loading of moved objects in six moves urllib_response
solve a system of linear equations
relative tolerance used as threshold for gradient
gemv on the gpu
log base e
@todo update replaces all occurrences of the input pattern by the output pattern
if true it will plot the inner graph of each scan op
compute the numeric derivative of a scalar-valued function at a particular point
op for the cudnn softmax
listener for functiongraph events which makes sure that no operation overwrites the contents of protected variables
an operation to wait on a previously sent array using mpi
sum or average over different patches
modified bessel function of the first kind of order v real
the current fgraph. this function uses the inputs and outputs attributes
...
the callable that persists certain objects in the
represents a symbolic input for use with function or functionmaker
**inherit from**: - :class enumlist
raised by get_scalar_constant_value if called on something that is not a scalar constant
parameters ----------
...
class wrapper for tensorinv() function theano utilization of numpy
...
dictionary with names of ops as keys and colors as
applies logical and to all the values of a tensor along the specified axis es
function that takes a variable and returns its
...
tensor containing images on which to apply convolution
dtype of output if complex (i.e. 'complex32' or
...
implements the scaled complementary error function exp(x**2)*erfc x in a numerically stable way for large x
if true this will print the storage map
converts samples from a uniform into sample from a multinomial
abstract op for the forward convolution
softsign activation function
symbolic thing to print
lazy loading of moved objects in six moves urllib_parse
bessel function of the first kind of order 1
return the indices of the elements that are non-zero
base class for abstractconv parameters
implement softmaxwithbias on the gpu
join for gpu
the file handle to the zip file to load the object from
increments a subtensor using advanced slicing list of index
op that provides conditional graph evaluation if used with the cvm/vm linkers
raised when the shape cannot be computed
this variable can take any value
double each element of a tensor
uses clinker on the individual ops that comprise an fgraph and loops over them in python
...
dot22 on the gpu
this special kind of variable is matched against a list and unifies an inner variable to an orvariable of the values in the list
...
a global optimizer for pushing out the variables inside the scan that depend only on constants and sequences
...
flatten the last 2 dimensions of the output. by default
parameters ----------
main subclasses - :class enumlist
functionmaker is the class to create function instances
the forward convolution
none or existing output storage see below
blas defines general rank-1 update ger as a <- a + alpha x y' for matrix a scalar alpha vectors x and y
...
the number of calls to gemm to do
...
return a sparse matrix having random values from a binomial density having number of experiment n and probability of succes
a function that accepts two symbolic variables and computes
absolute tolerance used as threshold for gradient
...
abstract class
random number generator used to sample u we test gradient
gemm for float16 using the nervena kernels
raised when grad encounters a nulltype
base class for gpucorr3dmm, gpucorr3dmm_gradweights and gpucorr3dmm_gradinputs
l{op} to return the shape of a matrix
fundamental way to create a sparse node
gpucareducecuda is a reduction along some dimensions by a scalar op
class used to encode the different things the inner function of scan can or needs to return
don't use debugmode for the numerical
graph optimizer for inserting gemm operations
structured dot csr is like dot except that only the gradient wrt non-zero elements of the sparse matrix
transfer the input to the gpu to start the rolling wave
the convolution gradient with respect to the weights
advancedsubtensor on the gpu
composite is an op that takes a graph of scalar operations and produces c code for the whole graph
if self.subsample[0] != 1 a variable giving the height
raised when functiongraph calls destroyhandler callbacks in an invalid way for example pruning or changing a node that has
...
print to this file ('str' means to return a string)
ger on the gpu
print graph to this depth -1 for unlimited
a string containing a file's worth of python code
...
a set of potential optimizations which should be applied in an arbitrary order until equilibrium is reached
a meta path importer to import six moves and its submodules
base op for cudnn batch normalization
creates c code for an fgraph compiles it and returns callables through make_thunk and make_function that make use of the compiled
...
careduce = commutative associative reduce reduces a scalar operation along the specified axis es
the variable we want its gradient inputs scale
used for recursive calls -- do not provide an argument for
...
parameters ----------
this op serves a dual purpose it can implement a vanilla 2d convolution as taught in any signal processing class or implement the
lazy loading of moved objects
if true will remove intermediate var that don't have name
writeme
...
an ndarray a list of ndarrays or tuple of ndarrays
...
the persistent loading function to use for
implement alloc on the cpu but without initializing memory
op for the cudnn softmaxgrad
scale of the gradient
an iterable of graph inputs not used in any
operand optimized for calculating the dot product dot(x, y t) = z
dimensions over which to sum for the tensordot operation
if true will print some theano flags and env variables
op for the cudnn softmax
subtensor on the gpu
shape of kernel to apply smaller than image
'valid' generates output only when kernel and
structured dot csc is like dot except that only the gradient wrt non-zero elements of the sparse matrix a are calculated and propagated
...
if self.subsample[1] != 1 a variable giving the depth
implement crossentropysoftmax1hotwithbiasdx on the gpu
l{op} to return the shape of a matrix
gradient wrt filters for abstractconv
implements the inverse error function
a python function that takes theano variables as inputs
when true if a node in the graph has a name
raised by get_scalar_const_value if called on something that is a zero dimensional constant
...
this variable could be any value from a finite list of values accessible via the options field
...
this class makes it easier to run several l{locallinker}s in parallel and offers some control over how each thunk is run
the convolution gradient with respect to the inputs
this optimizer performs two important functions 1) it has a 'requirement' of the destroyhandler
...
this exception should be thrown by replace_all_validate_remove when an optimization wanted to remove a variable or a node from
returns an inplace view of the input used internally by theano
an object for getting and setting this configuration
...
an iterable of graphs in which to replace variables
abstract op for the forward convolution
highlights a lazy if by sorrounding each of the 3
...
the output file where to put the graph
the zip file that func should write its data to
return a subtensor view
a type indicating that a variable is a result of taking the gradient of c with respect to x
...
an exception thrown when we put in a functiongraph a constant that is cached
compute the cross entropy between a coding distribution and a true distribution of the form [0 0
base op for cudnn batch normalization
an l{optimizer} can be applied to an l{functiongraph} to transform it
base class for functiongraph extensions
if the output is float32 and
symbolic type representing a numpy ndarray value
the pickling protocol to use. unlike python's built-in
base class for corr3dmm, corr3dmm_gradweights and corr3dmm_gradinputs
...
this class represents a possible unification of a group of variables with each other or with tangible values
...
takes a list of l{optimizer} instances and applies them sequentially
perform a reshape operation of the input x to the new shape shp
implement the grad of max pooling on the gpu
object to store runtime and memory profiling information for all of theano's operations compilation optimization execution
the lower bound of the gradient value
gpu correlation implementation using matrix multiplication
**inherit from**: - :class enumtype
3d convolution of multiple filters on a minibatch
softmax activation function :math \varphi(\mathbf{x})_j =
...
raised when grad is asked to compute the gradient with respect to a disconnected input and
the list of numpy.ndarrays to use as input values
compute a matrix-matrix product
...
if true it will create the image and return it
...
concatenate multiple tensorvariables along some axis
implements the r-operator for the downsample operation
logsoftmax activation function :math \varphi(\mathbf{x})_j =
replaces the application of a certain op by the application of another op that takes the same inputs as what they are replacing
...
generate a local optimizer of type localoptgroup instead of a global optimizer
...
2d tensor containing images on which to apply convolution
converts samples from a uniform into sample without replacement from a multinomial
base class for gpucorrmm, gpucorrmm_gradweights and gpucorrmm_gradinputs
...
if true and a variable have a name
build a basic theano op around a function
a global optimizer for pushing out the variables inside the scan that depend only on non-sequences
incomplete qr decomposition
the id to use for some object but maybe we only
implement advancedincsubtensor1 on the gpu
cosh x = (exp x + exp -x / 2
inverse complementary error function for gpu
print the toposort index of the node in the node name
this class is a wrapper for numpy sort function
if true the color that describes the respective
...
this variable can take any value but a finite amount of forbidden values accessible via the not_options field
a differentiable function such that f(*pt) is a scalar
compute the matrix exponential of a square array
class wrapper around release mechanism so that the lock is automatically released when the program exits even when crashing or being interrupted
log gamma function
