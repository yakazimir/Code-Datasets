...
converts the rdoc-…: links into a markdown
...
receives up to maxlen bytes from udpsocket using recvfrom 2 after o_nonblock is set for the underlying file descriptor
returns the encoding object that represents the encoding of the file
bits - integer safe - boolean add - bn rem - bn
with no arguments raises the exception in $! or raises a runtimeerror if $! is nil
fetch struct member name
aborts the previous command abor command
protected setter for the opaque component v see also #opaque=
...
returns dn
returns the second of the minute 0
...
string public setter for the scheme component v
...
class attributes
returns the index of the supplied child starting at 1 or -1 if the element is not a child an element child
...
removes this attribute from the tree and returns true if successful this method is usually not called directly
iterates over files in the tarball yielding each entry
writes the
resets http connection connection
check the fragment v component against the uri :parser regexp for :fragment string public setter for the fragment component v
...
returns the result of invoking exception
...
returns an expiry time based on sec which can be one of sec seconds into the future the expiry time is the start of 1970
...
get the parsable form of the current configuration given the following configuration being created you can parse get the serialized configuration using to_s and then parse it later
...
decodes a ber- or der-encoded value and creates an asn1data instance
...
converts a bigdecimal to a string of the form “nnnnnn
...
converts a string to a float see also string
...
should this codeobject be displayed in outputs
...
string parses uri and constructs either matching uri scheme object ftp http https ldap ldaps or mailto or uri :generic
returns whether or not the given entry point func can be found within lib
builds a methods for level meth
enables pretty-print for all tests
returns the ruby source filename and line number containing this proc or nil if this proc was not defined in ruby
many methods in this class aren’t documented
creates a new irb context
creates a new socket :option object for ip_multicast_ttl
returns the hour of the day 0
...
returns the ruby source filename and line number containing this method or nil if this method was not defined in ruby
...
converts a string to an integer see also string
...
deactivates the trace return true if trace was enabled
...
returns the index of the current cursor position in readline
...
string public setter for the typecode v
...
pass - string name - a string describing the key
...
example klass will determine the class of io returned using the io
...
extensions to build when installing the gem specifically the paths to extconf
...
start a stream emission with encoding see psych :handler#start_stream
...
creates a weak reference to orig raises an argumenterror if the given orig is immutable such as symbol fixnum or float
returns an array of headings in the document
barelement_id = [‘blt_stripchart_bar’
evaluates a string containing ruby source code or the given block within the context of the receiver obj
returns the list of break points where execution will be stopped
terminate execution immediately effectively by calling kernel
...
string public setter for the port component v
...
convert signal number to signal name produces
...
resumes the fiber from the point at which the last fiber
...
casts an integer as an openssl :bn see man bn for more info
adds rule to the output
...
key - the public key to be used for verifying the spki signature returns true if the signature is valid false otherwise
...
define new proc class if you want to modify the new class or create a new subclass you must do such operation in the block parameter
...
possible options elements ::new creates an instance of encoding :converter
instance methods
...
returns x/y syntax of string form see string#to_r
...
invokes the given block once for each element of self
removes an attribute either a string which is the name of the attribute to remove – namespaces are significant here – or the attribute to remove
generate url-encoded form data from given enum
returns true if strio is at end of file
finds all live tuples that match template
calls the block once for each value string in the database
true if the requirement will not always match the latest version
returns a string that represents the stringscanner object showing the current position the size of the string the characters surrounding the scan pointer s = ::new(“fri dec 12 1975 14 39”) s
...
private setter for the typecode v see also #typecode=
...
does this context and its methods and constants all have documentation? yes fully documented doesn’t mean everything
...
pass - string salt - string - should be at least 8 bytes long
...
adds data at the end of the chain of entries this is called by webrick :httputils
...
returns regexp that is default self
test case that calls @to
...
return a regexp object that is the union of the given patterns i
...
converts the rdoc markup tidylink into a markdown
...
creates an apispecification for the given set from the rubygems
...
creates a bestset for the given sources or gem
...
converts token_stream to html wrapping various tokens with <span> elements
...
look for directives in a normal comment block this method modifies the comment
...
creates a filehandler servlet on server that serves files starting at directory root options may be a hash containing keys from webrick :config :filehandler or true or false
...
an array of strings attempts to parse and merge a set of uris
...
adds a checksum for each entry in the gem to checksums
...
makes hsh compare its keys by their identity i
converts the contents of the database to an array of key value arrays and returns it
enable ssl for all new instances
defines a method method which delegates to accessor
...
converts a string to true or false raises an exception if str is not 0 or 1
...
create a new leveled notifier with the given base and prefix to send to irb :notifier :abstractnotifier
...
returns a list of gems available for each source in gem
...
string public setter for the query component v
...
the first two forms are used to create a new struct subclass class_name that can contain a value for each member_name
protocol scheme i
...
whether this specification is stubbed - i
...
sets a specific section name with a hash pairs given the following configuration being created it’s important to note that this will essentially merge any of the keys in pairs with the existing section
get the entity expansion limit
escapes a string so that it can be safely used in a bourne shell command line
creates a new tomdoc parser
...
plural accessor for setting licenses see license= for details
...
the date this gem was created do not set this it is set automatically when the gem is packaged
...
generates a xml-rpc methodresponse document when is_ret is false then the params array must contain only one element which is a structure of a fault return-value
...
start tracing you can also pass a block
...
creates a gzipreader or gzipwriter associated with io passing in any necessary extra options and executes the block with the newly created object just like file
...
turn an array of nametuple objects back into an array of tuples
...
this method provides the metavariables defined by the revision 3 of “the www common gateway interface version 1
...
creates a unix server socket on path if no block given it returns a listening socket
rename the file
...
returns an array containing the lines returned by calling kernel
...
returns true if stat generated a coredump when it terminated
...
protected setter for the path component v see also #path=
...
creates a new loc :size from arg which may be returns arg
...
sets bc mode which loads lib/mathn
exits the process immediately
...
string public setter for the opaque component v
helper commands
...
the release for this version e
...
print an http header and body to $default_output $>) if a string is passed it is assumed to be the content type
...
execute the provided block but preserve the rounding mode for use with the bigdecimal :round_* see ::mode
...
frozen_string_literal false cgi_runner
returns a frozen copy of the string passed in to match
calculates crc checksum for string and returns updated value of crc
retrieves remotefile in binary mode storing the result in localfile
protected setter for the port component v see also #port=
...
returns sym
subject of on_tail
ln_s(old new options = {}) creates a symbolic link new which points to old
...
returns 0 if the value is positive pi otherwise
extracts the files in this package into destination_dir if pattern is specified only entries matching that glob will be extracted
...
is there any content? this means any of comment aliases methods attributes external aliases require constant
...
returns an array with 14 elements representing the instruction sequence with the following data a string identifying the data format
...
returns the path to the trusted certificate with the given asn
...
installs the default context extensions as irb commands irb/ext/history
show pid and exit status as a string
closes the connection
creates a new ruby parser
parse a constant which might be qualified by one or more class or module names
the number of bytes that are immediately available for reading
adds an element if supplied is either an element string or source see element
returns a copy of self as local mode
waits for a child process to exit see ::waitpid for exact semantics) and returns an array containing the process id and the exit status a process :status object) of that child
...
protected setter for the password component v see also #password=
...
loads translation messages from locale_directory/+@name+/rdoc
returns the current value of the configuration option given by option
use the home and paths values for ::dir and ::path
based on tk/scrollable
create a new psych :nodes :mapping object
returns the current line number for io
yields each row of the data source in turn
does this dependency match spec? note this is not a convenience method
returns the source encoding as an encoding object
...
protected setter for the user component v see also #user=
...
returns true for ipv4 private address 10
...
resolves the requested dependencies into a {dependencygraph}, locking to the base dependency graph if specified @param array requested an array of ‘requested’ dependencies that the @param dependencygraph nil base the base dependency graph to which
...
creates a new http server according to config an http server uses the following attributes an array of access logs
returns true if there are no threads in the pool still running
returns the imaginary part
...
this method is insecure private information can leak out!!! dumps all parameters of a keypair to a string don’t use :-)) it’s up to you)
...
possible opt elements possible results #primitive_convert converts source_buffer into destination_buffer
returns true if obj is an element of the range false otherwise
...
builder for an rss object creates an object of the type passed in args executes the block to populate elements of the created rss object
...
string to search patterns to apply to str attempts to parse and merge a set of uris if no block given then returns the result else it calls block for each element in result
...
encodes string using ruby’s string
...
case equality—returns true if obj is an instance of mod or and instance of one of mod’s descendants
...
array to x509_extension structure or or or the same for sn
...
decodes the given str using xmlrpc :base64
...
parses tomdoc from text a string containing tomdoc-format text
...
extracts the gem :specification and raw metadata from the
...
end a stream emission see psych :handler#end_stream
...
initializes irb and creates a new irb
sets the preset dictionary and returns string
adds extra args from ~/
...
value please have a look at constructive and primitive to see how ruby types are mapped to asn
...
protected setter for the host component v see also #host=
...
does other match this platform? if other is a string it will be converted to a gem :platform first
...
calls end_accepting on your test case which needs to call @to
...
returns the contents of this tms object as a formatted string according to a format string like that passed to kernel
returns self
...
method name if the method has no assigned name it extracts it from call_seq
...
this method is insecure private information can leak out!!! stores all parameters of key to the hash
helper method
...
the fiddle :compositehandler instance will raise an error if no handlers are open
...
returns the content of this cdata object examples
...
returns a string usable in dir
...
::pipeline_start starts a list of commands as a pipeline
...
resolves the {#original_requested} dependencies into a full dependency @raise resolvererror if successful resolution is impossible @return dependencygraph the dependency graph of successfully resolved
...
creates a class to wrap the c struct with the value ty see also #struct
...
returns the value as a rational if possible the imaginary part should be exactly zero
similar to decode with the difference that decode expects one distinct value represented in der
returns the portion of the original string after the current match
true if the gems in the system satisfy dependency
removes the named instance variable from obj returning that variable s value
builds a heading from the token stream the level of heading to create returns an rdoc :markup :heading
...
equivalent to warning this method causes local vulnerability
extra files to add to rdoc such as readme or doc/examples
...
given the path to a gem file validates against its own md5 checksum path to gem file
...
constructs a new regular expression from pattern which can be either a string or a regexp in which case that regexp’s options are propagated), and new options may not be specified a change as of ruby 1
...
creates a new loc :coord from arg which may be returns arg
returns the socket option data as a string
returns a string representing this module or class
create a prettyprint :singleline object arguments output - string or similar to store rendered text
...
type of event see events at tracepoint for more information
...
the short name of the objectid as defined in <openssl/objects
...
with no associated block io
returns true for ipv4 loopback address 127
...
key - the private key to be used for signing this instance digest
returns a range object which represents the value of the content-range header field
loads ruby format gemspec from file
verifies that this gem contains a valid gem specification contains a contents archive the contents archive is not corrupt after verification the gem specification from the gem is available from spec
...
returns a hash that will be turned into a json object and represent this object
todo remove with rubygems 3
builds a paragraph from the token stream unused returns an rdoc :markup :paragraph
...
constructor the element of which this is an attribute
...
adds the multi-call handler system
...
the path to a built gem from @spec the path to a built gem from @user_spec a gem :installer for the @spec that installs into @gemhome a gem :installer for the @user_spec that installs into gem
creates an enumerator for each chunked elements
...
used to filter unwanted trace output example which only outputs lines of code executed within the kernel class
checks - integer trial_div - boolean
...
parses a c struct’s members example
...
returns true for ipv4 multicast address 224
...
create a new cgi :session :pstore instance this constructor is used internally by cgi :session
...
ln(old new options = {}) creates a hard link new which points to old
...
returns an array containing matrices corresponding to the real and imaginary parts of the matrix m
...
calls the given block once for each byte 0
call this after you have added all you handlers to the server
...
return a new version object where the next to the last revision number is one greater e
returns true if the named file is a directory or a symlink that points at a directory and false otherwise
...
make a copy of this object examples
...
returns the block length of the digest algorithm i
...
string to make safe regexp to apply
...
creates a new irb session see irb :irb
...
creates a new apiset that will retrieve gems from uri using the rubygems api url dep_uri which is described at guides
creates several default gems which all have a lib/code
...
creates a new date object by parsing from a string according to some typical rfc 2822 formats
the opposite of #absolute? it returns false if the pathname begins with a slash
...
updates the database with multiple values from the specified object
singular writer for authors usage
...
returns true if the database contains the given key
adds a hook that will get run after gem :specification
...
returns a ten-element array of values for time see the individual methods for an explanation of the valid ranges of each value
...
the long name of the objectid as defined in <openssl/objects
...
deletes a child element either an element which is removed directly an xpath where the first matching child is removed or an integer where the n’th element is removed
returns the successor to str
displays a warning using kernel#warn if we’re being verbose
returns the truncated value toward the nearest integer 0
...
uri or string merges two uri’s
extracts the files in this package into destination_dir
returns true if net/http is in version 1
...
returns true if this is an empty matrix i
sets a thread local with key to value
...
creates a new classmodule with name with optional superclass this is a constructor for subclasses and must never be called directly
...
wraps text to width
iterates through all of the child elements optionally filtering them by a given xpath optional
...
need to be able to set the sources without calling ::sources
...
returns ios
fetches an attribute the name by which to search for the attribute
...
construct a new class given a c class klass cunion cstruct or other that provide an entity_class) types fiddle :type_int fiddle :type_size_t etc
accepts an incoming connection using accept 2 after o_nonblock is set for the underlying file descriptor
...
test case that calls @to
returns the hexadecimal representation of a memory pointer address addr example
...
given a gem file’s contents validates against its own md5 checksum contents of the gem file
...
set value as the new date/time component
converts a the given str to a datetime
...
creates an array of handlers for the given libs can be an instance of fiddle :handle fiddle :importer or will create a new instance of fiddle :handle using fiddle
produces the summary text
reads the contents of filename and handles any encoding directives in the file
creates a message extractor for store
specifies the inspect mode with opt display inspect display to_s inspect mode in non-math mode non-inspect mode in math mode see irb :inspector for more information
turn on/off ssl
...
installs the default irb commands irb :context#main irb :context#change_workspace irb :context#workspaces irb :context#push_workspace irb :context#pop_workspace irb_load irb_require irb :irbloader#source_file irb
...
obsolete method
directs to accept specified class t
::new is private use ::rows columns [], etc
builds extensions
...
create a new drb :drbsslsocket :sslconfig instance the drb :drbsslsocket will take either a config hash or an instance of sslconfig and will setup the certificate for its session for the configuration
...
classifies the set by the return value of the given block and returns a hash of {value => set of elements} pairs
...
the ::double_fig class method returns the number of digits a float number is allowed to have
returns the offset in seconds between the timezone of time and utc
...
the object identifier as a string e
...
returns the symbol names of the binding’s local variables this method is short version of the following code
...
reads a one-character string from ios
return values corresponding to the value of float nil -1 1 for example
...
returns the ssl parameters see also ::enable_ssl
get an array of all text children
returns a proc object proc to call after the first prompt has been printed and just before readline starts reading input characters
...
or separated named) and return the ultimate name the associated container and the given name with the ::)
iterates the given block over all prime numbers
...
whether ^c control-c will be ignored or not
a copy of the default irb
the current input method can be either stdioinputmethod readlineinputmethod fileinputmethod or other specified when the context is created
current irb session
can be either name from irb
...
returns whether bc mode is enabled
irb prompt for continuated statement e
standard irb prompt see irb for more information
a copy of the default irb
see customizing the irb prompt at irb for more information
irb prompt for continuated strings see irb for more information
a copy of the default irb
a copy of the default irb
...
the top-level workspace of this context also available as irb
...
the global default options for the #dump method :max_nesting false :allow_nan true :quirks_mode true
returns the json generator module that is used by json
the global default options for the #load method :max_nesting false :allow_nan true :quirks_mode true
...
the maximum width of a line before it is separated in to a newline this defaults to 79 and should be a fixnum
...
the horizontal precision using ssize type values in meters using scientific notation as 2 integers of xey for precision use value/2 e
...
the vertical precision using ssize type values in meters using scientific notation as 2 integers of xey for precision use value/2 e
...
gemdependencyapi object which is set when
...
the list of hooks to be run after gem :specification
the list of hooks to be run after gem :uninstaller#uninstall completes installation
the list of hooks to be run before gem :installer#install does any work
the list of hooks to be run before gem :specification
...
a long description of this gem the description should be more detailed than the summary but not excessively long
:attr_accessor metadata the metadata holds extra data for this gem that may be useful to other consumers and is settable by gem authors without requiring an update to the rubygems software
a message that gets displayed after the gem is installed
the version of ruby required by this gem
the rubygems version required by this gem
the key used to sign this gem
a contact email address or addresses for this gem usage spec
the url of this gem’s home page usage spec
this gem’s name
...
@return [{string => vertex}] the vertices of the dependency graph keyed by {vertex#name}
...
@return [arrary<object>] the explicit requirements that required this vertex
@return [array<edge>] the edges of {#graph} that have self as their {edge#destination}
@return string the name of the vertex
@return [array<edge>] the edges of {#graph} that have self as their {edge#origin}
@return object the payload the vertex holds
@return boolean whether the vertex is considered a root vertex
@return boolean whether the vertex is considered a root vertex
the gems required by gem statements in the gem
...
display additional debug information defaults to false
typecode accessor see uri :ftp :component
...
the hash of regexp see also uri :parser
...
the complete request line such as get / http/1
...
returns the net :http object for the client
...
rdoc :markup :tottonly.accept_list_end rdoc :markup :tottonly.accept_list_start
io.gets io.lineno
sdbm.replace sdbm.update
xmlrpc :client.multicall xmlrpc :client.multicall_async
rdoc :codeobject.suppress rdoc :codeobject.suppressed?
prime.prime_division integer.prime_division
tsort.tsort_each_node tsort.each_strongly_connected_component_from
float.ceil numeric.ceil
basicsocket.local_address basicsocket.getsockname
thread.alive? thread.stop? thread.status
enumerator.peek_values enumerator.next_values enumerator.next
net :http.start net :http.read_timeout=
gem :security.trusted_certificates gem :security.trust_dir
gem :specification.require_path= gem :specification.require_path gem :specification.require_paths=
optparse.with optionparser.with
openssl :pkcs12.create openssl :ssl :sslsocket.client_ca
win32ole_event.handler= win32ole_event.on_event_with_outargs
socket.connect_nonblock socket.connect
shellwords.shellsplit string.shellsplit
rinda :ringserver.do_write rinda :ringserver.write_services
openssl :buffering.each openssl :buffering.readlines
date.italy date.england date.julian date.gregorian
gem.remove_unresolved_default_spec gem.find_unresolved_default_spec gem.register_default_spec
gem :resolver :dependencyrequest.match? gem :resolver :dependencyrequest.matches_spec?
shell.default_system_path shell.default_system_path=
kernel.caller_locations thread.backtrace_locations
json :ext :generator :state.quirks_mode= json :ext :generator :state.new
socket.udp_server_recv socket.udp_server_loop
string.=== string.str
gem :specification.author gem :specification.author=
array.bsearch_index array.bsearch
readline.input readline.output
cgi :htmlextension.file_field cgi :htmlextension.form
rdoc :codeobject.ignore rdoc :codeobject.ignored?
tsort.tsort tsort.tsort_each
net :imap.uid_thread net :imap.thread net :imap.uid_search
time.utc time.to_a time.gm
thread.wakeup thread.run thread :mutex.sleep
array.find_index array.rindex array.index
rdoc :ri :paths.raw_path rdoc :ri :paths.path
rdoc :codeobject.each_parent rdoc :classmodule.each_ancestor
net :httpheader.get_fields net :httpheader.add_field
uri :ldap.new uri :http.new
net :pop3.use_ssl? net :pop3.verify net :pop3.certs
basicsocket.getsockopt basicsocket.setsockopt
logger.info logger.warn logger.fatal logger.debug logger.unknown syslog :logger.add logger.error
gem :command.add_option gem :command.new
gem :specification.extra_rdoc_files= gem :specification.normalize
uri.encode_www_form uri.decode_www_form_component
kernel.exit process.exit
gem :requestset.install gem :requestset.new gem :requestset.install_from_gemdeps
array.keep_if array.select!
gem :requestset :gemdependencyapi.gem gem :requestset :gemdependencyapi.platform gem :requestset :gemdependencyapi.platforms
zlib :deflate.set_dictionary zlib :gzipwriter.new zlib :deflate.params
psych.load psych.parse_stream psych.parse psych.parse_file
array.ary index array.unshift array.push array.shift array.pop
tracepoint.event tracepoint.new
erb.set_eoutvar erb.result erb.run
fixnum.bit_length bignum.bit_length
openssl :pkey :dsa.public_key openssl :pkey :dsa.public?
shellwords.shellescape string.shellescape
rdoc :anymethod.call_seq= rdoc :anymethod.arglists rdoc :anymethod.name
net :pop3.reset net :popmail.delete
rdoc :store.classes_hash rdoc :store.modules_hash rdoc :classmodule.update_aliases
pathname.to_s pathname.to_path
prettyprint.fill_breakable prettyprint.new
yaml :dbm.replace yaml :dbm.update
irb :outputmethod.ppx irb :outputmethod.puts irb :outputmethod.pp irb :outputmethod.print
rexml :attributes.[] rexml :attributes.get_attribute
net :ftp.close net :ftp.connect
tempfile.path tempfile.unlink tempfile.close
objectspace.reachable_objects_from objectspace.internal_class_of objectspace.internal_super_of
psych :handler.start_sequence psych :emitter.start_sequence psych :nodes :sequence.new
tk :blt :stripchart.extents tk :blt :barchart.extents tk :blt :graph.extents
openssl :engine.cipher openssl :engine.ctrl_cmd openssl :engine.finish openssl :engine.load_public_key openssl :engine.load_private_key openssl :engine.digest
rdoc :classmodule.ancestors rdoc :mixin.module rdoc :mixin.full_name rdoc :classmodule.find_ancestor_local_symbol
rdoc :context.add_class rdoc :context.add_module
irb :jobmanager.irb irb :jobmanager.thread irb :jobmanager.search
rdoc :anymethod.marshal_load rdoc :attr.marshal_load rdoc :anymethod.marshal_dump rdoc :attr.marshal_dump
gem :uriformatter.unescape gem :uriformatter.escape
exception.set_backtrace exception.backtrace_locations
openssl :x509 :name.new openssl :x509 :name.add_entry
benchmark :tms.add benchmark :tms.add!
openssl :pkey :dh.compute_key openssl :pkey :dh.public_key openssl :pkey :dh.to_der openssl :pkey :dh.generate_key! openssl :pkey :dh.new
stringscanner.check_until stringscanner.exist?
array.sort array.sort!
cgi.header cgi.out cgi.http_header
uri :generic.merge uri :generic.merge!
net :http.request_get net :http.request net :http.request_post net :httpresponse.read_body
net :imap.uid_move net :imap.move
thread.kill thread.pending_interrupt? thread.terminate thread.handle_interrupt thread.exit thread.raise
rinda :tuplespaceproxy.notify rinda :tuplespace.notify
string.unicode_normalized? string.unicode_normalize!
gdbm.replace gdbm.update
encoding :converter.source_encoding encoding :converter.new encoding :converter.destination_encoding
rexml :streamlistener.tag_end rexml :streamlistener.tag_start
rexml :child.replace_with rexml :parent.replace_child
threadgroup.enclosed? threadgroup.enclose
rdoc :constant.marshal_dump rdoc :constant.marshal_load
observable.changed? observable.notify_observers observable.add_observer
gem :availableset.each_spec gem :availableset.each
psych :nodes :scalar.new psych :handler.scalar psych :emitter.scalar
fixnum.modulo bignum.modulo numeric.div fixnum.% bignum.% numeric.modulo
object.taint object.tainted? object.untaint
fiddle :cstructbuilder.create fiddle :importer.create_value
webrick :basiclog.new webrick :basiclog.info?
optparse.ver optionparser.ver
rdoc :codeobject.parent rdoc :constant.== rdoc :context.add_class_or_module rdoc :context.current_section rdoc :codeobject.section
net :smtp.start net :smtp.debug_output= net :smtp.new
encoding :converter.convert encoding :converter.last_error encoding :converter.primitive_convert
webrick :httputils :formdata.new webrick :httputils :formdata.append_data webrick :httputils :formdata.<<
gem :dependencyinstaller.new gem :installer.install gem :resolver :bestset.new gem :specfetcher.available_specs gem :specfetcher.new gem.sources=
pathname.each_child pathname.children pathname.entries
gem :testcase.util_setup_fake_fetcher gem :testcase.util_setup_spec_fetcher
rexml :document.entity_expansion_limit= rexml :document.entity_expansion_limit
sizedqueue.close queue.close
tempfile.new tempfile.open tempfile.create
irb :outputmethod.parse_printf_format irb :outputmethod.printf
fiddle :importer.dlload fiddle :compositehandler.new
irb :extendcommandbundle.install_alias_method irb :extendcommandbundle.extend_object
irb :notifier :compositenotifier.def_notifier irb :notifier :compositenotifier.level_notifier=
rexml :document.new psych :treebuilder.end_document psych :treebuilder.start_document psych :emitter.start_document psych :handler.start_document
shell.install_system_commands shell.def_system_command
array.first array.last
psych :emitter.start_mapping psych :handler.start_mapping psych :nodes :mapping.new
float.dclone fixnum.dclone array.dclone symbol.dclone object.dclone
rdoc :classmodule.update_includes rdoc :context.remove_from_documentation? rdoc :classmodule.documented? rdoc :codeobject.documented?
gem.find_files gem.find_latest_files
rdoc :ri :driver.find_store rdoc :store.friendly_path rdoc :store.source
array.take array.drop
drb.front drb.to_obj drb.current_server drb.to_id
rdoc :rdoc.document rdoc :rdoc.new rdoc :options.finish
pstore.fetch pstore.delete pstore.transaction pstore.commit pstore.abort pstore.[] pstore.[]= pstore.root? pstore.roots
io.new string.encode io.write_nonblock io.read io.select string.encode! encoding.default_internal io.open io.read_nonblock file.open kernel.open file.new kernel.select encoding.default_external
rdoc :store.remove_nodoc rdoc :store.complete rdoc :classmodule.complete
gem :unsatisfiabledependencyerror.new gem :testcase.dependency_request
json :ext :generator :generatormethods :string.to_json_raw_object json :ext :generator :generatormethods :string.to_json_raw
csv :row.delete csv :row.[]= csv :row.index csv :row.fields
stringscanner.getbyte stringscanner.get_byte
uri :ftp.new net :ftp.login
thread.add_trace_func kernel.set_trace_func tracepoint.defined_class thread.set_trace_func
uri :generic.normalize uri :generic.normalize!
openssl :buffering.read_nonblock openssl :buffering.write_nonblock
hash.include? hash.has_key? hash.key? hash.member?
process.waitpid2 process.wait2
openssl :cipher.new openssl :ssl :sslcontext.ciphers
uri :generic.set_host uri :generic.hostname=
benchmark.benchmark benchmark.measure benchmark.bmbm
getoptlong.each getoptlong.get getoptlong.set_options
openssl :pkey :ec.private_key? openssl :pkey :ec.public_key?
gem.ruby_version gem.rubygems_version
bigdecimal.power bigdecimal.**
json :ext :generator :state.depth= json :ext :generator :state.max_nesting json :ext :generator :state.max_nesting=
io.readpartial io.sysread
time.nsec time.subsec time.tv_nsec
fiddle :compositehandler.sym fiddle :importer.import_function fiddle :importer.import_symbol
rss :baselistener.class_name rss :baselistener.install_class_name
thread.thread_variable_set thread.thread_variables thread.thread_variable? thread.thr sym thread.thread_variable_get
webrick :httpauth :userdb.get_passwd webrick :httpauth :userdb.make_passwd webrick :httpauth :userdb.set_passwd
basicsocket.recvmsg ipsocket.peeraddr socket.getnameinfo basicsocket.recvmsg_nonblock basicsocket.sendmsg socket :ancillarydata.unix_rights addrinfo.new
array.shelljoin shellwords.shelljoin
kernel.scanf io.scanf
gem.pre_uninstall gem.post_uninstall
string.try_convert string.to_str
psych :nodes :stream.new psych :handler.start_stream psych :emitter.start_stream
optionparser :arguable.options= optionparser :arguable.options
array.drop_while array.take_while
yaml :dbm.store yaml :dbm.ydbm key
numeric.magnitude numeric.abs
gem :packagetask.define gem :packagetask.new
etc.endpwent etc.setpwent etc.getpwnam etc.getpwuid etc.passwd etc.getpwent
time.to_s time.inspect
xmlrpc :client.proxy xmlrpc :client.call2 xmlrpc :client.call xmlrpc :client.proxy_async xmlrpc :client.proxy2 xmlrpc :client.call_async
gem :specification.file_name gem :specification.spec_name
rdoc :context.add_section rdoc :context.methods_by_type rdoc :context.set_current_section rdoc :context.each_section rdoc :context.section_contents
random.rand kernel.rand
openssl :hmac.hexdigest openssl :hmac.new
open3.popen3 open3.popen2 open3.capture3 open3.capture2e open3.popen2e open3.capture2
gem :commands :unpackcommand.find_in_cache gem.use_paths gem :requestset.resolve_current
basicsocket.remote_address basicsocket.getpeername
gem :nametuple.full_name gem :resolver :specification.full_name gem :resolver :specspecification.full_name
float.to_int float.to_i float.truncate numeric.truncate
net :imap.examine net :imap.lsub net :imap.delete net :imap.capability net :imap.create net :imap.subscribe net :imap.unsubscribe net :imap.rename net :imap.authenticate net :imap.select net :imap.status net :imap.append net :imap.login net :imap.add_authenticator
addrinfo.getaddrinfo socket.tcp_server_loop socket.getaddrinfo addrinfo.foreach addrinfo.canonname
object.hash range.cover? array.uniq! range.hash array.eql? bignum.hash range.last struct.hash float.hash unboundmethod.hash string.hash proc.hash matchdata.hash struct.eql? regexp.hash array.uniq array.ary method.hash range.rng range.eql? time.hash hash.hash array.hash
rdoc :parser :c.do_struct_define_without_accessor rdoc :parser :c.do_aliases rdoc :parser :c.do_boot_defclass rdoc :parser :c.do_attrs rdoc :parser :c.do_classes rdoc :parser :c.do_singleton_class rdoc :parser :c.do_constants rdoc :parser :c.do_define_class rdoc :parser :c.do_define_module rdoc :parser :c.do_includes rdoc :parser :c.do_define_class_under rdoc :parser :c.do_define_module_under rdoc :parser :c.do_modules rdoc :parser :c.do_methods
etc.group etc.getgrgid etc.endgrent etc.getgrent etc.getgrnam etc.setgrent
rubyvm :instructionsequence.load_from_binary rubyvm :instructionsequence.to_binary
rinda :templateentry.match rinda :template.match
irb :irbloader.source_file irb :context.pop_workspace irb :context.change_workspace irb :extendcommandbundle.irb_load irb :context.main irb :context.prompting? irb :context.file_input? irb :irbloader.load_file irb :extendcommandbundle.install_extend_commands irb :context.push_workspace irb :irb.suspend_input_method
gem.post_build gem.post_install gem.pre_install
gem.pre_reset gem.post_reset
encoding :converter.putback encoding :invalidbytesequenceerror.readagain_bytes
string.sub string.sub!
fiddle :pointer.to_str fiddle :pointer.to_s
readline.completer_quote_characters readline.completion_proc
rdoc :markup :tobs.convert_special rdoc :markup :tobs.init_tags
rubyvm :instructionsequence.label rubyvm :instructionsequence.new rubyvm :instructionsequence.compile rubyvm :instructionsequence.compile_file rubyvm :instructionsequence.compile_option rubyvm :instructionsequence.path rubyvm :instructionsequence.of rubyvm :instructionsequence.inspect rubyvm :instructionsequence.base_label rubyvm :instructionsequence.absolute_path
json.fast_generate json.parse json.parse! json.pretty_generate json.generate
object.to_enum enumerator.new object.enum_for
addrinfo.connect_from addrinfo.family_addrinfo
tcltk.deletecallbackkey tcltk.dcb
zlib :gzipfile.finish zlib :gzipfile.wrap zlib :gzipfile.close
uri.decode_www_form uri.encode_www_form_component
racc :parser.on_error racc :parser.yyerror
pp.singleline_pp pp.pp
objectspace.allocation_generation objectspace.allocation_sourcefile objectspace.allocation_method_id objectspace.allocation_sourceline objectspace.trace_object_allocations objectspace.allocation_class_path
thread.value thread.join thread.abort_on_exception= thread.abort_on_exception
bigdecimal.to_f bigdecimal.split
rdoc :store.add_c_variables rdoc :parser.process_directive
bigdecimal.limit bigdecimal.div rational.to_d bigdecimal.quo bigdecimal.add bigdecimal.mult bigdecimal.sub bigdecimal.save_exception_mode bigdecimal.divmod bigdecimal.save_rounding_mode bigdecimal.modulo bigdecimal.round
csv :table.[] csv :table.[]= csv :table.values_at csv :table.delete
openssl :digest.update openssl :digest.reset
enumerable.grep_v enumerable.lazy
racc :parser.do_parse racc :parser.next_token
xmlrpc :basicserver.add_handler xmlrpc :cgiserver.new xmlrpc :modrubyserver.new xmlrpc :basicserver.new xmlrpc :server.new
string.to_r kernel.rational
rdoc :methodattr.search_record rdoc :toplevel.search_record rdoc :classmodule.search_record
gem :specification.executable gem :specification.executable=
rdoc :comment.parse rdoc :tomdoc.parse rdoc :ri :driver.method_document rdoc :rd :blockparser.parse rdoc :text.parse rdoc :ri :driver.class_document rdoc :classmodule.parse rdoc :rd :blockparser.new rdoc :parser :changelog.create_document rdoc :context :section.parse rdoc :parser :changelog.scan rdoc :markup.parse rdoc :rd.parse
file.fnmatch? file.fnmatch
drb :drbsslsocket.open_server drb :drbsslsocket.open drb :drbsslsocket.new
csv.instance csv.new csv.read csv.write_headers? csv.converters csv.return_headers? csv.open csv.generate csv.force_quotes? csv.filter csv.unconverted_fields? csv.foreach csv.skip_blanks? csv.headers csv.<< csv.header_converters csv.parse csv.shift
array.rassoc array.assoc
fiddle :cparser.parse_ctype fiddle :importer.sizeof
rdoc :markdown.paragraph rdoc :tomdoc.build_paragraph
kernel.complex string.to_c
resolv :ipv6.to_name resolv :dns.each_resource resolv :dns.each_name resolv :dns.each_address resolv :dns.getaddresses resolv :dns.getaddress resolv :dns.getnames resolv :dns.getresources resolv :dns.getname resolv :dns.getresource resolv :ipv4.to_name resolv :mdns.each_address
io.popen kernel.spawn kernel.exec process.getrlimit process.spawn process.exec
openssl :cipher.final openssl :cipher.key openssl :cipher.decrypt openssl :pkey :rsa.export openssl :cipher.authenticated? openssl :cipher.iv openssl :cipher.reset openssl :cipher.auth_data openssl :cipher.auth_tag([ openssl :cipher.encrypt openssl :pkey :ec.export openssl :cipher.auth_tag openssl :cipher.update
array.delete_if struct.values_at enumerable.reject struct.select array.reject array.reject! array.values_at enumerable.select array.select enumerable.find_all
rdoc :toplevel.== rdoc :toplevel.hash
zlib :deflate.flush zlib.inflate zlib.deflate zlib :deflate.deflate zlib :inflate.inflate zlib :inflate.set_dictionary zlib :gzipwriter.flush
optionparser.top optionparser.on optparse.base optparse.top optparse.make_switch optionparser.make_switch optionparser.on_head optparse.on_head optparse.on optionparser.base optparse.on_tail optionparser.on_tail
thread :backtrace :location.absolute_path thread :backtrace :location.path
udpsocket.recvfrom_nonblock socket.accept_nonblock socket.recvfrom_nonblock socket.sysaccept unixserver.accept_nonblock basicsocket.recv_nonblock tcpserver.accept tcpserver.accept_nonblock unixserver.accept socket.accept
irb :context.use_loader= irb.currentcontext irb :extendcommandbundle.irb_context irb.irb_at_exit irb.conf
gem :specification.licenses gem :specification.licenses= gem :specification.license
array.map! array.collect array.map array.collect!
the number of significant digits as a fixnum
none
...
user of proxy p_pass pass of proxy opt optional hash
pass of proxy opt optional hash
...
summary width
summary indent
output destination which must have method <<
...
width of left side option part
...
optional if supplied the value of the attribute
an attribute
the name by which to search for the attribute
must be a parent object
where to write the string indent an integer
an integer
ignored ie_hack ignored
ignored
an integer
encoding name as string
if not supplied will be set to the default value
if supplied must be a hash containing context items
can be either an attribute or a string
is either a hash or array of arrays
optional
the namespace uri
either an attribute or a string
must be an element string or integer
the text to search for
the prefix to search for
the parent element
the search parameter
used to find a matching element to replace
all elements matching this string path are removed
optional
an element child
deprecated and ignored
the context element path the xpath to search for
the xpath to search for
the context element path the xpath to search for
the xpath to search for
if supplied a hash which defines a namespace mapping
the node to write output a class implementing < <
a class implementing < <
an integer greater than 0
string contents of the gem file
string path to gem file
number of seconds to wait for the block to terminate
string to extract uris from
limit uri matching to a specific schemes
string with uri
protocol scheme i
user name and password i
server port registry registry of naming authorities
...
string to make safe unsafe regexp to apply
string to search schemes patterns to apply to str
an array of strings
string
string to remove escapes from unsafe regexp to apply
