...
if true, display the model that has been found
name of the corpus to take as input possible values are
a list of values to compare against the corresponding
the position of the new edge's dot. this position
the token that should be stemmed
a probability distribution for the outcomes of an experiment a
a dictionary from reentrance ids to values
a sequential tagger that uses a classifier to choose the tag for each token in a sentence
the number of total sentences that have to be used for
canvas the canvas on which to draw the drs. if none is given create a new canvas
extra keyword arguments for probdist_factory
a list of indices i1 i2 ..., in where
a list of bigrams whose presence/absence has to be
the grammar used to parse texts
given n featurelists if combinations=k all generated templates will have
this class implements - __init__, which specifies the location of the corpus
a list or iterable of tokens
the title for the graph
an abstract base class for read-only sequences whose values are computed as needed
closure properties for the extension of the concept
undirected combinator for type raising
a mod word to test as a modifier of 'head'
the fraction of the the corpus to be used for training (1=all)
the chunk types to be converted
if true then rename any variables in
the list of words to be put into the grid
a dialog box for entering
name of key marker at the start of each record. if set to
which button the user should use to click on
the arity of the concept
this is a python port of the moses tokenizer from https //github
path to parser model directory
the corner or side of the canvas widget that
the utterance id of the sample to play
the sequence of symbols which must contain the text
the frame relation to be printed
a regular expression based chunk parser regexpchunkparser uses a
the output of model_found()
...
capitalization flag
the list of fileids that make up this corpus
the context target display tuples for the item
the name abbreviation or id number of the semantic type
if true, ouput files are compressed with gzip
class for reading and processing standard format marker files and strings
the spanish snowball stemmer
lexical translation model that considers word order >>> bitext = []
a spanish word with or without accents
adjusts threshold similarity score for near-optimal alignments
only return productions with the given left-hand side
a canvas widget that displays a single tree
tokenize a string on whitespace space tab newline
a list or regexp specifying the ids of the files whose
a bottom-up parser for pcfg grammars that tries edges in whatever order
if 0 then the offset is from the start of the file
translation model that considers how a word can be aligned to multiple words in another language
the score to give to words which do not appear in each other's sets of synonyms
lexical translation model that ignores word order >>> bitext = []
the function that will be applied to each
the relative weighting of precision and recall
one of preorder, postorder, bothorder,
the correct tag for the *i*th token
the file identifier of the file to read
if true adjudicate ties deterministically
the skip distance
the separator string used to separate word strings
...
if true add this resource to a cache. if load()
instance of a pos tagger conforms to taggeri
list of strings to insert into the trie
feature with which to compare
the german snowball stemmer
give a sentiment intensity score to sentences
a common class extended by both prover9 and mace <mace mace>
the various verb taxonomies do not
the heldout frequency distribution
information about which synset relations
the name of the file containing the predicate-
the child widget to remove. child must be a
...
the parent of the leftcorners
corpus reader for the xml version of the british national corpus
nodes in the input file are numbered starting from 0
a string that is used to separate the node
...
the gold standard
the correct tagging of the corpus
an abstract base class for base values that define a custom unification method
function<list<t>,r> to combine the results of the
the english snowball stemmer
expression
list of str input sentences to parse as a single discourse
additional features for the new feature
the left-hand side of the new probabilisticproduction
a list of values to compare against the
if specified then filetype indicates that
greedy averaged perceptron tagger as implemented by matthew honnibal
the russian snowball stemmer
a list or regexp specifying the fileids in this corpus. default is oana-en.xml
...
the semantic type to be printed
the sentence to be parsed
if true then raise a value error if
the underlying lists
a list of reference sentences w.r.t. hypotheses
field delimiter
the maximum number of bytes to read. if no
...
buffer at least size bytes before writing to file
a list of the "known labels" for this
a corpus reader for the wordnet information content corpus
the encoding of the grammar if it is a binary string
data server has started unzipping a package
assumptions to be retracted
the probability associated with the object
the compresslevel argument is an integer from 1
a binary feature encoding which adds one new joint-feature to the joint-features defined by binarymaxentfeatureencoding: a
function with the same signature as the
the sequence of tokens
when set to true boxer removes all equalities from the
...
maximal phrase length if 0 or not specified
expression
an array that can be used to uncompress nf
a regular expression for filtering the fillers of
regular expression tagger the regexptagger assigns tags to tokens by comparing their
this class is the base class for settings files
number of bytes to buffer during calls to read() and write()
find contexts where more than one possible target value can appear
the list of classified tokens from which to extract
only generate edges e where e.lhs()==lhs
a character that is printed to the left of the
the words used to seed the similarity search
keyword arguments passed to standardformat.fields()
a dictionary which represents an assignment of values to variables
a sequence of labeled test instances
the hungarian snowball stemmer
option to trigger dash split rules
only generate edges e where e.rhs()==rhs
a mapping between first and second person expressions
the string to read
the item of interest
one of 'token', 'word', or 'chunk'
a first order model is a domain *d* of discourse and a valuation *v*
the new value to discount counts by
a list of words label tuples
the token separator
list of int
the tree to be converted
a list of fields that will be extracted from the json file and
a corpus reader used to access wordnet or its variants
a feature structure parser
first input synset
a frequency distribution for the outcomes of an experiment a
other input synset
first ngram to compare
a set containing the ids of all feature
this cell's y coordinate
arguments specifying various conditions under
a list of alternative file names to check
...
the training data a set of
a regular expression pattern used to search the
a dictionary where keys are lemmas and values are sets
list of words
...
a dictionary mapping from variables to values
the initial grammar to display
kneser-ney estimate of a probability distribution this is a version of
the iob tagger used by the chunk parser
whether the ngrams should be right-padded
interface to the stanford tokenizer >>> from nltk
tweet object returned by twitter api
a hypothesis sentence
a processing interface for tokenizing a string
an ordered list of transformation rules that
do not output any feature with 0 in any of its positions
starting index of the possible foreign language phrases
a collection of frequency distributions for a single experiment run under different conditions
bindingdict a starting set of bindings with which
a prover. if not set one will be created
...
bundle of metadata needed for constructing a concept
the dependency parser
format specification
only generate edges e where e.end()==end
reader for corpora of word-aligned sentences tokens are assumed
reference sentences
an iterable over featuresets each a dict mapping
a tokenizer that splits a string using a regular expression which matches either the tokens or the separators between tokens
str input sentence to parse
if true print a list of all unused templates
a sequence of nltk trees usually parentedtrees
a function used to transform each element
a function taking
the frequency distribution that the
...
the suggested leftcorner
a comparison represents a comparative sentence and its constituents
thesaurus fileid to search in. if none search all fileids
the model file
if specified neighbors that have a different
...
a list specifying the categories whose files have to
the index where the child widget should be
the initial x coordinate for the upper left hand
a string-based encoding of a particular chunking of a text
str of possible comment characters
a stream backed corpus view for corpus files that consist of sequences of serialized python objects (serialized using
tuple of two lists of atoms. the first lists the
a list of the files that make up this corpus
the frame to be printed
the parent of the leftcorner
extra parameters for the constructor such as
brill's transformational rule-based tagger brill taggers use an
a list of row values used to initialze the
a tagged corpus. each item should be
a regular expression pattern used to search the lu
the chunk structures to score the chunker on
the symbol to use for right padding default is none
this is an implementation of the smoothing techniques for segment-level bleu scores that was presented in
a string whose contents are used as stdin
a list of dependency graphs to train the scorer
a set containing the ids of all other
...
this is not intended to be a general purpose class for dealing with the local timezone
variableexpression the variable bind
str for the input to be parsed
this rule's tag pattern. when
regular expression that matches token separators must not be empty
the backward word alignment outputs from target-to-source
a dictionary used to record the trees that we've
path to the output file
a processing interface for labeling tokens with a single category label (or "class")
the sigma value for a gaussian
the hungarian word whose region r1 is determined
reparse the str form of subclasses of abstractboxerdrs
names of the chat-80 relations to extract
the backward equivalent of the forwardcombinator class
stores a token of text with annotations produced during sentence boundary detection
the address of the 'destination' node
a string used to connect collapsed node values (default = "+")
ngram to lookup
specifies the set of fileids for which paths should
a sentence whose subjectivity has to be classified
...
expression
if true then use the replaced intended word instead
a sequence of lists of the edges that
tokenizer for breaking sentences or
the actual word to be replace "<word>"
the name of the file containing the predicate-
...
list of tokens (words or word tag tuples)
the supervised training data a set of
number of dimensions to use in reducing vector
the iso 639 code of the language e.g. 'eng' for english 'rus' for russian
this command stores both a prover and a model builder and when either prove() or build_model() is called then both theorem tools are run in
if true, check that the result of adding the sentence is thread-consistent. updates self._readings
only generate edges e where
the multi-word expression we're adding into the word trie
words are stemmed by cutting them at this length
a discourse representation structure
class for storing lemmas stems and evaluation metrics
output the debugging messages during training
bigram association measure to use as score function
a dependency grammar a dependencygrammar consists of a set of
the position in the string to start parsing
a bidirectional index between words and their 'contexts' in a text
str output from c&c parser
optional parameter to start counting from ui oi
an input stream
a list of tuples of word and bio chunk tag
the new canvas widget's attributes
the string to be converted
a dictionary mapping from feature values
the index whose neighborhood should be returned
the positional alignments in giza format
the set of possible labels. if none is given then
...
if true remove tweets containing both happy
...
a list of the locations within tree of
the list specifying what kinds of children need to
an averaged perceptron as implemented by matthew honnibal
the new grammar
a base feature value that is a tuple of other base feature values
unit tests for cfg
the corpus containing the parse trees
the frame relation type to be printed
the maximum length for the parser's edge queue
an abstract bottom-up parser for pcfg grammars that uses a chart to record partial results
a concept class loosely based on skos (http //www
the feature path that led us to this unification
the sentence string from the source language
the username to authenticate with. use none to disable
bool alpha convert automatically to avoid name clashes?
...
the number of pixels to move this canvas widget
a specialized corpus view for chasenreader similar to taggedcorpusview,
the pos annotation set of the sentence to be printed
wrapper for scikit-learn classifiers
a list of target sentences' lengths
the new feature weight vector
input document
an msd tag that is used to filter all parts of the used corpus
a set of variable objects that should not be returned from
the container's child widget. child must not
the document that will be passed as argument to the
a generic chart parser a "strategy", or list of
an additional list of readings
information about the proposed expansion
the height of the new space widget
...
tokenize a string using the space character as a delimiter which is the same as s
a stream backed corpus view specialized for use with the bnc corpus
a bottom-up parser for pcfg grammars that tries edges in random order
the word whose regions r1 and r2 are determined
constant used to compute the cost of a shift
...
a byte count offset
only generate edges e where e.span()==span
input expressions to use as assumptions in the proof
...
the number of instances to consider (in case we want to use only a
a bottom-up parser for pcfg grammars that tries edges in descending order of the inside probabilities of their trees
input sentence to parse as word tag pairs
a string used to separate the node representation from its vertical annotation
the sentence that this chart will be used to parse
a tree that automatically maintains parent pointers for multi-parented trees
an annotation set or exemplar sentence to be printed
the epsilon value use in method 1
...
:ivar _root the root window :ivar _charts a dictionary mapping names to charts
use an optimized agenda-based algorithm
...
a list of strings and trees encoding
the encoding of the string if it is binary
a case-insensitive string specifying which
the feature label to use for the root semantics in the tree
additional arguments to pass to the training methods
a tuple of features default slash type
if true then return tuples of (str pos relation_list)
the symbol name string. this string can be
number of best words/tokens to use sorted by association
reader for the pros and cons sentence dataset
the graph represented as a dictionary of sets
a tuple of cell values one for each column
...
a list of documents. if labeled=true, the method expects
the list *nr*, where *nr[r]* is the number of
...
the word that is stemmed
a list of words/tokens whose presence/absence has to be
an *incremental* chart parser implementing jay earley's parsing algorithm
the file to be searched through
the initial feature values for this feature
new assumptions
approximate score for translating the
the patterns and responses
the child widget that changed
a list of rules that should be used to decide
keyword arguments passed to toolbox.standardformat.fields()
a chartparser using a top-down parsing strategy
the samples to plot default is all samples
this is the base abstract object for all logical expressions
the suggested leftcorner
are the grammar rules probabilistic?
...
the number of sample values that can be generated
if true, stopwords are thrown away
lancaster stemmer >>> from nltk
a text whose polarity has to be evaluated
displays an abbreviated repr of values where possible
contains the chunking rules used to parse the
maximum number of rule instances to create
flag to expand unary or not (default = true)
a context-free grammar a grammar consists of a start state and
an identifier specifying which class
expression, for the argument
error handling scheme for codec. same as decode() builtin method
the regular expression
the source data to be padded
an input sentence
a list of node addresses to check for
an optional function or class that maps a
...
this is the additional java arguments that
a sentence in the source language and its
the user-supplied binary location deprecated
...
the rightmost elements of the parser's
the regular expression for this regexpchunkrule
variable the variable to bind
a flag to specify whether the freqs are cumulative (default = false)
a variable acting as a key for the assignment
a specialized version of the fundamental rule that operates on nonterminals whose symbols are featstructnonterminals
the stream to which the megam input file should be
the name of the file to print the tree to
a verbnet class identifier or an elementtree
how much do we smooth synset counts default is 1.0
str indicating the format for displaying
represents an annotation task i e people assign labels to items
lists of features whose cartesian product will return a set of templates
...
the item to draw
this rule's right tag
words are stemmed by cutting at this length
a cached version of topdownpredictrule after the first time
a chunk tree
'pos' part of speech 'sem' semantic or 'both'
the chunked tagged text that should be used for
if true then create bindings that will
the list of input tokens
an interface for tag transformations on a tagged corpus as performed by tbl taggers
the canvas widget to remove
if true then include always-on
the new height
a tokenizer for breaking paragraphs into sentences
reference sentence
flag to set if case should be ignored when searching text
...
if true then the tags used will be the more detailed
a list of tags corresponding by index to the words in the tokens list
the portion of the text that is not yet
the location of the binary to call
the source language tokens a list of string
if true give warning when retracted is not present on
a gold standard alignment sure alignments
the trace level. a trace level of 0 will
the new edge's left-hand side specifying the
a reviewline instance that belongs to the review
bool should bound variables be replaced?
a parsed xml document
if true generate trace output
the symbol to use for left padding default is none
class representing combinators where the primary functor is on the left
alignment to be checked
a index representing the column of incoming arcs
...
'false' default will not collapse the parent of leaf nodes (ie
markov order for sibling smoothing in artificial nodes (none default = include all siblings)
this an abstract class for any expression that binds a variable in an expression
the tree that should be converted
the string being matched
a prior estimate of the probability of the label
if true then use word stems instead of word strings
elements and subelements to add blank lines before
this canvas widget's canvas
class for holding configuration which is the partial analysis of the input sentence
the width of each line in characters (default=80)
end of range (note inclusive!) where this feature should apply
...
a dictionary mapping from each id
reader for chunked and optionally tagged corpora paragraphs
if true strip spaces from word tokens
represents a single panlex meaning a meaning is a translation set derived
if true, assume that each document is represented by a
interface for trying to build a model of set of formulas
a tuple s e , where tokens[s e] is the
expected input list of pos-tagged words
determines the size of the cache used
the node label to use for the root
a tkinter frame containing a canvas and scrollbars
train method of a classifier
this class represents conjunctions
a string that will be checked to see if within which a
a rule specifying how to expand chunks in a chunkstring to the right using two matching tag patterns a left pattern and a
...
right or left factoring method (default = "right")
the node value corresponding to this
a coordinate pair
bool simplify the proof?
the maximum order of n-gram this function should extract
the list of chunked sentences to score the chunker on
a single sentence string
the encoding of the input string if it is binary
the class that should be used to create
corpus reader for the nombank corpus which augments the penn treebank with information about the predicate argument structure
the tk widget that contains the colorized list
data object to store counts of various parameters during training
a processing interface for removing morphological affixes from words
the source data to be converted into bigrams
the right-hand side of the new production
if true plot a visual representation of the sentence polarity
an assignment to individual variables
if true 'sent' is actually an annotation set within a sentence
the context in which the item of interest appears
the number of rows in the grid
the dimension of the smoothing window should be an odd integer
the root directory for this corpus. default points to location in multext config file
a set of values to compare against the reference set
the degree of the ngrams
the class of the subject named entity
the encoding of the files
flag to indicate normalization
a bytesio stream to read from instead of a file
if false then strip newlines
output format for displaying models
whether ot return matching leaf nodes
if true, only print out consistent thread ids and threads
...
the word that is transliterated
if specified then use this function to
false if we don't want to calculate the
bindings for the new edge
a hypothesis translation
the contents of the file
the most likely constituents table. this
the hypothesis component of the pair
the string representation of a tagged token
a stream backed corpus view specialized for use with text
bindingdict a dictionary of bindings used to simplify
bindingdict a starting set of bindings with which the
if true then return a list of
the list *tr*, where *tr[r]* is the total count in
the schema used in a set of relational tuples
the identifier we are searching for
class representing primitive categories
a lazy sequence formed by concatenating a list of lists this
a rule specifying how to add chunks to a chunkstring, using three matching tag patterns one for the left context one for the
the maximum number of items to print
list of list of str input discourses to parse
the initial graph represented as a dictionary of sets
a tagger that requires tokens to be featuresets a featureset
an expression of logic
the new child widget. child must not have a
the sentence to be printed
displays an abbreviated repr of only the first several elements not the whole list
this setting mimics older v2 behavior of nltk
tokenize a string into its lines optionally discarding blank lines
the number of tokens in the target language
the ngram that needs to be searched
hidden markov model class a generative model for labelling sequence data
rule output format one of "str", "repr", "verbose"
input sentence to parse
if true abbreviate labels longer than 5 characters
the alphabet to be used for filling blank cells
...
number of seconds before timeout set to 0 for
if true then substitute stems for words
feature which examines the text word of nearby tokens
the original json file containing tweets
the right hand side of a cfg production
if true skip the first line of the csv file (which usually
this is a python port of the tok-tok pl from
should duplicates be removed?
a synset
optional dictionary of parser options see
the child widget. child must not have a
the encoding of the given files default is utf8
a cutoff value that is used to discard
the tree to be collapsed
a sequence of labeled training instances
this is a prover decorator that adds domain closure assumptions before proving
the nonterminal used to form the right hand side
encoding used by settings file
the distribution from which to garner the
check properties of an ongoing discourse
specialized version of the standard bracket parse corpus reader that strips out (code
a base decorator for the provercommand class from which other prover command decorators can extend
a set of existing edges. the number of edges
the target words
reader for corpora that consist of plaintext documents paragraphs
a status message object used by incr_download to communicate its progress
the synset that this synset is being compared to
format of reported rules
the file where the learned tbl tagger will be saved
the node label typically a string
if specified select specific speaker s defined
...
a function that takes two arguments a featureset
a dictionary where the key is the alignment points and value is the cost referencing the languageindependent.priors
the label to use for the root of the tree
alignment under consideration
the stream to which the tadm input file should be
only generate edges e where e.dot()==dot
a dictionary that is used to hold the mapping
...
the sentence string from the target language
a list of feature specifications where
a list of node addresses each of which is in the cycle
if true, check that the result of adding the sentence is thread-informative. updates self._readings
a reference sentence
the length of the hypothesis
maximum number of rule instances to create
a list of strings i.e. tokenized text
a list specifying the categories whose words have to
if true, evaluate classifier f_measure
this class is used to read the list of word pairs from the subset of lexical pairs of the paraphrase database ppdb xxxl used in the monolingual word
if true double negation is considered affirmation
a tagger that chooses a token's tag based on a leading or trailing substring of its word string
bool should predicates be occurrence indexed?
a function that maps each token to a normalized
expression
...
the list of word tag tokens to be chunked
...
the filename of the file where the classifier
the samples whose frequencies should be returned
a set containing the ids of all self
the index of the word whose tag should be
an abstract base class for displaying a colorized list of items
the style to use for the format 3 4 10 columns
whether the individual tags should be
...
if true, omit the tag from tagged input strings
optional dictionary of reranker options see
the maximum number of models that mace will try before
the widget that should contain the new
the hunpos-tag binary
the string to be tokenized
additional parameters passed to categorizedcorpusreader
expression the atomic to which 'variable' should be bound
str for the filename of the training input data
a canvas widget that places a oval around a child widget
the number of times a thing is taken
abstract clusterer which takes tokens and maps them into a vector space
the discourse sentences
a list of names for the columns these
the function or class that maps
this class holds a prover, a goal and a list of assumptions when
name of file to which data is written
target word classes
a rule licensing edges corresponding to the grammar productions for the grammar's start symbol
a class for pos tagging with stanford tagger the input is the paths to
if specified then only show the specified
a list of tokens label tuples to use as gold set
if true then fileid will be deleted
...
(optional 'frame' must be a different frame) only show relations
a stream backed corpus view specialized for use with the bnc corpus
a list of featuresets whose label is unknown
a set containing tuples of str elements
the encoding of the input only used for text formats
if true then include unseen value
the path to the maltparser directory that
a scorer which implements the
filters out fillers which exceed this threshold
a base class used to mark deprecated classes a typical usage is to
the nonterminal from which to start generate sentences
the tokenizer instance that will be used to tokenize
data server has started downloading a package
input expressions to use as assumptions in
function to call on each subexpression
boundary value
the indentation level at which printing
templates to be used in training
the node value that should be used for
a list or regexp specifying the fileids in the corpus
...
the chunked sentence to be scored
the number of total tweets that have to be used for
collection of _hypothesis objects
list of lists of word tag tuples
variable that is being replaced. the new variable must
the frame element to be printed
an srl instance from a conll corpus which identifies and providing labels for the arguments of a single verb
a list of sentence strings
data object to store counts of various parameters during training
a dictionary mapping feature structures ids
the list of tokens to search from
prepended string that signals lines to be ignored
the number of target language words
if true, report on the updated list of sentences
a corpus view that selects out specified elements from an xml file and provides a flat list-like interface for accessing them
words used for the analysis
a rule that joins a given edge with adjacent edges in the chart to form combined edges
whether to produce colors with ansi escape sequences
attribute for the particular nlp task that the data was drawn from
variable
...
the alpha value use in method 6
...
clause
the samples to plot
function<expression t> to call on each subexpression
only arcs from cycle nodes are considered. this
a wrapper around elementtree element objects whose main purpose is to provide nicer __repr__ and __str__ methods
cache baseline tagger to this file (only interesting as a temporary workaround to get
if handle_negation == true apply mark_negation
the backoff tagger that should be used for this tagger
clause with which to unify
the width of the new space widget
probability parameters of the new probabilisticproduction
the name of the frame
the file-like object containing full tweets
the chunk tag that users want to extract e.g. 'np' or 'vp'
substitution permutation combinator
the left side of the current drawing area
source and target language sentence pair
tree node of a dendrogram
class for measuring labelled and unlabelled attachment score for dependency parsing
the file where errors will be saved
hypothesis being expanded
the current index
the amount of time that the eval demon took
the character that needs to be checked
string to parse as a standard format marker input file
a provercommand specific to the prover9 prover it contains
the number of possible sample outcomes. bins
a tagger that chooses a token's tag based its word string and on the preceding words' tag
a list of words augmented by an attribute num used to record the sentence identifier (the n attribute from the xml)
a dictionary where keys are stems and values are sets
if true remove retweets
string to insert into the trie
the index of child in self
the vowels of the respective language that are
a map that can be used to compress nf to a dense
...
word position span of the source phrase
...
number of iterations to run training algorithm
a trie implementation for strings
the text we are parsing. this is only used for
the number of words to generate (default=20)
the bracket characters used to mark the
the framenet id number of the frame
formulas which contain background information
a node dictionary to collapse the cycle nodes into
a feature-based grammar this is equivalent to a
>>> from nltk parse stanford import stanfordneuraldependencyparser
how many rules plotted
the extractor function to add to the list of feature extractors
the word alignment outputs as list of tuples where
a list of words or punctuation to be parsed
tokenize a string use the tab character as a delimiter the same as s
...
whether the training data should be a random subset of the corpus
...
the list of exemplar sentences to be printed
the samples to initialize the conditional
a tree represents a hierarchical grouping of leaves and subtrees
the class of the object named entity
filename of persistent store
a sentence whose polarity has to be classified
another list of independently generated test values
the word or phrase which is used for that comparative relation
...
strip trailing whitespace from the last line of each field
phrases represented by their spans in
pretty-print a tree in text format either as ascii or unicode
the framenet name or id number of the frame
indicates what download directory the data server is using
wordnum height*wordnum height*
a dictionary in which to store the attributes
whether to use sparse matrices internally
only generate edges e where
the user-supplied jar location or none
wordnum height*wordnum height*
an drtexpression to check equality against
a stream reader that automatically encodes the source byte stream into unicode (like codecs
a string joining two non-terminals in a unary production (default = "+")
only return productions with an empty right-hand side
the number of lines to display (default=25)
the left-hand side of the new production
data server has finished unzipping a package
a class that wraps a dict and allows accessing the keys of the dict as if they were attributes
a tag pattern that must match
phrase-based stack decoder for machine translation >>> from nltk
the initial y coordinate for the upper left hand
the character which can appear at the start of
when set to true boxer will resolve all anaphoric drss and perform merge-reduction
a function that given a tuple of cells returns a
maximum number of characters before a label starts to
whether to wrap output in html code default plain text
the expression's language variety as a seven-character
an interface for generating lists of transformational rules that apply at given sentence positions
the parser to be used to parse the logical expression
a canvaswidget constructor or a function that
the id number of the desired lu
the number of entities in the model determines the row length in the table
a dialog window for creating and editing context free grammars
the name of the pre-trained model with .mco file
a tool for the finding and ranking of quadgram collocations or other association measures
the left delimiter printed before the matched substring
if true, symbol set pairs are read into a valuation
a gzipfile subclass that buffers calls to read() and write()
set of {variable}s
if specified the search will be constrained to
a processing interface for assigning a tag to each token in a list
corpus reader for the xml version of the childes corpus
optional
set of marker names whose values are utf-8 encoded
the maximum number of collocations to print
the target language tokens a list of string
class representing leaf edges in a ccg derivation
a container for the nodes and labelled edges of a dependency structure
the maximal depth of the generated tree
if true then strip trailing spaces from word
the frequency counts upon which to base the
the full path to the megam binary. if not specified
a string separating the head node from its children in an artificial node (default = "|")
if true then return tuples of (stem index
if true the method will modify the original document in place
this canvas widget's hierarchical parent
tokenizer for breaking sentences or paragraphs
the base part of the preferred label for the concept
the lu to be printed
list int
...
data type used when building feature array
...
tuple of two coordinate pairs defining the first line segment
this class is used to represent two related types of logical expressions
additional parameters that will be passed as arguments to
an array containing the empirical
a path pointer that identifies a file which can be accessed directly via a given absolute path
identify sentiment-relevant string-level properties of input text
the conll string to be converted
formulas which express background assumptions
fields to add to each type of element and subelement
reader for the sinica treebank
the list of productions that defines the grammar
the start symbol
a file mode which can be any of 'r', 'rb', 'a', 'ab',
a rule checks the current corpus position for a certain set of conditions if they are all fulfilled the rule is triggered meaning that it
the default block size for reading. if an
an expression
this class holds a prover, a goal and a list of assumptions when
reader for the comparative sentence dataset by jindal and liu 2006
previous hypothesis before expansion to this one
alignment point to j_pegged
a canvas widget that displays a single string of text
attributes for the canvas widgets that make up the
...
a dependency graph to score
the number of samples to use in each demo
the number of times to run through the patterns
...
a graphical tool for exploring the regular expression based chunk parser nltk
a list of node addresses that belong to the cycle
a threshold for the number of items to include in the left and right context
this class represents disjunctions
what we expected to see instead
the canvas widgets watched by the
the romanian snowball stemmer
the keys to ignore. these keys will not be
identifier for the pair
the file position at which the view will
whether to store the probabilities as logarithms
the hypothetical segmentation
represents positions in a target sentence used to keep track of
random number generator
a parse tree
specifies which column to sort using
the fe relation to be printed
if true then sort by the count of each
the tagged corpus
an information content object (as returned by
if features is a mapping or none
str
filename containing the relations
an expression to check equality against
abstract class for representing a binary combinator
the string of text to display
the worder list output from word_rank_alignment
level of indentation for this element
start of range where this feature should apply
a function for parsing nonterminals
the size of the fixed-length joint-feature
generates a random probability distribution whereby each sample will be between 0 and 1 with equal probability (uniform random distribution
if true remove tweets containing ":p" and ":-p"
the number of total reviews that have to be used for
a word-to-word relation dependencygrammar
a string composed of element tags separated by
name of an encoding to use
window width
a variable assignment
the witten-bell estimate of a probability distribution this distribution
indicates how much progress the data server has made
a list of list of word tag tuples
bindingdict containing bindings that should be used
the right margin at which to do line-wrapping
the baseline tagger
a tgrep search pattern
hyperparameter used as a prior for the brevity penalty
a display widget for a table of values based on a multilistbox widget
where to index into the list of characters
if true create a deep copy if false create
...
the feature considered in the comparison relation
a sequence of unlabeled training instances
sample of the event
the id number of the lexical unit
the sentence to be parsed
the number of tokens in the source language tokens
attribute for the length of the text of the pair
path the reranker model's features file
keyword parameters used when loading the grammar
path the reranker model's weights file
expression the expression to which 'variable' should be bound
...
the positions where the transformation is to
class representing a variable ccg category
if set also make the closure reflexive
the name or path of the file
set for the indices of the glue formula from which the argument came
the synset to which the shortest path will be found
a list of reference translations
the specified beginning position of the string s
the data stream to print
an expression of logic
a simple top-down cfg parser that parses texts by recursively expanding the fringe of a tree and matching it against a
the word whose region rv is determined
the known-correct ("gold standard") chunked
a relation dictionary
a class used to access the nltk data server which can be used to download corpora and other data packages
a tbl template that generates a list of l{rule}s that apply at a given sentence position
the text component of the pair
if true then strip trailing spaces from
a list of dependency graphs to train from
chart parser for ccgs
a sequence of multi-word expressions to be merged where
the name or path of the file
the chunk structure to be further chunked
the currently active word
the preferred label for the concept
class implementing application of a binary combinator to a chart
a url specifying where the grammar is located
...
a common base class used by both prover9command and macecommand, which is responsible for maintaining a goal and a set of assumptions
if true create a compressed gzip file
convert newlines in a field to spaces
a tag pattern that must match for this
the group average agglomerative starts with each of the n vectors as singleton clusters
string that should be inserted between words in a multi-word
a regular expression pattern used to match against
the portion of the text that is not yet
this class represents variables that do not take the form of a single character followed by zero or more digits
the sequence of unlabeled symbols
the sequence of rules that should be used to
text to split into sentences
in-memory store of translations for a given phrase and the log
second ngram to compare
a word stemmer based on the porter stemming algorithm
expression for the consequent
the complete set of samples
a set of variable bindings to be used and
...
training data
ignored
indicates whether the given tokens are labeled
data server has started working on a collection of packages
tuple of two coordinate pairs defining the second line segment
if true then use the 'bernoulli' format. i.e.,
the second entity considered in the comparison relation
an ordered list of reference values
tokenize a string into individual characters if this functionality
url presented to user for download help
a valuation string
name of an encoding to use. if it is specified then
a parser for lambda-expressions
a collection of expressions
the number of tokens in the source language
this class represents variables that take the form of a single uppercase character followed by zero or more digits
dictionary from tags to string descriptions used
the new log probability
if true, evaluate classifier recall
error handling scheme for codec. same as the encode()
true to start a web browser and point it at the web
a list specifying the categories whose sentences
if true include sentence bracketing
dictionary of statistics collected during testing
the text that needs to be escaped
the http proxy server to use. for example
...
set of words used for analysis
error handling scheme for codec. same as the decode()
tab file as a file or file-like object
a processing interface for identifying non-overlapping groups in unrestricted text
a canvaswidget constructor or a function that
only return productions with the given first item
a list of options that should be passed to the
a trainer for tbl taggers
a hypothesis alignment aka. candidate alignments
a utility class for scoring chunk parsers chunkscore can
root directory containing thesaurus lisp files
dependency graph exception
the start position
if false, then return immediately after
interface covering basic clustering functionality
a dictionary which represents a model-theoretic valuation of non-logical constants
str an identifier to be inserted to each occurrence-indexed predicate
the tagset that should be used in the returned object
...
a list or regexp specifying the frameset
...
position in the record of the object of the predicate
str with the model builder's output
a canvas widget that displays special symbols such as the negation sign and the exists operator
verbosity level
the list of tokens to which feature_func should be
the item to visit
a reference to a page that may be generated by page_word
the function to filter all local trees
a specialized chart that 'instantiates' variables whose names start with '@', by replacing them with unique new variables
the positions at which this features should apply
str a filename for the output file
a stemmer that uses regular expressions to identify morphological affixes
a list of feature values where each feature value is either a basic value such as a string or an integer or a nested feature
the initial contents of the colorized list
bool
vacancy states of the slots in the target sentence
the chunkstring to which this rule is applied
a head word
the new edge's leaf value specifying the word
the relevant free individual variable in parsed
if the resulting tree has
data object to store counts of various parameters during training
the index to check
a list of tuples mapping variable expressions to the
use the weighted variant of windowdiff
the number of pixels to move this canvas widget
a subsequence produced by slicing a lazy sequence this slice
a pair of list str and tree, as generated by
the section of the text for which we are
the label used to classify a text/hypothesis pair
...
data object to store counts of various parameters during training
the top node of the tree being created
...
a real number used to parameterize the
if true then also return false if
the end position
the right-hand side of the new probabilisticproduction
the encoding that should be used to read the corpus
the level of tracing that should be used when
the italian snowball stemmer
a list of labels that should be used by the
a set containing selfid otherid pairs
a classifier based part of speech tagger
set of instances for a single sentence
feature which examines the tags of nearby tokens
impexpression
if true strip off emoticons from all tweets
the canvas widget that should replace
lazy module class
extra arguments for probdist_factory
exception raised by read_* functions when they fail
markov order for parent smoothing (0 default = no vertical annotation)
the node which cycle nodes are collapsed into
the sentence to be parsed
if true return months instead of year-month-date
a class for dependency parsing with maltparser the input is the paths to
whether to include semantic tags namely wordnet lemma
a tree that automatically maintains parent pointers for single-parented trees
the discount applied when retrieving counts of
isri arabic stemmer based on algorithm arabic stemming without a root dictionary
the default unicode encoding for the files
a dependency graph to score
a grammar of word-to-word relations
wrapper for the undirected binary combinator
only generate edges e where e.start()==start
the hypertext reference to be solved
a list of words/tokens or a tuple words label
sql query
the widget that should contain the new table
input line
data server has started working on a package
a dictionary of statistics collected
an interface for classes that can perform substitutions for variables
the name of the symbol to display
an optional function for transforming training
the sample whose probability
the source expression's language variety as a
stores data used to perform sentence boundary detection with punkt
the source text
the number of columns in the grid
data server has finished downloading a package
a flag to specify whether the plot is cumulative (default = false)
an nltk interface to the verbnet verb lexicon
the cfg production that licenses the tree
this is a python port of the moses detokenizer from https //github
tokenize a text into a sequence of alphabetic and non-alphabetic characters using the regexp \w+|[^\w\s]+
data object to store counts of various parameters during training
only generate edges e where
the display width (default=70)
name of the standard format marker input file
...
dict that maps variable names to types (or string
an integer to set the number of tweets to convert. after the
if true, store the sentimentanalyzer in a pickle file
python-crfsuite training options
featuregrammar or name of feature-based grammar
the forward word alignment outputs from source-to-target
...
the tagset to be used e.g. universal wsj brown
the html body corresponding to the word
a chart for feature grammars
the default total width reserved for
...
the block reader used to divide the
corpus reader for the york-toronto-helsinki parsed corpus of old english prose ycoe a 1
starting index of the possible source language phrases
the level of debugging which should be
a list specifying the categories whose sentences have
...
span of word positions covered by the
bool indicating whether debug statements should print
a base feature value that is a set of other base feature values
a list of test values
dict<str str> that maps variable names to type
additional parameters required by the function function
synset keys and relation types for which to display relations
text to split into words
specifies whether to sort the values in
the hungarian vowels that are used to determine
list of words one per line blank lines are ignored
a short description of the purpose and/or effect
the new child widget. child must not already
the maximum number of items to display
the parser that will be used to parse logical
a dictionary that maps from the id of
...
a character identifying the operation that
target language model. must define a
tnt - statistical pos tagger important notes
the maximum number of bytes to read. if not
the tuple representation of a tagged token
if true then the slice object may have a
the output file where results have to be reported
the synset for which we're building the relations
a conllcorpusreader whose data file contains three columns words pos and chunk
reader for corpora that consist of tweets represented as a list of line-delimited json
sequence of tokens to be tagged
this class stores both a prover and a model builder and when either prove() or build_model() is called then both theorem tools are run in
...
specifies what columns should be included in
the span to add
...
thread id
beam search degree see above
to create an edge rule make an empty base class that uses edgerule as the first base class and the basic rule as the
a regular expression
a dependency graph to assign scores to
a projective rule-based dependency parser a projectivedependencyparser
source word position under consideration
an abstract base class for a tree that automatically maintains pointers to parent nodes
...
include current centroid in computation
indication that the pos tagger is trained or not
...
a dictionary mapping from fname fval label
reader for the timit corpus or any other corpus with the same file layout and use of file formats
class representing a lexicon for ccg grammars
hyperparameter used as a prior for the unigram precision
the package download file is out-of-date or corrupt
the ngram order
the input csv filename
weights for unigrams bigrams trigrams and so on
the file where rules will be saved
the list of words that are being tagged
if not none, saves a pickled model in this location
an information content object (as returned by load_ic())
a mix-in class to associate probabilities with other classes (trees rules etc
the features to build this template on
a list of environment variable names to check
an exception class for framenet-related errors
the parser that will be used to parse the
...
the chunker being evaluated
the child that will be used if the value of
...
the russian word whose regions rv and r2 are determined
this class represents implications
class for lazy viewing the mte corpus
a corpus of lists of reference sentences w.r.t. hypotheses
if true, print out each thread id and the corresponding thread
...
an object that enumerates the code points of the cjk characters as listed on http //en
if true, evaluate classifier precision
the parameter to assign more importance to recall over precision
interface to the stanford segmenter >>> from nltk
the obj to be printed
the conditionalfreqdist specifying the
...
a corpus of list of reference sentences w.r.t. hypotheses
a tool for the finding and ranking of bigram collocations or other association measures
>>> dep_parser=stanforddependencyparser(
filename of plot of learning curve s train and also test if available
the tagged sentence
if true then call self.see() with the newly
the file identifier for the file whose path
algorithms for learning hmm parameters from training data these include
the full path to the java binary. if not specified
the unsupervised training data a set of
...
a list of row values used to initialze the table
number of best words/tokens to use sorted by frequency
whether to include part-of-speech tags
element to be indented. will be modified
this subclass encapsulates a method for defining the string region r1
list of sentences to be tagged
a probabilistic projective dependency parser
the source text
test just this sentence
the type of window from 'flat', 'hanning', 'hamming', 'bartlett', 'blackman'
the queue of edge objects to sort. each edge in
sentence-aligned parallel corpus
a non-terminal symbol for a context free grammar nonterminal
path to the unified model directory
the new edge
the total number of outcomes recorded by the heldout
the new edge's right-hand side specifying the
the minimum score for a rule in order for it to be considered
distinguish named entities from ordinary words
an interface to stanford taggers subclasses must define
the transpose of nfarray
a classifier instance previously trained
the training data a set of
methods for authenticating with twitter
the name of the tagset used by this corpus to be used
a feature encoding that generates vectors containing a binary joint-features of the form
the length of the hypothesis for a single sentence or the
the child canvas widget to remove
a token list with its original length and its index
list of int for the indices on which this atom is dependent
the tag pattern to convert to a regular
the distance number of edges from this hypernym to
...
flat representation of toolbox data whole database or single record
the information to be reported for each item
...
an integer defining the type of comparison expressed
the document list of tokens that this
a lambda calculus expression parser
the top of the current drawing area
an array that is 1 for features that are
a filesystem path
stores variables mostly regular expressions which may be language-dependent for correct application of the algorithm
stop training when no rules better than min_score can be found
iso 639 alpha-3 code. if specified filters returned varieties
a url specifying where the resource should be
a tag pattern that must match
a canvas widget that displays a single segment of a hierarchical tree
a nltk.inference.api.prover
a chartparser using a bottom-up left-corner parsing strategy
log probability that given src_phrase,
a component for viewing charts this is used by chartparserapp to
a character that marks comments. any lines
the path to the file that is read by this
the word used to seed the similarity search
the minimum number of distinct target forms
wraps an iterator loading its elements on demand and making them subscriptable
if true will print per-template statistics collected in training and optionally testing
...
the tokenizer that will be used to split each tweet into
a single string i.e. sentence text
window lengths where to look for feature
behaviour for encoding errors see https //docs.python.org/3/library/codecs.html#codec-base-classes
the widgets that should be aligned
a stream backed corpus view specialized for use with ann_morphosyntax
displays an abbreviated repr of only the first several elements not the whole list
the target language variety as a seven-character
a canvas widget that keeps a list of canvas widgets in a vertical line
a tagged sentence
a probability distribution that assigns equal probability to each sample in a given set and a zero probability to all other
the minimum order of n-gram this function should extract
the stream to print to. stdout by default
list of str identifiers to be inserted to each occurrence-indexed predicate
the number of possible event types. this must be at least
reader for corpora following the tei-p5 xml scheme such as multext-east
the tokens being tagged
a hiddenmarkovmodeltagger instance used to begin
...
bindingdict a dictionary of all current bindings
the new canvas widget. canvaswidget
bool true if the canvas size should be the exact size of the drs
tgrep exception type
source language phrase of interest
a transform that should be applied
list of str input sentences to parse as individual discourses
the grammar or a list of regexpchunkparser objects
type
a list of featureset label
length of source sentence being
if false then use non-logarithmic weights
...
only generate edges e where e.length()==length
the error handling strategy for the output writer
the output csv filename
lookup table that maps a source word
a collection of texts which can be loaded with list of texts or with a corpus consisting of one or more texts and which supports
how many sentences of training and testing data to use
a grammar based chunk parser chunk regexpparser uses a set of
translation of the source phrase in this
list of dependencygraph objects for training input data
list of expression for the conditions
a list specifying the categories whose words have
a grammar either in the form of a string or as a list of strings
a class that makes it easier to use regular expressions to search over tokenized strings
a wrapper around a sequence of simple string tokens which is intended to support initial exploration of texts (via the
class for loading the content of the multext-east corpus it
the samples that should be given uniform
glueformula for creating new glue formulas
retrieve data from the twitter rest api
the target word
a sting separating the node label from its parent annotation (default = "^")
the sample whose frequency
whether to allow transposition edits
the chart being used to parse the text. this
...
expression the expression with which to replace it
the current classifier
the word that the body corresponds to
reader for europarl corpora that consist of plaintext documents
toolbox data in an elementtree structure
a dependency grammar production each production maps a single
this class is used to read lists of characters from the perl unicode properties (see http //perldoc
if true then toks contains labeled tokens --
the label to use for chunk nodes
the set of variables that should be renamed
this class represents a constraint of the form (l =< n), where l is a label and n is a node a label or a hole
a grammar either in the form of a string or else
...
a tokenizer for breaking sentences or paragraphs
discard any rule with lower accuracy than min_acc
a list or regexp specifying the fileids in this corpus
deletion cost
minimum length of the ngrams aka. n-gram order/degree of ngram
a list of 1's and 0's that represent whether a relation holds in a mace4 model
the number of total tweets that have to be classified
...
the string to use to separate tokens
a gold standard reference of possible alignments
the port number for the server to listen on defaults to
a counter that auto-increments each time its value is read
included for compatibility with nltk.tag.hmm
regexp match of the problem token
the label to be appended to each tweet contained in the csv file
trace = 1 or trace = 2 for more verbose tracing
...
if specified the alignment point of j_pegged
the unicode encoding that should be used to
the number of times to attempt placing a word
data server has finished working on a package
the number of tokens in the target language tokens
a canvas widget that places a pair of parenthases around a child widget
the initial value for this feature
a list of hypothesis sentences
the number of possible event types. this must be
'false' default will not modify the root production
a class for named-entity tagging with stanford tagger the input is the paths to
whether or not to print path when a file is found
the tagger used to pos tag the raw string before
the source data to be converted into trigrams
learns parameters used in punkt sentence boundary detection
a 'view' of a corpus file that joins together one or more streambackedcorpusviews<streambackedcorpusview>
str the proof to decorate
the number of characters allotted to each
a dependency graph whose set of edges need to be
set or list of words corresponding to certain lemma
a boolean that determines whether the variable is restricted to primitives
the method that is used to find blocks
reader for corpora that consist of parenthesis-delineated parse trees like those found in the "combined" section of the penn treebank
an encoding that is used to convert the
the conditions to plot default is all
a hint giving an estimate of the number of
a mapping from thread ids to lists of reading ids
...
this rule's left tag
the list of the reviewlines that belong to the review
second input synset. must be the same part of
an abstract class defining a collection of generic association measures
the name of the feature that this
helper data object for training ibm models 3 and up read-only
a mapping from feature identifiers to feature values where each feature value is either a basic value (such as a string or an
...
the training set to be passed as argument to the
the k value use in method 4
the words to be plotted
if true then use the 'explicit' format. i.e.,
output at most max_rules rules
class representing a token
the canvas widget that should be inserted
the current word
the item to visit
a list of the frequency distributions
if true the don't actually set the child's
the finnish snowball stemmer
a list of rte corpus fileids
an elementtree containing the xml contents of
tokenize a document into topical sections using the texttiling algorithm
starting index of the possible foreign language phrases
keyword arguments for the new canvas. see the
insertion cost
if true print a message when loading a resource
use a fraction of the training data exclusively for training baseline
if true include sentence bracketing
if this is true gives all
a list of individual constants in the semantic representation
the base frequency distribution
if true do not output intersecting templates non-disjoint positions for some feature
bool also find discourse referents in subterms?
...
a tag specification indicating what xml
tokenizer for breaking the text of tweets into
training data represented as a list of
corpus reader for corpora in rte challenges
position in the record of the subject of the predicate
optional. use this to pass in custom
the name of the wordnet_ic file (e.g. "ic-brown.dat")
tuple of two clause objects. the first is a list of all
if true then the temporary file will be
the span of the production
the index of the token whose tag should be corrected
a stemmer for portuguese
unigram tagger the unigramtagger finds the most likely tag for each word in a training
the function that extracts features for each token of a sentence. this function should take
a list or iterator of sentences where each sentence
to see the api documentation for this lazily loaded corpus first run corpus
the class used for storing the chart
snowball stemmer the following languages are supported
the source expression's text
a class for word tokenization using the repp parser described in rebecca dridan and stephan oepen 2012 tokenization returning to a
the initial list of features for this feature
order of fields for each type of element and subelement
the danish snowball stemmer
lookup table that maps a target word
a sequence of start end offsets of the tokens
a base feature value that represents the union of two or more featurevalueset or variable
the length of the closest reference for a single
a sentence tokenizer which uses an unsupervised algorithm to build a model for abbreviation words collocations and words that start
a feature identifier that's specialized to put additional constraints default values etc
a unicode string or a byte string encoded in the given
the class used for parsing should be chartparser
an optional list of strings to use for labeling the leaves
this class holds a goal and a list of assumptions to be used in proving or model building
maximum length of the ngrams set to length of sequence by default
...
the name of the tagset to use or none for no tags
if true then create a scrollbar for the
the parent tkinter widget. if no parent is
classification label for the pair
the java command that should be called formatted as
the level of diagnostic tracing output to produce
the root directory for this corpus
the chunk string to which each rule should be
if true, evaluate classifier accuracy
boolean flag marking the input data
a list of instances e.g. documents that will be split
...
the number of source language words
a stream backed corpus view specialized for use with ann_segmentation
the minimum number of occurrencies of bigrams to take
a scorer for calculated the weights on the edges of a weighted dependency graph
a lambda calculus expression parser
a list of target sentences' lengths
a rule that inserts all empty productions as passive edges in every position in the chart
the corpus from which we create an information
if true remove tweets appearing more than once
the function to apply either _draw_command or _visit_command
a list of probability distributions over values to
...
the new end index
a single input sentence to parse
...
the framenet id number of the document
boolean flag indicating whether training should be
functional composition harmonic combinator
corpus reader for the propbank corpus which augments the penn treebank with information about the predicate argument structure
lists of token sequences sentences in some applications to be tagged
this corpusview is used to skip the initial readme block of the corpus
the grammar used to generate sentences
number of baum-welch interations to perform
a specialized corpus view for tagged documents it can be
an elementtree element
drs of the form ' drs + drs '
desired output length
corpus reader for corpora whose documents are xml files
the samples to initialize the frequency
a list of featuresets dicts or labelled featuresets
a feature encoding used to convert featuresets
the string to print consisting of words and spaces
the input signal
a list of reference values
the new edge's index specifying the position of
a tokenizer that processes tokenized text and merges multi-word expressions into single tokens
p(fname=fval|label), the probability
whether output should be verbose
a rule specifying how to add chunks to a chunkstring, using a matching tag pattern
whether the ngrams should be left-padded
the maximum change in log probability to
this class holds a goal and a list of assumptions to be used in proving or model building
...
the tokens to print
a string used in construction of the artificial nodes separating the head of the
tokenizer for breaking paragraphs
the right delimiter printed after the matched substring
the french word whose region rv is determined
the tagged tokens being tagged
the node value that should be used for the
sentences to be parsed
a tkinter window used to display a text showtext is
a path pointer identifying the root directory for
reader for simple part-of-speech tagged corpora paragraphs are
a list or iterable of tokens
input sentences to parse
if true, output files are compressed with gzip
the digraphs that are used to determine the
the sample for which to update the probability
if the most likely tag for a context occurs
the function that should be called when
the word whose region r1 is determined
input file object consisting of tweet ids one to a line
wrapper class with restricted functionality and fewer options
the list of sentences to be printed
the string to be tokenized
the input string
a set of variables whose names should not be
the french vowels that are used to determine
if true will tag incrementally and collect stats for each rule rather slow
dict(str -> list abstractvariableexpression
the model file
