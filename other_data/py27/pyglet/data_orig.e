...
datetime wrapper for an iso 8601 string or time tuple or localtime integer value to generate 'datetime
...
maximum line length iterator. each element
a distinct cursor used on a tablet
udp server class
dictionary that remembers insertion order
a procedurally generated fm waveform
+ this particular policy is the backward compatibility policy
...
a unicode string for the header. it must be possible
edge flag attribute
local_part contains non-ascii characters
at least mac os x doesn't free buffers when a source is deleted it just detaches them from the source
...
multipath xml-rpc server this specialization of simplexmlrpcserver allows the user to create
...
visible text insertion marker for pyglet
...
an import hook class that uses lib2to3 for source-to-source translation of py2 code to py3
...
this represents an objective-c method an imp which has been bound to some id which will be passed as the first parameter to the method
datetime(year month day[, hour[, minute[, second[, microsecond[,tzinfo]]]]]) the year month and day arguments are required
...
an authentication protocol defined by rfc 2069 digest authentication improves on basic authentication because it
...
a magical object class that provides python 2 compatibility methods : next
base class for message parsing errors
base64 encoded sequence had characters not in base64 alphabet
class to open urls
collection of http cookies
base class for request handler classes
an array of bytes a.k.a. octets
...
this class provides a set of methods to read parse and answer questions about a single robots
...
server -> a object used to boxcar method calls server should be a serverproxy object
...
uri [,options] -> a logical connection to an xml-rpc server uri is the connection point on the server given as
