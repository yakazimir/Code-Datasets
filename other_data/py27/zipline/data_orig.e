...
raised if a transact call returns a transaction with a direction that does not match the order
a helper for tmp_bcolz_equity_minute_bar_reader and tmp_bcolz_equity_daily_bar_reader
aroon technical indicator
retrieve prices until end date
an abstract pipeline loader for the estimates datasets that loads data from a blaze expression
...
a base class for readers which reindexes results filling in the additional indices with empty data
ziplinetestcase mixin providing cls bcolz_minute_bar_path
a pipelineloader that reads its input from dataframes
max drawdown
exponentially weighted moving average **default inputs ** none
an object that boxes results for calls to :meth ~zipline
...
an algorithm that ensures that 'phantom' positions do not appear in portfolio
raised when a 1d term is added as a column to a pipeline
a rule that only triggers when it is not a half day
base class for slippage models which compute a simulated price impact according to a history lookback
...
intermediate class that sits on dataset objects and returns memoized boundcolumn objects when requested
a marker for subdividing the period at the latest intraperiod capital change
raised if a user tries to call pipeline_output in an algorithm that hasn't set up a pipeline engine
...
:usage instantiate riskmetricscumulative once
...
raised when an assetfinder tries to build an asset that does not have a sid and that assetfinder is not permitted to assign sids
...
adjust open/high/low/close for splits and dividends
...
converts minute pricing data into a daily summary to be used for the last slot in a call to history with a frequency of 1d
moving average convergence/divergence macd signal line https //en
create a temporary assets sqlite database
raised when a trailing window is instantiated with a lookback greater than the length of the underlying array
error raised on attempt to perform operations between labelarrays with mismatched missing_values
...
raised if a user attempts to construct a term using parameterizedtermmixin without specifying a params list in the class body
raised if a user attempts to construct a term without specifying inputs and that term does not have class-level default inputs
raised if a users script calls set_commission magic after the initialize method has returned
...
a field that raises an exception indicating that the field was invalid
...
an algorithm that tries to make various invalid order calls verifying that appropriate exceptions are raised
...
raised when a pipeline api term that is not deemed window safe is specified as an input to another windowed term
...
rate of change percentage roc measures the percentage change in price from one period to the next
a precomputedloader that emits arrays containing 1s on the diagonal and 0s elsewhere
abstract base class representing a fail-safe control on the behavior of any algorithm
...
the calendarrollfinder calculates contract rolls based on when volume activity transfers from one contract to another
a factor representing the row-wise rank data of another factor
yields all events in event_list that match the given sid_filter
weighted average value of an arbitrary column
error raised on attempt to perform operations between labelarrays with mismatched category arrays
signal an error that should have a different message if invoked from the cli
raised when attempting to index into a non-sliceable term e g instances
raised when a get_supplementary_field() call contains a sid that does not have a value for the specified mapping type
...
test algorithm for verifying that executionstyles are properly forwarded by order api helper methods
decorator that marks that an attribute of an instance should not be computed until needed and that the value should be memoized
...
reader for data written by bcolzminutebarwriter parameters
...
raised if a user attempts to construct a term without specifying window length and that term does not have a class-level default window length
raised when a symbol() call contains a symbol that changed over time and is thus not resolvable without additional information
loads adjustments based on corporate actions from a sqlite database
class used to write data to an assets db
raised when a pipeline term is constructed with a dtype that isn't a numpy dtype object
raised if an order would violate a constraint set by a tradingcontrol
parameters ----------
this algorithm will send a specified number of orders to allow unit tests to verify the orders sent/received transactions created and positions
abstract base class representing a fail-safe control on the behavior of any algorithm
tradingcontrol representing a limit on the magnitude of any single order placed with the given asset
...
exchange calendar for the london stock exchange open time 8 00 am gmt
raised when a frequency is given to history which is not supported
metaclass for datasets supplies name and dataset information to column attributes
null context manager useful for conditionally adding a contextmanager in
a version of rollingpanel that exists for backwards compatibility with batch_transform
...
pipelineloader for us equity pricing data delegates loading of baselines and adjustments
average daily dollar volume **default inputs ** [usequitypricing
class encapsulating an order to be placed at the current market price
raised when a lookup_by_supplementary_mapping() call contains a value that changed over time for the specified field and is
a pipeline object represents a collection of named expressions to be compiled and executed by a pipelineengine
decorator that marks that an attribute of a class should not be computed until needed and that the value should be memoized
slippage model that fills a constant number of shares per tick for testing purposes
tradingcontrol representing a prohibition against ordering an asset before its start_date or after its end_date
writer for data to be read by sqliteadjustmentreader parameters
raised if any impossible parameters (nan negative limit/stop) are passed to an order call
...
raised if a user script calls set_cancel_policy with an object that isn't a cancelpolicy
validation mixin enforcing that a term gets a positive windowlength
raised when the static calendar registry already has a calendar with a given name
abstract restricted list interface representing a set of assets that an algorithm is restricted from trading
ziplinetestcase mixin providing cls env as a class-level fixture
raised if a set of mutually exclusive parameters are passed to an order call
mixin for aliased terms
customfactor that returns the assetid of each asset
graph represention of pipeline term dependencies that includes metadata about extra rows required to perform computations
...
ziplinetestcase mixin that provides cls
raised by api functions accepting percentile bounds when the passed bounds are invalid
...
model slippage for futures contracts according to the following formula new_price = price + (price * mi / 10000),
raised when an algorithm calls history() in initialize
wrapper around an adjustedarraywindow which supports monotonically increasing by datetime requests for a sized window of data
...
true range a technical indicator originally developed by j
pseudo-filter describing whether or not an asset existed on a given day
a factor that computes spearman rank correlation coefficients between the columns of a given factor and either the columns of another
calculates a commission for a transaction based on a per trade cost
orders are never automatically canceled
fast stochastic oscillator indicator [%k momentum indicator] https //wiki
preallocation strategies for rolling window over expanding data set
exchange calendar for nyse open time 9 31 am us/eastern
a stateless rule has no observable side effects
mixin for behavior shared by custom{factor filter classifier}
relative strength index **default inputs**: [usequitypricing
volume weighted average price **default inputs ** [usequitypricing
raised when a lookup_by_supplementary_mapping() call contains a value does not exist for the specified mapping type
the calendarrollfinder calculates contract rolls based purely on the contract's auto close date
raised whenever a rolling transform is called on an event that does not have the necessary properties
...
parameters ----------
an event is a pairing of an eventrule and a callable that will be invoked with the current algorithm context data and datetime only when the rule
raised during construction of a rank factor when supplied a bad rank method
ziplinetestcase mixin that provides self responses as an instance
manages a list of event objects
raised when trying to compute correlations/regressions between two 2d factors with different masks
raised when no source is given to the pipeline
tracks the performance of the algorithm
re-usable contextmanager for contextually managing warnings
mixin for behavior shared by downsampled{factor filter classifier} a downsampled term is a wrapper around the "real" term that performs actual
retrieve prices from start date on
base class for slippage models which only support futures
an abstract pipeline loader for the events datasets that loads data from a blaze expression
a factor that computes pearson correlation coefficients between the columns of a given factor and either the columns of another factor/boundcolumn or a
...
raised by an assetdbwriter or assetfinder if the version number in the versions table does not match the asset_db_version in asset_writer
the account object tracks information about the trading account the
raised if a user tries to set a futures slippage model for equities or vice versa
...
a sqliteadjustmentreader that stores no adjustments and uses in-memory sqlite
...
raised when a get_supplementary_field() call contains a value that changed over time for the specified field and is thus not resolvable
class property
...
a factor that transforms an input factor by applying a row-wise shape-preserving transformation on classifier-defined groups of that
dolce fa niente
a rule that composes the results of two rules with some composing function
create a temporary trading environment
raised when a pipeline term is constructed with a dtype that's not supported
base class for commission models which only support futures
ziplinetestcase mixin which makes it possible to synchronize date bounds across fixtures
algorithm that tries to buy with extremely low stops/limits and tries to sell with extremely high versions of same
pipelineengine class that computes each term independently
raised if a transact call returns a transaction with a volume greater than the corresponding order
...
raised by calendar operations that would ask for dates beyond the extent of our known data
warning used to signal that no deltas or checkpoints could be found and none were provided
...
accountcontrol representing a limit on the maximum leverage allowed by the algorithm
...
a metaclass template for classes the want to prevent subclassess from overriding a some methods or attributes
a filter that computes to true only for the given asset
algorithm that tries to order 1 share of sid 999 on every bar and has an assetdatebounds() trading control in place
raised when a retrieve_asset() or retrieve_all() call contains a non-existent sid
calculates adjustments for continuous futures based on the close and open of the contracts on the either side of each roll
...
calculates a commission for a transaction based on a per trade cost
historical restrictions stored in memory with effective dates for each asset
...
raised if a user attempts to construct a term without specifying dtype and that term does not have class-level default dtype
...
exponentially weighted moving standard deviation **default inputs ** none
context manager for making an algorithm instance available to zipline api functions within a scoped block
static restrictions stored in memory that are constant regardless of dt for each asset
an abstract pipeline loader for estimates data that can load data a variable number of quarters forwards/backwards from calendar dates
...
writer for files containing minute bar updates for consumption by a writer for a minutebarreader format
...
mock dataportal that returns fake data for history and non-fetcher spot value
an attribute that cannot be overridden
a factor that performs an ordinary least-squares regression predicting the columns of a given factor from either the columns of another
abstract class for business days since a previous event
raised if a user tries to set a futures commission model for equities or vice versa
tradingcontrol representing a limit on the number of orders that can be placed in a given trading day
raised when a trailing window would be instantiated with a length less than 1
...
create a context manager from a pre-execution callback and a post-execution callback
...
tradingcontrol representing a restricted list of assets that cannot be ordered by the algorithm
the financial simulations in zipline depend on information about the benchmark index and the risk free rates of return
...
tradingcontrol representing a limit on the maximum position size that can be held by an algo for a given asset
shared extensions to core unittest testcase
raised if a user script calls the set_commission magic
exchange calendar for the toronto stock exchange open time 9 30 am est
