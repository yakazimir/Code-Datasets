...
basic concurrency stuff
this module provides typed pointers to foreign data
deprecated safe is now the default please use control
deprecated safe is now the default please use foreign
...
the foreignptr type and operations
...
this module provides access to internal garbage collection and memory usage statistics
if you re using ghc
equality
...
the arbitrary-precision natural number type
this module is an internal ghc module
...
deprecated safe is now the default please use foreign instead
...
basic arrow definitions based on generalising monads to arrows by john hughes science of computer programming 37 pp67-111 may 2000
unsafe io operations
the strict st monad re-export of control
the maybe type and associated operations
unbounded channels
this module contains support for pooled memory management
symbolic references to values
converting values to readable strings the show class and associated functions
deprecated safe is now the default please use control
this legacy module provides access to the list-specialised operations of data
mutable references in the strict st monad re-export of data
...
a stable pointer is a reference to a haskell expression that is guaranteed not to be affected by garbage collection i
...
the type foreignptr represents references to objects that are maintained in a foreign language i
...
type of a device that can be used to back a handle see also mkfilehandle
a string is a list of characters
a parser for a type a represented as a function that takes a string and returns a list of possible parses as a string pairs
a finalizer is represented as a pointer to a foreign function that at finalisation time gets as an argument a plain pointer variant of the foreign pointer that the finalizer is associated with
exceptions generated by array operations
...
the cut-down haskell lexer used by text
a weak pointer object with a key and a value
qsemn is a quantity semaphore in which the resource is aqcuired and released in units of one
