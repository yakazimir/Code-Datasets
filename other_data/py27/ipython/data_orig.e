...
simple object for containing captured stdout/err and rich display stringio objects each instance c has three attributes
class for embedding a vimeo video in an ipython session based on its video id
context manager for capturing stdout/err
deprecated context manager that does nothing
...
raw_input was requested in a context where it is not supported for use in ipython kernels where only some frontends may support
...
formatter allowing itpl style $foo replacement for names and attribute access only
sys excepthook for ipython itself leaves a detailed report on disk
a private marker - used in parameter & signature
dispatch calls to a chain of commands until some func can handle it usage instantiate execute "add" to add commands (with optional
color escape sequences for input prompts
class for embedding a local file link in an ipython session based on path e
magics for displaying various output types with literals defines javascript/latex/svg/html cell magics for writing
...
baseclass for the representationprinter prettyprinter that is used to generate pretty reprs of objects
...
plugin that installs a knownfail error class for the knownfailureclass exception
...
class for embedding a scribd document in an ipython session use the start_page params to specify a starting point in the document
...
class for embedding local file links in an ipython session based on path e
...
magics for talking to scripts this defines a base %%script cell magic for running a cell
...
an inputhookcontext instance
extension which holds some state the last exception value
instances of this class are always autocalled this happens regardless of 'autocall' variable state
class to manage a pool of backgrounded threaded jobs
partial lexer for ipython tracebacks
deprecated class for local debugging similar to pdb
