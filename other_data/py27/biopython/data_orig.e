...
the measure r-work for the structure
...
the name of this group e.g. lys
...
fragments contained in the hsp object
...
wellrecord object stores all the time course signals of a phenotype microarray well
the integer atom index in the group of the second partner in the bond
...
the number of unique bonds in the group
...
the indices of the chains for this entity
...
the atom name e.g. ca for this atom
...
block sizes
the residue number of this group
...
store the information for one sequence line from a unigene file initialize with the text part of the sequence line or nothing
store a unigene record here is what is stored :
object to absorb
...
stores the information in a cel file example usage
...
sequence type
the list of doubles for the transform of this bioassmbly transform
a segment that is located at the end of a linear chromosome
...
either an integer specifying the number of taxa to create
an abstract support class to retrieve files
the integer atom index in the structure of the first partner in the bond
the formal atomic charge of the atom
...
lineardrawer abstractdrawer inherits from
...
affine_penalty(open extend[, penalize_extend_when_opening]) -> gap_fn create a gap function for use in an alignment
the integer index of the bioassembly
...
the integer atom index in the group of the first partner in the bond
...
the integer atom index in the structure of the second partner in the bond
...
identity_match([match][, mismatch]) -> match_fn create a match function for use in an alignment
...
the list of experimental methods in the structure
...
commandline object for the seqmatchall program from emboss e
the asym chain id from mmcif
find schemas using a genetic algorithm approach
hsp objects contained in the hit object
...
class to calculate the distance matrix from a dna or protein multiple sequence alignment msa and the given name of the
holds information from a psi-blast round
queryresult id of this hit
...
nexml tree object
the type of secondary structure used
the measured r-free for the structure
the single letter code of the group
...
parsed values of the hsp attributes
name of mapping between element and attribute names
...
a lower-case string supported by bio.phylo.write
...
kd tree implementation (c++, swig python wrapper) the kd tree data structure can be used for all kinds of searches that
...
key of the hit object to return
...
class to run svd alignment svdsuperimposer finds the best rotation and translation to put
object whose attribute value will be used
...
return value if no hit exists with the given key
dictionary_match(score_dict[, symmetric]) -> match_fn create a match function for use in an alignment
...
sequence to check
whether to do in-place sorting or no
...
function to define hit keys
iterator yielding hit objects
...
the serial id of the atom e.g. 1
the release date of the structure
whether the element has children or not
...
whether to reverse sorting results or no
...
a string indicating the type of group (as found in the chemcomp dictionary
...
length of the branch leading to the
...
mapping of format name and its processor object
base commandline object for new ncbi blast+ wrappers private
uppercase iupac protein single letter alphabet of the 20 standard amino acids
mauve/xmfa alignment writer
class for neighbor searching this class can be used for two related purposes
...
an array of length 6 with the unit cell parameters in order a b c alpha beta gamma
...
store the information for one sts line from a unigene file initialize with the text part of the sts line or nothing
...
base class for distance matrix or scoring matrix accepts a list of names and a lower triangular matrix
...
the index of this group in the sequence defined by the enttiy
...
function used to cast each item in the input string
...
program name
a node in the scop hierarchy sunid -- scop unique identifiers
...
store the information for one protsim line from a unigene file initialize with the text part of the protsim line or nothing
the number of chains in the model
...
the insertion code for this group
...
hsp whose properties will be set
...
element name
the number of groups this chain has
...
root element of the iteration_hits tag
...
base commandline object for emboss wrappers private
the list of integer indices for the chains of this bioassembly
