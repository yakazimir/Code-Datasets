...
extras
extras
frozen statespace model takes a snapshot of a statespace model
class for estimating and conducting inference in an aft model
gaussian normal kernel k u = 1 / (sqrt(2*pi)) exp(-0
univariate time series model for estimation with maximum likelihood
gram-charlier expansion of normal distribution class follows scipy
maximum likelihood estimation of poisson model this is an example for generic mle which has the same
parent class for covariance structure for categorical data models
results class for for an ols model
1d kernel density regression/kernel smoother requires
j-test for comparing non-nested models parameters
...
negative binomial variance function parameters
individual experimental unit for em implementation of repeated measures
tweedie family
fft tools for arma processes this class contains several methods that are providing the same or similar
...
a subclass of term that can be used to apply point transformations of another term i
...
class for two sample comparison the tests and the confidence interval work for multi-endpoint comparison
polynomial smoother up to a given order
control fdr in a regression procedure
first-order k-regime markov switching model parameters
vector autoregressive moving average with exogenous regressors model parameters
class that defines linalg operation on an array
diffusion that has an exact integral representation
descriptive statistics and tests with weights for case weights assumes that the data is 1d or 2d with nobs nvars observations in rows
cox proportional hazards regression model
statistical power calculations base class
a penalty that converges to +infinity as the argument matrix approaches the boundary of the domain of symmetric positive
markov switching regression model parameters
class to calculate outlier and influence measures for ols result parameters
...
data generating process for panel with within correlation allows various within correlation structures but no random intercept yet
class to hold results from fitting an ar model
class to contain results of fitting a linear mixed effects model
dictionary that remembers insertion order
the pseudo-huber penalty
univariate skew-normal distribution of azzalini class follows scipy
...
the probit standard normal cdf transform notes
andrew's wave for m estimation
the square-root transform notes
this class is very simple it is just a named term in a model formula
mixtures with a discrete distribution the mixing distribution is a discrete distribution like scipy
...
class to test equality of regression coefficients across groups this class performs tests whether the linear regression coefficients are
univariate skew-normal distribution of azzalini class follows scipy
pseudo-link function for fitting nominal multinomial models with gee
class to write numpy arrays into a python module calling save on the instance of this class write all attributes of the
...
statistics by groups another version groupstats as a class with lazy evaluation (not yet - decorators are still
heston stochastic volatility
an abstract statistical model class for panel longitudinal datasets
a class for representing a scalar-value penalty
the identity transform notes
the log transform notes
nonparametric test for functional form
a results class for the discrete dependent variable models
multivariate normal distribution with plain linalg
a wrapper class for two way anova type of analysis with ols currently mainly to bring things together
this class summarizes the fit of a linear regression model
a descriptor to make special properties that become normal attributes
multivariate kernel density estimator
class to hold results from fitting a markov switching model parameters
eubank p 179f
leave-p-out cross validation iterator
class to contain results from likelihood models parameters
an independence covariance structure for ordinal models
transformation for linear constraints r params = q note the transformation from the reduced to the full parameters is an
basic class for instrumental variables estimation using gmm a linear function for the conditional mean is defined as default but the
multivariate analysis of variance the implementation of manova is based on multivariate regression and does
...
class for runs in a binary sequence parameters
maximum likelihood estimation of linear model with t-distributed errors this is an example for generic mle
...
represent an arma process for given lag-polynomials this is a class to bring together properties of the process
provides a table row as a list of cells
an exchangeable working dependence structure
short panel with general intertemporal within correlation assumes data is stacked by individuals panel is balanced and
...
univariate mixture distribution for simple case for now unbound support
...
cosine kernel k u = pi/4 cos(0
principal component analysis parameters
hampel function for m-estimation
class to hold results from fitting a markov switching regression model parameters
forward selection effect sizes for fdr control
ornstein-uhlenbeck
distribution critical values and p-values from tables currently only 1 extra parameter e
distribution based on log/exp transformation
example -------
huber's proposal 2 for estimating location and scale jointly
cox test for non-nested models parameters
test whether variance is the same in 2 subsamples parameters
class that defines linalg operation on an array
...
class for linear instrumental variables models estimated with gmm uses closed form expression instead of nonlinear optimizers for each step
univariate density estimation by orthonormal series expansion uses an orthonormal polynomial basis to approximate a univariate density
...
class for the probability distribution of total runs this is the exact probability distribution for the wald-wolfowitz
class which wraps a statsmodels estimation results class and steps in to
additive model with non-parametric smoothed components parameters
class to contain results of fitting a cox proportional hazards survival model
the inverse squared transform notes
state space model for maximum likelihood estimation parameters
minimal implementation of wls optimized for performance
class to run monte carlo study on a statistical test''' todo
...
this class is needed to allow file-like object to be used as context manager but without getting closed
...
a class for step down methods this is currently for simple tree subset descend similar to homogeneous_subsets
...
univariate skew-t distribution of azzalini class follows scipy
a class that holds functions to be optimized over when conducting hypothesis tests and calculating confidence intervals
...
conditional logit data handling test parameters
negative binomial exponential family
conduct a mediation analysis
results class for pairwise comparisons based on p-values parameters
object to specify settings for density estimation or regression
class for absolute value transformation
orthonormal (for weight=1) fourier polynomial on [0 1] orthonormal polynomial but density needs corfactor that i don't see what
a first-order autoregressive working dependence structure
single index semiparametric model y = g(x * b) + e
class to reparameterize a model for standardized exog parameters
nonparametric kernel regression class
orthogonal (for weight=1) fourier polynomial on [0 pi]
generic n-dimensial kernel parameters
arma model with exact loglikelhood for short time series inverts nobs nobs matrix use only for nobs <= 200 or so
instrumental variables estimation using two-stage least-squares 2sls parameters
maximum likelihood estimation of linear model with t-distributed errors this is an example for generic mle which has the same
...
base class for estimation of a non-linear model with least squares this class is supposed to be subclassed and the subclass has to provide a method
inverse-gaussian ??? used by nig
distribution based on log/exp transformation
return the empirical cdf of an array as a step function
univariate arma model with t-distributed errors this inherit all methods except loglike from tsa
inversegaussian exponential family
maximum likelihood estimation of poisson model this is an example for generic mle which has the same
state space representation of a time series process with kalman filter parameters
garch model gjrgarch t-garch
results from looking at a particular contrast of coefficients in a parametric model
this is simply a monkey patch for numpy testing tester
notes -----
results for cohen's kappa attributes
the parent class for one-parameter exponential families
the logit transform
wls with heteroscedasticity that depends on explanatory variables note mixing gls sigma and weights for heteroscedasticity might not make
the complementary log-log transform cloglog inherits from logit in order to have access to its _clean method
class to hold parameters in markov switching models parameters
results from applying the hamilton filter to a state space model
multivariate linear model via least squares parameters
...
parameters
garch model still experimentation stage
...
a basic step function values at the ends are handled in the simplest way possible everything to the left of x[0] is set to ival everything
nested conditional logit runmnl data handling test
returns a dict-like object with keys accessible via attribute lookup
a results class for empirical likelihood regression through the origin parameters
...
two-stage least squares for simultaneous equations parameters
results from applying the kim smoother to a markov switching model
negative binomial regression parameters
distribution based on a non-monotonic u- or hump-shaped transformation the constructor can be called with a distribution class and functions
just a storage class right now
kalman filter code intended for use with the arma model
canonical correlation analysis using singluar value decomposition for matrices exog=x and endog=y find projections x_cancoef and y_cancoef
...
conversion between full rank dummy encodings y = x b + u
todo
distribution of success runs of length k or more classical definition the underlying process is assumed to be a sequence of bernoulli trials
variance gamma process
this class is used to construct contrast matrices in regression models
clipped continuous distribution with a masspoint at clip_lower notes
class to hold results from fitting a markov switching autoregression model parameters
class to contain model results parameters
class to hold tables for result summary presentation construction does not take any parameters
...
the cauchy standard cauchy cdf transform notes
univariate autoregressive moving average model conditional on initial values the arma model is estimated either with conditional least squares or with
asymptotic covariance by deltamethod the function is designed for 2d array with rows equal to
...
state space representation of a time series process parameters
quantile regression estimate a quantile regression model using iterative reweighted least
tests for multiple comparisons parameters
...
class for non-linear instrumental variables estimation wusing gmm the model is assumed to have the following moment condition
...
maximum likelihood estimation of poisson model this is an example for generic mle which has the same statistical model
results from tukey hsd test with additional plot methods can also compute and plot additional post-hoc evaluations using this
the negative binomial link function parameters
the schwartz type 1 stochastic process :math :
geometric brownian motion :math :
decorator for cachedattribute
from anne archibald in mailinglist http //www
...
univariate unobserved components time series model these are also known as structural time series models and decompose a
normal-inverse-gaussian
multivariate normal distribution with cholesky decomposition of sigma ignoring mean at the beginning maybe
a categorical factor
stub estimate one equation
class to perform kernel ridge regression gaussian process warning this class is memory intensive it creates nobs x nobs distance
class to keep track of varma polynomial format examples
first-order k-regime markov switching regression model parameters
restricted general least squares model that handles linear constraints parameters
estimated marginal effects for a regression model fit with gee
dict subclass for counting hashable objects sometimes called a bag
the inverse transform notes
huber's t for m estimation
estimate distribution parameters by gmm based on matching quantiles currently mainly to try out different requirements for gmm when we cannot
class for multivariate normal distribution
allows the fitting of any likelihood function via maximum likelihood
class to hold results from fitting an varmax model
results from applying the kalman smoother and/or filter to a state space model
class for estimation by generalized method of moments needs to be subclassed where the subclass defined the moment conditions
fit var and then estimate structural components of a and b defined
short panel with general intertemporal within correlation assumes data is stacked by individuals panel is balanced and
the power transform parameters
estimate var p process with fixed number of lags parameters
...
model for em implementation of repeated measures
one sample goodness of fit tests includes kolmogorov-smirnov d d+, d-, kuiper v cramer-von mises w^2 u^2 and
ramsay's ea for m estimation
class represents a known svar p process parameters
...
orthonormal (for weight=1) chebychev polynomial on -1 1 notes
