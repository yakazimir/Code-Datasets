...
'any' or 'all'
int default 1. number of rows to return
...
list of topic_name to consume
string name of the table
date datetime date data type
str list
output column for storing the distance between each result row and the key
latent dirichlet allocation lda a topic model designed for text documents
standardizes features by removing the mean and scaling to unit variance using column summary statistics on the samples in the
...
bool int long float string list or dict
an optional :class pyspark.sql.types.structtype for the input schema
user-facing configuration api accessible through sparksession conf
path specifying the directory to which the model
...
size of the rdd
string name of the database to use. default to the current database
a bisecting k-means algorithm based on the paper "a comparison of document clustering techniques" by steinbach karypis and kumar
...
list of row or tuple
a dictionary of jdbc database connection arguments. normally at
the minimum delay to wait to data to arrive late relative to the
interface used to write a streaming :class dataframe to external storage systems (e
...
an rdd of any kind of sql data representation(e.g. row tuple int boolean
list of column names string or list of :class column expressions that grouped
...
sets the single character used for escaping quoted values where the
note : experimental
...
path to hadoop file
list of host port s on which the spark sink is running
a :py class pyspark.ml.linalg.vector
the maximum number of events to be pulled from the spark sink
numeric data types
...
a set of methods for aggregations on a :class dataframe, created by :func dataframe
pipelined maps >>> rdd = sc
...
number of concurrent requests this stream should send to the sink
...
whether to load values from java system
note : experimental
exception that stopped a :class streamingquery
a distributed collection of data grouped into named columns
list of column names string or list of :class column expressions that have
model fitted by decisiontreeclassifier
test both vector operations and mllib algorithms with scipy sparse matrices if scipy is available
str default inner. must be one of inner, cross, outer,
a resilient distributed dataset rdd the basic abstraction in spark
serializes a stream of objects in batches by calling its wrapped serializer with streams of objects
deserializes streams written by string getbytes
any additional metadata default none
a :class pyspark.sql.types.structtype object
a discretized stream dstream the basic abstraction in spark streaming is a continuous sequence of rdds of the same type representing a
latent dirichlet allocation lda model
a :py class pyspark.ml.linalg.matrix
a dataset that contains labels/observations and predictions
verify data types of every row against schema
represents a distributed matrix in blocks of local matrices
spark configuration passed to spark-submit
optional parameters
kafka brokers for each topicandpartition in offsetranges. may be an empty
random seed
...
a :class pyspark.sql.types.datatype object
...
hadoop job configuration passed in as a dict none by default
...
file or directory path in any hadoop-supported file
a function used to convert kafkamessageandmetadata. you can assess
...
interface used to load a streaming :class dataframe from external storage systems (e
the start value
normalize a vector to have unit norm using the given p-norm
the target instance
vector or rdd of vector to be normalized
a column of :class pyspark.sql.types.stringtype
partition id of this kafka message
...
an rdd[labeledpoint] containing the labeled dataset
...
name of column containing the struct or array of the structs
number of partitions in the rdd
an internal type used to represent everything that is not null udts arrays structs and maps
parse records which may span multiple lines. if none is
one of append, overwrite, error, ignore default error
...
serializer that writes objects as a stream of length data pairs where c{length} is a 32-bit integer and data is c{length} bytes
...
topic name of this kafka message
a subclass of params mixed with hasmaxiter hasinputcol and hasseed
model fitted by prefixspan >>> data = [
note : experimental
if set to true truncate strings longer than 20 chars by default
compress the serialized data
note : experimental
test mllib algorithms on plain lists to make sure they're passed through as numpy arrays
...
a list of transformers or estimators
...
a name of the hint
model fitted by als
base class for :py class models that wrap java/scala implementations
model fitted by :py class countvectorizer
list of doubles as weights with which to split the dataframe. weights will
a clustering model derived from the bisecting k-means method
short data type i e a signed 16-bit integer
mixin for param labelcol label column name
serializes objects using python's marshal serializer http //docs
...
size of the vector
sets the string that indicates a timestamp format. custom date
python type list
...
random forest <http //en wikipedia org/wiki/random_forest>_
this class wraps a function rdd[x] -> rdd[y] that was passed to dstream
...
note : developerapi
utility class to help create wrapper classes from java/scala implementations of pipeline components
extra parameters
a string
note : experimental
resolves paths to files added through l{sparkcontext
extenalsorter will divide the elements into chunks sort them in memory and dump them into disks finally merge them back
...
the name of an integer column that will be used for partitioning
note : experimental
note : experimental
...
actual message payload of this kafka message the return data is
...
represents a mu sigma tuple >>> m = multivariategaussian(vectors
...
kafka partition id
turns the nested row as dict default false
...
a :py class transformer that maps a column of indices back to a new column of corresponding string values
...
a dataset that contains labels/observations and
...
additional params for kafka
a function used to decode body (default is utf8_decoder)
base class for :py class evaluators that wrap java/scala implementations
...
vector or rdd of vector to be standardized
...
vector or rdd of vector to be transformed
...
note : experimental
function that checks for termination conditions
private abstraction represents a tree ensemble model
base class for :py class transformers that wrap java/scala implementations
...
this class keeps track of different profilers on a per stage basis
...
a l{sparkconf} object setting spark properties
...
key payload of this kafka message can be null if this kafka message has no key
...
start position int or column
learning algorithm for a gradient boosted trees model for classification or regression
per-topic/partition kafka offsets defining the inclusive starting
none by default
a function used to decode value (default is utf8_decoder)
sets the single character used for skipping lines beginning with this
...
decision tree <http //en wikipedia org/wiki/decision_tree_learning>_
an optional param map that overrides embedded
sets the string representation of a positive infinity value. if none
test :py class javaparams classes to see if their default param values match those in their scala counterparts
...
normalizes samples individually to unit l\ :sup p\ norm for any 1 <= p < float('inf'), normalizes samples using
kafka message and metadata information including topic partition offset and message
interface used to load a :class dataframe from external storage systems (e
word2vec creates vector representation of words in a text corpus
fully qualified name of java class
string represents path to the json dataset or a list of paths
unique name for the query
base class for :py class estimators that wrap java/scala implementations
...
serializes objects using python's pickle serializer http //docs
...
source vectors
power iteration clustering pic a scalable graph clustering algorithm developed by [[http //www
versionadded : 0 9 0
private class to track supported impurity measures
...
sets the single character as a separator for each field and value. if none is
...
offset of this kafka message in the specific partition
a one-hot encoder that maps a column of category indices to a column of binary vectors with at most a single one-value per row
string name of the new column
mixin for param outputcol output column name
interface used to write a :class dataframe to external storage systems (e
...
note : experimental
length of the substring int or column
sparkcontext object
extra params to be copied
minimum splits in dataset
broker's hostname
...
note : experimental
a function used to decode key (default is utf8_decoder)
model produced by [[poweriterationclustering]]
std for the log normal distribution
sets the single character used for escaping quotes inside an already
note : experimental
...
hadoop job configuration passed in as a dict
gradient-boosted trees gbts <http //en wikipedia org/wiki/gradient_boosting>_
...
additional params overwriting embedded values
...
dict of (topic_name -> numpartitions) to consume
an accumulatorparam that uses the + operators to add values designed for simple types
url of kinesis service (e.g., https //kinesis.us-east-1.amazonaws.com)
a shared variable that can be accumulated i e has a commutative and associative "add"
serializes a stream of list of pairs split the list of values which contain more than a certain number of objects to make them
